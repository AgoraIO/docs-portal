...
describe('API Center local fragment index', () => {
  ...
  it('keeps insertion offsets stable when fenced code precedes a heading', () => {
    const aliasSource = '```ts\nconst longValue = true;\n```\n\n## 获取歌曲标签类别\n';
    const aliased = insertFragmentAliases(aliasSource, new Set(['获取歌曲标签']));
  ...
  it('resolves grouped routes and rewrites or safely drops stale fragments', async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'fragment-index-'));
    temporaryDirectories.push(repoRoot);
    await fs.mkdir(path.join(repoRoot, 'content/docs/zh-CN/api-reference/rtc'), {
      recursive: true,
    });
    const targetPath =
    ...
    expect(result.body).toContain(
      '[old](/zh-CN/api-reference/rtc/channel)',
    );
