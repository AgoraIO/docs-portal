...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--product': case '-p': opts.product = args[++i]; break;
      case '--platform': case '-P': opts.platform = args[++i]; break;
      case '--locale': case '-l': opts.locale = args[++i]; break;
      case '--route-base-path': case '-r': opts.routeBasePath = args[++i]; break;
      case '--version-dir': case '-V': opts.versionDir = args[++i]; break;
  ...
  if (!opts.source || !opts.output || !opts.product || !opts.platform) {
    console.error('Error: --source, --output, --product, and --platform are required.\n');
    printHelp();
    process.exit(1);
  }
...
Required:
  --source, -s      Source directory containing HTML docs
...
Options:
  ...
  --dry-run, -d         Print output to stdout instead of writing files
...
function cheerioHasClass(node, className) {
  return node?.attribs?.class?.includes(className) ?? false;
}

function cheerioGetId(node) {
  return node?.attribs?.id ?? null;
}

function escapeMarkdownSpecialChars(str) {
  return str
    .replaceAll('\\', '\\\\')
    .replaceAll('*', '\\*')
    .replaceAll('~', '\\~');
...
function isInlineElement(element) {
  return element.is('a, span, code, strong, b, em, i, br, sub, sup, small, mark, s, u');
}
...
function renderCodeBlock(pre) {
  ...
  const lang = classAttr.match(/language-([a-z0-9+-]+)/i)?.[1]?.toLowerCase() ?? 'text';
...
function renderTable($, table, pageTitleBySource, sourceToRoute, targetBasePath) {
  ...
  const matrix = rows.map((row) =>
    $(row)
      ...
      .toArray()
      .map((cell) =>
        inlineChildren($, $(cell), pageTitleBySource, sourceToRoute, targetBasePath)
          .replace(/\n+/g, '<br />')
          .replace(/\|/g, '\\|')
          .trim(),
      ),
...
function renderNote($, note, pageTitleBySource, sourceToRoute, targetBasePath) {
  ...
  for (const child of note.contents().toArray()) {
    ...
    if (isInlineElement(element)) {
      const rendered = renderInlineElement($, element, pageTitleBySource, sourceToRoute, targetBasePath);
      if (rendered) inlineParts.push(rendered);
      continue;
    }
    flushInline();
    const rendered = renderElement($, element, pageTitleBySource, sourceToRoute, targetBasePath);
...
function renderList($, list, pageTitleBySource, sourceToRoute, targetBasePath, ordered, options = {}) {
  const items = [];
  let index = 1;
  for (const li of list.find('> li').toArray()) {
    ...
    for (const child of $(li).contents().toArray()) {
      ...
      if (options.preferInlineOnly && !element.is('ul, ol')) {
        rendered = element.is('a')
          ? renderAnchor(element, pageTitleBySource, sourceToRoute, targetBasePath)
          : inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath);
      } else {
        rendered = element.is('ul, ol')
          ? renderList($, element, pageTitleBySource, sourceToRoute, targetBasePath, element.is('ol'), options)
          : renderElement($, element, pageTitleBySource, sourceToRoute, targetBasePath);
      }
      if (rendered) {
    ...
    if (blockParts.length === 0) {
      ...
      const nested = blockParts.map((part) =>
...
function renderDefinitionList($, dl, pageTitleBySource, sourceToRoute, targetBasePath, depth = 3) {
  const rows = [];
  ...
  for (const child of children) {
    const node = $(child);
    if (node.is('dt')) {
      currentTerm = inlineChildren($, node, pageTitleBySource, sourceToRoute, targetBasePath);
      continue;
    }
    if (node.is('dd')) {
      const value = renderChildren($, node, pageTitleBySource, sourceToRoute, targetBasePath, depth + 1);
      if (currentTerm) {
...
function routeSegmentsToDocPath(routeSegments, targetBasePath) {
  ...
  return clean.length === 0 ? targetBasePath : `${targetBasePath}/${clean.join('/')}`;
}

function renderInlineElement($, element, pageTitleBySource, sourceToRoute, targetBasePath) {
  if (element.is('a')) return renderAnchor(element, pageTitleBySource, sourceToRoute, targetBasePath);
  if (element.is('code')) return `\`${decodeHtml(element.text()).trim()}\``;
  if (element.is('strong, b')) {
    const text = inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath);
    return text ? `**${text}**` : '';
  }
  if (element.is('em, i')) {
    const text = inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  ...
  return inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath);
}

function inlineChildren($, node, pageTitleBySource, sourceToRoute, targetBasePath) {
  const parts = [];
  for (const child of node.contents().toArray()) {
    ...
    const element = $(child);
    const text = renderInlineElement($, element, pageTitleBySource, sourceToRoute, targetBasePath);
    if (text) parts.push(text);
  }
  return normalizeInlineFlow(parts.join(' '));
}

function renderElement($, element, pageTitleBySource, sourceToRoute, targetBasePath, depth = 2) {
  ...
  if (element.is('div.note')) return renderNote($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  if (element.is('div.codeblock')) return renderCodeBlock(element.find('pre').first());
  if (element.is('div, dd') && element.find('> ul, > ol, > table, > dl, > pre, > div.note, > section').length > 0) {
    return renderChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath, depth);
  }
  if (element.is('p')) return inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  if (element.is('ul')) return renderList($, element, pageTitleBySource, sourceToRoute, targetBasePath, false);
  if (element.is('ol')) return renderList($, element, pageTitleBySource, sourceToRoute, targetBasePath, true);
  if (element.is('section')) {
    const heading = element.find('> h2, > h3, > h4').first();
    const childDepth = heading.is('h4') ? 4 : heading.is('h3') ? 3 : 2;
    return renderSection($, element, pageTitleBySource, sourceToRoute, targetBasePath, Math.max(depth, childDepth));
  }
  if (element.is('pre')) return renderCodeBlock(element);
  if (element.is('table')) return renderTable($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  if (element.is('dl')) return renderDefinitionList($, element, pageTitleBySource, sourceToRoute, targetBasePath, depth + 1);
  if (element.is('article')) return renderNestedArticle($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  if (element.is('nav.related-links')) return renderRelatedLinks($, element, pageTitleBySource, sourceToRoute, targetBasePath);
  return inlineChildren($, element, pageTitleBySource, sourceToRoute, targetBasePath) || '';
}

function renderChildren($, parent, pageTitleBySource, sourceToRoute, targetBasePath, depth = 2) {
  const parts = [];
  let inlineParts = [];
  ...
  for (const node of parent.contents().toArray()) {
    ...
    const element = $(node);
    if (element.is('h1, h2, h3, h4, h5, h6') || element.is('.shortdesc') || element.is('nav.related-links')) continue;
    ...
    flushInline();
    const rendered = renderElement($, element, pageTitleBySource, sourceToRoute, targetBasePath, depth);
...
function renderSection($, section, pageTitleBySource, sourceToRoute, targetBasePath, depth = 2) {
  ...
  if (directList.length > 0 && directList.find('> li > a').length > 0 && directList.siblings().filter('ul').length === 0) {
    const renderedList = renderList($, directList, pageTitleBySource, sourceToRoute, targetBasePath, false, { preferInlineOnly: true });
    if (renderedList) parts.push(renderedList);
    return parts.join('\n\n').trim();
  }

  const rendered = renderChildren($, section, pageTitleBySource, sourceToRoute, targetBasePath, depth);
  if (rendered) parts.push(rendered);

  return parts.join('\n\n').trim();
}

function renderNestedArticle($, article, pageTitleBySource, sourceToRoute, targetBasePath) {
  ...
  for (const node of article.find('> .body > *').toArray()) {
    ...
    const rendered = renderElement($, $(node), pageTitleBySource, sourceToRoute, targetBasePath, headingDepth(level) + 1);
    if (rendered) content.push(rendered);
  }

  for (const nested of article.find('> article').toArray()) {
    const rendered = renderNestedArticle($, $(nested), pageTitleBySource, sourceToRoute, targetBasePath);
    if (rendered) content.push(rendered);
  }

  const related = renderRelatedLinks($, article.find('> nav.related-links').first(), pageTitleBySource, sourceToRoute, targetBasePath);
  if (related) content.push(related);

  return content.join('\n\n').trim();
}

function renderRelatedLinks($, nav, pageTitleBySource, sourceToRoute, targetBasePath) {
  ...
  for (const anchor of items) {
    const link = renderAnchor($(anchor), pageTitleBySource, sourceToRoute, targetBasePath);
    if (link) lines.push(`- ${link}`);
  }
  return lines.join('\n');
}

function renderPage({ $, currentSource, pageTitleBySource, pageDescriptionBySource, sourceToRoute, targetBasePath }) {
  ...
  for (const section of directSections) {
    const rendered = renderSection($, $(section), pageTitleBySource, sourceToRoute, targetBasePath);
    if (rendered) sections.push(rendered);
  }

  const nestedArticles = body.find('> article').toArray();
  for (const article of nestedArticles) {
    const rendered = renderNestedArticle($, $(article), pageTitleBySource, sourceToRoute, targetBasePath);
    if (rendered) sections.push(rendered);
  }

  if (sections.length === 0) {
    const fallback = renderChildren($, body.find('> .body').first(), pageTitleBySource, sourceToRoute, targetBasePath);
  ...
  const frontmatter = [
    ...
    description ? `description: ${escapeYaml(description)}` : `description: ${escapeYaml(`${pageTitle} API reference.`)}`,
    '---',
    '',
  ];
...
function assignRoutes(nodes, parentSegments, sourceToRoute) {
  for (const node of nodes) {
    ...
    if (isFolder) assignRoutes(node.children, [...parentSegments, slug], sourceToRoute);
  }
}
...
async function writeNode(output, node, pageTitleBySource, pageDescriptionBySource, sourceToRoute, cheerio) {
  if (node.children.length > 0) {
    ...
    await writeFile(
      ...
      renderPage({
        $: cheerio.load(await fs.readFile(path.join(output.sourceDir, node.sourceName), 'utf8')),
    ...
    for (const child of node.children) {
      await writeNode(output, child, pageTitleBySource, pageDescriptionBySource, sourceToRoute, cheerio);
    }
    return;
  }
...
async function main() {
  ...
  for (const name of fileNames) {
    const html = await fs.readFile(path.join(sourceDir, 'API', name), 'utf8');
  ...
  if (opts.dryRun) {
    console.log(`\n📋 Would process ${fileNames.length} files:`);
  ...
  // Write files
  const output = { sourceDir: path.join(sourceDir, 'API'), targetRoot, targetBasePath };
