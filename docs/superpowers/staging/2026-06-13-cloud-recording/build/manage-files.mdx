...
---
Agora Cloud Recording generates M3U8, TS/WebM, and MP4 files. To process these recorded files, such as [merging audio and video](/en/realtime-media/cloud-recording/build/merge-files), [converting file formats](/en/realtime-media/cloud-recording/build/convert-format), or [synchronizing playback](/en/realtime-media/cloud-recording/build/playback) with other streams, it is essential to understand the following key aspects of recorded files:
...
The naming conventions of [individual audio non-transcoding recording](/en/realtime-media/cloud-recording/build/individual-nontranscoding) are as follows:
...
:::info
If you face compatibility issues in some M3U8 players due to the missing comma after `#EXTINF:<length>`, set `privateParams` in `recordingConfig` in the [start](/en/api-reference/cloud-recording/restful#start) request as follows: `"recordingConfig": {"privateParams":"{\"correctEXTINF\":true}", ...}`.
:::
