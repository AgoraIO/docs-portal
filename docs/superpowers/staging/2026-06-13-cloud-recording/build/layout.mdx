...
---
This page explains the layout types you can choose in a composite recording. See [Composite Recording](/en/realtime-media/cloud-recording/build/composite-mode) for more details about this recording mode
...
Set the `mixedVideoLayout` parameter in `transcodingConfig` as 0 (floating layout), 1 (best fit layout), or 2 (vertical layout) when calling the [`start`](/en/api-reference/cloud-recording/restful#start) or [`updateLayout`](/en/api-reference/cloud-recording/restful#updatelayout) method.
...
To specify a uid, set the `maxResolutionUid` parameter when calling the [`start`](/en/api-reference/cloud-recording/restful#start) or [`updateLayout`](/en/api-reference/cloud-recording/restful#updatelayout) method.
...
Set `mixedVideoLayout` as 3 (customized layout) and pass in the `layoutConfig` parameter when calling the [`start`](/en/api-reference/cloud-recording/restful#start) method to start the recording.
```json
Body:
{
  ......
...
| `render_mode` | Integer | (Optional) The video display mode: - 0: (Default) Cropped mode.
...
During a recording, you can call the [`updateLayout`](/en/api-reference/cloud-recording/restful#updatelayout) method to update the video mixing layout multiple times.
