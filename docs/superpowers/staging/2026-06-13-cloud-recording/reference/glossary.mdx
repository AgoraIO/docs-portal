...
AA is a site for developers to track and analyze the usage and quality of calls.
...
Becoming an audience describes a use-case within an ILS channel (the channel profile is Live-Broadcast) when a host switches the user role and becomes an audience.
...
A callee is a SIG user who receives a [call invitation](#call-invitation).

### Caller

A caller is an SIG user who sends a [call invitation](#call-invitation).

###  Call invitation

Call invitation is a communication protocol based on the peer-to-peer messaging functionality of the Agora SIG SDK. Call invitation supports starting, ending, accepting, and refusing calls.
...
Channel attributes are tags added to SIG channels, including the property name, property value, the ID of the last SIG user who updated the attribute, and the time of the last update.

### Channel message

A channel message is a message that a SIG user sends to all SIG users in a channel.
...
CP is a proxy service that enables users to connect to Agora services through a firewall by using fixed IP addresses.
...
Becoming a host describes a use-case within an ILS channel (the channel profile is Live-Broadcast) when an audience switches the user role and becomes a host.
...
Inject online media stream refers to injecting an online media stream in an ILS channel to share the stream with all users in the channel. The Agora Video SDK provides a method for developers to inject an online mixed audio and video stream or an audio only stream to a channel.

### ILS

Enabled by either Agora’s Video SDK or ASDK, ILS gives you full control over the streaming experience from a standard one-to-many stream to a highly-interactive live event.
...
The mediaplayer kit is a plug-in of the Video SDK to play local and online media resources and publish the media streams to other users in an ILS channel.
...
Offline describes the status of an SIG user who has successfully logged out of SIG.

### Offline message

An offline message is a peer-to-peer message that an online SIG user sends to an offline SIG user.

### Online

Online describes the status of a user who has successfully logged in to the Agora SIG system or stays disconnected from the Agora SIG system for more than 30 seconds.
...
A peer-to-peer message is a message that an online SIG user sends to an online or offline user.
...
You use the SIG SDK to implement real-time messaging use-cases that require low latency and high concurrency for a global audience.
...
In the Agora Video SDK, subscribing is the action of receiving media streams published to the channel. In the Agora SIG SDK, subscribing is the action of monitoring the online status of one or multiple SIG users.
...
A token, also known as a dynamic key, is used for authentication when an app user joins an channel or logs onto the Agora SIG.
...
User attributes are tags added to SIG users, including property names and property values.
...
In the Agora SIG SDK, a user ID identifiers a user in SIG.

The user ID in the Agora Video SDK and the Agora SIG SDK are independent of each other.
