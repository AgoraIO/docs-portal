---
title: "Quickstart"
description: "Rapidly develop and easily enhance your social, work, and educational apps with face-to-face interaction."
---
...
<PlatformStructured platform="android">
<CodeBlockTabs defaultValue="new">
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="new">Create a new project</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="existing">Add to an existing project</CodeBlockTabsTrigger>
</CodeBlockTabsList>

<CodeBlockTab value="new">
...
<CodeBlockTabs defaultValue="tab1">
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="tab1">Maven Central</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="tab2">Manual integration</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
<CodeBlockTabs defaultValue="new">
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="new">npm integration</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="manual">Manual Integration</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
<CodeBlockTabs defaultValue="tab1">
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="tab1">Swift Package Manager</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="tab2">CocoaPods</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="tab3">Manual integration</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
<CodeBlock>{`https://github.com/AgoraIO/AgoraRtcEngine_macOS.git`}</CodeBlock>
    ...
    1. For basic Video Calling, select **RtcBasic**.
...
```
        ...
        Obtain the latest version number from the [release notes](/en/realtime-media/video/reference/release-notes).

    1. Run the `pod install` command in the terminal to install the Video SDK. After successful installation, the terminal shows **Pod installation complete!**.
...
1. Install the Agora server side Web SDK.
...
<CodeBlockTabsList>
  ...
  <CodeBlockTabsTrigger value="cdn">CDN</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
Take the following steps to add the Web Video SDK to your project:

1. Download the latest version of Agora Unreal Video SDK from [Download SDKs](/sdks) and unzip it.
...
- **Authentication token**: A dynamic key that authenticates a user when the client joins a channel. In a production environment, you obtain a token from a [token server](/en/realtime-media/video/build/deploy-token-server) in your security infrastructure. For the purpose of this guide [Generate a temporary token](../get-started/manage-agora-account#generate-temporary-tokens).
...
<Accordions>
  <Accordion title="Sample code to create the user interface">
    ```xml
...
<Accordions>
  <Accordion title="Complete sample code for Video Calling">
    ```dart
    import 'dart:async';
...
<Admonition title="Information" type="info">In the `appId` and `token` fields, enter the corresponding values you obtained from Agora Console. Use the same `channel` name you filled in when generating the temporary token.</Admonition>
...
To learn about the other SDK events, see [`AgoraRtcEngineDelegate`]({{Global.API_REF_IOS_ROOT_RTC_KIT}}/agorartcenginedelegate).
...
### Handle permissions
To access the camera and microphone on Web devices, add the required permissions for real-time interaction. Open the `info.plist` file from the project navigation bar, [edit the property list](https://help.apple.com/xcode/mac/current/#/dev3f399a2a6), to add the required permissions. These permissions are optional. However, if you do not add these permissions, you will not be able to use the corresponding devices.
...
<Accordions>
  <Accordion title="Complete code for `rtc.py`">
    <CodeRtcPy />
  </Accordion>
</Accordions>
</PlatformStructured>
...
- **App ID**: [`appid`](../get-started/manage-agora-account#get-the-app-id) identifies the project you created in Agora Console.
...
<Tabs defaultValue="tab1">
<TabsList>
  <TabsTrigger value="tab1">Android</TabsTrigger>
  <TabsTrigger value="tab2">iOS</TabsTrigger>
</TabsList>
...
<Accordions>
  <Accordion title="Steps to set up a script">
...
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="tab1">Android</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="tab2">iOS and macOS</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
1. When the app starts, ensure that device permissions have been granted.
...
1. To clean up all session-related resources when a user exits the app, call the `Dispose` method of the `IRtcEngine`.
...
<Accordions>
  <Accordion title="Sample code to implement Video Calling in your app">
    ```csharp
...
<Accordions>
  <Accordion title="Create a basic UI">
...
<CodeBlockTabsList>
  ...
  <CodeBlockTabsTrigger value="apple">iOS and macOS</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
When the local user leaves the channel, or exits the app, release memory by calling the `release` method of `IRtcEngine`. Override the `NativeDestruct()` method and add its implementation as follows:
...
<Accordions>
  <Accordion title="<code>Project/Source/Project/AgoraWidget.h</code>">
    ```cpp
    #pragma once
...
<Accordions>
  <Accordion title="<code>Project/Source/Project/AgoraWidget.cpp</code>">
    ```cpp
...
<Accordions>
  <Accordion title="<code>Project/Source/Project/Project.Build.cs</code>">
    ```csharp
...
- If the node is not retrieved at creation time, uncheck **Context Sensitive**. ![](/images/video-sdk/quickstart-implementation-blueprint-3.png)
:::
 |
...
| 4 | **Set Basic Video Call Widget** | Custom | Set up the user interface:1. Create the **BasicVideoCallWidget** variable and select the **Variable Type** of the variable as **BP_VideoWidget**, which is the user interface created to store a reference to the user interface in the blueprint.
...
:::info[Information]
If your target platform is macOS or iOS, please refer to [How do I add the permissions needed for real-time interaction to my Unreal Engine project?](/help/integration-issues/unreal_permissions)
:::
...
<Accordions>
  <Accordion title="Steps to create a minimalistic UI">
...
1. On a second target device, repeat the previous steps to install and launch the app.  Alternatively, use the [Web demo](https://webdemo.agora.io/basicVideoCall/index.html) to join the same channel and test the following use-cases:
...
1. On a second Web device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo.agora.io/basicVideoCall/index.html) to join the same channel and test the following use-cases:
...
4. On a second Web device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo.agora.io/basicVideoCall/index.html) to join the same channel and test the following use-cases:
...
<PlatformStructured platform="react-js">
Use [CodeSandbox](https://codesandbox.io/), to swiftly run the complete demo code and experience the name Video SDK functionality in just a few simple steps.
...
4. Invite a friend to install and run the app on a second device. Alternatively, use the [Web demo](https://webdemo.agora.io/basicVideoCall/index.html) to join the same channel. Once your friend joins the channel, you can see and hear each other.
</PlatformStructured>
<PlatformStructured platform="unity">
Take the following steps to test the sample code:
...
1. Invite a friend to run the demo app on a second device. Use the same `_appID_`, `_token`, and `_channelName` to join. Alternatively, use the [Web demo](https://webdemo.agora.io/basicVideoCall/index.html) to join the same channel.
...
1. Invite a friend to run the demo app on a second device. Use the same same app ID, channel name, and token to join. After your friend joins successfully, you can hear and see each other.
</PlatformStructured>
<PlatformStructured platform="web">
...
- If a firewall is deployed in your network environment, refer to [Connect with Cloud Proxy](/en/realtime-media/video/build/cloud-proxy) to use Agora services normally.
...
- [`RtcEngineConfig`]({{Global.API_REF_ANDROID_ROOT}}/class_rtcengineconfig.html)

- [`create`]({{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_create)
...
- [`enableVideo`]({{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_enablevideo)

- [`startPreview`]({{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_startpreview)

- [`leaveChannel`]({{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel)

- [`IRtcEngineEventHandler`]({{Global.API_REF_ANDROID_ROOT}}/class_irtcengineeventhandler.html#class_irtcengineeventhandler)
...
- [`createAgoraRtcEngine`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_createagorartcengine)

- [`joinChannel`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel2)

- [`enableVideo`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_enablevideo)

- [`startPreview`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_startpreview)

- [`leaveChannel`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel2)

- [`release`]({{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_release)
...
- [`enableVideo`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_enablevideo)

- [`registerEventHandler`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_addhandler)

- [`setClientRole`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_setclientrole2)

- [`setChannelProfile`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_setchannelprofile)

- [`startPreview`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_startpreview2)

- [`AgoraVideoView`]({{Global.API_REF_FLUTTER_ROOT}}/class_agoravideoview.html)

- [`joinChannel`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel2)

- [`leaveChannel`]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel2)
...
### Add a privacy manifest file
 ...
 The Agora Video SDK for Web provides the `PrivacyInfo.xcprivacy` file that contains the required reasons for the APIs used by the SDK. To add the privacy manifest to your app in Xcode, follow these steps:
...
### API reference
<PlatformStructured platform="macos">
- [`sharedEngine`]({{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/sharedengine(withappid:delegate:))

- [`joinChannel`]({{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/joinchannel(bytoken:channelid:info:uid:joinsuccess:))

- [`enableVideo`]({{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/enablevideo())

- [`startPreview`]({{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/startpreview())

- [`leaveChannel`]({{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/leavechannel(_:))

 - [`AgoraRtcEngineDelegate`]({{Global.API_REF_MACOS_ROOT_RTC_KIT}}/agorartcenginedelegate)
</PlatformStructured>

<PlatformStructured platform="ios">
- [`sharedEngine`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/sharedengine(withappid:delegate:))

- [`joinChannel`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/joinchannel(bytoken:channelid:info:uid:joinsuccess:))

- [`enableVideo`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/enablevideo())

- [`startPreview`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/startpreview())

- [`leaveChannel`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/leavechannel(_:))

- [`enableMultiCamera`]({{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/enablemulticamera(_:config:))

- [`AgoraRtcEngineDelegate`]({{Global.API_REF_IOS_ROOT_RTC_KIT}}/agorartcenginedelegate)
...
- [`LocalUser`]({{Global.API_REF_RNJS_ROOT}}/functions/LocalUser.html)
- [`RemoteUser`]({{Global.API_REF_RNJS_ROOT}}/functions/RemoteUser.html)
- [`useIsConnected()`]({{Global.API_REF_RNJS_ROOT}}/functions/useIsConnected.html)
- [`useJoin()`]({{Global.API_REF_RNJS_ROOT}}/functions/useJoin.html)
- [`useLocalMicrophoneTrack()`]({{Global.API_REF_RNJS_ROOT}}/functions/useLocalMicrophoneTrack.html)
- [`usePublish()`]({{Global.API_REF_RNJS_ROOT}}/functions/usePublish.html)
- [`useRemoteUsers()`]({{Global.API_REF_RNJS_ROOT}}/functions/useRemoteUsers.html)
</PlatformStructured>
<PlatformStructured platform="react-native">
### Sample project
...
* [`createAgoraRtcEngine`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_createagorartcengine)

* [`initialize`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_irtcengine_initialize)

* [`registerEventHandler`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_irtcengine_addhandler)

* [`setChannelProfile`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_irtcengine_setchannelprofile)

* [`joinChannel`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel2)

* [`leaveChannel`]({{Global.API_REF_RN_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel2)
...
- [`CreateAgoraRtcEngine`]({{Global.API_REF_UNITY_ROOT}}/class_irtcengine.html#api_createagorartcengine)
...
- [`JoinChannel`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel)

- [`EnableVideo`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_irtcengine_enablevideo)

- [`CreateAgoraRtcEngine`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_createagorartcengine)

- [`SetClientRole`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_irtcengine_setclientrole)

- [`LeaveChannel`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel)

- [`DisableVideo`]({{Global.API_REF_UE_ROOT}}/class_irtcengine.html#api_irtcengine_disablevideo)
...
- [`createAgoraRtcEngine`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_createagorartcengine)

- [`joinChannel`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel2)

- [`enableVideo`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_enablevideo)

- [`startPreview`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_startpreview)

- [`leaveChannel`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_leavechannel)

- [`release`]({{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_release)
...
* [Connection status management](/en/realtime-media/video/build/connection-status-management)
