...
description: "Integrate the Watermark extension to display image watermarks on local and remote audio tracks."
---
...
- You have implemented the [SDK quickstart](/en/realtime-media/video/index) in your project using the Web SDK version 4.24.0 or later.
...
:::info[Note]
- When loading images from URLs, the operation may fail due to [Cross-Origin Resource Policy (CORP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Cross-Origin_Resource_Policy) restrictions. If image loading fails, the `ready` Promise is rejected with an error.
