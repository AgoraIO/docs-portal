...
---
The Super Clarity extension leverages Agora's AI quality enhancement algorithm, which intelligently improves video quality without altering the resolution, thereby optimizing the viewing experience at the receiving end. The AI enhancement algorithm supports over 95% of mobile devices and ensures high-quality video rendering on low-end devices.

<PlatformStructured platform="web">
## Understand the tech

The media transmission pipeline of the Agora Web SDK consists of capture, pre-processing, encoding, transmission, decoding, post-processing, and playback stages. The Super Clarity extension processes video data in the post-processing stage to apply the AI enhancement effect.
...
This section shows you how to integrate the Super Clarity extension into your app.

<PlatformStructured platform="web">
### Integrate the extension

To install the [Super Clarity extension](https://www.npmjs.com/package/agora-extension-super-clarity), do the following:
    ...
    1. To import the Super Clarity extension, use one of the following methods:
...
When subscribing to a remote video track, follow these steps to enable the Super Clarity extension:

1. Call the extension's `createProcessor` method to create the extension's processor and listen to related events.
1. Call the SDK's `IRemoteVideoTrack.pipe` method to connect the pipeline between the Super Clarity extension and the video track.
1. Call the `enable` method of the extension to activate it. After enabling, play the video track to see the effect of the Super Clarity extension.

:::warning[Caution]
To avoid performance issues caused by multiple processors working simultaneously, you can create a maximum of two processors. As a result, the Super Clarity extension can only be applied to a maximum of two video tracks at the same time.
:::
```typescript
...
Although the extension limits the number of `processors` that can be created, you can use the same `processor` to perform Super Clarity processing on multiple video tracks by switching tracks.
...
To stop using the Super Clarity extension, follow these steps to ensure proper cleanup and avoid potential issues:
...
This section presents the minimum code to integrate the Super Clarity extension into your project. Copy the following into your script file:

<Accordions>
  <Accordion title="Complete sample code for the Super Clarity extension">
    ```js
    const extension = new SuperClarityExtension();
    AgoraRTC.registerExtensions([extension]);
...
The Super Clarity extension may not adaptively switch resolutions on iOS devices. As a workaround, when the remote video resolution changes, destroy the processor and then recreate it.

<Accordions>
  <Accordion title="Sample code to destroy the processor and then recreate it.">
    ```js
    const isIOS =
      // @ts-ignore
...
This section provides the API reference for the Super Clarity extension.
...
Enables the Super Clarity function.
```typescript
...
Disables the Super Clarity function.
```typescript
