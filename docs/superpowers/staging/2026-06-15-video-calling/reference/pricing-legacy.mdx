...
:::info
You're viewing information about Agora's previous monthly billing model. For the latest prepaid packages, see [Pricing](/en/realtime-media/video/reference/pricing).
:::
...
<Accordions>
  <Accordion title="Pricing Calculator">
    Use this calculator to estimate your Video Calling costs.
    ...
    :::warning[Disclaimer]
    The costs estimated by this calculator are indicative and may not match actual billing. Accuracy in all cases is not guaranteed, and the calculations should be used as estimates only. Actual billing is based on Agora's official pricing policy and may vary. Please refer to Agora's [Pricing](/en/realtime-media/video/reference/pricing) policy for complete and up-to-date information.
    :::
  </Accordion>
</Accordions>
...
| Additional charge for SDK side screenshot
 when QPS > 500 | $1.5/QPS | First 500 QPS per month are free |
...
If your use-case involves an Agora product other than Video Calling, such as Cloud proxy, WHITE,
or CREC, expect additional charges. See below for Cloud proxy pricing and dedicated pricing pages for
other products.

### Cloud proxy pricing

Typically, about 5% to 10% of the audio and video traffic may require [Cloud proxy](/en/realtime-media/video/build/cloud-proxy). For Cloud proxy, automatic mode is free of charge. Agora offers Force UDP and Force TCP cloud proxy modes with tiered capacity.
...
[AI Noise Suppression](/en/realtime-media/video/build/ai-noise-suppression) helps reduce noise and voice distortion in virtual communication. Agora charges for voice sent to the channel by a host while AI Noise Suppression is enabled.
...
[3D Spatial Audio](/en/realtime-media/video/build/spatial-audio) brings theater-like effects to virtual communication, making it seem as if the sound originates from all around the user.
...
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_ANDROID_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters2).
</PlatformStructured>
<PlatformStructured platform="ios">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_IOS_ROOT_RTC_KIT}}/agorascreencaptureparameters2).
</PlatformStructured>
<PlatformStructured platform="web">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenVideoTrackInitConfig](https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/screenvideotrackinitconfig.html).
</PlatformStructured>
<PlatformStructured platform="macos">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_MACOS_ROOT_RTC_KIT}}/agorascreencaptureparameters2).
</PlatformStructured>
<PlatformStructured platform="windows">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_CPP_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters2).
</PlatformStructured>
<PlatformStructured platform="electron">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_ELECTRON_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters).
</PlatformStructured>
<PlatformStructured platform="unity">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_UNITY_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters).
</PlatformStructured>
<PlatformStructured platform="flutter">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_FLUTTER_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters2).
</PlatformStructured>
<PlatformStructured platform="react-native">
In use-cases involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in [ScreenCaptureParameters]({{Global.API_REF_RN_ROOT}}/rtc_api_data_type.html#class_screencaptureparameters2).
</PlatformStructured>
...
[Billing policies and free-of-charge policy](/en/realtime-media/video/reference/billing-policies)
