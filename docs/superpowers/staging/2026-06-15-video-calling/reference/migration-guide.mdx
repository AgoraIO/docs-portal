...
Web is a new platform for Video SDK v4.x.
</PlatformStructured>
...
Agora SDK v4.0.0 is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Product overview](/en/realtime-media/video/product-overview).
...
Refer to the [quickstart](/en/realtime-media/video/index) to integrate the v4.0.0 SDK into your project.
...
- [What's changed](#whats-changed) between  Video Calling v3.x and v4.x.
...
-   If your channel profile is api-ref-web/interfaces/clientconfig.html#mode[live broadcast], the Video Calling v3.x automatically sets the user role as `host` when publishing the stream. However, in the Video Calling v4.x, you need to call `setClientRole` to set the user role as `host`.
...
1.  Integrate the SDK
    ...
    See [Get started](/en/realtime-media/video/index) for more information about integrating the v4.x SDK into your project.
...
-   Mix multiple video streams and publish them to the remote user through one `localUid`.
    ...
    `RtcChannel` and `RtcEngine` of v3.7.x are partially duplicated and overlap in their functionality, so 4.x hides the `RtcChannel` and `IRtcChannelEventHandler` classes. See the [JoinMultiChannel]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/MultiChannel) sample project for more details on how to replace `RtcChannel` with `joinChannel` and `ChannelMediaOptions`. The expected migration cost is one day or less.
...
See the [JoinChannelVideo]({{GLOBAL.AGORA_API_EXAMPLES_ANDROID_TREE}}/basic/JoinChannelVideo.java) sample project to update the code in your app.
...
-   Custom video source: [CustomVideoSourcePush]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/CustomVideoCapture)

-   Custom video renderer: [CustomVideoRender]({{GLOBAL.AGORA_API_EXAMPLES_ANDROID_TREE}}/advanced/CustomRemoteVideoRender.java)

-   Switching video source: [ScreenShare]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/ScreenShare)
...
In 4.x, you can achieve the same switching speed as `switchChannel` in v3.7.x by switching a channel through `leaveChannel` and `joinChannel`. Therefore, 4.x removes `switchChannel`. If you call `switchChannel` to quickly switch a channel in v3.7.x, see the [VideoQuickSwitch]({{GLOBAL.AGORA_API_EXAMPLES_ANDROID_TREE}}/advanced/VideoQuickSwitch.java) sample project to update the code in your app.
...
<table>
    ...
    <tbody>
    <tr class="odd">
        <td><code>setDefaultAudioRouteToSpeakerphone</code></td>
        <td>- You can only set the audio route before joining a channel.
- This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of <code>setDefaultAudioRouteToSpeakerphone</code> to <code>true</code> or <code>false</code>, calling <code>setEnableSpeakerphone(false)</code> changes the audio route to the earpiece.</td>
        <td>- You can set the audio route either before or after joining a channel.
- This method is a steady API and can change the default audio route of the system. For example, after calling <code>setDefaultAudioRouteToSpeakerphone(true)</code> to set the initial audio route to the speakerphone, calling <code>setEnableSpeakerphone(false)</code> cannot change the audio route to the earpiece.</td>
    </tr>
    <tr class="even">
        <td><code>setEnableSpeakerphone</code></td>
        <td>After connecting external playback devices such as Bluetooth and wired headphones, calling <code>setEnableSpeakerphone</code> cannot switch the audio route to the speakerphone or earpiece.</td>
...
See the [VideoProcessExtensionVideoProcessExtension]({{GLOBAL.AGORA_API_EXAMPLES_ANDROID_TREE}}/advanced/VideoProcessExtension.java) sample project to update the code in your app.
...
See the [JoinChannelVideo]({{GLOBAL.AGORA_API_EXAMPLES_IOS_TREE}}/Basic/JoinChannelVideo) sample project to update the code in your app.
...
-   Custom video source: [CustomVideoSourcePush]({{GLOBAL.AGORA_API_EXAMPLES_IOS_TREE}}/Advanced/CustomVideoSourcePush)

-   Custom video renderer: [CustomVideoRender]({{GLOBAL.AGORA_API_EXAMPLES_IOS_TREE}}/Advanced/CustomVideoRender)

-   Switching video source: [ScreenShare]({{GLOBAL.AGORA_API_EXAMPLES_IOS_TREE}}/Advanced/ScreenShare)
...
<td><code>setDefaultAudioRouteToSpeakerphone</code></td>
<td>- You can only set the audio route before joining a channel.
- This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of <code>setDefaultAudioRouteToSpeakerphone</code> to <code>YES</code> or <code>NO</code>, calling <code>setEnableSpeakerphone(NO)</code> changes the audio route to the earpiece.</td>
<td>- You can set the audio route either before or after joining a channel.
- This method is a steady API and can change the default audio route of the system. For example, after calling <code>setDefaultAudioRouteToSpeakerphone(YES)</code> to set the initial audio route to the speakerphone, calling <code>setEnableSpeakerphone(NO)</code> cannot change the audio route to the earpiece.</td>
</tr>
<tr class="even">
<td><code>setEnableSpeakerphone</code></td>
<td>After connecting external playback devices such as Bluetooth and wired headphones, calling <code>setEnableSpeakerphone</code> cannot switch the audio route to the speakerphone or earpiece.</td>
...
See the [VideoProcess]({{GLOBAL.AGORA_API_EXAMPLES_IOS_TREE}}/Advanced/VideoProcess) sample project to update the code in your app.
...
Agora SDK v4.0.0 is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Benefits and features](/en/realtime-media/video/product-overview).
...
See [Project setup](/en/realtime-media/video/index) for more information about integrating the v4.0.0 SDK into your project.
...
See the [LiveBroadcasting]({{GLOBAL.AGORA_API_EXAMPLES_MACOS_TREE}}/Basic/JoinChannelVideo) sample project to update the code in your app.
...
- Custom video source: [CustomVideoSourcePush]({{GLOBAL.AGORA_API_EXAMPLES_MACOS_TREE}}/Advanced/CustomVideoSourcePush)

- Custom video renderer: [CustomVideoRender]({{GLOBAL.AGORA_API_EXAMPLES_MACOS_TREE}}/Advanced/CustomVideoRender)
- Switching video source: [ScreenCapture]({{GLOBAL.AGORA_API_EXAMPLES_MACOS_TREE}}/Advanced/ScreenShare)
...
See [SDK quickstart](../get-started/get-started-sdk#project-setup) for more information about integrating the `agora_rtc_engine` 6.x SDK into your project.
...
In `agora_rtc_engine` 6.x, <Link to = "{{global.API_REF_FLUTTER_ROOT_PREV_SDK}}/class_rtc_local_view_textureview.html">SurfaceView</Link> are removed. [AgoraVideoView]({{Global.API_REF_FLUTTER_ROOT}}/class_agoravideoview.html) is used instead for video rendering.
...
`RtcChannel` and `RtcEngine` of `agora_rtc_engine` 5.x are partially duplicated and overlap in their functionality, so `agora_rtc_engine` 6.x hides the `RtcChannel` and `RtcChannelEventHandler` classes. See the [JoinMultiChannel]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/MultiChannel) sample project for more details on how to replace `RtcChannel` with `joinChannel` and `ChannelMediaOptions`. The expected migration cost is one day or less.
...
- Adds `options` and remove `info` in `joinChannel`. See [joinChannel]({{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_irtcengine_joinchannel2) for details.
...
Agora SDK v4.x is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Product overview](/en/realtime-media/video/product-overview).

This page introduces the main steps to upgrade the SDK from v3.7.x to v4.x, as well as the related changes.

## Migration steps

This section introduces the main steps to upgrade the SDK from v3.7.x  to v4.x.
...
2. **Update the Agora code in your app**
    ...
    The v4.x SDK has optimized or modified the implementation of some functions, resulting in incompatibility with the v3.7.x  SDK. In order to retain Agora functionality in your app, update the code in your app according to [What has changed](#what-has-changed).

## What has changed

This section is based on v3.7.x  and introduces the main changes of v4.x compared to v3.7.x in the following categories. You need to update the code of your app according to your business use-case.
    ...
    * _Function gaps_: Introduces functions that were supported in v3.7.x but are not supported in v4.x. However, these functions are intended to be added in a future release.
    * _Removed APIs_: Introduces APIs that were supported in v3.7.x but removed in v4.x. Most of these APIs have alternatives in v4.x. Modifying the related implementation should require less time.
...
See the [LiveBroadcasting]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Basic/LiveBroadcasting) sample project to update the code in your app.
...
- Custom video source/Custom video renderer: [CustomVideoCapture]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/CustomVideoCapture)
- Switching video source: [ScreenShare]({{GLOBAL.AGORA_API_EXAMPLES_WINDOWS_TREE}}/Advanced/ScreenShare)
...
Agora SDK v4.0.0 is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see see [Benefits and features](/en/realtime-media/video/product-overview).
...
See the [quickstart](/en/realtime-media/video/index) for more information about integrating the v4.0.0 SDK into your project.
