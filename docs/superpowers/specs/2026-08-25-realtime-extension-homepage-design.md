...
- 每个分组使用现有 `SolutionCardGrid`，设置 `size="compact"`。紧凑网格使用约 12rem 的最小列宽，在首页桌面内容宽度下形成三列布局，在窄屏下为单列。
...
- 在桌面宽度下确认四个分组顺序正确，并呈现三列或当前内容宽度可容纳的密集布局。
- 在窄屏下确认卡片为单列，标题无文本或横向溢出。
...
- 首页扩展能力在桌面为三列或内容宽度可容纳的密集布局，窄屏为单列；卡片仅显示图标、标题和箭头，且无文本或横向溢出。
- 内容生成、类型检查和相关现有测试通过。
