# 播报自定义消息 (/zh-CN/api-reference/conversational-ai/rest-api/agent/speak)
...
- `text` (string, required) - 播报的文本内容，最长不超过 512 字节 (bytes)。
No schema.
- `priority` (string) - 播报行为的优先级，支持设为以下值：
...
No schema.
- `interruptable` (boolean) - 是否允许用户说话打断智能体播报：
- `true`：（默认）允许。
- `false`：不允许。
No schema.
