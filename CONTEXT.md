...
- "Server SDK" was used as a generic label for the in-portal `api-ref/server-sdk` references; resolved: this is **Agora Agents** — the named Conversational AI server SDK (TypeScript/Python/Go) for integrating Agora's Conversational AI server-side, a distinct DevX product. Avoid the generic "Server SDK" name for it.
