...
将 `reliable` 设置为 `true` 后，如果接收方 5 秒没有收到发送方所发送的数据，会触发 [`OnStreamMessageError`](/zh-CN/api-reference/rtc-server-sdk/go-api/localuserobserver.go#onstreammessageerror) 回调并获得相应报错信息。
...
调用 [`createDataStream`](#createdatastream) 创建数据流后，你可以调用该方法向频道内所有用户发送数据流消息。
...
成功调用该方法后，远端会触发 [`OnStreamMessage`](/zh-CN/api-reference/rtc-server-sdk/go-api/localuserobserver.go#onstreammessage) 回调，远端用户可以在该回调中获取接收到的流消息；若调用失败，远端会触发 [`OnStreamMessageError`](/zh-CN/api-reference/rtc-server-sdk/go-api/localuserobserver.go#onstreammessageerror) 回调。

#### 参数

| 参数 | 描述 |
| ---- | ---- |
| `streamId` | 数据流 ID，可以通过 [`CreateDataStream`](#createdatastream) 获取。 |
| `msg` | 待发送的消息数据。 |
...
:::warning[注意]
- 请确保在调用该方法前，已调用 [`create_data_stream`](#createdatastream-2) 创建了数据通道。
- 直播场景下，该方法仅适用于主播用户。
:::
...
| 参数        | 描述   |
|-------------|------------|
| `stream_id` | 数据流 ID。可以通过 [`CreateDataStream`](#createdatastream-2) 获取。|
| `data`      | 待发送的数据。|
