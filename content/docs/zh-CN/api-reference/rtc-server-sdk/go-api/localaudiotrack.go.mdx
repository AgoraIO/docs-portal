...
| `mixMode` | 音频轨道混音模式，详见 [`AudioTrackMixingState`](/zh-CN/api-reference/rtc-server-sdk/go-api/apidatatype.go)。 |
...
SDK 默认的音频发送缓冲区最大容量是 6000，表示支持存放最多 6000 帧数据，即 60 s 的数据。如果你需要更大的缓冲区容量，你可以调用该方法进行调整。如果你还需要设置音频发送缓冲区允许发送的最小数据时长，调用 [`SetSendDelayMs`](/zh-CN/api-reference/rtc-server-sdk/go-api/localaudiotrack.go#setsenddelayms) 方法。
...
| 参数 | 描述 |
|------|------|
| `delayMs` | 缓冲区允许发送的最小数据时长（ms）。该参数需要设置为 10 的倍数，取值范围为大于 0 且小于 [`SetMaxBufferedAudioFrameNumber`](/zh-CN/api-reference/rtc-server-sdk/go-api/localaudiotrack.go#setmaxbufferedaudioframenumber) 方法中 frameNum 参数值的 10 倍。 |
