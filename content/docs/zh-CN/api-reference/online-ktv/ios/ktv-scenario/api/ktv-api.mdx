---
title: K 歌房场景化 Swift API
_migration:
  type: manual-mdx
  status: warning
  sourceUrl: https://doc.shengwang.cn/doc/online-ktv/ios/ktv-scenario/api/ktv-api
  sourcePath: docs/online-ktv/ktv-scenario/api/ktv-api.ios.mdx
  ...
  warnings:
    ...
    - code: unresolved-link
      severity: warning
      message: No local target route for /doc/rtm2/javascript/user-guide/token/token-generation.
    - code: source-only-link-removed
      severity: info
      message: >-
        Rendered unresolved legacy MDX link
        https://doc.shengwang.cn/doc/rtm2/javascript/user-guide/token/token-generation as local
        text.
    ...
    - code: manual-fragment-normalized
      severity: info
      message: Normalized 6 legacy fragment link(s) to stable local anchors.
...
- `rtmToken`：用于音乐内容中心鉴权的 RTM Token，详见 部署 RTM Token 服务器。
...
- `songCode`: 歌曲编号，用于标识一个音乐资源。你可以通过 `searchMusic[1/2]` 或 `searchMusic[2/2]` 获取需要加载的歌曲编号，也可以通过 RESTful API 来获取[曲库所有歌曲列表](/zh-CN/api-reference/online-ktv/ios/ktv-scenario/api/music-content-center#获取曲库所有歌曲列表)或[增量歌曲列表](/zh-CN/api-reference/online-ktv/ios/ktv-scenario/api/music-content-center#获取增量歌曲列表)。
...
KTV API 初始化时默认用户角色为听众，如果需要开始独唱或加入合唱，需要调用 `switchSingerRole` 来切换至相应的角色。你可以参考[切换说明](/zh-CN/api-reference/online-ktv/ios/ktv-scenario/get-started/integrate-ktvapi#5-切换角色)来进行角色切换。
...
-  [`AgoraMusicPlayerProtocol`](/zh-CN/api-reference/rtc/ios/rtc-interface-class#class_imusicplayer) 实例。
...
- [`AgoraMusicContentCenter`](/zh-CN/api-reference/rtc/ios/rtc-interface-class#class_imusiccontentcenter) 实例。
...
由于目前 [`AgoraRtcEngineKit`](/zh-CN/api-reference/rtc/ios/rtc-interface-class#class_irtcengine) 不支持多代理，因此你需要实现该回调获取实时的人声音调来传递给 KTV API 内部，便于打分组件使用。
...
- `status`：歌曲加载状态，详见 [`AgoraMusicContentCenterPreloadStatus`](/zh-CN/api-reference/rtc/ios/enum-preloadstate)。
...
- `state`：播放器的当前状态。详见 [`AgoraMediaPlayerState`](/zh-CN/api-reference/rtc/ios/enum-mediaplayerstate)。
- `error`：播放器的错误码。详见 [`AgoraMediaPlayerError`](/zh-CN/api-reference/rtc/ios/enum-mediaplayerreason)。
- `isLocal`: 是否为本地事件：
...
- `speakers`：领唱的音量信息，详见 [`AgoraRtcAudioVolumeInfo`](/zh-CN/api-reference/rtc/ios/class-audiovolumeinfo) 数组。
...
- `noPermission`：不支持从当前角色切换为目标角色，请参考[切换说明](/zh-CN/api-reference/online-ktv/ios/ktv-scenario/get-started/integrate-ktvapi#5-切换角色)进行角色切换。
...
- `engine`：[`AgoraRtcEngineKit`](/zh-CN/api-reference/rtc/ios/rtc-interface-class#class_irtcengine) 对象。
...
- `musicType`：音乐资源类型，详见 [KTVMusicType](/zh-CN/api-reference/online-ktv/ios/ktv-scenario/api/ktv-api)。
