...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/doc/online-ktv/ios/online-ktv-sdk/advanced-features/lyrics-scoring
  sourcePath: docs/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios.mdx
...
调用 `setScoreLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/api-reference/online-ktv/ios/online-ktv-sdk/api/lyrics-api#setscorelevel)来进行设置。
...
![歌词打分组件](/img/api-center-generated/f0df80064f74-lyrics-scoring-ios.svg)
