...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/doc/console/general/api/guarantee-rest
  sourcePath: docs/console/api/guarantee-rest.mdx
...
4. 如果步骤 3 的重试依然失败，使用与当前解析区域相邻的区域域名重试。
    举例说明：假设你的业务服务器位于欧洲，你设置的主域名为 `api.agora.io`，且主域名解析到了德国。德国位于欧洲中部 api-eu-central-1.agora.io，查域名信息表可知，相邻区域为欧洲西部 api-eu-west-1.agora.io 或 api-eu-west-1.sd-rtn.com。因此，出现网络故障且步骤 2、3 的重试失败时，请使用 api-eu-west-1.agora.io 或 api-eu-west-1.sd-rtn.com 域名重试。
