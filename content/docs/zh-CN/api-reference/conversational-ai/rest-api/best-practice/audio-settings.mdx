...
_migration:
  type: manual-mdx
  status: warning
  sourceUrl: https://doc.shengwang.cn/doc/convoai/restful/best-practice/audio-settings
  sourcePath: docs/convoai/best-practice/audio-settings.mdx
...
集成方式请参考[减小 App 体积](/zh-CN/realtime-media/rtc/build/quality-and-operations/reduce-app-size.android#%E5%88%A0%E9%99%A4%E4%B8%8D%E9%9C%80%E8%A6%81%E7%9A%84%E6%8F%92%E4%BB%B6)。
...
集成方式请参考[减小 App 体积](/zh-CN/realtime-media/rtc/build/quality-and-operations/reduce-app-size.ios#%E5%88%A0%E9%99%A4%E4%B8%8D%E9%9C%80%E8%A6%81%E7%9A%84%E6%8F%92%E4%BB%B6)。
...
将 `convoaiApi` 文件夹拷贝到你的项目中，并在后续调用组件 API 前引入组件。你可以前往[组件结构](#组件结构)了解组件源码结构。
...
你可以通过 CocoaPods、Swift Package Manager 或源码拷贝集成组件。
...
将 `ConversationalAIAPI` 文件夹拷贝到你的项目中，并在后续调用组件 API 前引入组件。你可以前往[组件结构](#组件结构)了解组件源码结构。
...
将 `conversational-ai-api` 文件夹拷贝到你的项目中，并在后续调用组件 API 前引入组件。你可以前往[组件结构](#组件结构)了解组件源码结构。
...
1. **设置音频场景**：在初始化引擎时将音频场景设置为 AI 对话场景。你也可以在加入频道前调用 [`setAudioScenario`](/zh-CN/api-reference/rtc/android/audio/audio-basic#api_irtcengine_setaudioscenario) 方法将音频场景设置为 AI 对话场景。
2. **设置音频相关参数**：在加入频道前和音频路由变化（触发 [`onAudioRouteChanged`](/zh-CN/api-reference/rtc/android/device-management/audio-route#onaudioroutechanged) 回调）时，调用 [`setParameters`](/zh-CN/api-reference/rtc/android/network#api_irtcengine_setparameters) 方法设置音频相关参数。主要包括音频 3A（回声消除（AEC）、降噪（ANS）和增益控制（AGC））插件的开关，以及音频采样率、音频处理模式等设置。各参数推荐设定的值参考示例代码。
...
1. **设置音频场景**：在初始化引擎时将音频场景设置为 AI 对话场景。你也可以在加入频道前调用 [`setAudioScenario`](/zh-CN/api-reference/rtc/ios/audio/audio-basic#api_irtcengine_setaudioscenario) 方法将音频场景设置为 AI 对话场景。
2. **设置音频相关参数**：在加入频道前和音频路由发生变化（触发 [`rtcEngine:didAudioRouteChanged:`](/zh-CN/api-reference/rtc/ios/device-management/audio-route#rtcenginedidaudioroutechanged) 回调）时，调用 [`setParameters`](/zh-CN/api-reference/rtc/ios/network#api_irtcengine_setparameters) 方法设置音频相关参数。主要包括音频 3A（回声消除（AEC）、降噪（ANS）和增益控制（AGC））插件的开关，以及音频采样率、音频处理模式等设置。各参数推荐设定的值参考示例代码。
...
- [`onAudioRouteChanged`](/zh-CN/api-reference/rtc/android/device-management/audio-route#onaudioroutechanged)
...
- [`rtcEngine:didAudioRouteChanged:`](/zh-CN/api-reference/rtc/ios/device-management/audio-route#rtcenginedidaudioroutechanged)
...
- [`loadAudioSettings`](/zh-CN/api-reference/conversational-ai/android/iconversationalaiapi#loadaudiosettings)
...
- [`loadAudioSettings [1/2]`](/zh-CN/api-reference/conversational-ai/ios/conversationalaiapi#loadaudiosettings[1/2])
- [`loadAudioSettings [2/2]`](/zh-CN/api-reference/conversational-ai/ios/conversationalaiapi#loadaudiosettings[2/2])
