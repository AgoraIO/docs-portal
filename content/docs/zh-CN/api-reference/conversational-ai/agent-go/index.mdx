...
设置 LLM vendor。传入 [`NewAliyun`](#newaliyun)、[`NewBytedance`](#newbytedance)、[`NewDeepSeek`](#newdeepseek) 或 [`NewTencentLLM`](#newtencentllm) 的实例。
...
设置 avatar vendor。当前提供 [`NewSensetimeAvatar`](#newsensetimeavatar)。如果已经配置了 TTS，且其采样率与 avatar 所需采样率不一致，则会 **panic**。
