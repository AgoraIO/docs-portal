...
各鉴权模式的详细说明可参考[实现 HTTP 安全认证](/zh-CN/ai/build/http-basic-auth)。
...
所有参数均为可选项。创建后可通过 [builder methods](#builder-methods) 配置各 vendor。
...
<a id="withllmvendor"></a>

#### `with_llm(vendor)`

为级联流程设置 LLM vendor。可传入任意 [LLM vendor class](#llm-vendors)（`AliyunLLM`、`BytedanceLLM`、`DeepSeekLLM`、`TencentLLM` 或 `CustomLLM`）的实例。

```python
with_llm(vendor: BaseLLM) -> Agent
```

<a id="withttsvendor"></a>

#### `with_tts(vendor)`

设置 TTS vendor。该方法会记录 vendor 的 `sample_rate`，用于进行[数字人](#avatar-vendors)校验。

```python
with_tts(vendor: BaseTTS) -> Agent
```

<a id="withsttvendor"></a>
...
使用已绑定到 `Agent(client=...)` 的 client 创建一个 [`AgentSession`](#agentsession-asyncagentsession)。该方法不会启动智能体，需调用 [`session.start()`](#start) 让其入会。智能体实例名通过 `name` 参数在此传入，会在会话启动时发送给 Start Agent API。
...
| `expires_in` | `Optional[int]` | No | token 有效期，单位为秒。仅在自动生成 token 时生效。有效范围为 1–86400。可使用 [`expires_in_hours()`](#expiresinhours-expiresinminutes) 提升可读性 |
...
<a id="agentsession-asyncagentsession"></a>

## AgentSession / AsyncAgentSession

`AgentSession`（sync）和 `AsyncAgentSession`（async）负责管理运行中智能体的完整生命周期。通常通过 [`agent.create_session()`](#createsession) 获取 session；也可在高级场景下直接构造。
...
CN vendor 从 `agora_agent.cn` 导入；可在 CN 场景使用的 common/custom vendor 从 `agora_agent` 顶层导入。

```python
from agora_agent import CustomLLM, GenericTTS
from agora_agent.cn import (
    AliyunLLM, BytedanceLLM, DeepSeekLLM, TencentLLM,
    MiniMaxTTS, TencentTTS, BytedanceTTS, MicrosoftTTS,
    CosyVoiceTTS, BytedanceDuplexTTS, StepFunTTS,
    FengmingSTT, TencentSTT, MicrosoftSTT,
    SenseTimeAvatar, SpatiusAvatar,
)
```
...
与 [`with_llm()`](#withllmvendor) 配合使用。CN provider helper 均复用 OpenAI 兼容协议（`style=openai`），内部设置各自的 vendor 标识。CN 场景下这些 helper 均为 BYOK，要求传入 `base_url`、`model` 和 `api_key`。
...
| `CustomLLM` | `custom` | 任意兼容 OpenAI Chat Completions 协议的 LLM |

以上四个 CN provider helper 共用相同的构造参数：

```python
from agora_agent.cn import AliyunLLM
...
用于接入任意兼容 OpenAI Chat Completions 协议的自定义 LLM 服务。

```python
from agora_agent import CustomLLM

llm = CustomLLM(
    base_url='https://your-llm.example.com/v1/chat/completions',
    model='your-model',
    api_key='your-api-key',
)
```
...
| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| `base_url` | `str` | Yes | — | 自定义 LLM 服务的 OpenAI 兼容 chat completions endpoint |
| `model` | `str` | Yes | — | 模型名称 |
| `api_key` | `str` | Yes | — | 自定义 LLM 服务鉴权 API key |
| `system_messages` | `List[Dict]` | No | `None` | 额外的 system message 列表 |
| `greeting_message` | `str` | No | `None` | 智能体问候语 |
| `failure_message` | `str` | No | `None` | LLM 调用失败时播报的消息 |
| `max_history` | `int` | No | `None` | 保留在 LLM 上下文中的最大对话轮次数 |
| `params` | `Dict[str, Any]` | No | `None` | 附加模型参数 |
| `headers` | `Dict[str, str]` | No | `None` | 转发给 LLM 服务的自定义 HTTP Header |

### TTS vendors

与 [`with_tts()`](#withttsvendor) 配合使用。`sample_rate` 选项决定数字人兼容性，详见 [`with_avatar()`](#withavatarvendor)。
...
```python
from agora_agent.cn import MiniMaxTTS
```
...
```python
from agora_agent.cn import TencentTTS
```
...
```python
from agora_agent.cn import BytedanceTTS
```
...
```python
from agora_agent.cn import MicrosoftTTS
```
...
自定义 OpenAI 兼容 HTTP TTS。`url` 必填，且必须是包含 host 的 HTTP 或 HTTPS endpoint；地址格式错误或使用 `ws`、`wss` 等非 HTTP(S) 协议时会抛出 `ValueError`。合法地址会序列化为 `tts.vendor = "generic_http"`。
...
| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| `url` | `str` | Yes | — | 自定义 TTS 服务的 HTTP(S) endpoint |
| `headers` | `Dict[str, str]` | No | `None` | 转发给 TTS 服务的自定义 HTTP Header；未设置时不会写入 `headers` |
| `api_key` | `str` | No | `None` | TTS 服务鉴权 API key |
| `model` | `str` | No | `None` | TTS 模型名|
| `voice` | `str` | No | `None` | 音色名 |
| `speed` | `float` | No | `None` | 语速 |
| `sample_rate` | `int` | No | `None` | 输出音频采样率，单位为 Hz。如果你的 TTS 服务暂不支持多采样率，请确保返回音频的采样率与该配置一致 |
| `response_format` | `str` | No | `None` | 输出音频格式；当前 ConvoAI 支持 `pcm` |
| `instruction` | `str` | No | `None` | 语音风格、情绪或其他播报指令 |
| `additional_params` | `Dict[str, Any]` | No | `None` | 透传给 TTS 服务的其他参数；同名字段会被显式参数覆盖 |
| `skip_patterns` | `List[int]` | No | `None` | 控制 TTS 跳过特定文本模式。 |

#### CosyVoiceTTS

```python
from agora_agent.cn import CosyVoiceTTS
```
...
```python
from agora_agent.cn import BytedanceDuplexTTS
```
...
```python
from agora_agent.cn import StepFunTTS
```
...
```python
from agora_agent.cn import FengmingSTT
...
```python
from agora_agent.cn import TencentSTT
```
...
```python
from agora_agent.cn import MicrosoftSTT
```
...
与 [`with_avatar()`](#withavatarvendor) 配合使用。
...
```python
from agora_agent.cn import SenseTimeAvatar
```

| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| `agora_token` | `str` | No | `None` | 数字人发布者使用的 RTC token；未传入时由 `AgentSession` 生成 |
| `agora_uid` | `str` | Yes | — | 数字人 RTC 发布者 uid |
| `appId` | `str` | No | `None` | 商汤 app id |
| `app_key` | `str` | Yes | — | 商汤 app key |
| `sceneList` | `List[Dict[str, Any]]` | No | `None` | 商汤场景列表 |
...
数字人 vendor。`required_sample_rate` 返回 `sample_rate` 字段值；未设置时返回 `0`，即不强制要求特定的 TTS 采样率。

```python
from agora_agent.cn import SpatiusAvatar
```
...
| `agora_uid` | `str` | Yes | — | 数字人服务使用的 Agora UID |
| `agora_token` | `str` | No | `None` | 数字人发布者使用的 RTC token；未传入时由 `AgentSession` 生成 |
| `region` | `str` | No | `None` | Spatius 服务区域，例如 `cn-beijing` |
| `sample_rate` | `int` | No | `None` | 音频采样率，单位 Hz |
| `session_expire_minutes` | `int` | No | `None` | Spatius session 有效时长，单位分钟 |
| `enable` | `bool` | No | `None` | 是否启用数字人 |
| `additional_params` | `Dict[str, Any]` | No | `None` | 附加 Spatius 数字人参数 |
...
生成一个同时具备 RTC 和 RTM 权限的 Conversational AI token。这与 SDK 在 app-credentials 模式下自动生成的 token 相同。当你需要把预生成 token 传给 [`create_session()`](#createsession) 时，可以使用该方法。
...
<a id="expiresinhours-expiresinminutes"></a>

### `expires_in_hours()` / `expires_in_minutes()`

用于指定 token 生命周期的辅助函数。可与 [`create_session()`](#createsession) 或 token 生成函数配合使用。函数会校验输入值，并在超过声网上限 86400 秒（24 小时）时自动截断。
...
控制智能体在处理 [`say()`](#saytext-prioritynone-interruptablenone) 调用时，相对于当前行为应如何处理消息。通过字符串传给 `session.say()`。
