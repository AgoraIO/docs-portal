...
为级联流程设置 LLM vendor。可传入任意 [LLM vendor class](#llm-vendors)（`AliyunLLM`、`BytedanceLLM`、`DeepSeekLLM` 或 `TencentLLM`）的实例。
...
与 [`with_llm()`](#withllmvendor) 配合使用。LLM helper 均复用 OpenAI 兼容协议（`style=openai`），内部设置各自的 vendor 标识。CN 场景下这些 helper 均为 BYOK，要求传入 `base_url`、`model` 和 `api_key`。
