...
import {
  ...
  CosyVoiceTTS, BytedanceDuplexTTS, StepFunTTS, GenericTTS,
...
自定义 OpenAI 兼容 HTTP TTS。`url` 必填，且必须是 HTTP 或 HTTPS 绝对地址；缺失 `url`、地址格式错误或使用 `ws:`、`wss:` 等非 HTTP(S) 协议时，constructor 会抛出错误。合法地址会序列化为 `tts.vendor = "generic_http"`。
```typescript
...
| `headers` | `Record<string, string>` | No | 转发给 TTS 服务的自定义 HTTP Header；未设置时不会写入 `headers` |
| `apiKey` | `string` | No | TTS 服务鉴权 API key |
| `model` | `string` | No | TTS 模型名 |
| `voice` | `string` | No | 音色名 |
| `speed` | `number` | No | 语速 |
| `sampleRate` | `number` | No | 输出音频采样率，单位为 Hz。如果你的 TTS 服务暂不支持多采样率，请确保返回音频的采样率与该配置一致。 |
| `responseFormat` | `"pcm"` | No | 输出音频格式；当前 ConvoAI 支持 `pcm` |
| `instruction` | `string` | No | 语音风格、情绪或其他播报指令 |
| `additionalParams` | `Record<string, unknown>` | No | 透传给 TTS 服务的其他参数；同名字段会被显式字段覆盖 |
| `skipPatterns` | `number[]` | No | 控制 TTS 跳过特定文本模式 |
...
与 [`withAvatar()`](#withavatarvendor) 配合使用。数字人 vendor 会声明所需 TTS 采样率；如果返回 `0`，则不强制要求特定的 TTS 采样率。
...
Spatius 数字人。如果 `spatiusApiKey`、`spatiusAppId`、`spatiusAvatarId` 或 `agoraUid` 为空，构造函数会抛出错误。
...
| `sampleRate` | `16000 \| 24000` | No | 写入 Spatius vendor 参数的音频采样率，单位 Hz |
| `sessionExpireMinutes` | `number` | No | Spatius 会话过期时间，单位分钟 |
| `enable` | `boolean` | No | 是否启用数字人。默认值：`true` |
| `additionalParams` | `Record<string, unknown>` | No | 附加 Spatius 数字人参数，合并进 `params` |
