...
RTM 提供消息、状态等事件通知能力。通过设置事件监听，你可以接收已订阅频道中的消息和事件。关于如何添加和设置事件监听，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.harmonyos#事件监听)章节。

通过调用 `subscribe` 方法，客户端可以订阅 Message Channel 并开始接收频道中的消息和事件通知。成功调用该方法后，订阅该频道且开启 Presence 事件监听的用户会收到 `REMOTE_JOIN_CHANNEL` 类型的 `presence` 事件，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.harmonyos#事件监听)。
...
如果你不再需要关注某个频道，可调用 `unsubscribe` 方法取消订阅该频道。成功取消订阅该频道后，其他订阅该频道并开启事件监听的用户会收到 `REMOTE_LEAVE_CHANNEL` 类型的 `presence` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.harmonyos#事件监听)。
...
成功离开频道后，频道中的远端用户会收到 `REMOTE_LEAVE_CHANNEL` 类型的 `presence` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.harmonyos#事件监听)。
...
- `0`：方法调用成功。
- 非 `0`：方法调用失败，返回对应的错误码。详见[错误排查](/zh-CN/realtime-media/rtm/error-codes.android.harmonyos)。
