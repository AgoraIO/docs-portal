...
_migration:
  type: manual-mdx
  status: warning
  sourceUrl: https://doc.shengwang.cn/api-ref/rtm2/cpp/toc-channel/channel
  sourcePath: docs-api-reference/rtm2/toc-channel/channel.cpp.mdx
  generator: legacy-mdx
  warnings:
    - code: manual-mdx-normalized
      severity: info
      message: Normalized 15 legacy MDX syntax signal(s).
...
- 方法调用成功：一个指向 [`IStreamChannel`](/zh-CN/api-reference/rtm/toc-channel/channel.cpp) 对象的指针。
- 方法调用失败：`NULL`。
