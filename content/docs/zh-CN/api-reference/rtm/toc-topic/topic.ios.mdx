...
成功加入 Topic 后，订阅该 Topic 并添加事件监听的用户会收到 `AgoraRtmTopicEventTypeRemoteJoinTopic` 类型的 `didReceiveTopicEvent` 事件通知。详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.ios#事件监听)。
...
|     属性     |  类型  |  是否必填  |  默认值  |                     描述                     |
| -------------- | ---- | ----- | --- | ---------- |
| `qos` | `AgoraRtmMessageQos` |  选填  | `AgoraRtmMessageQosUnordered`    | 定义此 Topic 中传输数据是否保序。详见 [`AgoraRtmMessageQos`](/zh-CN/api-reference/rtm/enumv.android.ios#messageqos)。 |
|     `priority`     |  `AgoraRtmMessagePriority` |  选填  |   `AgoraRtmMessagePriorityNormal`      | 定义此 Topic 中传输数据的优先级（相比于同频道中的其他 Topic）。详见 [`AgoraRtmMessagePriority`](/zh-CN/api-reference/rtm/enumv.android.ios#messagepriority)。 |
...
用户发送的消息在传输的过程中会被 TLS 加密，数据链路加密默认开启且无法关闭。用户也可以在出初始化的时候开启端侧加密，以获得更高的数据安全等级，详见[初始配置](/zh-CN/api-reference/rtm/toc-configuration/configuration.ios)。
...
成功调用该方法后，订阅该频道并开启事件监听的用户会收到 `AgoraRtmTopicEventTypeRemoteLeaveTopic` 类型的 `didReceiveTopicEvent` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.ios#事件监听)。
