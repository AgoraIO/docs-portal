...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtm2/android/toc-topic/topic
  sourcePath: docs-api-reference/rtm2/toc-topic/topic.android.mdx
  generator: legacy-mdx
  sourceUrls:
    - https://doc.shengwang.cn/api-ref/rtm2/android/toc-topic/topic
    - https://doc.shengwang.cn/api-ref/rtm2/flutter/toc-topic/topic
    - https://doc.shengwang.cn/api-ref/rtm2/harmonyos/toc-topic/topic
    - https://doc.shengwang.cn/api-ref/rtm2/ios/toc-topic/topic
    - https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-topic/topic
    - https://doc.shengwang.cn/api-ref/rtm2/unity/toc-topic/topic
  sourcePaths:
    - docs-api-reference/rtm2/toc-topic/topic.android.mdx
    - docs-api-reference/rtm2/toc-topic/topic.flutter.mdx
    - docs-api-reference/rtm2/toc-topic/topic.harmonyos.mdx
    - docs-api-reference/rtm2/toc-topic/topic.ios.mdx
    - docs-api-reference/rtm2/toc-topic/topic.javascript.mdx
    - docs-api-reference/rtm2/toc-topic/topic.unity.mdx
  platforms:
    - android
    - flutter
    - harmonyos
  ...
  warnings:
    - code: manual-mdx-normalized
      severity: info
      message: Normalized 21 legacy MDX syntax signal(s).
    - code: manual-platform-merge
      severity: info
      message: >-
        Merged 6 platform-specific MDX sources for
        content/docs/zh-CN/api-reference/rtm/toc-topic/topic.mdx.
    - code: manual-fragment-normalized
      severity: info
      message: Normalized 11 legacy fragment link(s) to stable local anchors.
---
...
- [Topic](/zh-CN/realtime-media/rtm/build/channels-and-topics/topics/topic-basic)
...
- 在执行此操作之前，用户需要首先创建 `StreamChannel` 对象实例，并调用 `join` 方法加入频道，详见[频道](/zh-CN/realtime-media/rtm/build/channels-and-topics/channel-basic)。
:::
...
|      属性       |     类型     | 是否必填 | 默认值               |    描述    |
| ------------ | -------------- | ----- | ---------------- | ----------- |
|  `messageQos`   | `RtmMessageQos` |   选填   | `UNORDERED` |                  定义此 Topic 中传输数据是否保序。详见 [`RtmMessageQos`](/zh-CN/api-reference/rtm/ios/enumv.android#messageqos)。                  |
|   `priority`    |  `RtmMessagePriority`  |   选填   | `NORMAL`    | 定义此 Topic 中传输数据的优先级（相比于同频道中的其他 Topic）。详见 [`RtmMessagePriority`](/zh-CN/api-reference/rtm/ios/enumv.android#messagepriority)。 |
...
同一个用户在同一个频道中最多只能同时订阅 50 个 Topic，且每个 Topic 中最多只能订阅 64 个消息发布者。详见 [API 使用限制](/zh-CN/realtime-media/rtm/reference/usage-limits-and-errors/api-limits)。
...
| `qos` | `RtmMessageQos` | 选填 | `unordered` | 定义此 Topic 中传输数据是否保序。详见 [`RtmMessageQos`](/zh-CN/api-reference/rtm/ios/enumv.flutter#messageqos)。 |
| `priority` | `RtmMessagePriority` | 选填 | `normal` | 定义此 Topic 中传输数据的优先级（相比于同频道中的其他 Topic）。详见 [`RtmMessagePriority`](/zh-CN/api-reference/rtm/ios/enumv.flutter#messagepriority)。 |
...
|      属性       |     类型     | 是否必填 | 默认值               |    描述    |
| ------------ | -------------- | ----- | ---------------- | ----------- |
|  `qos`   | `RtmMessageQos` |   选填   | `UNORDERED` |                  定义此 Topic 中传输数据是否保序。详见 [`RtmMessageQos`](/zh-CN/api-reference/rtm/ios/enumv.harmonyos#messageqos)。                  |
|   `priority`    |  `RtmMessagePriority`  |   选填   | `NORMAL`    | 定义此 Topic 中传输数据的优先级（相比于同频道中的其他 Topic）。详见 [`RtmMessagePriority`](/zh-CN/api-reference/rtm/ios/enumv.harmonyos#messagepriority)。 |
|   `meta`   |    string    |   选填   | `''`                    |         加入 Topic 时携带的其他辅助信息。          |
| `syncWithMedia` |   boolean    |   选填   | `false`              |        此 Topic 中发送的数据是否与共频道的 RTC 音视频数据流实现数据同步（时间戳对齐）。        |
...
:::info[信息]
- 当前 RTM 支持单客户端在同一个 Stream Channel 中同时加入 8 个 Topic。
- 在执行此操作之前，用户需要首先创建 `AgoraRtmStreamChannel` 对象实例，并调用 `joinWithOption` 方法加入频道，详见[频道](/zh-CN/realtime-media/rtm/build/channels-and-topics/channel-basic)。
:::
...
|     属性     |  类型  |  是否必填  |  默认值  |                     描述                     |
| -------------- | ---- | ----- | --- | ---------- |
| `qos` | `AgoraRtmMessageQos` |  选填  | `AgoraRtmMessageQosUnordered`    | 定义此 Topic 中传输数据是否保序。详见 [`AgoraRtmMessageQos`](/zh-CN/api-reference/rtm/ios/enumv.ios#messageqos)。 |
|     `priority`     |  `AgoraRtmMessagePriority` |  选填  |   `AgoraRtmMessagePriorityNormal`      | 定义此 Topic 中传输数据的优先级（相比于同频道中的其他 Topic）。详见 [`AgoraRtmMessagePriority`](/zh-CN/api-reference/rtm/ios/enumv.ios#messagepriority)。 |
...
:::info[信息]
- 当前 RTM 支持单客户端在同一个 Stream Channel 中同时加入 8 个 Topic。
- 在执行此操作之前，用户需要首先创建 `RTMStreamChannel` 对象实例，并调用 `join` 方法加入频道，详见[频道](/zh-CN/realtime-media/rtm/build/channels-and-topics/channel-basic)。
:::
...
:::info[信息]
- 当前 RTM 支持单客户端在同一个 Stream Channel 中同时加入 8 个 Topic。
- 在执行此操作之前，用户需要首先创建 `IStreamChannel` 对象实例，并调用 `JoinAsync` 方法加入频道，详见[频道](/zh-CN/realtime-media/rtm/build/channels-and-topics/channel-basic)。
:::
