...
- [用户鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication.android.flutter)

## renewToken

### 接口描述

为保证及时更新 Token，声网推荐你监听 `token` 回调，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.flutter#事件监听)。成功添加事件监听后，当 Token 还有 30 秒过期时，SDK 会触发 `token` 回调，提醒用户及时更新。
