...
- [用户鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication.unity)
...
为保证及时更新 Token，声网推荐你监听 `OnTokenPrivilegeWillExpire` 回调，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.unity#事件监听)。成功添加事件监听后，当 RTM Token 还有 30 秒过期时，SDK 会触发 `OnTokenPrivilegeWillExpire` 回调，提醒用户 Token 即将过期。收到该回调时，你可以在服务端重新生成 RTM Token，然后调用 `RenewTokenAsync` 方法，将新生成的 RTM Token 传给 SDK。
...
为保证及时更新 Token，声网推荐你监听 `OnTokenPrivilegeWillExpire` 回调，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.unity#事件监听)。成功添加事件监听后，当 RTC Token 还有 30 秒过期时，SDK 会触发 `OnTokenPrivilegeWillExpire` 回调，提醒用户 Token 即将过期。收到该回调时，你可以在服务端重新生成 RTC Token，然后调用 `RenewTokenAsync` 方法，将新生成的 RTC Token 传给 SDK。
