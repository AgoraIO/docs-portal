...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtm2/cpp/toc-lock/lock
  sourcePath: docs-api-reference/rtm2/toc-lock/lock.cpp.mdx
...
配置成功后，频道中的所有用户会收到 `RTM_LOCK_EVENT_TYPE_LOCK_SET` 类型的 `onLockEvent` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.cpp#事件监听)。
...
成功获取锁后，频道中的其他用户会收到 `RTM_LOCK_EVENT_TYPE_LOCK_ACQUIRED` 类型的 `onLockEvent` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.cpp#事件监听)。
...
成功释放后，频道中的其他用户会收到 `RTM_LOCK_EVENT_TYPE_LOCK_RELEASED` 类型的 `onLockEvent` 事件，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.cpp#事件监听)。
...
成功收回后，频道中的所有用户会收到 `RTM_LOCK_EVENT_TYPE_LOCK_RELEASED` 类型的 `onLockEvent` 事件，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.cpp#事件监听)。
...
成功删除锁后，频道中的所有用户会收到 `RTM_LOCK_EVENT_TYPE_LOCK_REMOVED` 类型的 `onLockEvent` 事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.cpp#事件监听)。
