...
成功调用该方法后，订阅了该频道并开启了事件监听的其他用户会通过 `didReceiveMessageEvent` 收到这条消息，详见[事件监听](/zh-CN/api-reference/rtm/ios/configuration#事件监听)。
...
:::info[信息]
成功调用该方法后，SDK 会触发 `didReceiveMessageEvent` 事件通知。订阅了该频道并且开启了事件监听的用户将会收到该事件通知，详见[事件监听](/zh-CN/api-reference/rtm/ios/configuration#事件监听)。
:::
...
关于如何添加和设置事件监听，详见[事件监听](/zh-CN/api-reference/rtm/ios/configuration#事件监听)章节。
