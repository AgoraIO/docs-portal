...
| `areaCode` | `RTM_AREA_CODE` | 选填 | `RTM_AREA_CODE_GLOB` | 服务区域代码，你可以根据自己业务部署的区域进行选择，详见 [`RTM_AREA_CODE`](/zh-CN/api-reference/rtm/enumv.android.cpp#areacode)。 |
| `protocolType` | `RTM_PROTOCOL_TYPE` | 选填 | `RTM_PROTOCOL_TYPE_TCP_UDP` | 消息传输协议类型。RTM 默认使用一路 TCP 和一路 UDP 协议进行传输，你可以根据需要修改协议类型。详见 [`RTM_PROTOCOL_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#protocoltype)。 |
...
| `level` | `RTM_LOG_LEVEL` | 选填 | `RTM_LOG_LEVEL_INFO` | 日志信息的输出等级，详见 [`RTM_LOG_LEVEL`](/zh-CN/api-reference/rtm/enumv.android.cpp#loglevel)。 |
...
| `proxyType` | `RTM_PROXY_TYPE` |  选填  | `RTM_PROXY_TYPE_NONE` | Proxy 协议类型。详见 [`RTM_PROXY_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#proxytype)。 |
...
| 属性 | 类型 | 是否必填 | 默认值 | 描述 |
| --- | --- | --- | --- | --- |
| `encryptionMode` | `RTM_ENCRYPTION_MODE` | 选填 | `RTM_ENCRYPTION_MODE_NONE` | 加密方式。详见 [`RTM_ENCRYPTION_MODE`](/zh-CN/api-reference/rtm/enumv.android.cpp#encryptionmode)。 |
...
| 属性          | 类型                  |  是否必填  |  默认值  | 描述                      |
|-----|-------|--------|------------| ---------------- |
| `serviceType`    | `RTM_SERVICE_TYPE` |  选填  | - | 服务类型。详见 [`RTM_SERVICE_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#rtmservicetype)。 |
...
| <code className="index-group-item-rtmeventlistener index-api-onconnection" id="onConnectionStateChanged">onConnectionStateChanged</code> | <span className="index-desc-onconnection">（已废弃）接收客户端网络连接状态变更的事件通知。详见 [SDK 连接状态](/zh-CN/api-reference/rtm/enumv.android.cpp#connectionstate)和 [SDK 连接状态改变原因](/zh-CN/api-reference/rtm/enumv.android.cpp#connectionreason)。</span> |
...
| `messageType`            | `RTM_MESSAGE_TYPE`    |  消息类型。详见 [`RTM_MESSAGE_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#messagetype)。   |
...
|----|-----| ----- |
| `channelType`            | `RTM_CHANNEL_TYPE`    |  频道类型。详见 [`RTM_CHANNEL_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#channeltype)。   |
| `eventType`            | `RTM_LOCK_EVENT_TYPE`    |  Lock 事件类型。详见 [`RTM_LOCK_EVENT_TYPE`](/zh-CN/api-reference/rtm/enumv.android.cpp#locktype)。    |
| `channelName`              | const char*                |  频道名称。   |
...
登录 RTM 系统的 Token。
    ...
    - 如果你的项目开启了 Token 鉴权，你可以填入 [RTM 临时 Token](/zh-CN/realtime-media/rtm/get-started/enable-service#5-获取临时-token) 或 Token 服务器生成的 RTM 正式 Token。详见[客户端鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication.android.cpp)和[部署 RTM Token 服务器](/zh-CN/realtime-media/rtm/user-guide/token/token-generation)。
