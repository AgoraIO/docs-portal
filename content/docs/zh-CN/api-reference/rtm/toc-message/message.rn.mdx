...
成功调用该方法后，订阅了该频道并开启了事件监听的其他用户会通过 `message` 收到这条消息，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.rn#事件监听)。
...
|     属性     |  类型  | 是否必填 | 默认值 | 描述          |
| --------- | --- | ----- | --- | -------- |
|   `channelType`   |  RtmChannelType  |   选填   |  message   | 频道类型。支持的取值见[频道类型](/zh-CN/api-reference/rtm/enumv.rn#channeltype)。 |
| `messageType` | RtmMessageType | 选填 | binary | 消息类型。详见[消息类型](/zh-CN/api-reference/rtm/enumv.rn#messagetype)。
...
:::info[信息]
成功调用该方法后，SDK 会触发 `messsage` 事件通知。订阅了该频道并且开启了事件监听的用户将会收到该事件通知，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.rn#事件监听)。
:::
...
| `channelType` | RtmChannelType | 必填 | - | 频道类型。详见 [RtmChannelType](/zh-CN/api-reference/rtm/enumv.rn#channeltype)。 |
...
关于如何添加和设置事件监听，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.rn#事件监听)章节。
