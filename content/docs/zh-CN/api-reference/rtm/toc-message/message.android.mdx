...
成功调用该方法后，订阅了该频道并且开启了事件监听的用户会通过 `onMessageEvent` 收到这条消息，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.android#事件监听)。
...
关于如何添加和设置事件监听，详见[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration.android#事件监听)章节。
