...
- 用户选择的区域错误。 请检查 [`launch`](/api-ref/flexible-classroom/javascript/classroom-sdk#launch) 方法的 `region` 字段是否设置了正确的区域。
...
`600001-1` 和 `401` 错误一般是因为 [`launch`](/api-ref/flexible-classroom/javascript/classroom-sdk#launch) 方法传入的 Token 错误，或者 Token 和 App ID 不匹配。请检查 Token 是否正确，以及保证 App ID 和 Token 匹配。

声网推荐你在后端生成 Token，前端调用生成的 Token，详情参考[使用 Token 鉴权](/doc/flexible-classroom/javascript/basic-features/generate-token)。
...
其他相关服务端错误码可参考[响应状态码](/doc/flexible-classroom/restful/response-code)。

### Error Domain=last launch not finished Code=-1 "(null)"

该错误错误一般是因为 [`launch`](/api-ref/flexible-classroom/javascript/classroom-sdk#launch) 方法传入的 Token 错误或过期，或者 Token 和 App ID 或 User ID 不匹配。请检查 Token 是否正确和有效，以及保证 Token 和 App ID 以及 User ID 匹配。
