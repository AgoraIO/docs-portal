...
如果你需要添加更多语言，只需要在指定目录找到语言相关的 key 值，进行修改即可。
...
![增加语言文件](https://web-cdn.agora.io/docs-files/1680084679972)

  2. 在 `packages/agora-classroom-sdk/src/infra/api/index.tsx` 文件中增加一行对应的 `addResourceBundle();` 代码，位置如下图所示：

![增加 addResourceBundle](https://web-cdn.agora.io/docs-files/1680084689520)
  ...
  :::info[信息]
  `addResourceBundle()` 中所填的参数只要具有唯一性且容易对应到该语言即可。
  :::
...
![增加语言](https://web-cdn.agora.io/docs-files/1680084701486)

  2. 在 `packages/agora-plugin-gallery/src/gallery/answer/i18n/config.ts` 文件中增加一行 `import` 声明和一行对应的 `addResourceBundle();` 代码，位置如下图所示：

![增加 addResourceBundle](https://web-cdn.agora.io/docs-files/1680084710288)
...
![AgoraClassSDK 配置](https://web-cdn.agora.io/docs-files/1680084721653)

- `/AgoraEduUIKit/src/main/res`

![AgoraEduUIKit 配置](https://web-cdn.agora.io/docs-files/1680084729669)

  例如：你想要添加日语，那么你需要在以上目录下增加一个 `value-jp` 文件夹，内有一个 `strings.xml` 文件。

## iOS 端多语言配置

参考如下步骤在 iOS 端添加多语言配置：

1. 在 `others ` 文件夹下，复制一个原有的语言文件夹（例如 `en.Iproj`），重命名为新语言对应的名称（例如 `jp.Iproj`）。

   :::info[信息]
   文件夹名只要具有唯一性且容易对应到该语言即可，但后缀必须为 `.lproj`。
   :::
