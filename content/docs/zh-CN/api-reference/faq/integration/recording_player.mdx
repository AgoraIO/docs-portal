...
- 在单流录制模式下，会生成每个用户的音频文件和视频文件，通常需要[使用转码脚本](/doc/recording/cpp/advanced-features/merge-files)将这些文件合成为一个 MP4 文件播放。

- 在单流录制模式下，会生成每个用户的音频文件和视频文件，通常需要[使用转码脚本](/doc/recording/java/advanced-features/merge-files)将这些文件合成为一个 MP4 文件播放。
