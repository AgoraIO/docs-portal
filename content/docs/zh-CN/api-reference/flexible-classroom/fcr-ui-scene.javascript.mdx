...
| `token` | （必填）用于鉴权的 RTM Token。详见[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/basic-features/generate-token.android.javascript)。 |
...
| `devicePretest` | （必填）是否开启课前设备检测：<li>`true`: 开启课前设备检测。开启后，在加入课堂前会弹出设备检测页面，测试终端用户的摄像头、麦克风和扬声器是否能正常工作。</li><li>`false`: 不开启课前设备检测。</li> |
...
（必填）用户 ID。这是用户的全局唯一标识，**需要与你生成 RTM Token 时使用的 UID 一致**。填写方式见 [userUuid 的填写方式](/zh-CN/solutions/flexible-classroom/get-started/integrate.javascript)。同时需满足长度在 64 字节以内。以下为支持的字符集范围（共 89 个字符）:
