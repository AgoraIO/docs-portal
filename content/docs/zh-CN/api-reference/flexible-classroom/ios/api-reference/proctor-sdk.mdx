...
| `appId` | 声网 App ID。 |
| `token` | 用于鉴权的 Token。 详见[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/build/setup-and-access/generate-token)。 |
...
| `userProperties` | （选填）由开发者自定义的用户属性，会传入 [`AgoraEduUserContext`](/zh-CN/api-reference/flexible-classroom/ios/api-reference/edu-context/class-usercontext) 的 `userProperties`，详见[如何设置自定义用户属性](/zh-CN/api-reference/faq/integration/agora_class_custom_properties)。 |
