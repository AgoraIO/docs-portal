...
| `userProperties` | （选填）由开发者自定义的用户属性，会传入 [`AgoraEduUserContext`](/zh-CN/api-reference/flexible-classroom/ios/api-reference/edu-context/class-usercontext) 的 `userProperties`，详见[如何设置自定义用户属性](/zh-CN/reference/faq/integration/agora_class_custom_properties)。 |
