...
本页提供声网 Classroom SDK for Electron 的 TypeScript API 参考。
...
| 参数 | 描述 |
| --- | --- |
| `rtmToken` | （必填）用于鉴权的 RTM Token。详见[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/build/create-classroom/generate-token)。 |
...
| `widgets` | （选填）扩展插件集合，扩展教室能力，详见[自定义插件](/zh-CN/solutions/flexible-classroom/build/customize-classroom-experience/widget)。 |
| `courseWareList` | （选填）教育机构指派的课件配置，客户端无法编辑。详见 [`CourseWareList`](#coursewarelist)。配置后，SDK 会在启动课堂时将相应的课件从声网云盘组件中下载至本地。 |
| `userFlexProperties` | （选填）由开发者自定义的用户属性。详见[如何设置自定义用户属性？](/zh-CN/reference/faq/integration/agora_class_custom_properties)。 |
