...
| [IAccountMgr](#iaccountmgr) | 账号注册、登录与登出 |
| [ICallkitMgr](#icallkitmgr) | 来电呼叫管理，包括拨号、接听、挂断等 |
| [IDeviceMgr](#idevicemgr) | 设备管理接口，包括设备绑定、设备解绑、设备列表查询、设备分享管理等 |
| [IDeviceMgrCallback](#idevicemgrcallback) | 设备管理事件 |
| [IAlarmMgr](#ialarmmgr)  | 设备告警信息管理，包括移动侦察、有人经过等 |
| [INotificationMgr](#inotificationmgr) | 设备系统信息管理，包括设备上线、下线、绑定、解绑等事件管理 |
| `INotificationMgrCallback` | 设备系统信息管理事件 |
...
| 参数 | 描述 |
| --- | --- |
| `incoming` | <Slot name="full-ios-html-0-1-1" /> |
| `memberState` | <Slot name="full-ios-html-0-2-1" /> |

<Slot for="full-ios-html-0-1-1">
...
| `memberState` | <Slot name="full-ios-html-1-4-1" /> |

<Slot for="full-ios-html-1-4-1">
...
| --- | --- |
| `actionAck` | 通话中产生的事件。 |
| `memberState` | <Slot name="full-ios-html-2-2-1" /> |

<Slot for="full-ios-html-2-2-1">
...
| 参数 | 描述 |
| --- | --- |
| `mute` | <Slot name="full-ios-html-3-1-1" /> |

<Slot for="full-ios-html-3-1-1">
...
| 参数 | 描述 |
| --- | --- |
| `mute` | <Slot name="full-ios-html-4-1-1" /> |

<Slot for="full-ios-html-4-1-1">
...
| 参数 | 描述 |
| --- | --- |
| `mute` | <Slot name="full-ios-html-5-1-1" /> |

<Slot for="full-ios-html-5-1-1">
...
| 参数 | 描述 |
| --- | --- |
| `mute` | <Slot name="full-ios-html-6-1-1" /> |

<Slot for="full-ios-html-6-1-1">
...
| 参数       | 描述                                                         |
| ---------- | ------------------------------------------------------------ |
| `listener` | `IDeviceStateListener` 类型的设备状态回调接口。 |

**返回**

错误码，详见 [ErrCode](#errcode)。

<a id="queryproductlist"></a>
### queryProductList
...
| `deviceId` | 被分享的设备编号。 |
| `userId` | 分享对象的用户 ID。 |
| `type` | <Slot name="full-ios-html-7-3-1" /> |

<Slot for="full-ios-html-7-3-1">
...
| --- | --- |
| `deviceId` | 设备 ID。 |
| `statusUpdated` | <Slot name="full-ios-html-8-2-1" /> |

<Slot for="full-ios-html-8-2-1">

- `status`：RTM 状态。详见 `MessageChannelStatus`。
    - `msg`：提示信息。
    - `data`：提示描述。
...
<a id="idevicemgrcallback"></a>
## IDeviceMgrCallback

设备状态改变回调。

<a id="ondeviceonoffline"></a>
### onDeviceOnOffline
...
| 参数 | 描述 |
| --- | --- |
| `online` | <Slot name="full-ios-html-9-1-1" /> |
| `deviceId` | 设备 ID，必须是唯一的。 |
| `productId` | 产品 ID。 |

<Slot for="full-ios-html-9-1-1">
...
| --- | --- |
| `deviceId` | 设备 ID，必须是唯一的。 |
| `actionType` | <Slot name="full-ios-html-10-2-1" /> |

<Slot for="full-ios-html-10-2-1">
...
<a id="ondevicepropertyupdated"></a>
...
| 参数         | 描述                                                   |
| ------------ | ------------------------------------------------------ |
| `queryParam` | 告警信息查询参数，详见 [AlarmQueryParam](#alarmqueryparam) 类。 |
...
| `productId` | 要查询的产品 ID。 |
| `deviceId` | 要查询的设备 ID。 |
| `messageType` | <Slot name="full-ios-html-11-3-1" /> |
| `status` | <Slot name="full-ios-html-11-4-1" /> |
| `createdDateBegin` | 开始查询的时间。 |
| `createdDateEnd` | 结束查询的时间。 |

<Slot for="full-ios-html-11-3-1">
...
| 参数         | 描述                                                   |
| ------------ | ------------------------------------------------------ |
| `queryParam` | 系统信息查询参数，详见 [SysAlarmQueryParam](#sysalarmqueryparam) 类。 |
...
| `productId` | 要查询的产品 ID。 |
| `deviceIds` | 要查询的设备 ID。 |
| `messageType` | <Slot name="full-ios-html-12-3-1" /> |
| `status` | <Slot name="full-ios-html-12-4-1" /> |
| `createdDateBegin` | 开始查询的时间。 |
| `createdDateEnd` | 结束查询的时间。 |

<Slot for="full-ios-html-12-3-1">
...
| 参数 | 描述 |
| --- | --- |
| `bool` | <Slot name="full-ios-html-13-1-1" /> |

<Slot for="full-ios-html-13-1-1">
...
| `publishAudio` | <Slot name="full-ios-html-14-3-1" /> |
| `publishVideo` | <Slot name="full-ios-html-14-4-1" /> |
| `subscribeAudio` | <Slot name="full-ios-html-14-5-1" /> |
| `subscribeVideo` | <Slot name="full-ios-html-14-6-1" /> |
...
| `name` | 用户名。 |
| `avatar` | 用户头像。 |
| `sex` | <Slot name="full-ios-html-15-3-1" /> |
| `age` | 年龄。0：未知 |
| `birthday` | 生日。 |
| `height` | 身高，单位：cm。 |
| `weight` | 体重，单位：kg |
...
| --- | --- |
| `alertMessageId` | 告警信息 ID。 |
| `messageType` | <Slot name="full-ios-html-16-2-1" /> |
| `desc` | 告警描述。 |
| `fileUrl` | 告警视频 URL。 |
| `status` | <Slot name="full-ios-html-16-5-1" /> |
| `tenantId` | 租户 ID。 |
| `productId` | 产品 ID。 |
| `deviceId` | 设备 ID。 |
| `deviceName` | 设备名称。 |
| `deleted` | <Slot name="full-ios-html-16-10-1" /> |
| `imageId` | 告警图片 ID。 |
...
<Slot for="full-ios-html-16-10-1">
...
| `productId` | 产品 ID。 |
| `deviceId` | 设备 ID。 |
| `messageType` | <Slot name="full-ios-html-17-3-1" /> |
| `status` | <Slot name="full-ios-html-17-4-1" /> |
| `createdDateBegin` | 开始查询的时间。 |
| `createdDateEnd` | 结束查询的时间。 |
| `currentPage` | 要查询的页面索引，从 1 开始。 |
| `pageSize` | 查询结果中每页告警消息的数量。 |
| `desc` | <Slot name="full-ios-html-17-9-1" /> |

<Slot for="full-ios-html-17-3-1">
...
| 参数 | 描述 |
| --- | --- |
| `messageType` | <Slot name="full-ios-html-18-1-1" /> |
| `status` | <Slot name="full-ios-html-18-2-1" /> |
| `createdDateBegin` | 开始查询的时间。 |
| `createdDateEnd` | 结束查询的时间。 |
| `currentPage` | 要查询的页面索引，从 1 开始。 |
| `pageSize` | 查询结果中每页告警消息的数量。 |
| `desc` | <Slot name="full-ios-html-18-7-1" /> |
| `deviceIds` | 要查询的设备 ID。当查询所有设备的信息时，需要填上所有设备的 ID。 |

<Slot for="full-ios-html-18-1-1">
...
| --- | --- |
| `userId` | 用户 ID。 |
| `userType` | <Slot name="full-ios-html-19-2-1" /> |
| `deviceId` | 设备 ID。 |
| `deviceName` | 设备名称。 |
| `deviceNumber` | 设备编号。 |
| `tenantId` | 租户 ID。 |
...
| `connected` | <Slot name="full-ios-html-19-13-1" /> |
| `alias` | 产品别名。 |
| `props` | 设备属性。 |

<Slot for="full-ios-html-19-2-1">
...
<Slot for="full-ios-html-19-13-1">
...
| --- | --- |
| `alias` | 产品别名。 |
| `bindType` | <Slot name="full-ios-html-20-2-1" /> |
| `connectType` | <Slot name="full-ios-html-20-3-1" /> |
| `createTime` | 创建时间。 |
| `deleted` | <Slot name="full-ios-html-20-5-1" /> |
| `id` | 产品 ID。 |
| `imgBig` | 产品大图。 |
| `imgSmall` | 产品小图。 |
| `merchantId` | 商户 ID。 |
...
| `status` | <Slot name="full-ios-html-20-15-1" /> |
| `updateBy` | 更新人 ID。 |
| `updateTime` | 最后更新时间。 |

<Slot for="full-ios-html-20-2-1">
...
<Slot for="full-ios-html-20-15-1">
...
| `params` | 预留参数。 |
| `pointName` | 数据点名称。 |
| `pointType` | <Slot name="full-ios-html-21-7-1" /> |
| `readType` | <Slot name="full-ios-html-21-8-1" /> |
| `remark` | 备注 |
| `status` | <Slot name="full-ios-html-21-10-1" /> |
| `createBy` | 创建人 ID。 |
| `createTime` | 创建时间。 |
| `deleted` | <Slot name="full-ios-html-21-13-1" /> |
| `tip` | 描述。 |

<Slot for="full-ios-html-21-7-1">
...
<Slot for="full-ios-html-21-10-1">
...
<Slot for="full-ios-html-21-13-1">
...
| `appuserId` | 用户 ID。 |
| `avatar` | 用户头像。 |
| `connect` | <Slot name="full-ios-html-22-3-1" /> |
| `createTime` | 创建时间。 |
| `deviceNumber` | 设备编号。 |
| `deviceNickname` | 设备昵称。 |
| `email` | 用户邮箱. |
...
| `productKey` | 产品编号。 |
| `sharer` | 分享人 ID。 |
| `uType` | <Slot name="full-ios-html-22-14-1" /> |
| `updateTime` | 更新时间。 |

<Slot for="full-ios-html-22-3-1">
...
<Slot for="full-ios-html-22-14-1">
...
| 参数 | 描述 |
| --- | --- |
| `auditStatus` | <Slot name="full-ios-html-23-1-1" /> |
| `content` | 推送内容。 |
| `createBy` | 创建人 ID。 |
| `createTime` | 创建时间。 |
| `deleted` | <Slot name="full-ios-html-23-5-1" /> |
| `id` | 该条消息 ID。 |
| `merchantId` | 商户 ID。 |
| `merchantName` | 商户名称。 |
| `msgType` | <Slot name="full-ios-html-23-9-1" /> |
| `para` | 分享口令。 |
| `permission` | <Slot name="full-ios-html-23-11-1" /> |
| `status` | <Slot name="full-ios-html-23-12-1" /> |
| `title` | 推送标题。 |
| `type` | <Slot name="full-ios-html-23-14-1" /> |
| `updateBy` | 更新人 ID。 |
| `updateTime` | 更新时间。 |
| `userId` | 被分享者 ID。 |

<Slot for="full-ios-html-23-1-1">
...
<Slot for="full-ios-html-23-11-1">
...
<Slot for="full-ios-html-23-12-1">
...
<Slot for="full-ios-html-23-14-1">
...
| 参数 | 描述 |
| --- | --- |
| `auditStatus` | <Slot name="full-ios-html-24-1-1" /> |
| `content` | 推送内容。 |
| `createBy` | 创建人 ID。 |
| `createTime` | 创建时间。 |
| `deleted` | <Slot name="full-ios-html-24-5-1" /> |
| `deviceNumber` | 设备编号。 |
| `id` | 该条消息 ID。 |
| `img` | 产品图片地址。 |
| `merchantId` | 商户 ID。 |
| `merchantName` | 商户名称。 |
| `msgType` | <Slot name="full-ios-html-24-11-1" /> |
| `para` | 分享口令。 |
| `permission` | <Slot name="full-ios-html-24-13-1" /> |
| `productName` | 产品名称。 |
| `pushTime` | 推送时间。 |
| `shareName` | 分享设备的名称。 |
| `status` | <Slot name="full-ios-html-24-17-1" /> |
| `title` | 推送标题。 |
| `type` | <Slot name="full-ios-html-24-19-1" /> |
| `userId` | 被分享者 ID。 |

<Slot for="full-ios-html-24-1-1">
...
<Slot for="full-ios-html-24-11-1">
...
<Slot for="full-ios-html-24-13-1">
...
<Slot for="full-ios-html-24-17-1">
...
<Slot for="full-ios-html-24-19-1">
...
| `status` | <Slot name="full-ios-html-25-5-1" /> |

<Slot for="full-ios-html-25-5-1">
...
| 参数 | 描述 |
| --- | --- |
| `isBusy` | <Slot name="full-ios-html-26-1-1" /> |
