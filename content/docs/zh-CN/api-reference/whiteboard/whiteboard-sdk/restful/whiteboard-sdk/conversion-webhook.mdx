...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/doc/whiteboard/restful/whiteboard-sdk/conversion-webhook
  sourcePath: docs/whiteboard/whiteboard-sdk/conversion-webhook.restful.mdx
...
![控制台中的生成 webhook 密钥入口](/img/api-center-generated/7d93dea94e82-webhook.png)
...
| data | JSON Object | <Slot name="conversion-webhook-restful-markdown-1-3-2" /> |

<Slot for="conversion-webhook-restful-markdown-1-3-2">
...
- `images`：JSON Object。文档转图片结果的地址和图片参数，每页对应一张图片。
  - `width`：Number。图片宽度 (px) 。
  - `height`：Number。图片高度 (px)。
  - `url`：String。图片连接地址。

  该参数仅在该参数仅在[发起文档转换](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)时，请求包体中的 `type` 设为 `static` 时才生效。
