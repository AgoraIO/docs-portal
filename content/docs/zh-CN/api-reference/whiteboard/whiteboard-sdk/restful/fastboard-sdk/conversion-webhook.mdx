...
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/doc/whiteboard/restful/fastboard-sdk/conversion-webhook
  sourcePath: docs/whiteboard/fastboard-sdk/conversion-webhook.restful.mdx
...
- 已开启文档转换服务并配置第三方云存储的有效声网项目。详见[使用文档转换服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/build/extend-whiteboard/convert-files)。
- 满足以下条件的服务器：
...
调用[发起文档转换](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)时，在请求包体中的 `webhookEndpoint` 字段传入 Webhook 回调地址即可对本次转换启用文档转换 Webhook 回调，接收来自服务端的文档转换进度和运行状态信息，回调内容和字段解释详见 [Webhook 回调内容](#webhook-回调内容)。
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| code | Number | 转换任务状态码，转换成功时为 `0`，失败时会返回错误码，详见[文档转换错误码](/zh-CN/realtime-media/whiteboard/fastboard-sdk/reference/response-code#文档转换错误码)。 |
...
- `previews`：JSON Object。转换后的文档预览图地址，每页对应一个预览图地址。该参数仅在[发起文档转换](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)时，请求 Body 中的 `preview` 设为 `true `，且 `type` 设为 `dynamic` 时才生效。

- `images`：JSON Object。文档转图片结果的地址和图片参数，每页对应一张图片。
  - `width`：Number。图片宽度 (px) 。
  - `height`：Number。图片高度 (px)。
  - `url`：String。图片链接地址。
  - 该参数仅在调用[发起文档转换](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)时，请求 Body 中的 `type` 设为 `static` 时生效。
...
- [使用文档转换服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/build/extend-whiteboard/convert-files)
