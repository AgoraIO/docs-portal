...
[Room](/zh-CN/api-reference/whiteboard/web/interfaces/room) 对象和 [Room](/zh-CN/api-reference/whiteboard/web/interfaces/room) 对象共用的参数配置。
...
[Room](/zh-CN/api-reference/whiteboard/web/interfaces/room) 对象的参数设置，用于加入互动白板实时房间。该类型继承 [ConstructRoomParams](/zh-CN/api-reference/whiteboard/web/globals#constructroomparams) 且包含以下属性：
...
设置是否待服务器确认 [dispatchMagixEvent](/zh-CN/api-reference/whiteboard/web/interfaces/room#dispatchmagixevent) 方法调用成功后再发送事件回调：

- true：调用 [dispatchMagixEvent](/zh-CN/api-reference/whiteboard/web/interfaces/room#dispatchmagixevent) 后立即触发事件回调。
...
- **onCanUndoStepsUpdate**: *(canUndoSteps: number)=&gt;void*
  ...
  当本地用户调用 [undo](/zh-CN/api-reference/whiteboard/web/interfaces/room#undo) 撤销上一步操作时，会触发该回调，报告剩余的可撤销次数。
...
- **onCanRedoStepsUpdate**: *(canRedoSteps: number)=&gt;void*
  ...
  当本地用户调用 [undo](/zh-CN/api-reference/whiteboard/web/interfaces/room#undo) 重做上一步操作时，会触发该回调，报告剩余的可重做次数。
...
[Plugins](/zh-CN/api-reference/whiteboard/web/interfaces/plugins) &lt; C_MAP &gt;
