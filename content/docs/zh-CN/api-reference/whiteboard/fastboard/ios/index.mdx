...
成功[发起文档转换任务](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)并调用[查询文档转换任务进度](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)获取文档转换结果后，可以调用该方法并传入获取的文档转换结果，SDK 会自动创建子窗口，插入并分页展示转换后的文档。
...
静态文档指通过[声网互动白板文档转换服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/build/extend-whiteboard/convert-files)，由 PPT、PPTX、DOC、DOCX、PDF 格式的文件转换成的 PNG、JPG/JPEG 格式的静态图片。
