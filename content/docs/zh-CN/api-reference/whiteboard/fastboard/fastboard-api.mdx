...
- `token`：String。房间的 Room Token，用于加入房间时的用户鉴权。可以通过以下方式获取：
  ...
  - 在 app 服务端用代码生成，详见[在 app 服务端生成 Token](/zh-CN/realtime-media/whiteboard/fastboard-sdk/basic-features/generate-token#在-app-服务端生成-token)。
...
- `src`：String。图片或动态 PPT 页的地址。
  - 图片：URL 地址，可以是你自己生成的 URL 地址，也可以是通过[文档转换功能](/zh-CN/realtime-media/whiteboard/fastboard-sdk/advanced-features/convert-files)生成的 URL 地址。你可以从[查询转换任务进度](/doc/whiteboard/restful/fastboard-sdk/restful-wb/operations/get-v5-projector-tasks-uuid)返回的 `image` 字段中获取该地址。请确保 App 客户端能够访问该 URL，否则无法正常加载图片。
  - 动态 PPT 页：通过[文档转换功能](/zh-CN/realtime-media/whiteboard/fastboard-sdk/advanced-features/convert-files)生成的 URI 地址。
...
该方法可以插入通过[动态文档转换](/zh-CN/realtime-media/whiteboard/fastboard-sdk/advanced-features/convert-files#动态文档转换)转换后的动态 HTML 网页。
...
动态文档指通过[声网互动白板文档转换服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/advanced-features/convert-files)，由 PPTX 格式的文件转换成的 HTML 网页。转换后的文件会保留源文件里的动画效果。
...
在白板子窗口中插入并展示文档。该方法适用于[新版文档转换服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/advanced-features/convert-files)。
...
- `enableAppInMainViewPlugin`: （可选）启用 [`@netless/app-in-mainview-plugin`](https://www.npmjs.com/package/@netless/app-in-mainview-plugin) 插件，支持在白板主视图管理[小窗应用](/zh-CN/realtime-media/whiteboard/fastboard-sdk/overview/concepts#小窗应用)：
  - `true`：采用默认配置启用插件。
...
- `replayRoom`：Object。（必需）加入房间的参数设置，必须传入以下参数：
  - `room`：String。房间的 UUID，即房间的唯一标识符。详见[创建房间](/doc/whiteboard/restful/fastboard-sdk/restful-wb/operations/post-v5-rooms)请求成功后响应包体中 `uuid` 参数的值。
  - `roomToken`：String 型。房间的 Room Token，用于加入房间时的用户鉴权。可以通过以下方式获取：
    - 调用[生成 Room Token ](/doc/whiteboard/restful/fastboard-sdk/restful-wb/operations/post-v5-tokens-rooms-uuid)。
    - 在 app 服务端用代码生成，详见[在 app 服务端生成 Token](/zh-CN/realtime-media/whiteboard/fastboard-sdk/basic-features/generate-token#在-app-服务端生成-token)。
