...
| roomPromise | `Promise<Room>` 接口实例，详见 `Promise`。你可以通过该接口获取 `joinRoom` 的调用结果：<br />- 如果方法调用成功，将返回房间对象，详见 [Room](/zh-CN/api-reference/whiteboard/android/classcom-1-1herewhite-1-1sdk-1-1-room)。<br />- 如果方法调用失败，将返回错误信息。 |
...
| roomPromise | `Promise<Room>` 接口实例，详见 `Promise`。你可以通过该接口获取 `joinRoom` 的调用结果：<br />- 如果方法调用成功，则返回房间对象，详见 [Room](/zh-CN/api-reference/whiteboard/android/classcom-1-1herewhite-1-1sdk-1-1-room)。<br />- 如果方法调用失败，则返回错误信息。 |
...
| playerPromise | `Promise<Player>` 接口实例，详见 `Promise`。你可以通过该接口获取 `createPlayer` 的调用结果：<br />- 如果方法调用成功，将返回新创建的回放房间对象，详见 [Player](/zh-CN/api-reference/whiteboard/android/classcom-1-1herewhite-1-1sdk-1-1-player)。<br />- 如果方法调用失败，将返回错误信息。 |
