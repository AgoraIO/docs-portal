...
| `joinRTCStart` | `10` | 本地已经加入 RTC 频道，但是还未成功（调用了 [`joinChannelExByToken[1/2]`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengineex_joinchannelex) 或 [`joinChannelExByToken[2/2]`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengineex_joinchannelwithuseraccountex)，但由于延迟还未收到成功回调（v2.1.0 开始支持）。） |
