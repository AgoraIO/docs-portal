...
_migration:
  type: generated-html
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtc/flutter/API/class_remotevideostats
  sourcePath: html-docs/rtc/Flutter/API/class_remotevideostats.html
  generator: oxygen
  warnings: []
---
...
:::note
注： 弃用：在有音画同步机制的音视频场景中，你可以参考 [RemoteAudioStats](/zh-CN/api-reference/rtc/flutter/class-remoteaudiostats) 里的 `networkTransportDelay` 和 `jitterBufferDelay` 成员的值，了解视频的延迟数据。
:::
...
视频流类型，大流或小流。详见 [VideoStreamType](/zh-CN/api-reference/rtc/flutter/enum-videostreamtype)。
