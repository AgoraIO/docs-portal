...
3: 指定 48 kHz 采样率，音乐编码，双声道，编码码率最大值为 80 Kbps。 如需实现立体声，你还需要调用 [setAdvancedAudioOptions](/zh-CN/api-reference/rtc/flutter/audio/audio-prenpost/audio-effect#api_irtcengine_setadvancedaudiooptions) 并在 [AdvancedAudioOptions](/zh-CN/api-reference/rtc/flutter/class-advancedaudiooptions) 中设置 `audioProcessingChannels` 为 `audioProcessingStereo`。
