...
直播视频上的水印。图片格式需为 PNG。详见 [RtcImage](/zh-CN/api-reference/rtc/flutter/class-rtcimage)。 你可以添加一个水印，或使用数组的方式添加多个水印。

### `backgroundImage`

直播视频上的背景图。图片格式需为 PNG。详见 [RtcImage](/zh-CN/api-reference/rtc/flutter/class-rtcimage)。 你可以添加一张背景图，或使用数组的方式添加多张背景图。该参数与 `backgroundImageCount` 搭配使用。
