...
5：SDK 正在与推流服务器和 CDN 服务器断开连接。当你调用 [stopRtmpStream](/zh-CN/api-reference/rtc/flutter/media-push#api_irtcengine_stoprtmpstream) 方法正常结束推流时，SDK 会依次报告推流状态为 `rtmpStreamPublishStateDisconnecting`、 `rtmpStreamPublishStateIdle`。
