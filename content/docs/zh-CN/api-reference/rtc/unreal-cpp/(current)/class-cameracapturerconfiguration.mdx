...
- 部分 iOS 设备的后置摄像头为多个摄像头组成的融合镜头，如双摄（广角和超广角）或三摄（广角、超广角和长焦），对于这种具备超广角能力的融合镜头，你可以通过以下任意一种方式实现超广角的采集效果：
  ...
  - 方式二：将该参数设置为 CAMERA_FOCAL_LENGTH_DEFAULT (0)（标准镜头），然后调用 [setCameraZoomFactor](/zh-CN/api-reference/rtc/unreal-cpp/video-device#api_irtcengine_setcamerazoomfactor) 将相机缩放比例设置为小于 1.0 的数值，最小可以设置为 0.5。
