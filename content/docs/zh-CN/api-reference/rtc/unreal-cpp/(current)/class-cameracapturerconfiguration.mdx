...
- 部分 iOS 设备的后置摄像头为多个摄像头组成的融合镜头，如双摄（广角和超广角）或三摄（广角、超广角和长焦），对于这种具备超广角能力的融合镜头，你可以通过以下任意一种方式实现超广角的采集效果：
  ...
  - 方式二：将该参数设置为 CAMERA_FOCAL_LENGTH_DEFAULT (0)（标准镜头），然后调用 [setCameraZoomFactor](/zh-CN/api-reference/rtc/unreal-cpp/toc-video-device#api_irtcengine_setcamerazoomfactor) 将相机缩放比例设置为小于 1.0 的数值，最小可以设置为 0.5。
...
（可选）是否跟随 [setVideoEncoderConfiguration](/zh-CN/api-reference/rtc/unreal-cpp/toc-video-basic#api_irtcengine_setvideoencoderconfiguration) 中设置的视频宽高比：

- `true`：(默认) 跟随。SDK 会将采集到的视频按照已设置的视频宽高比进行裁剪，会同步改变本地预览画面、 [onCaptureVideoFrame](/zh-CN/api-reference/rtc/unreal-cpp/toc-video-raw#callback_ivideoframeobserver_oncapturevideoframe) 和 [onPreEncodeVideoFrame](/zh-CN/api-reference/rtc/unreal-cpp/toc-video-raw#callback_ivideoframeobserver_onpreencodevideoframe) 中的视频画面。
