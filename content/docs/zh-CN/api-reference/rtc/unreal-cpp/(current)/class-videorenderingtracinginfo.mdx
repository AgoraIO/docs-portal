...
从调用 [startMediaRenderingTracing](/zh-CN/api-reference/rtc/unreal-cpp/video-rendering#api_irtcengine_startmediarenderingtracing) 到触发 [onVideoRenderingTracingResult](/zh-CN/api-reference/rtc/unreal-cpp/video-rendering#callback_irtcengineeventhandler_onvideorenderingtracingresult) 回调的时间间隔 (ms)。建议在加入频道前调用 startMediaRenderingTracing。
