...
（11）：（仅适用于 macOS 平台）调用 [startScreenCaptureByWindowId:regionRect:captureParams:](/zh-CN/api-reference/rtc/macos/screencapture#api_irtcengine_startscreencapturebywindowid) 方法共享窗口时，被共享窗口已最小化，无法进行共享。请提示用户取消最小化该窗口。
