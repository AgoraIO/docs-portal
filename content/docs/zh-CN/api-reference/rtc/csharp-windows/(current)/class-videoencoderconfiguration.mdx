...
_migration:
  type: generated-html
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtc/csharp/API/class_videoencoderconfiguration
  sourcePath: html-docs/rtc/Csharp/API/class_videoencoderconfiguration.html
  generator: oxygen
  warnings: []
---
...
视频编码的分辨率 (px)，详见 [VideoDimensions](/zh-CN/api-reference/rtc/csharp-windows/class-videodimensions)。该参数用于衡量编码质量，以长 × 宽表示，默认值为 960 × 540。用户可以自行设置分辨率。

</Parameter>
<a id="codectype"></a>
<Parameter name="codecType">

视频编码类型，详见 [VIDEO_CODEC_TYPE](/zh-CN/api-reference/rtc/csharp-windows/enum-videocodectype)。

</Parameter>
<a id="framerate"></a>
<Parameter name="frameRate">

视频编码的帧率(fps)，默认值为 15。详见 [FRAME_RATE](/zh-CN/api-reference/rtc/csharp-windows/enum-framerate)。
...
视频编码的方向模式，详见 [ORIENTATION_MODE](/zh-CN/api-reference/rtc/csharp-windows/enum-orientationmode)。

</Parameter>
<a id="degradationpreference"></a>
<Parameter name="degradationPreference">

带宽受限时，视频编码降级偏好。详见 [DEGRADATION_PREFERENCE](/zh-CN/api-reference/rtc/csharp-windows/enum-degradationpreference)。
...
发送编码视频时是否开启镜像模式，只影响远端用户看到的视频画面。详见 [VIDEO_MIRROR_MODE_TYPE](/zh-CN/api-reference/rtc/csharp-windows/enum-videomirrormodetype)。
...
视频编码的高级选项。详见 [AdvanceOptions](/zh-CN/api-reference/rtc/csharp-windows/class-advanceoptions)。
