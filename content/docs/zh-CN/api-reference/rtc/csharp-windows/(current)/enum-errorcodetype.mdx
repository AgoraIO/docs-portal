...
113：调用 [SendStreamMessage](/zh-CN/api-reference/rtc/csharp-windows/datastream#api_irtcengine_sendstreammessage) 方法时用户不在频道内。
...
116：在调用 [CreateDataStream [2/2]](/zh-CN/api-reference/rtc/csharp-windows/datastream#api_irtcengine_createdatastream2) 时，创建的数据流超过限制（5 个）。
