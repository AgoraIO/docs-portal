...
- 开启该参数时，建议设定一个规律的周期（比如 30 ms），然后调用 [UpdatePlayerPositionInfo](/zh-CN/api-reference/rtc/csharp-windows/spatial-audio#api_ibasespatialaudioengine_updateplayerpositioninfo)、 [UpdateSelfPosition](/zh-CN/api-reference/rtc/csharp-windows/spatial-audio#api_ibasespatialaudioengine_updateselfposition) 和 [UpdateRemotePosition](/zh-CN/api-reference/rtc/csharp-windows/spatial-audio#api_ilocalspatialaudioengine_updateremoteposition) 方法持续更新声源和接收方的相对距离。以下因素会导致多普勒效应达不到预期或者声音出现抖动：更新距离的周期过长，更新周期不规律，网络丢包或延迟导致距离信息丢失。
:::
