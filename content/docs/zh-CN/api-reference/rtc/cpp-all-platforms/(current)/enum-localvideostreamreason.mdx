...
（11）：（仅适用于 macOS 和 Windows 平台）调用 [startScreenCaptureByWindowId](/zh-CN/api-reference/rtc/cpp-all-platforms/screencapture#api_irtcengine_startscreencapturebywindowid) 方法共享窗口时，被共享窗口已最小化。SDK 无法共享最小化的窗口。请提示用户取消最小化该窗口。
