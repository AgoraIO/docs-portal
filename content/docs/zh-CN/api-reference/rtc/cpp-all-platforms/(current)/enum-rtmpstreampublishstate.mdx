...
（5）：SDK 正在断开与声网推流服务器和 CDN 的连接。调用 [stopRtmpStream](/zh-CN/api-reference/rtc/cpp-all-platforms/media-push#api_irtcengine_stoprtmpstream) 正常停止推流时，SDK 会依次上报状态为 `RTMP_STREAM_PUBLISH_STATE_DISCONNECTING` 和 `RTMP_STREAM_PUBLISH_STATE_IDLE`。
