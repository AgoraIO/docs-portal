...
_migration:
  type: generated-html
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtc/android/API/class_screenvideoparameters
  sourcePath: html-docs/rtc/Android/API/class_screenvideoparameters.html
  generator: oxygen
  warnings: []
---
...
:::note
注： 屏幕共享流的计费基于此参数值。当不传入该参数时，声网按 1280 × 720 收费；传入该参数时，声网按设置值计费。
该参数不代表输出视频的旋转方向，设置方向详见 [ORIENTATION_MODE](/zh-CN/api-reference/rtc/android/enum-orientationmode)。
是否支持 720p 或更高分辨率取决于设备性能，如设备不支持，则帧率可能低于设置值。
在文档共享（`SCREEN_SCENARIO_DOCUMENT`）场景中设置编码分辨率时，建议如下：
