...
_migration:
  type: generated-html
  status: migrated
  sourceUrl: https://doc.shengwang.cn/api-ref/rtc/android/API/class_videoencoderconfiguration
  sourcePath: html-docs/rtc/Android/API/class_videoencoderconfiguration.html
  generator: oxygen
  warnings: []
---
...
:::note
注： 是否支持 720p 或更高分辨率取决于设备性能。如果设备无法支持 720p，帧率可能低于设定值。默认值为 960 × 540。
:::
...
详见 [VideoDimensions](/zh-CN/api-reference/rtc/android/class-videodimensions)。

</Parameter>
<a id="framerate"></a>
<Parameter name="frameRate">

编码视频帧的帧率（fps）。默认值为 15。详见 [FRAME_RATE](/zh-CN/api-reference/rtc/android/enum-framerate)。

</Parameter>
<a id="minframerate"></a>
<Parameter name="minFrameRate">
...
编码视频的方向模式。详见 [ORIENTATION_MODE](/zh-CN/api-reference/rtc/android/enum-orientationmode)。

</Parameter>
<a id="degradationprefer"></a>
<Parameter name="degradationPrefer">

带宽受限时的视频编码降级偏好。详见 [DEGRADATION_PREFERENCE](/zh-CN/api-reference/rtc/android/enum-degradationpreference)。

:::note
注： 当该参数设置为 `MAINTAIN_FRAMERATE (1)` 或 `MAINTAIN_BALANCED (2)` 时，`orientationMode` 需同时设为 `ORIENTATION_MODE_ADAPTIVE (0)`，否则该设置无效。
:::
...
视频编码的高级选项。详见 [AdvanceOptions](/zh-CN/api-reference/rtc/android/class-advanceoptions)。

</Parameter>
<a id="codectype"></a>
<Parameter name="codecType">

本地视频流的首选编码器类型。详见 [VIDEO_CODEC_TYPE](/zh-CN/api-reference/rtc/android/enum-videocodectype)。
