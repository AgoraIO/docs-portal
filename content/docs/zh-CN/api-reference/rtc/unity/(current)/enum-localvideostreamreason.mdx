...
11：（仅适用于 macOS 和 Windows）调用 [StartScreenCaptureByWindowId](/zh-CN/api-reference/rtc/unity/screencapture#api_irtcengine_startscreencapturebywindowid) 方法共享窗口时，共享窗口处于最小化的状态。SDK 无法共享被最小化的窗口。请提示用户将共享窗口取消最小化。
