...
| 参数名   | 类型             | 是否必选 | 描述 |
| -------- | ---------------- | -------- | ---- |
| `client` | [`IAgoraRTCClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient) &VerticalLine; `null` | 可选 | 通过 Web SDK 的 [`IAgoraRTC.createClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtc#createclient) 创建。 |

#### 返回值

| 类型              | 描述   |
| ----------------- | ---------------------|
| `ConnectionState` | SDK 与服务器的连接状态。详见 [`ConnectionState`](/zh-CN/api-reference/rtc/web/globals#connectionstate)。 |
...
| 类型              | 描述   |
| ----------------- | ---------------------|
| `boolean` | - `true`：SDK 已连接到服务器。<br />- `false`：SDK 没有连接到服务器。 |
...
| 参数名   | 类型             | 是否必选 | 描述 |
| -------- | ---------------- | -------- | ---- |
| `client` | [`IAgoraRTCClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient) &VerticalLine; `null`| 可选 | 如果传入该参数，则使用传入的 `IAgoraRTCClient` 对象；如果不传入该参数，则使用从[父组件的 Context](./components#agorartcprovider)中获取的 `IAgoraRTCClient` 对象。 |

#### 返回值

| 类型              | 描述                     |
| ----------------- | ------------------------ |
| [`IAgoraRTCClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient) | `IAgoraRTCClient` 对象。 |
...
| `client`    | [`IAgoraRTCClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient) | 可选                | 通过 Web SDK 的 [`IAgoraRTC.createClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtc#createclient) 创建。 |
...
| `isLoading` | `boolean` |- `true`：正在执行加入频道相关操作。<br />- `false`：已经执行加入频道相关操作，但不代表成功加入频道。 |
| `isConnected` | `boolean` | - `true`：SDK 已连接到服务器。即加入频道成功。<br />- `false`：SDK 没有连接到服务器。 |
...
| 属性              | 类型                   | 描述  |
| ----------------------| ---------------------- | ------------------------ |
| `isLoading` | `boolean` |- `true`：正在执行发布相关操作。<br />- `false`：已经执行发布相关操作，但不代表成功发布。 |
...
| `audioTrackConfig` | `MicrophoneAudioTrackInitConfig` | 可选     | 麦克风音频轨道的初始化配置，默认为 `{ ANS: true, AEC: true }`。详见 [`MicrophoneAudioTrackInitConfig`](/zh-CN/api-reference/rtc/web/interfaces/microphone-audio-track-init-config)。 |
...
| `isLoading` | `boolean` |- `true`：正在执行创建轨道相关操作。<br />- `false`：已经执行创建轨道相关操作，但不代表成功创建轨道。 |
...
| -------- | ---------------- | -------- | ---- |
| `ready`  | `boolean`         | 可选     | 是否准备好创建轨道，默认为 `true`。                                                                                                           |
| `cameraVideoTrackConfig` | `CameraVideoTrackInitConfig` | 可选     | 摄像头视频轨道的初始化配置。详见 [`CameraVideoTrackInitConfig`](/zh-CN/api-reference/rtc/web/interfaces/camera-video-track-init-config)。 |
...
| -------- | ---------------- | -------- | ---- |
| `ready`  | `boolean`         | 可选     | 是否准备好创建轨道，默认为 `true`。         |
| `screenVideoTrackInitConfig`  | [`screenVideoTrackInitConfig`](/zh-CN/api-reference/rtc/web/interfaces/screen-video-track-init-config)         | 必选     | 屏幕共享的视频配置，包括编码配置、采集配置等。         |
| `withAudio`  | `string`         | 必选     | 屏幕共享时是否同时分享屏幕共享输入源的音频。支持传入 `"enable"`、`"disable"` 或 `"auto"`。详见[`createScreenVideoTrack`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtc#createscreenvideotrack)的参数解释。       |
...
| 属性              | 类型                   | 描述  |
| ----------------------| ---------------------- | ------------------------ |
| `screenTrack` | `[ILocalVideoTrack, ILocalAudioTrack]` &VerticalLine; `ILocalVideoTrack` &VerticalLine; `null` | 根据传入的 `withAudio`，返回创建的屏幕共享视频轨道或者屏幕共享视频轨道和音频轨道。详见 [`createScreenVideoTrack`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtc#createscreenvideotrack)的返回值。  |
| `isLoading` | `boolean` |- `true`：正在执行创建轨道相关操作。<br />- `false`：已经执行创建轨道相关操作，但不代表成功创建轨道。 |
| `error` | `AgoraRTCReactError` &VerticalLine; `null` | 创建轨道成功则返回 `null`，创建轨道失败则抛出错误。错误详见 [`AgoraRTCReactError`](./data-types#agorartcreacterror)。 |
...
- 如果你从返回值中获取了视频轨道对象和音频轨道对象，但只发布了其中一个轨道，在停止屏幕共享之前还需要调用 Web SDK 的 [`ILocalTrack.close`](/zh-CN/api-reference/rtc/web/interfaces/ilocal-track#close) 方法手动关闭未发布的轨道。已发布的轨道会自动关闭。
...
| `isLoading` | `boolean` |- `true`：正在执行订阅相关操作。<br />- `false`：已经执行订阅相关操作，但不代表成功订阅。 |
...
| `isLoading` | `boolean` |- `true`：正在执行获取轨道相关操作。<br />- `false`：已经执行获取轨道相关操作，但不代表成功获取轨道。 |
...
| `play`  | `boolean`          | 可选              | - `true`：播放该轨道。<br />- `false`：停止播放该轨道。 |
| `videoPlayerConfig`  |`VideoPlayerConfig` | 可选 | 视频轨道的播放配置。可以设置播放参数（镜像/显示模式），详见 [`VideoPlayerConfig`](/zh-CN/api-reference/rtc/web/interfaces/video-player-config)。对于本地视频轨道，镜像模式默认开启。|
...
| 参数名   | 类型             | 是否必选 | 描述 |
| -------- | ---------------- | -------- | ---- |
| `client` | [`IAgoraRTCClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient) | 必选 | 通过 Web SDK 的 [`IAgoraRTC.createClient`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtc#createclient) 创建。 |
| `event` | `string` | 必选               | 事件名称。支持的事件详见[`IAgoraRTCClient.on`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient?platform=All%20Platforms#on)。 |
| `listener`  | `Function`          | 必选               | 回调函数。当传入的事件触发时，该函数会被调用。支持的回调详见 [`IAgoraRTCClient.on`](/zh-CN/api-reference/rtc/web/interfaces/iagora-rtcclient?platform=All%20Platforms#on)。 |
...
| -------- | ---------------- |
| `ILocalTrack` &VerticalLine; `ILocalVideoTrack` &VerticalLine; `null` | 详见 [`ILocalTrack.on`](/zh-CN/api-reference/rtc/web/interfaces/ilocal-track#on)。 |
| `IBufferSourceAudioTrack`  &VerticalLine; `null` |详见 [`IBufferSourceAudioTrack.on`](/zh-CN/api-reference/rtc/web/interfaces/ibuffer-source-audio-track#on)。            |
| `ILocalVideoTrack` &VerticalLine; `null` | 详见 [`ILocalVideoTrack.on`](/zh-CN/api-reference/rtc/web/interfaces/ilocal-video-track#on)。 |
| `IRemoteTrack` &VerticalLine; `null` | 详见 [`IRemoteTrack.on`](/zh-CN/api-reference/rtc/web/interfaces/iremote-track#event_first_frame_decoded)。 |
