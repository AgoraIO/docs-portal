...
:::info[注]
弃用：在有音画同步机制的音视频场景中，你可以参考 [RemoteAudioStats](/zh-CN/api-reference/rtc/react-native/class-remoteaudiostats) 里的 `networkTransportDelay` 和 `jitterBufferDelay` 成员的值，了解视频的延迟数据。
:::
