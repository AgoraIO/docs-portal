...
声网为 App 开发者签发的 App ID。 使用同一个 App ID 的 App 才能进入同一个频道进行通话或直播。一个 App ID 只能用于创建一个 IRtcEngine。如需更换 App ID，必须先调用 [Release](/zh-CN/api-reference/rtc/unreal-blueprint/toc-initialize#api_irtcengine_release) 销毁当前 IRtcEngine 再重新创建。
...
- `false`：初始化 IRtcEngine 时不注册声网插件。你需要调用 [EnableExtension](/zh-CN/api-reference/rtc/unreal-blueprint/toc-extension#api_irtcengine_enableextension) 来注册声网插件。
