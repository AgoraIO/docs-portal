...
从调用 [StartMediaRenderingTracing](/zh-CN/api-reference/rtc/unreal-blueprint/video-rendering#api_irtcengine_startmediarenderingtracing) 到触发 [OnVideoRenderingTracingResult](/zh-CN/api-reference/rtc/unreal-blueprint/video-rendering#callback_irtcengineeventhandler_onvideorenderingtracingresult) 回调的时间间隔 (ms)。建议在加入频道前调用 StartMediaRenderingTracing。
