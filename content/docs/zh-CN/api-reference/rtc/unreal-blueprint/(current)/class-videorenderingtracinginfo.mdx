...
从调用 [StartMediaRenderingTracing](/zh-CN/api-reference/rtc/unreal-blueprint/toc-video-rendering#api_irtcengine_startmediarenderingtracing) 到触发 [OnVideoRenderingTracingResult](/zh-CN/api-reference/rtc/unreal-blueprint/toc-video-rendering#callback_irtcengineeventhandler_onvideorenderingtracingresult) 回调的时间间隔 (ms)。建议在加入频道前调用 StartMediaRenderingTracing。

### `start2JoinChannel`

从调用 StartMediaRenderingTracing 到调用 [JoinChannelWithOptions](/zh-CN/api-reference/rtc/unreal-blueprint/toc-channel#api_irtcengine_joinchannel2) 的时间间隔 (ms)。负数表示在调用 JoinChannelWithOptions 后调用 StartMediaRenderingTracing。
