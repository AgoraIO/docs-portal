...
- NONE (0)：将背景处理为 Alpha 数据，不进行替换，仅分割人像和背景。设置后，你可以调用 [startLocalVideoTranscoder](/zh-CN/api-reference/rtc/harmonyos/local-transcoder#api_irtcengine_startlocalvideotranscoder)，实现人像画中画的效果。
- COLOR (1)：（默认）背景为纯色。
