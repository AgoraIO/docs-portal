---
title: 使用 Go SDK 开始云端录制
_migration:
  type: manual-mdx
  status: migrated
  sourceUrl: https://doc.shengwang.cn/doc/cloud-recording/restful/get-started/quick-start-go
  sourcePath: docs/cloud-recording/get-started/quick-start-go.mdx
...
- 参考[开通服务](/zh-CN/api-reference/cloud-recording/restful/get-started/enable-service)在声网控制台开通云端录制，并复制保存 App ID、临时 Token、客户 ID 和客户密钥。
...
- 在项目中实现了基础的音视频互动场景。你也可以参考[文档](/zh-CN/api-reference/cloud-recording/restful/get-started/quick-start#模拟音视频互动场景)使用声网实时互动 [Web Demo](https://webdemo.agora.io/index.html) 在浏览器中模拟一个音视频互动场景。
...
- `Vendor` 值设为 `2` 代表使用阿里云；`Region` 值设为 `1` 表示 `CN_Shanghai`。如果你使用其他云存储厂商，参考[第三方云存储地区说明](/zh-CN/api-reference/cloud-recording/restful/api/reference#第三方云存储地区说明)赋值。
...
![云存储中 quickstart 录制文件目录示意图](/img/api-center-generated/8953b581ba1d-quick-oss-2.png)
...
根据你在 `start` 方法中设置的录制时间，云存储中可能会有多个 .ts 文件。录制文件介绍请参考[录制文件命名和切片规则](/zh-CN/api-reference/cloud-recording/restful/user-guides/manage-file/introduce-recorded-files#页面录制模式)。
...
- [响应状态码和错误码](/zh-CN/api-reference/cloud-recording/restful/response-code)
