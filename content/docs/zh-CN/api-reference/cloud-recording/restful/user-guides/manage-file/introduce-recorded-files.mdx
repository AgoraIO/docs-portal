...
_migration:
  ...
  status: migrated
  sourceUrl: >-
    https://doc.shengwang.cn/doc/cloud-recording/restful/user-guides/manage-file/introduce-recorded-files
  sourcePath: docs/cloud-recording/user-guides/manage-file/introduce-recorded-files.mdx
...
本文介绍云端录制生成文件，以帮助你理解录制文件的组成、命名规则、切片规则以及如何估算文件大小。

## 录制文件组成及命名规则

本节具体介绍无异常情况下各录制模式生成的录制文件及命名规则。

### 单流录制模式

在[单流录制模式](/zh-CN/api-reference/cloud-recording/restful/overview/concepts#单流录制)下，录制服务会针对不同 UID 的音视频流分别生成对应的音频和视频文件。生成的文件类型可能包括 M3U8、TS 和 WebM 文件。

生成的录制文件命名规则如下：

- M3U8 文件名：`<sid>_<cname>__uid_s_<uid>__uid_e_<type>.m3u8`
- TS 文件名：`<sid>_<cname>__uid_s_<uid>__uid_e_<type>_utc.ts`
- WebM 文件名：`<sid>_<cname>__uid_s_<uid>__uid_e_<type>_utc.webm`（VP8视频编码格式下生成）

### 合流录制模式

在[合流录制模式](/zh-CN/api-reference/cloud-recording/restful/overview/concepts#录制模式)下，录制过程中会实时生成一个 M3U8 文件和多个 TS 文件。若在录制配置中将 `avFileType` 设置为 `["hls","mp4"]`，录制服务会同时生成 MP4 文件；若未设置或未包含 `"mp4"`，则仅生成 M3U8/TS，你也可使用[转码脚本](/zh-CN/api-reference/cloud-recording/restful/user-guides/manage-file/convert-format)将 M3U8/TS 合并并转换为 MP4 格式。
...
在[页面录制模式](/zh-CN/api-reference/cloud-recording/restful/overview/concepts#录制模式)下，录制过程中会实时生成一个 M3U8 文件和多个 TS 文件。若在录制配置中将 `avFileType` 设置为 `["hls","mp4"]`，录制服务会同时生成 MP4 文件；若未设置或未包含 `"mp4"`，则仅生成 M3U8/TS，你也可使用[转码脚本](/zh-CN/api-reference/cloud-recording/restful/user-guides/manage-file/convert-format)将 M3U8/TS 合并并转换为 MP4 格式。

生成的录制文件命名规则如下：

- M3U8 文件名：`<sid>_<cname>.m3u8`
- TS 文件名：`<sid>_<cname>_<timestamp>.ts`
- MP4 文件名：`<sid>_<cname>_<index>.mp4`
...
| --- | --- | --- |
| `<sid>` | 录制 ID。 |  |
| `<cname>` | 频道名。由于文件名不支持特殊字符，`cname` 中的特殊字符在生成文件时会被替换成 `-`。特殊字符包括："!"、 "@"、 "#"、 "$"、 "%"、 "^"、 "&"、 "*"、 "("、 ")"、 "+"、 "_"、 "."、 "="、 "["、 "]"、 "&#123;"、 "&#125;"、 "~"、 " | "、 "、"、 ";"、 ":"、 "?"、 "&lt;"、 "&gt;"。 |
| `<uid>` | 用户 ID。 |  |
| `<type>` | 文件类型，`audio` 或 `video`。 |  |
| `<utc>` / `<timestamp>` | <Slot name="introduce-recorded-files-markdown-0-5-1" /> |  |
| `<index>` | MP4 文件的索引数，`0` 表示生成的第一个 MP4 文件。录制服务会在当前 MP4 文件时长超过约 3 小时或大小超过 2 GB 左右时创建一个新的 MP4 文件。 |  |

<Slot for="introduce-recorded-files-markdown-0-5-1">

切片文件开始录制时的时间标识：

- `&lt;utc&gt;`：用于**单流录制**和**合流录制**场景。采用 UTC 时间（UTC+0），格式：`yyyyMMddHHmmssSSS`。示例：`20190611073246073` 表示 `2019-06-11 07:32:46.073`。
- `&lt;timestamp&gt;`：用于**页面录制**场景，表示该切片开始录制时自 `1970-01-01 00:00:00 UTC` 起经过的毫秒数。示例：`1627613641393` 表示已过 1627613641393 毫秒。

</Slot>

例如，在单流录制文件 `sid713476478245_cnameagora__uid_s_123__uid_e_video_20190920125142485.ts` 中：

- `sid713476478245` 为录制 ID
- `cnameagora` 为频道名
- `123` 为用户 ID
- `video` 为录制类型
- `20190920125142485` 为录制时间（2019 年 9 月 20 日 12 点 51 分 42 秒 485 毫秒）

## 录制文件切片规则

录制文件的切片是将连续的音视频流分割成一系列小文件的过程，这一过程对于视频点播、直播回放以及文件存储和传输至关重要。正确的切片规则能够确保视频内容的连续性、完整性，并适应不同的播放平台和网络条件。以下是对视频和音频文件切片规则的具体介绍：
...
:::info[信息]
当云存储中存在带有 `_<tick>_<index>` 后缀的 M3U8 文件，你需要将 `index` 最大的 M3U8 文件与无后缀的 M3U8 文件对比，选择内容较多的一个，作为最终使用的 M3U8 文件。通过备份云转存的 TS/WebM 文件以及 MP4 文件的文件名不会附上该后缀。
:::

#### 服务器断网或进程被杀

当出现[服务器断网、进程被杀](/zh-CN/api-reference/faq/integration/high_availability)时，云端录制会自动启用高可用机制，在 90 秒内切换到新的服务器，自动恢复录制服务。启用高可用机制后，会生成新的 M3U8 文件，包含录制服务恢复之后的切片文件索引信息。文件名增加 `bak<n>` 前缀，`n` 为高可用机制在该次录制中被启用的 `index`, `0` 表示第一次启用。
...
如需将原 M3U8 文件与启用高可用机制后生成的新 M3U8 文件合并为一个 MP4 文件，请参考[转换录制文件格式](/zh-CN/api-reference/cloud-recording/restful/user-guides/manage-file/convert-format)。
...
:::info[信息]
如果某些播放器因 `#EXTINF:<length>` 后没有逗号而出现兼容性问题，你可以在 `start` 请求中的 `recordingConfig` 字段中配置字段 `privateParams`，即 `"recordingConfig": {"privateParams":"{\"correctEXTINF\":true}", ...}`。
:::
