...
description: "agora_chat_sdk API docs, for the Dart programming language."
---

<a id="get-started-with-agora-chat"></a>
...
:::info
You need to run `flutter doctor` to check whether both the development environment and the deployment environment are correct.
:::

<a id="token-generation"></a>
...
1. Click **Config** for **Chat(Beta)** under **Features**.

1. Click **Create User** on the **User** page under **Operation Management**.

1. Fill in the user information and click **Save** to create a user.

<a id="generate-a-user-token"></a>
...
<a id="1-create-a-flutter-project"></a>
...
1. In the `quick_start/android/app/proguard-rules.pro` file, add the following lines to prevent code obfuscation:
...
<a id="3-integrate-the-agora-chat-sdk"></a>
...
<a id="implement-peer-to-peer-messaging"></a>
...
<a id="3-log-out-of-an-account"></a>
...
1. In the `_addChatListener` method, add the following to add the chat event handler:
...
1. Under the `initState` method, add the `dispose` method to remove the chat event handler, as shown in the following:
...
<a id="run-and-test-the-project"></a>
...
<a id="agora_chat_sdk"></a>

### [agora_chat_sdk](/zh-CN/api-reference/im/flutter/agora-chat-sdk)
