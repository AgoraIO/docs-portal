...
声网服务端 SDK 支持在单流和合流录制下，对录制视频添加水印。支持的水印类型包括：
...
开始前，请确保你已经参考[开始录制音视频](/zh-CN/realtime-media/recording/local-server-recording/get-started/start-recording)在项目中集成了录制 SDK 并实现了基础的录制功能。

## 实现方法

无论是单流录制，还是合流录制，SDK 都支持在录制视频过程中添加水印。下文以合流录制为例，介绍实现水印功能时核心的参数设置。

你可以通过 [`enableAndUpdateVideoWatermarks`](/api-ref/recording/cpp/iagoramediartcrecorder#enableandupdatevideowatermarks) 开启或更新水印设置：
...
如果需要关闭水印功能，可以调用 [`disableVideoWatermarks`](/api-ref/recording/cpp/iagoramediartcrecorder#disablevideowatermarks) 实现。

:::info[信息]
单流录制场景下，实现的水印功能与合流录制一致。两者的差异在于不同场景下，调用的 API 不同。单流下：

- 开启或更新水印：[`enableAndUpdateVideoWatermarksByUid`](/api-ref/recording/cpp/iagoramediartcrecorder#enableandupdatevideowatermarksbyuid)
- 关闭水印：[`disableVideoWatermarksByUid`](/api-ref/recording/cpp/iagoramediartcrecorder#disablevideowatermarksbyuid)
:::
...
无论添加哪种水印，都需要设置水印在视频中的位置。SDK 在 [`WatermarkConfig`](/api-ref/recording/cpp/struct#watermarkconfig) 结构体中提供了两种位置设置模式。
...
- [API 参考](/api-ref/recording/cpp/api-overview)
