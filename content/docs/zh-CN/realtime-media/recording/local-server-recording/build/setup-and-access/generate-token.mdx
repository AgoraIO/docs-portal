---
title: "使用 Token 鉴权"
description: "了解如何在本地服务端录制中 Token 鉴权，包括适用场景、配置步骤、实现要点和注意事项。"
---
鉴权是指在用户访问你的系统前，对用户权限进行校验，声网用 Token 对用户鉴权。

Token 也称为动态密钥，是在加入频道时用于校验用户权限的一组字符串。当录制服务加入 RTC 频道时传入 Token，声网服务端会根据 Token 中的信息校验用户的权限。

下图展示声网使用 Token 对录制服务端鉴权的流程：

![token 流程](https://web-cdn.agora.io/docs-portal/img/recording/token-authentication.svg)
...
- 参考[开始录制音视频](/zh-CN/realtime-media/recording/local-server-recording/get-started/start-recording)在你的项目中集成录制 SDK 并实现基础的录制功能。
...
```cpp tab="C++" tabGroup="zh-cn-realtime-media-recording-local-server-recording-build-setup-and-access-generate-token-1"
recorder->joinChannel(config.token.c_str(), config.ChannelName.c_str(), config.UserId.c_str());
```

```java tab="Java" tabGroup="zh-cn-realtime-media-recording-local-server-recording-build-setup-and-access-generate-token-1"
...
你也可以直接前往声网控制台获取临时 Token 来调试你的项目，详见[获取临时 Token](/zh-CN/realtime-media/recording/local-server-recording/build/setup-and-access/enable-service#获取临时-token)。
