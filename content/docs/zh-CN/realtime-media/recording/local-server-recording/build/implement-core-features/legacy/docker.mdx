---
title: "使用云容器部署录制 SDK"
description: "了解如何在本地服务端录制中云容器部署录制 SDK，包括适用场景、配置步骤、实现要点和注意事项。"
---
我们推荐使用云容器（云主机 + Docker）的方式部署录制 SDK。

使用这种方式，你无需管理底层服务器，只要提供打包好的镜像，即可运行容器，并仅为容器实际运行消耗的资源付费，可以节约成本。本文以阿里云[弹性容器实例](https://help.aliyun.com/product/87486.html) (Elastic Container Instance) 为例介绍具体的部署方法，你也可以选择其他的云容器实例服务。

## 前提条件

开始前请确保你的本地开发环境满足如下要求：

- 安装 [Docker](https://docs.docker.com/install/) 环境。
- 下载[录制 SDK](/zh-CN/realtime-media/recording/local-server-recording/reference/downloads)。
...
- 启动参数：录制的具体设置，可以参考[设置录制选项](/zh-CN/realtime-media/recording/local-server-recording/build/implement-core-features/legacy/record-by-cmd#设置录制格式)。

  ```shell
  --appId <yourAppId> --channelKey <token> --channelProfile 1 --recordFileRootDir /data/recorder --channel demo --uid 886 --appliteDir /usr/src/recording --idle 30 --lowUdpPort 40000 --highUdpPort 41004 --isMixingEnabled 1 --mixedVideoAudio 2
  ```
...
阿里云 [CloudShell](https://help.aliyun.com/document_detail/90256.html) 支持使用命令行的方式来创建和管理弹性容器实例，下面给出创建实例的命令行示例供你参考。
