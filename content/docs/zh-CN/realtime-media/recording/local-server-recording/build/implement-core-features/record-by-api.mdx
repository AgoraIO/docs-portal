...
你可以根据自己的开发习惯选择一种方式。本文介绍如何通过调用 API 实现录制。

## 前提条件

开始前请确保你已经完成[集成录制 SDK](/zh-CN/realtime-media/recording/local-server-recording/build/implement-core-features/legacy/integrate-sdk)。
...
- `channelKey`：如果待录制的频道使用了 Token，你需要在该参数中传入 Token。详见[校验用户权限](/zh-CN/realtime-media/recording/local-server-recording/build/setup-and-access/generate-token)。
...
- 如果想了解如何通过命令行录制，可以参考[使用命令行录制](/zh-CN/realtime-media/recording/local-server-recording/build/implement-core-features/legacy/record-by-cmd)。
...
开始前请确保你已经完成[集成录制 SDK](/zh-CN/realtime-media/recording/local-server-recording/build/implement-core-features/legacy/integrate-sdk)，并将以下文件复制到你的项目：
