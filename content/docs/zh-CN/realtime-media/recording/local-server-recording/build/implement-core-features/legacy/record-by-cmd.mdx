...
- `triggerMode`：选择录制启动模式。
  ...
  - 1：手动模式，手动输入命令开始和结束录制。详见[当使用命令行录制时，如何控制录制进程？](/faq/integration-issues/cmd-control-record)。
...
- `mixedVideoAudio`：如果 `isMixingEnabled` 设为 `1` ，该参数可以设置是否实时混合语音和视频:
 ...
 具体的播放器支持请参考[录制生成的文件支持哪些播放器？](/faq/integration-issues/recording-player)。
