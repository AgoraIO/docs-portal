...
前往[资源获取](/zh-CN/realtime-media/recording/local-server-recording/resources)下载本地服务端录制 SDK 并解压，在 `tools` 文件夹下找到 `ffmpeg.tar.gz` 和 `video_convert.py` 文件。
...
| `-p` | 设置转码后视频的帧率，单位为 fps。默认值为 15 fps。取值范围为 5 fps 到 120 fps。如果设置低于 5 fps，则按 5 fps 执行。具体设置请参考[分辨率、帧率、码率对照表](/zh-CN/realtime-media/recording/local-server-recording/legacy/set-output-video#分辨率帧率码率对照表)。 |
| `-r` | 设置转码后视频的分辨率，格式为 `宽 高`，例如 `-r 640 360`。具体设置请参考[分辨率、帧率、码率对照表](/zh-CN/realtime-media/recording/local-server-recording/legacy/set-output-video#分辨率帧率码率对照表)。 |
