...
title: "上传模块事件"
---
当 `payload` 中的 `serviceType` 返回 `2` 时，表示该回调事件与上传模块相关。本页介绍上传模块相关的回调事件。如果你想查看其他云端录制的相关事件，可以参考[回调事件概览](/zh-CN/realtime-media/recording/cloud-recording/reference/ncs-events)。
