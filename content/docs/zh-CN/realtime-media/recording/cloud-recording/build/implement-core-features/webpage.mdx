---
title: "页面录制事件"
description: "了解如何在云端录制中页面录制事件，包括适用场景、配置步骤、实现要点和注意事项。"
---
当 payload 中的 serviceType 返回 6 时，表示该回调事件的服务类型是页面录制。

本页介绍页面录制相关的回调事件。如果你想查看其他云端录制的相关事件，可以参考[回调事件概览](/zh-CN/realtime-media/recording/cloud-recording/reference/ncs-events)。
...
| `4`    | `page_load_timeout`   |使用[检测页面加载超时](/zh-CN/realtime-media/recording/cloud-recording/build/optimize-and-operate/webpage#检测页面加载超时)功能的情况下，页面加载超时导致录制停止。 |
...
`eventType` 为 `73` 表示页面录制过程中，页面重新加载， `details` 中包含以下字段：
 ...
 - `reason`：String 类型，页面重新加载的原因。包含以下原因：
  ...
  - `"page_load_timeout"`：检测到页面加载超时。该回调仅在以下条件下返回该字段：启用[检测页面加载超时](/zh-CN/realtime-media/recording/cloud-recording/build/optimize-and-operate/webpage#检测页面加载超时)功能，且页面加载超时。
