...
在[单流录制模式](/zh-CN/realtime-media/recording/cloud-recording/reference/concepts#单流录制)下，录制服务会针对不同 UID 的音视频流分别生成对应的音频和视频文件。生成的文件类型可能包括 M3U8、TS 和 WebM 文件。
...
在[合流录制模式](/zh-CN/realtime-media/recording/cloud-recording/reference/concepts#合流录制模式)下，录制过程中会实时生成一个 M3U8 文件和多个 TS 文件。若在录制配置中将 `avFileType` 设置为 `["hls","mp4"]`，录制服务会同时生成 MP4 文件；若未设置或未包含 `"mp4"`，则仅生成 M3U8/TS，你也可使用[转码脚本](/zh-CN/realtime-media/recording/cloud-recording/build/manage-recorded-files/convert-format)将 M3U8/TS 合并并转换为 MP4 格式。
...
在[页面录制模式](/zh-CN/realtime-media/recording/cloud-recording/reference/concepts#页面录制模式)下，录制过程中会实时生成一个 M3U8 文件和多个 TS 文件。若在录制配置中将 `avFileType` 设置为 `["hls","mp4"]`，录制服务会同时生成 MP4 文件；若未设置或未包含 `"mp4"`，则仅生成 M3U8/TS，你也可使用[转码脚本](/zh-CN/realtime-media/recording/cloud-recording/build/manage-recorded-files/convert-format)将 M3U8/TS 合并并转换为 MP4 格式。
...
如需将原 M3U8 文件与启用高可用机制后生成的新 M3U8 文件合并为一个 MP4 文件，请参考[转换录制文件格式](/zh-CN/realtime-media/recording/cloud-recording/build/manage-recorded-files/convert-format)。
