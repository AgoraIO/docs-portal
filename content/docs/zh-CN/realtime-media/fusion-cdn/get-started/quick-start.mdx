...
开始前，请确保你已经[开通融合 CDN 服务](/zh-CN/realtime-media/fusion-cdn/build/enable-service)。
...
- 为保障安全性，声网强烈建议你对每个推流域名和播放域名都设置防盗链密钥。防盗链密钥需要你自行提供，用于生成 URL 中的鉴权字符串。详见[计算鉴权字符串](/zh-CN/realtime-media/fusion-cdn/build/streaming-url#key)。
:::
...
推流支持 RTMP 和 RTMPS 协议。详见[拼接推流 URL](/zh-CN/realtime-media/fusion-cdn/build/streaming-url#拼接推流-url)。

## 开始推流

你可以通过以下方式开始推流：

- 使用 OBS 等推流软件。详见[推流软件配置](/zh-CN/realtime-media/fusion-cdn/build/obs-config)。
- 使用旁路推流 RESTful API 将声网频道内的流转换成 RTMP/RTMPS 协议的流。详见[旁路推流](/zh-CN/realtime-media/media-push)。

## 拼接播流 URL

播流支持 RTMP、HTTP-FLV、HLS 协议。详见[拼接播流 URL](/zh-CN/realtime-media/fusion-cdn/build/streaming-url#拼接播流-url)。
...
如果你需要接收推流、录制、截图等业务事件的通知，你还可以[使用消息通知服务](/zh-CN/realtime-media/fusion-cdn/build/enable-ncs)。
