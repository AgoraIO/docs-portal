...
- `sign`：防盗链签名。字符串中的字母必须为小写。详见[计算鉴权字符串](/zh-CN/realtime-media/fusion-cdn/build/manage-media-streams/streaming-url)。
...
防盗链密钥为不超过 128 字节的字符串，你需要自行设置。每个推流域名和播流域名都可以分别设置防盗链密钥，详见[直播流鉴权配置](/zh-CN/api-reference/api-ref/fusion-cdn/timestamp-auth-key-update)。
