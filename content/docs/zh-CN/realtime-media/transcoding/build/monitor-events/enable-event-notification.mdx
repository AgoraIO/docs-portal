...
如果你的网络环境部署了防火墙，请将声网消息通知服务器的 IP 地址添加到白名单，以正常使用声网消息通知服务。详见[查询消息通知服务器的 IP 地址](../cloud-transcoder/operations/get-v1-ncs-ip )。
...
- 声网系统将自动生成 `secret`。`secret` 是字符串类型的密钥，可以用来生成签名，你需要保存这个密钥以备后续验签，详见[验证签名](#验证签名)。
