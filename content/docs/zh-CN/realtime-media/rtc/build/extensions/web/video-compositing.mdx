...
- 已集成 v4.17.0 或以上版本的 Web SDK，实现基础的实时音视频互动功能，具体参考[快速开始](../../get-started/quick-start?platform=web)文档。
...
- 屏幕共享视频轨道。
- 两张本地图片。
- 源视频轨道 1：通过摄像头采集的视频流创建，并且使用[虚拟背景插件](/zh-CN/realtime-media/rtc/build/extensions/web/virtual-background)移除该轨道的背景。
- 源视频轨道 2：通过本地视频文件创建。
...
1. 集成虚拟背景插件并了解注意事项。具体请参考[使用虚拟背景插件](/zh-CN/realtime-media/rtc/build/extensions/web/virtual-background)。
...
1. 分别调用 [`createScreenVideoTrack`](/zh-CN/api-reference/rtc/web/interfaces/iagorartc#createscreenvideotrack)、 [`createCameraVideoTrack`](/zh-CN/api-reference/rtc/web/interfaces/iagorartc#createcameravideotrack) 和 [`createCustomVideoTrack`](/zh-CN/api-reference/rtc/web/interfaces/iagorartc#createcustomvideotrack) 方法，创建三个视频轨道：
