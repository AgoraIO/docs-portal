---
title: "使用 AI 降噪插件"
description: "了解如何在实时互动 RTC 中 AI 降噪插件，包括适用场景、配置步骤、实现要点和注意事项。"
titlePlatforms:
  - web
---
## 使用 AI 降噪插件
...
- 已集成 v4.15.0 或以上版本的 Web SDK，实现基础的实时音视频互动功能，具体参考[快速开始](../../get-started/quick-start?platform=web)文档。

## 实现步骤

:::warning[注意]
本节介绍如何使用**最新版本**的 AI 降噪插件。历史版本的实现步骤可能会有所不同，具体差异详见[插件发版说明](/zh-CN/realtime-media/rtc/build/extensions/web/release)。
:::
...
调用 [`AgoraRTC.registerExtensions`](/zh-CN/api-reference/rtc/web/interfaces/iagorartc#registerextensions) 方法并传入创建的 `AIDenoiserExtension` 实例注册 AI 降噪插件。你还可以监听 Wasm 文件加载失败的事件。
...
2. 调用 SDK 的 [`pipe`](/zh-CN/api-reference/rtc/web/interfaces/imicrophoneaudiotrack#pipe) 方法并指定 [`processorDestination`](/zh-CN/api-reference/rtc/web/interfaces/imicrophoneaudiotrack#processordestination) 属性，将插件注入音频处理管道。
...
该方法必须在 [`pipe`](/zh-CN/api-reference/rtc/web/interfaces/imicrophoneaudiotrack#pipe) 之后调用。
...
设置降噪时延等级，详见 [AIDenoiserProcessorLatency](#aidenoiserprocessorlatency)。
