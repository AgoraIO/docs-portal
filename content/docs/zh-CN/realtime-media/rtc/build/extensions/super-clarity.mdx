...
- 已集成 v4.19.0 或以上版本的 Web SDK，实现基础的实时音视频互动功能，具体参考[快速开始](../../get-started/quick-start?platform=web)文档。
...
在加入频道之前，创建 `SuperClarityExtension` 对象并调用 SDK 的[`registerExtensions`](/zh-CN/api-reference/rtc/web/interfaces/iagorartc#registerextensions) 方法注册插件。
...
2. 调用 SDK 的 [`IRemoteVideoTrack.pipe`](/zh-CN/api-reference/rtc/web/interfaces/iremotevideotrack#pipe) 方法连接 AI 画质插件与该视频轨道之间的管线。
...
需要切换视频轨道时，调用 SDK 的 [`IRemoteVideotrack.unpipe`](/zh-CN/api-reference/rtc/web/interfaces/iremotevideotrack#unpipe) 方法断开 `processor` 与当前视频轨道的连接，然后调用 SDK 的 `IRemoteVideotrack.pipe` 方法连接新的视频轨道。
