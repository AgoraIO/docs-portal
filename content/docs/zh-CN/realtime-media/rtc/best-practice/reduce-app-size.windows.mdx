...
视频 SDK 支持音频和视频功能，包体积大。如果你只需使用音频功能，建议你集成音频 SDK，详见[实现纯语音互动](/zh-CN/realtime-media/rtc/basic-features/audio-quick-start.windows#集成-sdk)。
...
通过官网 [SDK 下载链接](/zh-CN/realtime-media/rtc/resources.windows)集成时，你可以将不需要使用的插件文件直接删除。
...
自 v4.1.0 起，SDK 提供版权音乐插件（DRM, Data Rights Management），实现实时互动场景中播放版权音乐相关功能，例如检索音乐资源、获取音乐榜单及榜单详情、预加载及播放音乐资源、下载歌词及海报等。详见 [v4.1.0 发版说明](/zh-CN/realtime-media/rtc/overview/release-notes.windows#v4.1.0)。
...
如需使用该功能，详见[使用面部捕捉插件](/zh-CN/realtime-media/rtc/advanced-features/face-capture.windows)。
...
屏幕共享插件提供屏幕共享功能，支持将自己的屏幕内容分享给观众或其他用户观看，以提高沟通效率。用法详见[屏幕共享](/zh-CN/realtime-media/rtc/basic-features/screen-share.windows.windows)。
