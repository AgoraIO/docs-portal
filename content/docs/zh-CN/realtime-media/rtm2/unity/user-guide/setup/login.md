...
你可以通过 `LoginAsync` 的返回值或监听 `` 事件通知来判断登录是否成功以及登录失败的错误码和错误原因。执行登录操作时，客户端的网络连接状态为 ``；登录成功后，其状态会更新为 `` 状态。为持续监测客户端的网络连接状态，声网建议你在应用的整个生命周期内持续监听 `` 事件通知，详见<a href=>事件监听</a >。
...
当不再需要使用 RTM 服务时，你可以通过显式调用 `LogoutAsync` 退出登录。退出登录意味着客户端与 RTM 服务器关闭长连接，你会自动退出或取消订阅所有频道（Stream Channel 及 Message Channel），频道中的其他用户也会收到你的离开频道的 `OnPresenceEvent` 事件通知。
