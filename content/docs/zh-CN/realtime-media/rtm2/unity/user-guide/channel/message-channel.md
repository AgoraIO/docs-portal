...
监听了 `OnMessageEvent` 事件通知后，你就可以调用 `SubscribeAsync` 方法订阅频道，从而接收频道中的消息和事件通知。
...
`SubscribeAsync` 方法只允许你一次订阅一个频道。如需订阅多个频道，你可以多次调用该方法。例如，当你想同时订阅频道 `chats.room1` 和频道 `chats.room2` 时，你可以参考如下示例代码：
...
如需停止接收某个频道中的所有消息和事件通知，你可以调用 `UnsubscribeAsync` 方法。
