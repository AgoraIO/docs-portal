...
**Stream** 频道是 RTM 提供的一种基于 Room 模型的频道类型。与 Message 频道不同的是，使用 Stream 频道的相关方法前你需要先创建 `IStreamChannel` 对象实例。加入 Stream 频道后，你就可以监听频道中的事件通知了，但如需收发消息，你还需要通过 Topic 特性来实现，详见 [Topic](../topic/topic-basic)。RTM 允许你的 App 中同时存在成千上万个 Stream 频道，但由于端侧性能和带宽的限制，单个客户端同时只能加入有限数量的频道，详见 [API 使用限制](../setup/api-limits)。
...
你可以通过以下方式创建 `IStreamChannel` 对象实例：

```csharp
IStreamChannel streamChannel = rtmClient.CreateStreamChannel("chat_room");
```

该方法一次只能创建一个 `IStreamChannel` 对象实例，如需同时创建多个 `IStreamChannel` 对象实例，你需要多次调用该方法。
...
RTM 支持你在单个客户端创建无限个 `IStreamChannel` 对象实例，但声网建议你结合实际需求和端侧性能情况合理创建。例如，如果你同时持有多个暂时用不到的 `IStreamChannel` 实例，为避免浪费资源，声网建议你及时销毁该实例，并在需要使用的时候再创建。

## 加入频道

创建 `IStreamChannel` 对象实例后，调用 `JoinAsync` 方法加入 Stream Channel，以便监听其中的事件通知。
...
{
    Debug.Log(string.Format("User: Join stream channel success! at Channel:", response.UserId, response.ChannelName));
}
```

加入频道时，你需要在 `JoinChannelOptions` 中设置 `token` 参数。为方便体验和测试 App，你可以在创建项目时选择**鉴权机制**为**调试模式：APP ID**，并在加入频道时将 App ID 填写至 `token` 参数中。
...
如果你不再需要接收某个频道中所有消息和事件通知，你可以调用 `LeaveAsync` 方法离开该频道。
...
离开频道后，只要当前的 `IStreamChannel` 对象实例没有被销毁，你还可以调用 `JoinAsync` 方法再次加入该频道。

## 销毁频道

对于不再需要的 `IStreamChannel` 对象实例，你可以调用 `Dispose` 方法销毁它以释放资源。
