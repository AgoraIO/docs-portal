...
RTM 目前只支持字符串和二进制类型的消息数据格式。如果你的业务中使用了 JSON、Object 等其他数据类型，或者 protobuf 等第三方数据构建工具，那么你需要在发送消息之前对消息数据进行序列化。关于如何有效构建负载数据结构及推荐的序列化方式可以参考[消息负载结构化](./constructed.md)和[消息负载序列化](./serialized.md)。

## Message Channel 中发送消息

在 Message Channel 中，你只需调用 `PublishAsync` 方法即可发送消息，无需提前订阅频道。该方法一次只能向一个频道发送消息，如需向多个频道发送消息，你需要多次调用该方法。RTM 并不限制你向多少个频道发送消息，也不限制有多少用户向一个频道发送消息，但是对你同时向一个频道发送消息的频率有一定的限制，详见 [API 使用限制](../setup/api-limits)。

> **信息**
> `PublishAsync` 方法仅适用于 Message Channel，你无法使用该方法向 Stream Channel 发送消息。
...
方法调用成功后，RTM 会在 100 毫秒内将你的消息发送给订阅该频道的所有在线用户。同时，你会收到 `PublishResult` 类型的返回值，虽然其中的内容是空的。
...
2. 你需要调用 `JoinAsync` 方法加入该频道。

3. 你需要调用 `JoinTopicAsync` 方法注册成为指定 Topic 的消息发布者。

满足上述条件后，你就可以调用 `PublishTopicMessageAsync` 方法向指定的 Topic 发送消息了。该方法一次只能向一个 Topic 发送消息，如需向多个 Topic 发送消息，你可以多次调用该方法。
...
成功向 Topic 发送消息后，RTM 会在 100 毫秒内将你的消息发送给订阅该 Topic 的所有在线用户。同时，你会收到 `RtmResult` 类型的返回值，虽然当前其中的内容是空的。该消息和在 Message Channel 中发送的消息具备同样的延迟特性和 SLA 保障。
