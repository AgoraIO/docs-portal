...
- 1 次登录 (`LoginAsync`) 请求计作 1 条消息。
- 1 次登出 (`LogoutAsync`) 请求计作 1 条消息。
- 1 次更新 Token  (`RenewTokenAsync`)  计作 1 条消息。
- 接收到 1 条 SDK 连接状态的变更通知计作 1 条消息。
- 接收到 1 条 Token 即将过期通知计作 1 条消息。

### 频道操作

- 1 次订阅 (`SubscribeAsync`) Message Channel 计作 1 条消息。
- 1 次取消订阅 (`UnsubscribeAsync`) Message Channel 计作 1 条消息。
- 1 次加入 (`JoinAsync`) Stream Channel 计作 1 条消息。
- 1 次离开 (`LeaveAsync`)  Stream Channel 计作 1 条消息。

### Topic 操作

- 1 次加入 (`JoinTopicAsync`) Topic 方法调用计作 1 条消息。
- 1 次离开 (`LeaveTopicAsync`) Topic 方法调用计作 1 条消息。
- 1 次订阅 (`SubscribeTopicAsync`) Topic 方法调用计作 1 条消息。
- 1 次取消订阅 (`UnsubscribeTopicAsync`) Topic 方法调用计作 1 条消息。
- 接收到 1 条 Topic 变更通知计作 1 条消息。

### 收发消息

- 向 Message Channel 中发布 (`PublishAsync`) 任何 1 条消息计作 1 条消息。
- 在 Message Channel 中收到的任何 1 条消息计作 1 条消息。例如，一个客户端向一个 Message Channel 发送 1 条消息，而该频道被 10 人订阅，则计作 1 条发送消息和 10 条接收消息，总计 11 条消息。
- 向 Stream Channel 中的 Topic 中发布 (`PublishTopicMessageAsync`) 的任何 1 条消息计作 1 条消息。
...
- 1 次 `WhoNowAsync` 查询计作 1 条消息。
- 1 次 `WhereNowAsync` 查询计作 1 条消息。
- 1 次设置 (`SetStateAsync`)、查询 (`GetStateAsync`)、删除 (`RemoveStateAsync`) 任何 1 条临时用户状态，计作 1 条消息。
...
- 设置 (`SetChannelMetadataAsync`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 查询 (`GetChannelMetadataAsync`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 更新 (`UpdateChannelMetadataAsync`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 删除 (`RemoveChannelMetadataAsync`) 任何 1 条 Channel Metadata，计作 1 条消息。
...
- 设置 (`SetUserMetadataAsync`) 任何 1 条 User Metadata，计作 1 条消息。
- 查询 (`GetUserMetadataAsync`) 任何 1 条 User Metadata，计作 1 条消息。
- 更新 (`UpdateUserMetadataAsync`) 任何 1 条 User Metadata，计作 1 条消息。
- 删除 (`RemoveUserMetadataAsync`) 任何 1 条 User Metadata，计作 1 条消息。
...
- 任意 1 次对锁的设置 (`SetLockAsync`) 操作计作 1 条消息。
- 任意 1 次对锁的获取 (`AcquireLockAsync`) 操作计作 1 条消息。
- 任意 1 次对锁的查询 (`GetLocksAsync`) 操作计作 1 条消息。
- 任意 1 次对锁的释放 (`ReleaseLockAsync`) 操作计作 1 条消息。
- 任意 1 次对锁的剥夺 (`RevokeLockAsync`) 操作计作 1 条消息。
- 任意 1 次对锁的删除 (`RemoveLockAsync`) 操作计作 1 条消息。
