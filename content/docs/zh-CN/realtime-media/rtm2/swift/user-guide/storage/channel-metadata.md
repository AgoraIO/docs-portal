...
同时，RTM 还会触发一个 `update` 类型的 `didReceiveStorageEvent` 事件通知，并在 100 ms 内通知到频道中的其他人，详见[事件通知](./event.md)。
...
同时，RTM 还会触发一个 `update` 类型的 `didReceiveStorageEvent` 事件通知，并在 100ms 内通知到频道中的其他人。`didReceiveStorageEvent` 详见[事件通知](event)。
...
同时，RTM 还会触发一个 `update` 类型的 `didReceiveStorageEvent` 事件通知，并在 100ms 内通知到频道中的其他人，详见[事件通知](./event.md)。
