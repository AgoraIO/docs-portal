...
成功加入频道后，SDK 会触发 `remoteJoinTopic` 类型的 `didReceiveTopicEvent` 事件，频道中所有开启 Topic 事件监听的用户都会收到此事件通知。
...
当你不再需要向某个 Topic 发送消息，或者同时加入的 Topic 超出数量限制时，你可以离开某个 Topic 以释放资源。成功离开 Topic 后，SDK 会触发 `remoteLeaveTopic` 类型的 `didReceiveTopicEvent` 事件，频道中所有开启 Topic 事件监听的用户都会收到此事件通知。
