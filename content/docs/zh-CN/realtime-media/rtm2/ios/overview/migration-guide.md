...
相比于 v1，v2 对初始化参数做了很大的调整，增加了很多新特性，例如端侧加密、云代理等，详见 [API 参考](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#create)。此外，v2 还丰富了接口调用的错误信息，你可以通过 `AgoraRtmErrorInfo` 数据结构获取错误码、错误原因及 API 操作名称，以便于你能更快地排查故障。结合[错误排查](../error-codes)文档，你也可以快速找到解决方法。
...
关于事件通知的更多信息及负载数据结构见[事件监听](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#事件监听)。
...
你也可以随时通过 `getState` 方法获取用户的在线状态，或者通过 `removeState` 方法删除用户的状态。用户临时状态变更后，RTM 服务器会触发 `AgoraRtmPresenceEventTypeRemoteStateChanged` 类型的 `didReceivePresenceEvent` 事件通知。具体用法见[临时用户状态](../user-guide/presence/temporary-user-state)。
