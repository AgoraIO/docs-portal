...
- 修复部分场景下，设置 [`Presence`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-presence/presence) 的临时用户状态后，事件通知无法正常下发的问题。
...
- 该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmClientConfig) 中新增 `callbackQueue` 参数，支持对回调所在的线程进行设置。
...
该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#rtmconfig) 中增加 `reconnectTimeout` 字段。你可以通过该字段配置 SDK 在登录或断线重连时，返回超时错误的时间，单位为秒。当登录或重连超过设置的时间，SDK 会通过回调报告当前的连接状态和原因。
...
在 IoT 场景下，设备可能受限于互联网服务提供商 (ISP)。为解决这一限制，该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#rtmconfig) 中增加 `ispPolicyEnabled` 字段。通过将该字段设为 `true`，SDK 会仅连接到已向运营商报备的域名或 IP 白名单中的服务器。
...
为反馈更新 Token 操作的结果，该版本新增错误码 `AgoraRtmErrorChannelNotJoined` (10026)。
...
- 取消了 `AgoraRtmStateItem` 类，并将如下参数由 `NSArray` 类型变为 `NSDictionary` 类型，参考对应 API 文档修改你的实现代码。
  - [`setState`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-presence/presence#setState) 中的 `items` 参数。
  - [`PresenceEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmPresenceEvent) 中的 `states` 参数。
  - [`UserState`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-presence/presence#getState) 中的 `states` 参数。

### 新特性

#### 新增支持 macOS 和 VisionOS 平台

该版本新增支持 macOS 和 VisionOS 平台，支持的最小版本和目标架构见[平台支持](./platform-support.md)。同时，该版本也将包名由 `AgoraRtm_iOS` 变更为 `AgoraRtm`。升级至该版本后，在使用 CocoaPods 集成方式时，你需要将 `Podfile` 文件修改为以下内容：
...
该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmClientConfig) 中新增 `privateConfig` 参数，用于实现私有化部署功能。详见[私有化配置](../user-guide/setup/private-setup)。

#### 新增心跳时间配置

该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmClientConfig) 中新增 `heartbeatInterval` 参数，用于配置 SDK 向 RTM 服务器发送心跳包的时间间隔。详见 [Heartbeat Interval 与 Presence Timeout](../user-guide/link/link-basic#heartbeatinterval-and-presencetimeout)。

#### 新增双环境配置

该版本在 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmClientConfig) 中新增 `protocolType` 参数，用于自定义网络传输协议，以便于更好地适配你的网络环境。详见[连接协议配置](../user-guide/setup/application-setup#protocol)。

#### 新增用户频道类型

该版本在 [`AgoraRtmChannelType`](https://doc.shengwang.cn/api-ref/rtm2/ios/enumv#channeltype) 中新增 `AgoraRtmChannelTypeUser` 类型，用于向指定用户发送消息。该功能可以替代 v1 的点对点消息功能。详见[在 User Channel 中发送消息](../user-guide/message/send-message#userchannel)。

#### 新增静默模式配置

该版本在 [`AgoraRtmSubscribeChannelFeature`](https://doc.shengwang.cn/api-ref/rtm2/ios/enumv#subscribechannelfeature) 中新增 `AgoraRtmSubscribeChannelFeatureBeQuiet` 枚举，在 [`AgoraRtmJoinChannelFeature`](https://doc.shengwang.cn/api-ref/rtm2/ios/enumv#joinchannelfeature) 中新增 `AgoraRtmJoinChannelFeatureBeQuiet` 枚举，用于设置静默订阅或加入频道。设置成功后，频道中其他用户无法感知该用户的出席状态。

### 改进

#### 改进连接状态管理逻辑

该版本新增 [`didReceiveLinkStateEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#事件监听) 回调，用于替代废弃的 `connectionChangedToState` 回调，以便更好地管理连接状态。详见[连接管理](../user-guide/link/link-basic)。
...
- [`AgoraRtmMessageEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#messageevent)
- [`AgoraRtmPresenceEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#presenceevent)
- [`AgoraRtmTopicEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#topicevent)
- [`AgoraRtmStorageEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#storageevent)
- [`AgoraRtmLockEvent`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#lockevent)

#### 优化 API 行为

该版本优化了以下 API 的行为：

- [`loginByToken`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#login)：
...
- [`subscribeWithChannel`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-channel/channel#subscribe)：
  - 在之前版本中，不支持连续多次调用该方法。
  - 自 2.2.0 版本起，支持连续多次调用该方法。
- [`joinWithOption`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-channel/channel#join)：
  - 在之前版本中，不支持连续多次调用该方法，也不支持在该方法的 `token` 参数中传入空字符串。
  - 自 2.2.0 版本起，支持连续多次调用该方法。此外，当 `token` 参数为空字符串时，SDK 会默认使用你初始化时传入的 App ID 替代 Token。

#### 修改 presenceTimeout 取值范围

该版本将 [`AgoraRtmClientConfig`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-configuration/configuration#AgoraRtmClientConfig) 中 `presenceTimeout` 参数的取值范围由 [10,300] 修改为 [5,300]。
...
- 在 `AgoraRtmPublishOptions` 中删除了 `sendTs` 参数，并新增了 `channelType` 参数。详见 [`AgoraRtmPublishOptions`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-message/message#options)。
- 将 `publishTopicMessage` 方法中的 `options` 参数类型由 `AgoraRtmPublishOptions` 修改为 `AgoraRtmTopicMessageOptions`。详见 [`AgoraRtmTopicMessageOptions`](https://doc.shengwang.cn/api-ref/rtm2/ios/toc-topic/topic#options)。

### 改进

该版本新增了 `AgoraRtmErrorInvalidChannelType`、`AgoraRtmErrorInvalidEncryptionParameter` 和 `AgoraRtmErrorOperationRateExceedLimitation` 错误码。错误码描述及解决方法见[错误排查](../error-codes)。
