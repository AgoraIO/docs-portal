...
同时，RTM 还会触发一个 `AgoraRtmStorageEventTypeUpdate` 类型的 `didReceiveStorageEvent` 事件通知，并在 100 ms 内通知到订阅了此 User Metadata 的其他人，详见[事件通知](./event.md)。
...
同时，RTM 还会触发一个 `AgoraRtmStorageEventTypeUpdate` 类型的 `didReceiveStorageEvent` 事件通知，并在 100ms 内通知到订阅此 User Metadata 的其他人。`didReceiveStorageEvent` 详见[事件通知](event)。
...
以上示例中我们订阅了用户名为 `Tony` 的 User Metadata，当 `Tony` 的 User Metadata 变化时 RTM 会触发一个 `AgoraRtmStorageEventTypeUpdate` 类型的 `didReceiveStorageEvent` 事件通知，并在 100 ms 内通知到你。
