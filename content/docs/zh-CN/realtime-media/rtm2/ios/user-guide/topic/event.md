...
当用户第一次加入频道时，SDK 会投递 `AgoraRtmTopicEventTypeSnapshot` 类型的 Topic 事件通知给到当前用户，你可以通过该事件通知获得当前频道中 Topic 的历史状态信息。
...
| 事件通知                                                     | 描述                                                                                                          |
| ------ | --------- |
| `AgoraRtmTopicEventTypeSnapshot`  | 本地用户首次加入 Stream 类型频道时触发，通知用户该频道中所有 Topic 的详情。 |
| `AgoraRtmTopicEventTypeRemoteJoinTopic`  | 远端用户加入 Topic 并注册成为该 Topic 的发布者时触发。                                                        |
| `AgoraRtmTopicEventTypeRemoteLeaveTopic` | 远端用户离开 Topic 并取消注册该 Topic 的发布者时触发。                                                        |
