...
监听了 `didReceiveMessageEvent` 事件通知后，你就可以调用 `subscribeWithChannel` 方法订阅频道，从而接收频道中的消息和事件通知。
...
默认情况下，你会收到你所订阅的所有频道的消息事件。如果你无需接收某些特定频道的消息，但仍需接收这些频道中其他类型的事件通知或其他频道的消息，请勿将 `AgoraRtmSubscribeOptions` 参数中的 `features` 字段设为 `AgoraRtmSubscribeChannelFeatureMetadata`。
...
`subscribeWithChannel` 方法只允许你一次订阅一个频道。如需订阅多个频道，你可以多次调用该方法。例如，当你想同时订阅频道 `chats.room1` 和频道 `chats.room2` 时，你可以参考如下示例代码：
...
如需停止接收某个频道中的所有消息和事件通知，你可以调用 `unsubscribeWithChannel` 方法。
