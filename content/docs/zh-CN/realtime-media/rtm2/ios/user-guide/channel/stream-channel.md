...
创建 `AgoraRtmStreamChannel` 对象实例后，调用 `joinWithOption` 方法加入 Stream Channel，以便监听其中的事件通知。
...
离开频道后，只要当前的 `AgoraRtmStreamChannel` 对象实例没有被销毁，你还可以调用 `joinWithOption` 方法再次加入该频道。
