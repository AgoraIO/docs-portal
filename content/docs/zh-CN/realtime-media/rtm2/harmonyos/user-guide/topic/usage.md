...
成功加入频道后，SDK 会触发 `REMOTE_JOIN` 类型的 `topic` 事件，频道中所有开启 Topic 事件监听的用户都会收到此事件通知。
...
try {
  ...
  if (error instanceof RtmError) {
    console.error(`Join topic failed: $ (code: $)`);
  }
}
```
...
当你不再需要向某个 Topic 发送消息，或者同时加入的 Topic 超出数量限制时，你可以离开某个 Topic 以释放资源。成功离开 Topic 后，SDK 会触发 `REMOTE_LEAVE` 类型的 `topic` 事件，频道中所有开启 Topic 事件监听的用户都会收到此事件通知。
...
try {
  ...
  if (error instanceof RtmError) {
    console.error(`Leave topic failed: $ (code: $)`);
  }
}
```
...
try {
  ...
  if (error instanceof RtmError) {
    console.error(`Subscribe topic failed: $ (code: $)`);
  }
}
```
...
try {
  ...
  if (error instanceof RtmError) {
    console.error(`Unsubscribe topic failed: $ (code: $)`);
  }
}
```
