...
当用户临时状态发生变更时，RTM 会实时触发 `remoteStateChanged` 事件通知，加入频道时设置了 `withPresence = true` 的用户会收到事件通知。
...
`setState` 方法和 `removeState` 方法都会触发 `remoteStateChanged` 事件通知，加入频道时设置了 `withPresence = true` 的用户会收到事件通知，其中包含了当前用户临时状态的全量数据。
