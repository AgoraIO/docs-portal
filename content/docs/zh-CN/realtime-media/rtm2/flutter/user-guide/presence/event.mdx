...
| `stateItems`  | 用户临时状态信息列表，`remoteStateChanged` 事件发生时有效。                                     |
...
| `remoteTimeout` | `5`: 当用户掉线超时时触发，频道中的其他人会收到此类通知。                 |
...
如果频道中的在线人数小于实时通知最大人数参数设定的值（默认值为 50），则 Presence 事件通知处于实时通知模式。在此模式下，`remoteJoinChannel`、`remoteLeaveChannel`、`remoteTimeout`、`remoteStateChanged` 事件在触发时会立即发送到客户端。
...
当频道中的在线人数超过实时通知最大人数的值时，频道会进入定时通知模式。在这种模式下，`remoteJoinChannel`、`remoteLeaveChannel`、`remoteTimeout`、`remoteStateChanged` 事件会被 `interval` 事件代替，并以一定的时间间隔发送给频道中所有用户。
