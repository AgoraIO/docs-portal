...
创建 `IStreamChannel` 对象实例后，调用 `join` 方法加入 Stream Channel，以便监听其中的事件通知。
...
离开频道后，只要当前的 `IStreamChannel` 对象实例没有被销毁，你还可以调用 `join` 方法再次加入该频道。

## 销毁频道

对于不再需要的 `IStreamChannel` 对象实例，你可以调用 `release` 方法销毁它以释放资源。
