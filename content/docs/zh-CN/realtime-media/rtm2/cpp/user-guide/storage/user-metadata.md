...
调用该方法后，SDK 会触发 `onSetUserMetadataResult` 回调并返回 API 调用结果。
...
同时，RTM 还会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100 ms 内通知到订阅了此 User Metadata 的其他人，详见[事件通知](./event.md)。
...
调用该方法后，SDK 会触发 `onGetUserMetadataResult` 回调并返回 API 调用结果。
...
调用该方法后，SDK 会触发 `onUpdateUserMetadataResult` 回调并返回 API 调用结果。
...
同时，RTM 还会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100ms 内通知到订阅此 User Metadata 的其他人。`onStorageEvent` 详见[事件通知](event)。
...
调用该方法后，SDK 会触发 `onRemoveUserMetadataResult` 回调并返回 API 调用结果。
...
```
调用该方法后，SDK 会触发 `onSubscribeUserMetadataResult` 回调并返回 API 调用结果。
...
以上示例中我们订阅了用户名为 `Tony` 的 User Metadata，当 `Tony` 的 User Metadata 变化时 RTM 会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100 ms 内通知到你。
...
```
调用该方法后，SDK 会触发 `onUnsubscribeUserMetadataResult` 回调并返回 API 调用结果。
