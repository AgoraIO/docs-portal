...
调用该方法后，SDK 会触发 `onSetChannelMetadataResult` 回调并返回 API 调用结果。
...
同时，RTM 还会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100 ms 内通知到频道中的其他人，详见[事件通知](./event.md)。
...
调用该方法后，SDK 会触发 `onGetChannelMetadataResult` 回调并返回 API 调用结果。
...
调用该方法后，SDK 会触发 `onUpdateChannelMetadataResult` 回调并返回 API 调用结果。
...
同时，RTM 还会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100ms 内通知到频道中的其他人。`onStorageEvent` 详见[事件通知](event)。
...
```
调用该方法后，SDK 会触发 `onRemoveChannelMetadataResult` 回调并返回 API 调用结果。
...
同时，RTM 还会触发一个 `RTM_STORAGE_EVENT_TYPE_UPDATE` 类型的 `onStorageEvent` 事件通知，并在 100ms 内通知到频道中的其他人，详见[事件通知](./event.md)。
