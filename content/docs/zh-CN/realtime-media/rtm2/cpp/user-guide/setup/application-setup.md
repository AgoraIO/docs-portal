...
- `MESSAGE` 服务涵盖了 `Message Channel`、`User Channel`、`Presence`、`Storage`、`Lock`、`Token` 鉴权等功能特性，通过 `IRtmClient` 实例调用 API。
- `STREAM` 服务则提供了 `Stream Channel`、`Topic` 等功能，通过 `IStreamChannel` 实例调用 API。
...
所有传输连接都由 SDK 统一管理，并通过 `onLinkStateEvent` 事件通知给用户，详见[连接管理](../link/link-basic)。深入理解 RTM 服务类型的区别，有助于你更准确地选择和使用相关能力。
...
> **信息**
> 在你的应用上线时，你需要将日志等级参数设置为 `RTM_LOG_LEVEL_INFO`。
