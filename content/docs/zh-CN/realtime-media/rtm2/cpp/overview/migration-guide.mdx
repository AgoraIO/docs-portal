...
v2 和 v1 的 SDK 包名、集成方式都不变。详见[项目配置](../get-started/quick-start#1-项目配置)。

## 初始化实例

相比于 v1，v2 对初始化参数做了很大的调整，增加了很多新特性，例如端侧加密、云代理等，详见 [API 参考](https://doc.shengwang.cn/api-ref/rtm2/cpp/toc-configuration/configuration#create)。此外，v2 还丰富了接口调用的错误信息，你可以通过 `RTM_ERROR_CODE` 数据类型获取错误码，以便于你能更快地排查故障。结合[错误排查](../error-codes)文档，你也可以快速找到解决方法。
...
关于事件通知的更多信息及负载数据结构见[事件监听](https://doc.shengwang.cn/api-ref/rtm2/cpp/toc-configuration/configuration#事件监听)。
...
你也可以随时通过 `getState` 方法获取用户的在线状态，或者通过 `removeState` 方法删除用户的状态。用户临时状态变更后，RTM 服务器会触发 `RTM_PRESENCE_EVENT_TYPE_REMOTE_STATE_CHANGED` 类型的 `onPresenceEvent` 事件通知。具体用法见[临时用户状态](../user-guide/presence/temporary-user-state)。
