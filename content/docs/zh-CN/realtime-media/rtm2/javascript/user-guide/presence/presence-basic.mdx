...
当频道中人数众多时，你可能只关心频道中在线用户数量，而不关心他们是谁及其临时状态如何。此时可以设置 `whoNow` 方法中的 `includedState` 和 `includedUserId` 字段为 `false`。例如：
...
> **信息**
> 不可以同时设置 `includedState = true` 且 `includedUserId = false`，即获取用户的临时状态就必须要返回用户 ID，否则会设置失败并收到错误码。
