...
该版本在 [`LinkStateChangeReasonCode`](https://doc.shengwang.cn/api-ref/rtm2/javascript/enumv#连接状态改变原因类型) 中增加原因 `LOGIN_TOO_FREQUENT`，表示当前登录操作过于频繁。
...
该版本在 [`publish`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-message/message#publish) 的 `PublishOptions` 参数中新增了 `storeInHistory` 属性，支持在频道中发送消息的同时，将消息存储到云端，后续用户可以通过 [`getMessages`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-message/message#getMessages) 方法获取。
...
该版本在 SDK 连接状态 `linkState` 中添加 [`LinkStateChangeReasonCode`](https://doc.shengwang.cn/api-ref/rtm2/javascript/enumv#linkstatechangereason) 枚举类，报告连接状态变更的原因。
...
该版本在 `RTMConfig` 中删除了 `token` 参数，并在 [`login`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration#login) 方法中新增了 `token` 参数作为替代，你需要在升级后修改你的实现代码：
...
try {
    ...
    const  = status;
    console.log(`$ failed, the error code is $, because of: $.`);
}
```
...
该版本在 [`RTMConfig`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration) 中新增 `privateConfig` 参数，用于实现私有化部署功能。详见[私有化配置](../user-guide/setup/private-setup)。

#### 新增心跳时间配置

该版本在 [`RTMConfig`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration) 中新增 `heartbeatInterval` 参数，用于配置 SDK 向 RTM 服务器发送心跳包的时间间隔。详见 [Heartbeat Interval 与 Presence Timeout](../user-guide/link/link-basic#heartbeatinterval-and-presencetimeout)。

#### 新增用户频道类型

该版本在 [`channelType`](https://doc.shengwang.cn/api-ref/rtm2/javascript/enumv) 中新增 `USER` 类型，用于向指定用户发送消息。该功能可以替代 v1 的点对点消息功能。详见[在 User Channel 中发送消息](../user-guide/message/send-message#userchannel)。

#### 新增静默模式配置

该版本在 [`subscribe`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-channel/channel#subscribe) 和 [`join`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-channel/channel#join) 方法的 `options` 对象中新增 `beQuiet` 属性。该属性设置为 `true` 时表示静默订阅或加入频道，频道中其他用户无法感知该用户的出席状态。

### 改进

#### 改进连接状态管理逻辑

该版本新增 [`linkState`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration#事件监听) 回调，用于替代废弃的 `status` 回调，以便更好地管理连接状态。详见[连接管理](../user-guide/link/link-basic)。
...
- [`login`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration#login)：
...
- [`subscribe`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-channel/channel#subscribe)：
  - 在之前版本中，不支持连续多次调用该方法。
  - 自 2.2.0 版本起，支持连续多次调用该方法。
- [`join`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-channel/channel#join)：
  - 在之前版本中，不支持连续多次调用该方法，也不支持在该方法的 `token` 参数中传入空字符串。
  - 自 2.2.0 版本起，支持连续多次调用该方法。此外，当 `token` 参数为空字符串时，SDK 会默认使用你初始化时传入的 App ID 替代 Token。

#### 修改 presenceTimeout 取值范围

该版本将 [`RTMConfig`](https://doc.shengwang.cn/api-ref/rtm2/javascript/toc-configuration/configuration) 中 `presenceTimeout` 参数的取值范围由 [10,300] 修改为 [5,300]。
...
该版本支持定时通知模式。当频道中的在线人数超过 Announce Max 的值时，频道会进入定时通知模式。SDK 会以一定的时间间隔触发 `presence` 事件通知，并在 `interval` 属性中返回当前频道在上一个周期内用户加入、离开、超时、状态变更等事件通知的聚合增量信息。详见[定时通知模式](../user-guide/presence/event#定时通知模式)。
