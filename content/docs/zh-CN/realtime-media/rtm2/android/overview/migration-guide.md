...
相比于 v1，v2 对初始化参数做了很大的调整，增加了很多新特性，例如端侧加密、云代理等，详见 [API 参考](https://doc.shengwang.cn/api-ref/rtm2/android/toc-configuration/configuration#create)。此外，v2 还丰富了接口调用的错误信息，你可以通过 `ErrorInfo` 数据结构获取错误码、错误原因及 API 操作名称，以便于你能更快地排查故障。结合[错误排查](../error-codes)文档，你也可以快速找到解决方法。

> **信息**
> RTM v2 版本在命名字符集的支持上与 v1 版本存在差异。例如，v2 版本不支持以 `"_"` 开头或包含 `"."` 字符的频道名、用户名和 Topic 名等。在从 v1 升级到 v2 或混合使用这两个版本时，需注意字符集差异可能导致的不兼容问题。建议在迁移时统一使用 v2 版本[支持的字符集](../user-guide/channel/channel-basic.md#合法字符)。
...
- v1 的示例代码如下：
    ...
    try {
        rtmClient = RtmClient.createInstance(getBaseContext(), "your_appId", new RtmClientListener() {
            ...
            @Override
            public void onPeersOnlineStatusChanged(Map status) {
            }
        });
    } catch (Exception e) {
...
- v1 的示例代码如下：
    ...
    ```java
    rtmClient.login("your_token", "your_userId", new ResultCallback() {
        @Override
...
- v2 的示例代码如下：
    ...
    ```java
    rtmClient.login("your_token", new ResultCallback() {
        @Override
        public void onSuccess(Void responseInfo) {
          // 处理登录结果
        }
...
关于事件通知的更多信息及负载数据结构见[事件监听](https://doc.shengwang.cn/api-ref/rtm2/android/toc-configuration/configuration#事件监听)。
...
- v1 的示例代码如下：
    ...
    // 加入频道
    rtmChannel.join(new ResultCallback() {
        @Override
        public void onSuccess(Void responseInfo) {
            // 处理加入频道结果
        }
    ...
    rtmChannel.sendMessage(message, new ResultCallback() {
        @Override
...
- v2 的示例代码如下：
    ...
    rtmClient.publish("channelName", message, options, new ResultCallback() {
        @Override
        public void onSuccess(Void responseInfo) {
            // 处理消息发送结果
        }
    ...
    rtmClient.subscribe("channelName", options, new ResultCallback() {
        @Override
        public void onSuccess(Void responseInfo) {
            // 处理消息订阅结果
        }
...
rtmClient.sendMessageToPeer("Tony", message, options, new ResultCallback() {
    @Override
    public void onSuccess(Void aVoid) {
        // 处理消息发送结果
    }
...
rtmClient.publish("Tony", "Hello world", options, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
        // 处理消息发送结果
    }
...
PublishOptions options = new PublishOptions();
rtmClient.publish("receiver", jsonObject.toString(), options, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
        // 处理消息发送结果
    }
...
rtmClient.getPresence().whoNow("channelName", RtmChannelType.MESSAGE, options, new ResultCallback() {
    @Override
...
```java
// v2
rtmClient.getPresence().whereNow("Tony", new ResultCallback>() {
    @Override
    public void onSuccess(ArrayList channels) {
        // 处理 whereNow 调用结果
    }
...
```java
// v2
HashMap stateItems = new HashMap();
stateItems.put("mood", "pumped");

rtmClient.getPresence().setState("channelName", RtmChannelType.MESSAGE, stateItems, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
        // 处理 setState 调用结果
    }
...
你也可以随时通过 `getState` 方法获取用户的在线状态，或者通过 `removeState` 方法删除用户的状态。用户临时状态变更后，RTM 服务器会触发 `REMOTE_STATE_CHANGED` 类型的 `onPresenceEvent` 事件通知。具体用法见[临时用户状态](../user-guide/presence/temporary-user-state)。
...
rtmClient.getStorage().setChannelMetadata("channelName", RtmChannelType.MESSAGE, metadata, options, "lockName", new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
