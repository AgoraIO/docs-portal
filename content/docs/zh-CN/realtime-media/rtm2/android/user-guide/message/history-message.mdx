...
String userId = "Tony";
String message = "Hello world!";
rtmClient.publish(userId, message, options, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
        log(CALLBACK, "send message success");
    }
...
String channelName = "chatRoom";
rtmClient.getHistory().getMessages(channelName, RtmChannelType.MESSAGE, options, new ResultCallback() {
    @Override
