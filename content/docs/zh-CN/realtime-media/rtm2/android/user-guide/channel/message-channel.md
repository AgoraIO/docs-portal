...
监听了 `onMessageEvent` 事件通知后，你就可以调用 `subscribe` 方法订阅频道，从而接收频道中的消息和事件通知。
...
mRtmClient.subscribe(channelName1, options, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
...
mRtmClient.subscribe(channelName2, options, new ResultCallback() {
    @Override
    public void onSuccess(Void responseInfo) {
