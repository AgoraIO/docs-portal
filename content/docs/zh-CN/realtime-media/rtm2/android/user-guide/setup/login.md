...
```java
mRtmClient.login("your_token", new ResultCallback() {
    @Override
...
你可以通过 `login` 的返回值或监听 `onLinkStateEvent` 事件通知来判断登录是否成功以及登录失败的错误码和错误原因。执行登录操作时，客户端的网络连接状态为 `CONNECTING`；登录成功后，其状态会更新为 `CONNECTED` 状态。为持续监测客户端的网络连接状态，声网建议你在应用的整个生命周期内持续监听 `onLinkStateEvent` 事件通知。
