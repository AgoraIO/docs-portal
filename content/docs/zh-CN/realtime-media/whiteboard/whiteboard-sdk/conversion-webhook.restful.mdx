...
- 已开启文档转换服务并配置第三方云存储的有效声网项目。详见[使用文档转换服务](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/advanced-features/convert-files)。
- 满足以下条件的服务器：
...
![控制台中的生成 webhook 密钥入口](/img/whiteboard/webhook.png)
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| code | Number | 转换任务状态码，转换成功时为 `0`，失败时会返回错误码，详见[文档转换错误码](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/response-code#文档转换错误码)。 |
...
- `taskId`：String。转换任务的 UUID，即转换任务的唯一标识符。</li><li>`taskType`：转换任务类型，取值如下： - `dynamic_convert`：动态文档转换，即文档转网页。 - `static_convert`：静态文档转换，即文档转图片。</li><li>`prefixUrl`：String。转换结果文件地址前缀路径。</li><li>`pageCount`：Number。文档页数，转换任务失败时没有该字段。</li><li>`previews`：JSON Object。转换后的文档预览图地址，每页对应一个预览图地址。该参数仅在[发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)时，请求包体中的 `preview` 设为 `true `，且 `type` 设为 `dynamic` 时才生效。</li><li>`images`：JSON Object。文档转图片结果的地址和图片参数，每页对应一张图片。 - `width`：Number。图片宽度 (px)。 - `height`：Number。图片高度 (px)。 - `url`：String。图片连接地址。 该参数仅在该参数仅在[发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)时，请求包体中的 `type` 设为 `static` 时才生效。</li><li>`note`：String。从文档中提取出的所有备注内容。</li><li>`noticeTimestamp`：Number。发送请求的时间戳。
...
- [使用文档转换服务](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/advanced-features/convert-files)
