...
互动白板 Token，包括不同权限的 SDK Token、Room Token 和 Task Token，是互动白板服务对用户采用的一种鉴权方式。详见[互动白板 Token 类型和权限](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/reference/concepts#token)。
...
- 在[声网控制台](https://console.shengwang.cn/)生成 SDK Token。详见[获取互动白板项目的安全密钥](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/build/setup-and-access/enable-service#获取互动白板项目的安全密钥)。

    :::warning[注意]
    该方法只能生成 `admin` 角色的永久 SDK Token。请勿将该 Token 下发给客户端，否则会有泄露的风险。
    :::

- 在 App 服务端调用互动白板服务端 RESTful API，详见[使用 RESTful API 生成 Token](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-tokens-teams)。
...
声网支持对 Token 设置 `admin`、`writer` 或 `reader` 角色，每种角色的 Token 具有的权限详见[互动白板 Token 类型和权限](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/reference/concepts#token)。
...
- 如果房间已经存在，你可以调用[获取房间列表](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/get-v5-rooms)查询已创建的房间列表。请求成功后，响应包体中包含查询房间的 UUID。
- 如果房间尚未创建，你可以调用[创建房间](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-rooms)。请求成功后，响应包体中包含新创建房间的 UUID。
...
要获取文档转换任务的 UUID，你需要调用互动白板 RESTful API [发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)。请求成功后，响应包体会包含转换任务的 UUID。
...
6. App 客户端使用获取的 Room Token 和房间 UUID 等信息加入互动白板房间。
...
开始前，确保你已参考[开启和配置互动白板服务](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/build/setup-and-access/enable-service)完成了以下准备工作：
...
| 权限                         | admin（管理员） | writer（读写） | reader（只读） |
| :--------------------------- | :-------------- | :------------- | :------------- |
| 以[互动模式](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/reference/concepts#互动模式)加入特定房间       |  ✔               |  ✔             |✘               |
| 以[订阅模式](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/reference/concepts#订阅模式)加入特定房间       | ✘                | ✘              |  ✔              |
| 获取特定房间信息             |  ✔             |  ✔              | ✘               |
| 封禁特定房间                 |  ✔              | ✘               | ✘               |
| 对指定场景进行截图           |  ✔           |  ✔              | ✘               |
| 对场景组下的所有场景进行截图 | ✔              |  ✔              | ✘               |
...
- 在 App 服务端用代码生成 Token，详见[在 App 服务端生成 Token](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/build/setup-and-access/generate-token#在-app-服务端生成-token)。
