...
- 在 App 服务端用代码生成 Token，详见[在 App 服务端生成 Token](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/build/setup-and-access/generate-token)（推荐）。
...
- 如果房间尚未创建，你可以调用[创建房间](/zh-CN/api-reference/api-ref/whiteboard/restful/create-room)。请求成功后，响应包体中包含新创建房间的 UUID。
...
要获取文档转换任务的 UUID，你需要调用互动白板 RESTful API [发起文档转换](/zh-CN/api-reference/api-ref/whiteboard/restful/start-file-conversion)。请求成功后，响应包体会包含转换任务的 UUID。
...
- **角色**：包括 `admin`（管理员）、`writer`（读写）或 `reader`（只读）三种角色，每种角色的 Token 具有的权限详见[Token 的类型和权限](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/build/setup-and-access/generate-token)。
