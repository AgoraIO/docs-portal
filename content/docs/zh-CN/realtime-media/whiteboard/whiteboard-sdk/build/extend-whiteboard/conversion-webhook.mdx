...
调用[发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)时，在请求 Body 中的 `webhookEndpoint` 字段传入 Webhook 回调地址即可对本次转换启用文档转换 Webhook 回调。你也可以通过 `webhookRetry` 字段配置回调失败时的重试次数。回调内容和字段解释详见 [Webhook 回调内容](#webhook-回调内容)。
...
- `previews`：JSON Object。转换后的文档预览图地址，每页对应一个预览图地址。该参数仅在[发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)时，请求包体中的 `preview` 设为 `true `，且 `type` 设为 `dynamic` 时才生效。
- `images`：JSON Object。文档转图片结果的地址和图片参数，每页对应一张图片。该参数仅在[发起文档转换](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/post-v5-projector-tasks)时，请求包体中的 `type` 设为 `static` 时才生效。
...
- [查询转换任务进度](/doc/whiteboard/restful/whiteboard-sdk/restful-wb/operations/get-v5-projector-tasks-uuid)
