...
如果你将 SDK 升级到 v2.15.0 或之后版本，为保证你的业务功能不受影响，调用 `joinRoom` 时必须在 `JoinRoomParams` 传入字符串格式的 `uid`，长度不超过 1024 字节，并确保同一房间内每个用户的 `uid` 唯一。详见[加入实时房间](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/get-started/join-room-wb.javascript)。
