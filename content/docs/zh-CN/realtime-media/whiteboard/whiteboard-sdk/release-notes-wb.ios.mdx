...
:::info[桥接说明]
为了确保 Whiteboard SDK 在不同平台上提供一致的用户体验，同时允许各端以原生方式接入，Whiteboard iOS SDK 通过桥接 Whiteboard Web SDK 实现，不是完全原生的 SDK。因此，iOS SDK 的版本更新内容会默认包含指定 Web SDK 版本及以前的更新内容，你可以根据 iOS 各版本的桥接说明，前往 [Web SDK 发版说明](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/release-notes-wb.javascript)查看包含的更新内容。
:::
...
如果你将 SDK 升级到 v2.15.0 或之后版本，要加入房间，你必须调用 `joinRoomWithConfig`，并确保初始化 `WhiteRoomConfig` 对象时在 `initWithUuid` 中传入字符串格式的 `uid`。每个用户的 `uid` 长度不能超过 1024 字节，且在同一房间内必须唯一。详见[加入实时房间](/zh-CN/realtime-media/whiteboard/whiteboard-sdk/get-started/join-room-wb.ios#3-初始化-sdk-并加入房间)。
