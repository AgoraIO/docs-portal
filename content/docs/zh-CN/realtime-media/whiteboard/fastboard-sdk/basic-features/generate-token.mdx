...
互动白板 Token，包括不同权限的 SDK Token、Room Token 和 Task Token，是互动白板服务对用户采用的一种鉴权方式。详见[互动白板 Token 类型和权限](/zh-CN/realtime-media/whiteboard/fastboard-sdk/overview/concepts#token)。
...
- 在[声网控制台](https://console.shengwang.cn/)生成 SDK Token。详见[获取互动白板项目的安全密钥](/zh-CN/realtime-media/whiteboard/fastboard-sdk/get-started/enable-service#获取互动白板项目的安全密钥)。
...
声网支持对 Token 设置 `admin`、`writer` 或 `reader` 角色，每种角色的 Token 具有的权限详见[互动白板 Token 类型和权限](/zh-CN/realtime-media/whiteboard/fastboard-sdk/overview/concepts#token)。
...
开始前，确保你已参考[开启和配置互动白板服务](/zh-CN/realtime-media/whiteboard/fastboard-sdk/get-started/enable-service)完成了以下准备工作：
...
- 在 App 服务端用代码生成 Token，详见[在 App 服务端生成 Token](/zh-CN/realtime-media/whiteboard/fastboard-sdk/basic-features/generate-token#在-app-服务端生成-token)。
