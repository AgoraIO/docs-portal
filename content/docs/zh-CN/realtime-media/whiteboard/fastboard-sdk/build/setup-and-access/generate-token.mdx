...
- 在 App 服务端用代码生成 Token，详见[在 App 服务端生成 Token](/zh-CN/realtime-media/whiteboard/fastboard-sdk/build/setup-and-access/generate-token)（推荐）。
...
- **角色**：包括 `admin`（管理员）、`writer`（读写）或 `reader`（只读）三种角色，每种角色的 Token 具有的权限详见[ Token 类型和权限](/zh-CN/realtime-media/whiteboard/fastboard-sdk/build/setup-and-access/generate-token)。
