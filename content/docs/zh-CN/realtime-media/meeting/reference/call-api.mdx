...
在调用 RESTful API 之前，确保已经[开通灵动会议服务](/zh-CN/realtime-media/meeting/build/enable-service)并获取 App ID 和临时 Token。
...
- `api.sd-rtn.com`：声网服务端域名。详见[保障 REST 服务高可用](/zh-CN/realtime-media/meeting/build/high-availability)。
...
- `Authorization`：用于 HTTP Token 鉴权的鉴权字段。每次发送 HTTP 请求时，都必须填入 `Authorization` 字段。详见[实现 HTTP Token 鉴权](/zh-CN/realtime-media/meeting/build/http-token-auth)。
