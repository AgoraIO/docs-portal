...
在调用 RESTful API 之前，确保已经[开通灵动会议服务](/zh-CN/realtime-media/meeting/build/setup-and-access/enable-service)并获取 App ID 和临时 Token。
...
- `api.sd-rtn.com`：声网服务端域名。详见[保障 REST 服务高可用](/zh-CN/realtime-media/meeting/build/manage-classroom/high-availability)。
...
- `Authorization`：用于 HTTP Token 鉴权的鉴权字段。每次发送 HTTP 请求时，都必须填入 `Authorization` 字段。详见[实现 HTTP Token 鉴权](/zh-CN/realtime-media/meeting/build/customize-and-extend/http-token-auth)。
...
- 如果 HTTP 状态码不为 `200`，参考[响应状态码](/zh-CN/realtime-media/meeting/reference/response-code)排查问题。
...
当你遇到问题时，可以使用开发者工具查看请求和响应，结合[响应状态码](/zh-CN/realtime-media/meeting/reference/response-code)进行排查。如未能解决问题，可[联系技术支持](https://ticket.shengwang.cn/)。
