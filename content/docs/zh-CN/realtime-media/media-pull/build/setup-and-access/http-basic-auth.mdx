---
title: "实现 HTTP 基本认证"
description: "了解如何在输入在线媒体流中 HTTP 基本认证，包括适用场景、配置步骤、实现要点和注意事项。"
---
## 前提条件

本文介绍如何进行 HTTP 基本认证。

已经从声网控制台获取客户 ID 和客户密钥，详见[开通拉流服务](/zh-CN/realtime-media/media-pull/build/setup-and-access/enable-service)。
...
开通服务、实现 HTTP 基本认证后，你可以进行如下操作：

1. 参考 [RESTful API 说明](../media-pull/operations/get-region-v1-projects-appId-cloud-player-players) 集成 media-pull 服务。
2. 参考 [接收 Webhook 事件](/zh-CN/realtime-media/media-pull/build/monitor-events/enable-event-notification) 开启消息通知服务，通过事件回调监听服务状态。
3. 参考 [集成最佳实践](/zh-CN/realtime-media/media-pull/build/optimize-and-operate/ensure-player-created) 检查你的集成步骤，保障服务可靠性。
