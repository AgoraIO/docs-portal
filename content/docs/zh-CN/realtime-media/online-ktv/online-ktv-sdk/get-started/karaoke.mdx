...
3. 用户可以点歌，从[声网内容中心](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/api/music-content-center)获取歌单，并下载歌曲文件和歌词文件，成为主唱。
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。
...
5. 合唱者通过 `onStreamMessage` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。
...
| 声网歌词打分组件 | [源码](https://github.com/AgoraIO-Community/LyricsView/tree/main) | [歌词组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring) |
...
参考[获取版权音乐](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/get-music)来创建并初始化音乐播放器，获取并加载版权音乐。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `onAudioVolumeIndication` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。

### 相关文档

- [RESTful API](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/api/music-content-center)
- [歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)
- [实时互动 API 参考](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/api/rtc-api)
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。
7. 主唱通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
5. 合唱者通过 `receiveStreamMessageFromUid` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。
6. 合唱者通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
| 声网歌词组件 | [源码](https://github.com/AgoraIO-Community/LrcView-iOS/tree/main) | [歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring) |
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `reportAudioVolumeIndicationOfSpeakers` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring)。
