...
:::info[信息]
如果你想简化集成步骤，请参考[场景化 API 解决方案](/zh-CN/realtime-media/online-ktv/ktv-scenario/get-started/integrate-ktvapi.ios)。
:::
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios)。
...
5. 合唱者通过 `receiveStreamMessageFromUid` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios)。
...
| 声网歌词组件 | [源码](https://github.com/AgoraIO-Community/LrcView-iOS/tree/main) | [歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios) |
...
参考[获取版权音乐](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/get-music.ios)来创建并初始化音乐播放器，获取并加载版权音乐。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `reportAudioVolumeIndicationOfSpeakers` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios)。

### 相关文档

- [RESTful API](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/api/music-content-center)
- [歌词打分组件](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/advanced-features/lyrics-scoring.ios)
- [实时互动 API 参考](/zh-CN/realtime-media/online-ktv/online-ktv-sdk/api/rtc-api.ios)
