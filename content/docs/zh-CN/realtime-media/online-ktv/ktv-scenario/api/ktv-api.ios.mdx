...
KTV API 初始化时默认用户角色为听众，如果需要开始独唱或加入合唱，需要调用 `switchSingerRole` 来切换至相应的角色。你可以参考[切换说明](/zh-CN/realtime-media/online-ktv/ktv-scenario/get-started/integrate-ktvapi.ios#5-切换角色)来进行角色切换。
...
- `noPermission`：不支持从当前角色切换为目标角色，请参考[切换说明](/zh-CN/realtime-media/online-ktv/ktv-scenario/get-started/integrate-ktvapi.ios#5-切换角色)进行角色切换。
