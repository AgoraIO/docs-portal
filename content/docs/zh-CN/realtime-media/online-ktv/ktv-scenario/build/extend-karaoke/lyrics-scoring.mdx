...
:::info[信息]
你还可以通过 [`LyricsView`](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/lyrics-api#lyricsview) 类下的 API 来自定义更多界面元素。
:::
...
调用 `setScoringLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/lyrics-api#setscoringlevel)来进行设置。
...
:::info[信息]
你还可以通过 [`ScoringView`](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/lyrics-api#scoringview) 类下的 API 来自定义更多界面元素。
:::
...
调用 `setScoreLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/lyrics-api#setscorelevel)来进行设置。
