---
title: "实现收发消息"
description: "了解如何快速接入实时消息 RTM，完成环境准备、参数配置、示例运行和核心流程验证。"
---
...
开始构建项目前，你需要完成[开通 RTM 服务](/zh-CN/realtime-media/rtm/build/rtm-initialization/enable-service)，并检查你的开发环境是否满足[平台支持](/zh-CN/realtime-media/rtm/reference/capabilities-and-compatibility/platform-support)中的最低版本要求。
...
1. [点击此处](https://download.shengwang.cn/rtm2/release/RTM_JAVA_SDK_for_Android_v2.3.0.zip)下载最新版本的 Java SDK。
...
2. 在 `/Gradle Scripts/build.gradle(Module: <projectname>.app)` 文件中添加如下代码，将 SDK 集成到你的 Android 项目中：
   ...
   ...
   dependencies {
       // 将 x.y.z 替换为具体的 SDK 版本号，如 2.3.0
       // 对于 2.2.7 及之后版本:
   ...
   :::info[信息]
   如果你同时集成了 2.2.0 及以上版本的 RTM SDK 和 4.3.0 及以上版本的 RTC SDK，请参考 [FAQ](/zh-CN/reference/faq/integration/rtm2_rtc_integration_issue) 处理集成问题。
   :::
...
如需了解更多初始化的信息，查看[初始配置](/zh-CN/api-reference/rtm/android/configuration)。
...
private RtmEventListener eventListener = new RtmEventListener() {
    ...
    public void onLinkStateEvent(LinkStateEvent event) {
        String text = "Link state event: " + event + "\n";
        writeToMessageHistory(text);
    }
};
```
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/android/message)。
...
现在，你已经学会了如何使用 RTM Java SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/android/configuration)了解更多功能的使用方法。
...
1. [点击此处](https://download.shengwang.cn/rtm2/release/RTM_OC_SDK_v2.3.0.zip)下载最新版本的 Objective-C SDK。
...
如需了解更多初始化的信息，查看[初始配置](/zh-CN/api-reference/rtm/ios/configuration)。
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/ios/message)。
...
现在，你已经学会了如何使用 RTM Objective-C SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/ios/configuration)了解更多功能的使用方法。
...
开始构建项目前，你需要完成[开通 RTM 服务](/zh-CN/realtime-media/rtm/build/rtm-initialization/enable-service)，并检查你的浏览器是否满足[平台支持](/zh-CN/realtime-media/rtm/reference/capabilities-and-compatibility/platform-support)中的最低版本要求。
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/web/message)。
...
现在，你已经学会了如何使用 RTM JavaScript SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/web/configuration)了解更多功能的使用方法。
...
[点击此处](https://download.shengwang.cn/rtm2/release/RTM_ArkTS_SDK_for_HarmonyOS_v2.3.0.zip)下载最新版本的 HarmonyOS SDK。
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/harmonyos/message)。
...
现在，你已经学会了如何使用 RTM HarmonyOS SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/harmonyos/configuration)了解更多功能的使用方法。
...
开始构建项目前，你需要完成[开通 RTM 服务](/zh-CN/realtime-media/rtm/build/rtm-initialization/enable-service)，并检查你的开发环境和运行环境是否满足如下要求：
...
:::tip[初始化配置]
初始化 RTM 的时候可以通过配置某些参数改变 `IRtmClient` 其后续行为，例如配置事件监听函数、端侧消息加密等。详见[初始配置](/zh-CN/api-reference/rtm/unity/configuration)。
:::
...
你也可以使用 `JSON` 或 `Protobuf` 等方式来构建你的负载结构以便于简化业务逻辑，此部分内容可以参考[消息负载序列化](/zh-CN/realtime-media/rtm/build/message-design-and-history/serialized)。但请记住在把消息负载转交给 RTM 发送前，需要对消息负载进行序列化。
...
你可以通过调用 `SubscribeTopicAsync` 方法订阅某个 Topic 中的一位或多位消息发布者以便于接收他们发送的消息。单个客户端在单个 Topic 中最多只能订阅 64 个消息发布者，详见 [API 调用限制](/zh-CN/realtime-media/rtm/reference/usage-limits-and-errors/api-limits)。
...
现在，你已经学会了如何使用 RTM SDK 来实现 Message Channel 和 Stream Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/unity/configuration)了解更多功能的使用方法。
...
2. 使用的你的 IDE 工具打开项目，编辑 `pubspec.yaml` 文件，添加以下依赖：
   ...
   :::info[信息]
   如果你同时集成了 2.2.1 及以上版本的 RTM SDK 和 4.3.0 及以上版本的 RTC SDK，请参考 [FAQ](/zh-CN/reference/faq/integration/rtm2_rtc_integration_issue) 处理集成问题。
   :::
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/flutter/message)。
...
现在，你已经学会了如何使用 RTM Flutter SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/flutter/configuration)了解更多功能的使用方法。
...
2. [点击此处](https://download.shengwang.cn/rtm2/release/RTM_C%2B%2B_SDK_for_Windows_v2.3.0.zip)下载最新版本的 C++ SDK 并解压。将 SDK 包中的如下文件复制到项目的对应目录下：
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/cpp/message)。
...
现在，你已经学会了如何使用 RTM C++ SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/cpp/configuration)了解更多功能的使用方法。
...
开始构建项目前，你需要完成[开通 RTM 服务](/zh-CN/realtime-media/rtm/build/rtm-initialization/enable-service)，并检查你的开发环境和运行环境是否满足[平台支持](/zh-CN/realtime-media/rtm/reference/capabilities-and-compatibility/platform-support)中的最低版本要求。
...
如需了解更多初始化的信息，查看[初始配置](/zh-CN/api-reference/rtm/swift/configuration)。
...
如需了解更多收发消息的信息，查看[消息](/zh-CN/api-reference/rtm/swift/message)。
...
现在，你已经学会了如何使用 RTM Swift SDK 来实现 Message Channel 的收发消息功能。下一步，你可以通过 SDK 的 [API 参考](/zh-CN/api-reference/rtm/swift/configuration)了解更多功能的使用方法。
