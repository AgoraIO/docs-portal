...
RTM 提供两种不同的服务类型，即 `MESSAGE` 服务和 `STREAM` 服务，详见[服务类型](/zh-CN/realtime-media/rtm/user-guide/setup/application-setup#servicetype)。具体的服务类型选择取决于你的实际需求和预算，你可以选择部署其中一种或同时部署两种。以下代码展示同时部署了两种服务的私有化环境配置：
