...
所有传输连接都由 SDK 统一管理，并通过 `config.onlinkstateevent` 事件通知给用户，详见[连接管理](/zh-CN/realtime-media/rtm/user-guide/link/link-basic.android)。深入理解 RTM 服务类型的区别，有助于你更准确地选择和使用相关能力。

此外，服务类型还涉及私有化配置的选择。用户可以选择私有化一个服务，或者同时私有化两种服务，具体取决于应用场景和预算。详见[私有化配置](/zh-CN/realtime-media/rtm/user-guide/setup/private-setup)。
...
详细流程参考[快速开始](/zh-CN/realtime-media/rtm/get-started/quick-start.android)。
...
你也可以查看[错误排查](/zh-CN/realtime-media/rtm/error-codes.android)了解更详细地调试及错误处理方法。
