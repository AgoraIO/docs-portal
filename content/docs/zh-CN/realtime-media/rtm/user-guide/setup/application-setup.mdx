...
所有传输连接都由 SDK 统一管理，并通过 `config.onlinkstateevent` 事件通知给用户，详见[连接管理](/zh-CN/realtime-media/rtm/user-guide/link/link-basic)。深入理解 RTM 服务类型的区别，有助于你更准确地选择和使用相关能力。
...
你也可以查看[错误排查](/zh-CN/realtime-media/rtm/error-codes)了解更详细地调试及错误处理方法。
