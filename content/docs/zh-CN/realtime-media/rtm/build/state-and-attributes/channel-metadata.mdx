...
---
Storage 服务为你提供了 Channel Metadata 的能力，可以用来存储和分发你应用中的上下文数据，例如：道具、公告、成员列表和关系链等。Channel Metadata 在被设置、更新和删除的时候会触发 `config.onstorageevent` 事件通知，频道中的其他人会在 100 ms 内收到此信息。Storage 服务对于 Message Channel 和 Stream Channel 都有效，在使用的时候可以通过 `channelType` 参数区分。
...
你可以为指定频道设置一组 Channel Metadata，用以实现业务上房间级别的数据存储和实时通知，例如房间内的道具信息、拍卖品价格更新、群公告等。每个频道只能有一组 Channel Metadata，但每组 Channel Metadata 可以包含一个或多个 Metadata Item，相关限制详见 [API 调用限制](/zh-CN/realtime-media/rtm/reference/usage-limits-and-errors/api-limits)。每个 Metadata Item 包含 `key`、`value`、`revision` 等预定义字段。

如果当前 Channel Metadata 或者 Metadata Item 不存在，该方法将为指定频道新增属性；如果 Channel Metadata 或者 Metadata Item 已存在，则会使用新值覆盖原有值。关于 `storage.setchannel` 接口的更多信息详见 [Storage API 参考](/zh-CN/api-reference/rtm/android/storage)。
...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100 ms 内通知到频道中的其他人，详见[事件通知](/zh-CN/realtime-media/rtm/build/state-and-attributes/metadata-events)。

## 获取频道元数据

你可以调用 `storage.getchannel` 方法获得指定频道的全部 Channel Metadata。关于 `storage.getchannel` 接口的更多信息见 [Storage API 参考](/zh-CN/api-reference/rtm/android/storage)。
...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100ms 内通知到频道中的其他人。`config.onstorageevent` 详见[事件通知](/zh-CN/realtime-media/rtm/build/state-and-attributes/metadata-events)。
...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100ms 内通知到频道中的其他人，详见[事件通知](/zh-CN/realtime-media/rtm/build/state-and-attributes/metadata-events)。
