...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100 ms 内通知到频道中的其他人，详见[事件通知](/zh-CN/realtime-media/rtm/build/manage-metadata/event)。
...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100ms 内通知到频道中的其他人。`config.onstorageevent` 详见[事件通知](/zh-CN/realtime-media/rtm/build/manage-metadata/event)。
...
同时，RTM 还会触发一个 `enumv.storageeventtypeupdate` 类型的 `config.onstorageevent` 事件通知，并在 100ms 内通知到频道中的其他人，详见[事件通知](/zh-CN/realtime-media/rtm/build/manage-metadata/event)。
