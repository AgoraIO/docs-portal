...
如果想要接收 Presence 事件通知，你需要实现事件监听程序，详见[事件监听](/zh-CN/realtime-media/rtm/build/manage-messages/add-event-listener)。除此之外，在订阅或加入频道时还需将 `features` 参数设为 `enumv.joinchannelfeaturepresence` 或 `enumv.subscribechannelfeaturepresence`，或将 `withPresence` 参数设置成 `true`。`config.presenceevent` 数据结构包含以下属性：
