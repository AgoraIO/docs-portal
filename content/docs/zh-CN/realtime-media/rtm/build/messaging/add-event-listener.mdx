---
title: "添加事件监听"
description: "了解实时消息 RTM 中添加事件监听相关内容的适用场景、配置步骤、实现要点和注意事项。"
---
## 添加事件监听
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```java tab="Java" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```swift tab="Swift" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```objc tab="Objective-C" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-add-event-listener-1"
...
```dart tab="Dart" tabGroup="rtm-build-manage-messages-add-event-listener-1" showLineNumbers
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-add-event-listener-2"
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-add-event-listener-2" showLineNumbers
...
```dart tab="Dart" tabGroup="rtm-build-manage-messages-add-event-listener-2"
// 定义消息时间处理函数
...
```typescript tab="TypeScript" tabGroup="rtm-build-manage-messages-add-event-listener-2"
...
RTM 一共有 7 种类型事件，对应的有 7 种事件监听程序。你可以查看 API 参考的[事件监听](/zh-CN/api-reference/rtm/toc-configuration/configuration#事件监听)章节，了解每个事件监听程序传入参数的详细信息。
...
完成事件监听处理程序后，你可以通过订阅频道或 Topic 来接收指定的 Message Channel 或 Stream Channel 中的消息和事件通知了。详见[订阅 Message Channel](/zh-CN/realtime-media/rtm/build/channels-and-topics/message-channel#订阅频道) 和[订阅 Topic](/zh-CN/realtime-media/rtm/build/channels-and-topics/topics/usage#订阅-topic)。
