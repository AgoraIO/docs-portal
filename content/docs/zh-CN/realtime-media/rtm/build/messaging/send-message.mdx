---
title: "发送消息"
description: "了解实时消息 RTM 中发送消息相关内容的适用场景、配置步骤、实现要点和注意事项。"
---
...
RTM 目前只支持字符串和二进制类型的消息数据格式。如果你的业务中使用了 JSON、Object 等其他数据类型，或者 protobuf 等第三方数据构建工具，那么你需要在发送消息之前对消息数据进行序列化。关于如何有效构建负载数据结构及推荐的序列化方式可以参考[消息负载结构化](/zh-CN/realtime-media/rtm/build/message-design-and-history/constructed)和[消息负载序列化](/zh-CN/realtime-media/rtm/build/message-design-and-history/serialized)。

## 在消息频道中发送消息

在 Message Channel 中，你只需调用 `message.publish` 方法即可发送消息，无需提前订阅频道。该方法一次只能向一个频道发送消息，如需向多个频道发送消息，你需要多次调用该方法。RTM 并不限制你向多少个频道发送消息，也不限制有多少用户向一个频道发送消息，但是对你同时向一个频道发送消息的频率有一定的限制，详见 [API 调用限制](/zh-CN/realtime-media/rtm/reference/usage-limits-and-errors/api-limits)。
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-send-message-1"
// 发送字符串消息
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-send-message-1"
// 发送字符串消息
...
```java tab="Java" tabGroup="rtm-build-manage-messages-send-message-1"
// 发送字符串消息
String message = "Hello World";
...
```swift tab="Swift" tabGroup="rtm-build-manage-messages-send-message-1"
...
```objc tab="Objective-C" tabGroup="rtm-build-manage-messages-send-message-1"
// 发送字符串消息
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-send-message-1" showLineNumbers
// 发送字符串消息
...
```dart tab="Dart" tabGroup="rtm-build-manage-messages-send-message-1"
// 发送字符串消息
...
```typescript tab="TypeScript" tabGroup="rtm-build-manage-messages-send-message-1"
// Send string message
...
```typescript tab="TypeScript" tabGroup="rtm-build-manage-messages-send-message-2"
// Send binary message
import { PublishOptions, RtmChannelType } from '@shengwang/rtm-full';
import { RtmError } from '@shengwang/rtm-full';
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
...
```java tab="Java" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
byte[] message = new byte[] { 00, 01, 35, 196 };
String channelName = "my_channel";
var options = new PublishOptions();
...
```swift tab="Swift" tabGroup="rtm-build-manage-messages-send-message-2"
...
```objc tab="Objective-C" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
PublishOptions options;
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-send-message-1"
// 异步回调
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
string message = "left";
string topic = "Motion";
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
const payload = "Hello Agora!";
...
```java tab="Java" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
...
```swift tab="Swift" tabGroup="rtm-build-manage-messages-send-message-3"
let message = "Hello Agora!"
...
```objc tab="Objective-C" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
NSString* message = @"Hello Agora!";
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
std::string message = "Hello Agora!";
...
```dart tab="Dart" tabGroup="rtm-build-manage-messages-send-message-3"
// 发送字符串消息
...
```typescript tab="TypeScript" tabGroup="rtm-build-manage-messages-send-message-3"
...
```typescript tab="TypeScript" tabGroup="rtm-build-manage-messages-send-message-4"
...
```csharp tab="C#" tabGroup="rtm-build-manage-messages-send-message-4"
// 发送二进制消息
byte[] message = new byte[] { 00, 01, 35, 196 };
string topic = "Motion";
var options = new TopicMessageOptions();
...
```javascript tab="JavaScript" tabGroup="rtm-build-manage-messages-send-message-4"
// 发送二进制消息
const message = "Hello Agora!";
const payload = new TextEncoder().encode(message);
const topicName = "chat_topic";
...
```java tab="Java" tabGroup="rtm-build-manage-messages-send-message-4"
// 发送二进制消息
byte[] message = new byte[] { 00, 01, 35, 196 };
String topicName = "chat_topic";
var options = new TopicMessageOptions();
...
```swift tab="Swift" tabGroup="rtm-build-manage-messages-send-message-4"
...
```objc tab="Objective-C" tabGroup="rtm-build-manage-messages-send-message-4"
// 发送二进制消息
unsigned char bytes[32] = { /*your raw values */ };
NSData* raw_message = [[NSData alloc] initWithBytes:bytes length:32];
...
```cpp tab="C++" tabGroup="rtm-build-manage-messages-send-message-4"
// 发送二进制消息
char message[5] = {0x00, 0x01, 0x02, 0x03, 0x04};
...
```dart tab="Dart" tabGroup="rtm-build-manage-messages-send-message-2"
// 发送二进制消息
final topicName = 'chat_topic';
var payload = Uint8List.fromList([155, 26, 88, 0, 0]);
