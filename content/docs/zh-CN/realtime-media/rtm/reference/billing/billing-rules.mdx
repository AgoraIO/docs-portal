...
- 存储占用 (Storage Usage) ：一个自然月内所有开通 RTM 服务的项目所占用的存储空间之和，详见[数据存储](/zh-CN/realtime-media/rtm/build/state-and-attributes/data-storage)。
...
企业套餐用户的消息总数、峰值连接数和存储占用三种计量指标的用量额度没有限制，系统会基于这三种计量指标在一个自然月内的实际使用情况形成账单。这种计费方式可以让用户更好地了解他们的使用情况，并根据实际需求进行调整。你可以查看[计费说明](/zh-CN/realtime-media/rtm/reference/billing/billing-strategy)了解更多信息。
...
- 1 次登录 (`config.login`) 请求计作 1 条消息。
- 1 次登出 (`config.logout`) 请求计作 1 条消息。
- 1 次更新 Token  (`token.renewtoken`)  计作 1 条消息。
- 接收到 1 条 SDK 连接状态的变更通知计作 1 条消息。
- 接收到 1 条 Token 即将过期通知计作 1 条消息。

### 频道操作

- 1 次订阅 (`channel.subscribe`) Message Channel 计作 1 条消息。
- 1 次取消订阅 (`channel.unsubscribe`) Message Channel 计作 1 条消息。
- 1 次加入 (`channel.join`) Stream Channel 计作 1 条消息。
- 1 次离开 (`channel.leave`)  Stream Channel 计作 1 条消息。

### Topic 操作

- 1 次加入 (`topic.join`) Topic 方法调用计作 1 条消息。
- 1 次离开 (`topic.leave`) Topic 方法调用计作 1 条消息。
- 1 次订阅 (`topic.subscribe`) Topic 方法调用计作 1 条消息。
- 1 次取消订阅 (`topic.unsubscribe`) Topic 方法调用计作 1 条消息。
- 接收到 1 条 Topic 变更通知计作 1 条消息。

### 收发消息

- 向 Message Channel 中发布 (`message.publish`) 任何 1 条消息计作 1 条消息。
- 在 Message Channel 中收到的任何 1 条消息计作 1 条消息。例如，一个客户端向一个 Message Channel 发送 1 条消息，而该频道被 10 人订阅，则计作 1 条发送消息和 10 条接收消息，总计 11 条消息。
- 向 Stream Channel 中的 Topic 中发布 (`topic.publish`) 的任何 1 条消息计作 1 条消息。
...
- 1 次 `presence.whonow` 查询计作 1 条消息。
- 1 次 `presence.wherenow` 查询计作 1 条消息。
- 1 次设置 (`presence.setstate`)、查询 (`presence.getstate`)、删除 (`presence.removestate`) 任何 1 条临时用户状态，计作 1 条消息。
...
- 设置 (`storage.setchannel`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 查询 (`storage.getchannel`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 更新 (`storage.updatechannel`) 任何 1 条 Channel Metadata，计作 1 条消息。
- 删除 (`storage.removechannel`) 任何 1 条 Channel Metadata，计作 1 条消息。
...
- 设置 (`storage.setuser`) 任何 1 条 User Metadata，计作 1 条消息。
- 查询 (`storage.getuser`) 任何 1 条 User Metadata，计作 1 条消息。
- 更新 (`storage.updateuser`) 任何 1 条 User Metadata，计作 1 条消息。
- 删除 (`storage.removeuser`) 任何 1 条 User Metadata，计作 1 条消息。
...
- 任意 1 次对锁的设置 (`lock.set`) 操作计作 1 条消息。
- 任意 1 次对锁的获取 (`lock.acquire`) 操作计作 1 条消息。
- 任意 1 次对锁的查询 (`lock.get`) 操作计作 1 条消息。
- 任意 1 次对锁的释放 (`lock.release`) 操作计作 1 条消息。
- 任意 1 次对锁的剥夺 (`lock.revoke`) 操作计作 1 条消息。
- 任意 1 次对锁的删除 (`lock.remove`) 操作计作 1 条消息。
...
![RTM 存储占用量计算公式](https://web-cdn.agora.io/docs-portal/img/rtm2/rtm2billingformula.png)
