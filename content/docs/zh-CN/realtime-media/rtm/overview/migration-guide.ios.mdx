...
相比于 v1，v2 对初始化参数做了很大的调整，增加了很多新特性，例如端侧加密、云代理等，详见 [API 参考](/api-ref/rtm2/ios/toc-configuration/configuration#create)。此外，v2 还丰富了接口调用的错误信息，你可以通过 `AgoraRtmErrorInfo` 数据结构获取错误码、错误原因及 API 操作名称，以便于你能更快地排查故障。结合[错误排查](/zh-CN/realtime-media/rtm/error-codes.android.ios)文档，你也可以快速找到解决方法。
...
除上述版本之间的功能差异之外，v2 还额外增加了很多被广泛验证和使用的新功能，你可以根据项目的需要选择和使用，相信革新版本的 v2 一定可以帮助你快速接入实时互动领域。v2 所具备的功能特性见[特性列表](/zh-CN/realtime-media/rtm/overview/feature-list.ios)。
