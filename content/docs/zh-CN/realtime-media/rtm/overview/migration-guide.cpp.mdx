...
v2 和 v1 的 SDK 包名、集成方式都不变。详见[项目配置](/zh-CN/realtime-media/rtm/get-started/quick-start.cpp#1-项目配置)。

## 初始化实例

相比于 v1，v2 对初始化参数做了很大的调整，增加了很多新特性，例如端侧加密、云代理等，详见 [API 参考](/api-ref/rtm2/cpp/toc-configuration/configuration#create)。此外，v2 还丰富了接口调用的错误信息，你可以通过 `RTM_ERROR_CODE` 数据类型获取错误码，以便于你能更快地排查故障。结合[错误排查](/zh-CN/realtime-media/rtm/error-codes.android.cpp)文档，你也可以快速找到解决方法。
...
除上述版本之间的功能差异之外，v2 还额外增加了很多被广泛验证和使用的新功能，你可以根据项目的需要选择和使用，相信革新版本的 v2 一定可以帮助你快速接入实时互动领域。v2 所具备的功能特性见[特性列表](/zh-CN/realtime-media/rtm/overview/feature-list.cpp)。
