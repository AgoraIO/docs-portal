---
title: "快速接入 RTC 服务端 SDK"
description: "了解如何快速接入 RTC 服务端 SDK，完成环境准备、参数配置、示例运行和核心流程验证。"
---
<PlatformStructured platform="cpp">

## 准备开发环境

本文介绍如何集成 C++ SDK 并跑通声网提供的示例项目。确保你的服务端开发机满足以下要求。
...
参考[开通服务](/zh-CN/realtime-media/rtc-server-sdk/build/setup-and-access/enable-service)获取声网 App ID 和 RTC 临时 Token。
...
2. 打开 `pom.xml` 文件并将文件内容替换为以下文本：
   ...
   :::info[信息]
   `dependencies` 中的 `version` 字段为 SDK 版本号，需随你集成的 SDK 版本更换。最新的 SDK 版本号可参考[发版说明](/zh-CN/realtime-media/rtc-server-sdk/reference/release-notes)。
   :::
...
- 一个有效的声网账号以及声网项目。请参考[开通服务](/zh-CN/realtime-media/rtc-server-sdk/build/setup-and-access/enable-service)从声网控制台获得 App ID。
