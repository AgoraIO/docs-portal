...
3. 调用 `Connect` 与声网 RTC 频道建立连接，你需要传入以下参数：
   ...
   - `token`: RTC Token。如果你的项目开启了 App 证书，你需要传入 RTC Token，详见[获取临时 Token](/zh-CN/realtime-media/rtc-server-sdk/get-started/enable-service#get-temp-token)；如果你的项目未开启 App 证书，则只需传入空字符串。
