...
请求失败。你可以根据返回的[响应状态码](/zh-CN/api-reference/console/general/api/response-code)和响应 Body 中 `message` 字段的描述排查错误。
...
4. 在 RTSA SDK 中，调用 `agora_rtc_init` 并在 `rtc_service_option_t` 的 `license_value` 参数中传入 License。

5. RTSA SDK 会在用户加入频道时自动验证 License。

   - 如果 License 验证成功，则用户可以正常使用 SDK 的功能。
   - 如果 License 验证失败，则用户会被踢出频道并收到 `on_license_validation_failure` 回调，你可以根据收到的错误码处理问题。
...
需要续期的 License 值。你可以在[激活接口](#activate)的 `license` 响应参数中获取。
...
4. 在 SDK 中，调用 `agora_rtc_init` 并在 `rtc_service_option_t` 的 `license_value` 参数中传入新的 License。
