---
title: "申请和使用许可证"
description: "了解 RTSA 中申请和使用许可证相关内容的适用场景、配置步骤、实现要点和注意事项。"
---
## 许可证介绍
...
![license workflow](https://web-cdn.agora.io/docs-portal/img/rtsa/license.svg)
...
请求失败。你可以根据返回的[响应状态码](/zh-CN/api-reference/api-ref/console/response-codes)和响应 Body 中 `message` 字段的描述排查错误。
...
![use license](https://web-cdn.agora.io/docs-portal/img/rtsa/use-license.png)
...
声网控制台 RESTful API 仅支持 HTTPS 协议。发送请求时，你需要使用声网提供的客户 ID 和客户密钥生成一个 Base64 算法编码的凭证，并填入 HTTP 请求 Header 的 `Authorization` 字段中。详见[实现 HTTP 基本认证](/zh-CN/realtime-media/rtc/build/setup-and-access/http-basic-auth)。

#### 调用频率限制

对于每个声网账号（非每个 App ID），本页每个 API 的调用频率上限为每秒 10 次。如果调用频率超出限制，参考[如何处理服务端 RESTful API 调用超出频率限制](/zh-CN/api-reference/faq/integration/restful_api_call_frequency)优化调用频率。
