...
- 一个有效的 License。RTSA 通过 License 对设备鉴权，详见[申请和使用 License](/zh-CN/realtime-media/rtsa/build/project-preparation/license)。
- 一台开发机，可以是电脑或嵌入式设备。环境要求见[平台兼容](/zh-CN/realtime-media/rtsa/reference/supported-platforms)。本文以安装 Ubuntu 18.04 系统的电脑为例。
- 参考[开通服务](/zh-CN/realtime-media/rtsa/build/project-preparation/enable-service)获取 App ID、RTC 和 RTM 临时 Token。
...
- 传入能标识 RTM 系统用户角色和权限的 [RTM Token](/zh-CN/realtime-media/rtm/build/authentication-and-connection/token-generation)。如果安全要求不高，使用的是 App ID，可以将 `token` 设为空。
...
- [API 参考](/zh-CN/api-reference/rtsa/c/overview)
