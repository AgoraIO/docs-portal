---
title: "在录制中添加水印"
description: "了解本地服务端录制在不同平台上的在录制中添加水印方式，包括接入流程、关键配置和平台差异。"
---
...
开始前，请确保你已经参考[开始录制音视频](/zh-CN/realtime-media/local-server-recording/get-started/start-recording)在项目中集成了录制 SDK 并实现了基础的录制功能。

## 实现方法

无论是单流录制，还是合流录制，SDK 都支持在录制视频过程中添加水印。下文以合流录制为例，介绍实现水印功能时核心的参数设置。

你可以通过 [`enableAndUpdateVideoWatermarks`](/zh-CN/api-reference/local-server-recording/cpp/iagoramediartcrecorder.cpp#enableandupdatevideowatermarks) 开启或更新水印设置：

```cpp
int enableAndUpdateVideoWatermarks(WatermarkConfig* watermark_configs, int num)
```

其中，`watermark_configs` 为具体的水印数组指针；`num` 为水印个数。

如果需要关闭水印功能，可以调用 [`disableVideoWatermarks`](/zh-CN/api-reference/local-server-recording/cpp/iagoramediartcrecorder.cpp#disablevideowatermarks) 实现。

:::info[信息]
单流录制场景下，实现的水印功能与合流录制一致。两者的差异在于不同场景下，调用的 API 不同。单流下：

- 开启或更新水印：[`enableAndUpdateVideoWatermarksByUid`](/zh-CN/api-reference/local-server-recording/cpp/iagoramediartcrecorder.cpp#enableandupdatevideowatermarksbyuid)
- 关闭水印：[`disableVideoWatermarksByUid`](/zh-CN/api-reference/local-server-recording/cpp/iagoramediartcrecorder.cpp#disablevideowatermarksbyuid)
:::
...
1. 调用 `enableAndUpdateVideoWatermarks` 方法，并设置相应的 `watermark_configs` 参数。
2. 设置水印类型为文字水印：`watermark_configs.type = agora::rtc::LITERA`。
...
1. 调用 `enableAndUpdateVideoWatermarks` 方法，并设置相应的 `watermark_configs` 参数
2. 设置水印类型为时间戳水印：`watermark_configs.type = agora::rtc::TIMESTAMPS`
...
1. 调用 `enableAndUpdateVideoWatermarks` 方法，并设置相应的 `watermark_configs` 参数
2. 设置水印类型为图片水印：`watermark_configs.type = agora::rtc::PICTURE`。
...
无论添加哪种水印，都需要设置水印在视频中的位置。SDK 在 [`WatermarkConfig`](/zh-CN/api-reference/local-server-recording/cpp/struct#watermarkconfig) 结构体中提供了两种位置设置模式。
...
- 横屏模式的水印位置
  ...
  watermarks[i].options.positionInLandscapeMode.width = 200;  // 水印宽度
  watermarks[i].options.positionInLandscapeMode.height = 100; // 水印高度
  ```
...
- [API 参考](/zh-CN/api-reference/local-server-recording/cpp/api-overview)
...
你可以通过 [`enableAndUpdateVideoWatermarks`](/zh-CN/api-reference/local-server-recording/java/agoramediartcrecorder.java#enableandupdatevideowatermarks) 开启或更新水印设置：
...
如果需要关闭水印功能，可以调用 [`disableVideoWatermarks`](/zh-CN/api-reference/local-server-recording/java/agoramediartcrecorder.java#disablevideowatermarks) 实现。

:::info[信息]
单流录制场景下，实现的水印功能与合流录制一致。两者的差异在于不同场景下，调用的 API 不同。单流下：

- 开启或更新水印：[`enableAndUpdateVideoWatermarksByUid`](/zh-CN/api-reference/local-server-recording/java/agoramediartcrecorder.java#enableandupdatevideowatermarksbyuid)
- 关闭水印：[`disableVideoWatermarksByUid`](/zh-CN/api-reference/local-server-recording/java/agoramediartcrecorder.java#disablevideowatermarksbyuid)
:::
...
无论添加哪种水印，都需要设置水印在视频中的位置。SDK 在 [`Constants.WatermarkFitMode`](/zh-CN/api-reference/local-server-recording/java/enum#watermarkfitmode) 中提供了两种位置设置模式。
...
[API 参考](/zh-CN/api-reference/local-server-recording/java/api-overview)
