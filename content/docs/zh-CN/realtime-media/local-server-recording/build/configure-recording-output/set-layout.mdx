---
title: "设置合流布局"
description: "了解本地服务端录制中设置合流布局相关内容的适用场景、配置步骤、实现要点和注意事项。"
---
在使用合流录制模式时，你需要设置频道内所有发流用户子视频的大小及其在视频画布上的位置，进行合流布局。

本文介绍如何使用录制 4.x SDK 在合流录制时设置合流布局。

## 功能介绍

下图为合流后的视频示例。我们将视频的背景称为**画布**，每个发流用户占据一个**子视频**。

![画布视窗示例](https://web-cdn.agora.io/docs-portal/img/recording/canvas-view.png)

## 前提条件

开始前，请确保你已经参考[开始录制音视频](/zh-CN/realtime-media/local-server-recording/get-started/start-recording)在项目中实现了基础的录制功能，且录制模式为合流录制。

## 实现方式

录制 SDK 提供了一个 [`setVideoMixingLayout`](/zh-CN/api-reference/local-server-recording/java/legacy/classio-1-1agora-1-1recording-1-1-recording-s-d-k#setvideomixinglayout) 方法用来设置合流布局，通过对 `VideoMixingLayout` 对象的定义，来设置合流画布和参与合流的用户的视窗：
...
录制 SDK 提供了一个 [`setVideoMixingLayout`](/zh-CN/api-reference/local-server-recording/cpp/legacy/classagora-1-1recording-1-1-i-recording-engine#setvideomixinglayout) 方法用来设置合流布局，通过对 `VideoMixingLayout` 结构体的定义，来设置合流画布和参与合流的用户的视窗：
...
```java tab="Java" tabGroup="zh-cn-realtime-media-recording-local-server-recording-build-recording-modes-set-layout-1"
...
```cpp tab="C++" tabGroup="zh-cn-realtime-media-recording-local-server-recording-build-recording-modes-set-layout-1"
...
![合流布局示意](https://web-cdn.agora.io/docs-portal/img/recording/video-mixing.png)
