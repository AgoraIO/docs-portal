...
开始前请确保你已经完成录制 SDK 的环境准备和集成工作，且编译了 SDK 包中的代码示例，详见[集成客户端](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/legacy/integrate-sdk)。
...
- `appId`：App ID，必须和声网 RTC SDK 的 App ID 一致，详见[获取 App ID](/zh-CN/realtime-media/local-server-recording/build/setup-and-access/enable-service)。
...
- `layoutMode`：设置视频合流布局。详见[设置合流布局](/zh-CN/realtime-media/local-server-recording/build/recording-modes/legacy/set-layout)。
...
- `mixResolution`：如果 `isMixingEnabled` 设为 1，可以通过该参数设置合流视频的分辨率，格式为：`width`，`height`，`fps`，`Kbps`，从左至右分别对应合图的宽、高、帧率和码率。你可以参考[视频属性表](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/legacy/set-output-video#分辨率帧率码率对照表)进行设置。
...
- `enableCloudProxy`：是否启用云代理服务。使用云代理需要先联系声网申请开通，详见[应对防火墙限制](/zh-CN/realtime-media/local-server-recording/build/setup-and-access/cloud-proxy)。
...
- 录制完成后，你可能需要使用转码脚本将录制的文件进行合成，详见[使用转码脚本](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/legacy/merge-files)。
- 如果你希望了解如何通过调用 API 实现录制，可以参考[调用 API 录制](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/record-by-api)。

- 录制过程中，如果出现错误码或警告码，请参考[警告码](/zh-CN/api-reference/local-server-recording/cpp/legacy/namespaceagora-1-1linuxsdk#a467894afed372a4fe76594a6223869a0)。

- 录制过程中，如果出现错误码或警告码，请参考[警告码](/zh-CN/api-reference/local-server-recording/java/legacy/enumio-1-1agora-1-1recording-1-1common-1-1-common-1-1-e-r-r-o-r-c-o-d-e-t-y-p-e)和[错误码](/zh-CN/api-reference/local-server-recording/java/legacy/enumio-1-1agora-1-1recording-1-1common-1-1-common-1-1-e-r-r-o-r-c-o-d-e-t-y-p-e)。
