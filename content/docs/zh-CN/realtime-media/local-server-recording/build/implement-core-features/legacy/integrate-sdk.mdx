...
:::info[信息]
声网本地服务端录制 SDK 需要集成在你的 Linux 服务器，而不是你的 App 上。如果你不想自行部署 Linux 服务器，可尝试[声网云端录制](/zh-CN/realtime-media/cloud-recording)。
:::
...
| 带宽 | 根据需要同时录制的频道数量和频道内情况确定所需带宽。以下数据可供参考：录制一个分辨率为 640 × 480 的画面需要的带宽约为 500 Kbps；录制一个有两个人的频道则需 1 Mbps；同时录制 100 个这样的频道，需要带宽为 100 Mbps。关于分辨率和带宽的关系，详见[录制 API](/zh-CN/api-reference/local-server-recording/cpp/legacy/overview)。 |
...
你可参考上述云主机配置和对应的录制性能，根据自己的录制需要选择和配置云主机，详见[使用云容器部署录制 SDK](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/legacy/docker)。

## 下载 SDK

前往[资源中心](/zh-CN/realtime-media/local-server-recording/reference/downloads)获取最新的声网本地服务端录制 SDK 软件包。软件包内容如下：
...
2. 安装编译器：gcc 4.4+。
3. 如果你的网络环境设置了防火墙限制外网访问，请使用[云代理服务](/zh-CN/realtime-media/local-server-recording/build/setup-and-access/cloud-proxy)。
...
4. 如果你的网络环境设置了防火墙限制外网访问，请使用[云代理服务](/zh-CN/realtime-media/local-server-recording/build/setup-and-access/cloud-proxy)。
...
- [使用命令行录制](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/legacy/record-by-cmd)
- [调用 API 录制](/zh-CN/realtime-media/local-server-recording/build/implement-core-features/record-by-api)
