...
* 参考[开通本地服务端录制服务](/zh-CN/realtime-media/local-server-recording/build/setup-and-access/enable-service)在控制台创建一个声网项目，并获取项目的 App ID。
* 在项目中实现了基础的音视频互动场景。
* 服务器硬件条件：
  * CPU：8 核，1.8 GHz 主频
  * 内存：推荐 4 GB 或更高
...
- [单流录制和合流录制](/zh-CN/realtime-media/local-server-recording/build/recording-modes/recording-mode)
- [自定义合流录制视频布局](/zh-CN/realtime-media/local-server-recording/build/recording-modes/set-layout)
- [配置水印](/zh-CN/realtime-media/local-server-recording/build/recording-modes/watermark)
- [录制崩溃后修复录制文件](/zh-CN/realtime-media/local-server-recording/build/optimize-and-operate/restore-files)
...
4. 按照场景需要填写 `recorder.json`（合流录制的配置文件）或 `singleVideo.json`（单流录制的配置文件）。单流和合流的场景区别可以参考[单流录制和合流录制](/zh-CN/realtime-media/local-server-recording/build/recording-modes/recording-mode)。
