...
当你调用 [`Update`](/zh-CN/api-reference/api-ref/media-push/update) 成功更新一个 Converter 配置时，消息通知服务器会向你的服务器通知推流配置更新事件。
...
- `destroyReason`: String 型字段，描述 Converter 被销毁的原因。
    - `Delete Request`: 业务服务器收到你的 [`Delete`](/zh-CN/api-reference/api-ref/media-push/delete) 请求。
    - `Idle Timeout`: 超时退出。
