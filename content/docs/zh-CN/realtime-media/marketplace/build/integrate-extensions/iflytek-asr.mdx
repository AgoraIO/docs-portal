...
讯飞转写及翻译插件会通过 `onEvent` 回调返回语音转写和文本翻译结果。识别结果的含义详见 [onEvent 回调的 key](/zh-CN/realtime-media/marketplace/build/integrate-extensions/iflytek-asr-api#回调-key-)。
...
- [插件的 key-value 说明](/zh-CN/realtime-media/marketplace/build/integrate-extensions/iflytek-asr-api)
...
`setOptions` 方法中的参数设置详见[插件独有 API](/zh-CN/realtime-media/marketplace/build/integrate-extensions/iflytek-asr-api)。你可以使用 `setOptions` 方法中的 `callback` 参数来处理识别结果，示例如下：
...
- [插件独有 API](/zh-CN/realtime-media/marketplace/build/integrate-extensions/iflytek-asr-api)
