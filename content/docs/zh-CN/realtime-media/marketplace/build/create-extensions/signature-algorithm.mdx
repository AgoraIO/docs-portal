---
title: "签名算法"
description: "了解如何在云市场中签名算法，包括适用场景、配置步骤、实现要点和注意事项。"
---
服务开通 RESTful API 和计量计费 RESTful API 使用签名（signature 字段）进行身份验证。

名是经过加密处理的字符串，仅在服务商和声网之间共享。服务商可以通过签名鉴定声网发送的 HTTP 请求。
