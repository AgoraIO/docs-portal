...
| `agoraRtcConfig.encryption` | 可选   | Object | 如果 RTC 频道设置了加密，该参数必须设置。该参数包含 mode、secret、salt 字段，分别对应 [recordingConfig 的 decryptionMode、secret、salt 字段](/zh-CN/api-reference/api-ref/cloud-recording/start)。 |
| `agoraRtmConfig.appId` | 必填   | String | 用于声网 RTM 的 App ID。详见 [RTM 2.x API](/zh-CN/api-reference/rtm/android/configuration)。                 |
| `agoraRtmConfig.uid` | 必填   | String | 用于登录 RTM 系统的用户 ID。详见 [RTM 2.x API](/zh-CN/api-reference/rtm/android/configuration)。                        |
| `agoraRtmConfig.token` | 必填   | String | 用于登录 RTM 系统的 Token。详见 [RTM 2.x API](/zh-CN/api-reference/rtm/android/configuration)。                    |
