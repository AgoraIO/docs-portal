...
description: "查看 K 歌场景化方案的 K 歌房场景化 Kotlin API，了解关键概念、参数说明、使用限制和接入注意事项。"
---
本文提供在线 K 歌房场景定制化 Kotlin API。

<PlatformStructured platform="android">

你可以在 GitHub 上查看源码文件 [KTVApi.kt](https://github.com/AgoraIO-Community/KTVApi/blob/main/KTVAPI/Android/lib_ktvapi/src/main/java/io/agora/ktvapi/KTVApi.kt) 和 [KTVApiImpl.kt](https://github.com/AgoraIO-Community/KTVApi/blob/main/KTVAPI/Android/lib_ktvapi/src/main/java/io/agora/ktvapi/KTVApiImpl.kt)。
...
- `songCode`: 歌曲编号，用于标识一个音乐资源。你可以通过 `searchMusicByMusicChartId` 或 `searchMusicByKeyword` 获取需要加载的歌曲编号，也可以通过 RESTful API 来获取[曲库所有歌曲列表](/zh-CN/solutions/online-ktv/ktv-scenario/reference/music-content-center#获取曲库所有歌曲列表)或[增量歌曲列表](/zh-CN/solutions/online-ktv/ktv-scenario/reference/music-content-center#获取增量歌曲列表)。
...
KTV API 初始化时默认用户角色为听众，如果需要开始独唱或加入合唱，需要调用 `switchSingerRole` 来切换至相应的角色。你可以参考[切换说明](/zh-CN/solutions/online-ktv/ktv-scenario/get-started/integrate-ktvapi#5-切换角色)来进行角色切换。
...
- `NO_PERMISSION`：(1) 不支持从当前角色切换为目标角色，请参考[切换说明](/zh-CN/solutions/online-ktv/ktv-scenario/get-started/integrate-ktvapi#5-切换角色)进行角色切换。
...
- `songCode`: 歌曲编号，用于标识一个音乐资源。你可以通过 `searchMusic[1/2]` 或 `searchMusic[2/2]` 获取需要加载的歌曲编号，也可以通过 RESTful API 来获取[曲库所有歌曲列表](/zh-CN/solutions/online-ktv/ktv-scenario/reference/music-content-center#获取曲库所有歌曲列表)或[增量歌曲列表](/zh-CN/solutions/online-ktv/ktv-scenario/reference/music-content-center#获取增量歌曲列表)。
...
- `noPermission`：不支持从当前角色切换为目标角色，请参考[切换说明](/zh-CN/solutions/online-ktv/ktv-scenario/get-started/integrate-ktvapi#5-切换角色)进行角色切换。
