---
title: "歌词打分组件"
description: "了解 K 歌场景化方案在不同平台上的歌词打分组件方式，包括接入流程、关键配置和平台差异。"
---
...
:::info[信息]
你还可以通过 [`LyricsView`](/zh-CN/solutions/online-ktv/ktv-scenario/reference/lyrics-api#lyricsview) 类下的 API 来自定义更多界面元素。
:::
...
调用 `setScoringLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/solutions/online-ktv/ktv-scenario/reference/lyrics-api#setscoringlevel)来进行设置。
...
:::info[信息]
你还可以通过 [`ScoringView`](/zh-CN/solutions/online-ktv/ktv-scenario/reference/lyrics-api#scoringview) 类下的 API 来自定义更多界面元素。
:::
...
调用 `setScoreLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/solutions/online-ktv/ktv-scenario/reference/lyrics-api#setscorelevel)来进行设置。
