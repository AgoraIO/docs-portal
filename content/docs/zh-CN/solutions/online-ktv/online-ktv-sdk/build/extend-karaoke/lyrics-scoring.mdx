---
title: "歌词打分组件"
description: "了解在线 K 歌 SDK 在不同平台上的歌词打分组件方式，包括接入流程、关键配置和平台差异。"
---
声网歌词打分组件（KaraokeView）包含歌词组件（LyricsView）和打分组件（ScoringView）。

<PlatformStructured platform="android">

在 K 歌房场景中，你可以通过歌词组件实现歌曲播放时歌词同步展示以及自定义歌词界面，通过打分组件实现根据当前播放进度展示对应的原唱音准线、展示演唱者和原唱音准线的匹配效果、演唱者的分数、自定义打分逻辑以及评分组件界面。
...
![UI](https://web-cdn.agora.io/docs-files/1687162089264)

## 前提条件

在使用歌词组件前，请确保你已在项目中集成声网视频 SDK v4.x，并实现在线 K 歌功能。你可以参考场景化 API 方案、PaaS 方案、UIKit 开源方案来实现在线 K 歌功能。
...
1. **初始化歌词下载器并设置回调事件**
   ...
   调用 `getInstance` 初始化一个歌词下载器 `LyricsFileDownloader` 单例，然后调用 `setLyricsFileDownloaderCallback` 设置歌词文件下载回调事件。

   ```java
   // 设置可缓存的歌词文件的最大数量，默认为 50
   LyricsFileDownloader.getInstance(appContext).setMaxFileNum(50);
...
2. **下载歌词**
   ...
   调用 `download` 方法来下载歌词。默认情况下，最多可以缓存 **50** 个歌词文件，每个歌词文件的有效期为 8 小时。当歌词文件过期或缓存数量超出限制，歌词打分组件会自动移除已过期的或最早缓存的歌词文件。

   ```java
   int requestId = LyricsFileDownloader.getInstance(appContext).download(lrcUri);
   if (requestId < 0) {
       return;
   }

   // 保存返回的 requestId
   int saveRequestId = requestId;
   ```

3. **取消下载（可选）**

   如果你要取消下载歌词文件，调用 `cancelDownload` 方法。

   ```java
   LyricsFileDownloader.getInstance(appContext).cancelDownload(saveRequestId);
   ```

4. **清空本地缓存（可选）**

   如果你需要清空本地所有已下载的歌词文件，调用 `cleanAll` 方法。
...
2. **歌词同步**
   ...
   当播放器开始播放歌曲后，你可以通过内置媒体播放器的 [`getPlayPosition`](/api-ref/rtc/android/API/toc_mediaplayer_info#getPlayPosition) 获取歌曲当前的播放进度，然后通过 `setProgress` 来把歌词的播放进度同步给歌词组件，从而实现歌词同步。
   ...
   :::info[信息]
   为保证歌词 UI 界面的流畅性，声网建议调用 `setProgress` 方法的时间间隔不超过 20 ms。
   :::
...
:::info[信息]
你还可以通过 [`LyricsView`](/zh-CN/solutions/online-ktv/online-ktv-sdk/reference/lyrics-api#lyricsview) 类下的 API 来自定义更多界面元素。
:::
...
当演唱者开始唱歌，你可以通过 `RtcEngine` 下的 [`onAudioVolumeIndication`](/api-ref/rtc/android/API/toc_audio_basic#callback_irtcengineeventhandler_onaudiovolumeindication) 来获取本地用户的人声音调，然后通过 `setPitch` 来将用户实时的人声音调同步给打分组件。
...
调用 `setScoringLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/solutions/online-ktv/online-ktv-sdk/reference/lyrics-api#setscoringlevel)来进行设置。

```java
mKaraokeView.setScoringLevel(15);
```
...
:::info[信息]
声网不建议你同时使用 `setScoringLevel` 和 `setScoreCompensationOffset` 来调整打分逻辑。
:::
...
:::info[信息]
你还可以通过 [`ScoringView`](/zh-CN/solutions/online-ktv/online-ktv-sdk/reference/lyrics-api#scoringview) 类下的 API 来自定义更多界面元素。
:::

下图展示歌词打分组件的 API 调用时序：

![歌词打分组件](https://web-cdn.agora.io/docs-portal/img/online-ktv/lyrics-scoring.android.svg)
...
声网在 GitHub 上提供歌词组件开源的[示例项目](https://github.com/AgoraIO-Community/LrcView-iOS/tree/main)供你参考。
...
1. **初始化歌词下载器并设置代理**
   ...
   创建一个歌词下载器 `LyricsFileDownloader` 的实例并设置代理。
...
3. **取消下载（可选）**
   ...
   ```swift
   lyricsFileDownloader.cancelDownload(requestId: saveRequestId)
   ```
...
2. **歌词同步**
   ...
   当播放器开始播放歌曲后，你可以通过内置媒体播放器的 [`getPosition`](/api-ref/rtc/ios/API/toc_mediaplayer_info#getPosition) 获取歌曲当前的播放进度，然后通过 `setProgress` 来把歌词的播放进度设置给歌词组件，从而实现歌词同步。
...
当演唱者开始唱歌，你可以通过 `AgoraRtcEngineDelegate` 下的 [`reportAudioVolumeIndicationOfSpeakers`](/api-ref/rtc/ios/API/toc_audio_basic#callback_irtcengineeventhandler_onaudiovolumeindication) 来获取本地用户的人声音调，然后通过 `setPitch` 来将用户实时的人声音调同步给打分组件。
...
调用 `setScoreLevel` 来设置演唱者得分的难易程度。你可以通过 `level` 参数来调节难易程度，取值范围为 [0,100]，默认值为 15。值越小难度越低，演唱者越容易得分。你也可以参考[不同难度的推荐取值](/zh-CN/solutions/online-ktv/online-ktv-sdk/reference/lyrics-api#setscorelevel)来进行设置。

```swift
// 设置打分难易程度
karaokeView.setScoreLevel(level: 15)
```
...
:::info[信息]
声网不建议你同时使用 `setScoreLevel` 和 `setScoreCompensationOffset` 来调整打分逻辑。
:::
...
![歌词打分组件](https://web-cdn.agora.io/docs-portal/img/online-ktv/lyrics-scoring.ios.svg)
