...
- RTC SDK 和 RTSA Lite SDK 的音视频接口使用 [RTC Token](/zh-CN/realtime-media/rtc/basic-features/token-authentication.android) 进行鉴权，RTM SDK 和 RTSA Lite SDK 的云信令接口使用 [RTM Token](https://docs.agora.io/cn/Real-time-Messaging/token2_server_rtm) 进行鉴权。两者生成方式不同且不可混用。
