...
使用信令接口需要通过 RTM Token 鉴权。你需要自行在业务后台生成 [RTM Token](/zh-CN/realtime-media/rtm/build/security-and-auth/token-generation)。业务后台会分配此次通话的主叫用户 ID、被叫用户 ID 及对应的 RTM Token，并分别下发至客户端和设备端。这些参数都是后续客户端使用的 RTM SDK 和设备端使用的 RTSA Lite SDK 登录 RTM 系统所需要的。
...
- RTC SDK 和 RTSA Lite SDK 的音视频接口使用 [RTC Token](/zh-CN/realtime-media/rtc/build/setup-and-access/token-authentication) 进行鉴权，RTM SDK 和 RTSA Lite SDK 的云信令接口使用 [RTM Token](/zh-CN/realtime-media/rtm/build/security-and-auth/token-generation) 进行鉴权。两者生成方式不同且不可混用。
