...
在调用 RESTful API 之前，请确保已获取声网 App ID 和 Token（可选，用于安全鉴权）；对于服务端产品，还需要获取客户 ID 和客户密钥。可参考[开通灵动课堂服务](/zh-CN/solutions/flexible-classroom/build/setup-and-access/enable)。
...
- `Authorization`：声网 RESTful API 要求 HTTP Token 鉴权。每次发送 HTTP 请求时，都必须在请求 Header 中填入 `Authorization` 字段。关于如何生成该字段的值，请参考[实现 HTTP Token 鉴权](/zh-CN/solutions/flexible-classroom/build/setup-and-access/http-token-auth)。
...
响应的 HTTP 状态码分 200 和非 200 两种情况，详情可参考[响应状态码](/zh-CN/solutions/flexible-classroom/reference/response-code)。
...
当你遇到问题时，可以使用开发者工具查看请求和响应，结合[响应状态码](/zh-CN/solutions/flexible-classroom/reference/response-code)进行排查。如未能解决问题，可[联系技术支持](https://ticket.shengwang.cn/)。
