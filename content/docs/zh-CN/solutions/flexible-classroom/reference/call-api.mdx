...
- `Authorization`：声网 RESTful API 要求 HTTP Token 认证。每次发送 HTTP 请求时，都必须在请求 Header 中填入 `Authorization` 字段。关于如何生成该字段的值，请参考[实现 HTTP Token 认证](/zh-CN/solutions/flexible-classroom/build/setup-and-access/http-token-auth)。
