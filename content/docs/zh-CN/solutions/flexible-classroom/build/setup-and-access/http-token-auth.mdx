---
title: "实现 HTTP Token 鉴权"
description: "了解如何在灵动课堂中 HTTP Token 鉴权，包括适用场景、配置步骤、实现要点和注意事项。"
---
在使用灵动课堂 RESTful API 前，你需要通过 HTTP Token 鉴权。

灵动课堂支持使用 EducationToken 进行认证。

认证时，你需要在 HTTP 请求的请求 Header 中，在 `Authorization: agora token=` 字段填入服务端生成的 EducationToken。关于如何生成 EducationToken，可参考[生成器代码](/zh-CN/solutions/flexible-classroom/build/setup-and-access/generate-token#accesstoken2-%E7%94%9F%E6%88%90%E5%99%A8%E4%BB%A3%E7%A0%81)。
...
// 基于 Java 实现的 HTTP Token 鉴权示例，使用 RTC 的服务端 RESTful API
...
// 基于 Golang 实现的 HTTP Token 鉴权示例，使用 RTC 的服务端 RESTful API
