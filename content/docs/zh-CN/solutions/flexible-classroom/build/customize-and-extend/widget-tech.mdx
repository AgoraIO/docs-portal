...
Widget 内部注入了 `EduClassroomStore` 实例。`EduClassroomStore` 实例内拥有各个模块 Store 的实例，例如 `CloudDriveStore`、`GroupStore`，详见 [Edu Store API](/api-ref/flexible-classroom/javascript/overview)。
...
Widget 内部注入了 `EduClassroomStore` 实例。`EduClassroomStore` 实例内拥有各个模块 Store 的实例，例如 `CloudDriveStore`、`GroupStore`，详见 [Edu Store API](/api-ref/flexible-classroom/electron/overview)。
