...
:::warning[注意]
示例项目中使用了临时 Token，仅适用于运行 App 模块快速测试。在正式环境中，为确保安全，你必须参考[使用 Token 鉴权文档](/zh-CN/solutions/flexible-classroom/basic-features/generate-token.android.javascript)，在服务端部署并生成 Token。生成的 Token 对应的 `userId` 需要和 `launch` 方法中传入的参数 `userUuid` 保持一致，否则生成的 Token 无效。
:::
...
现在你已经初步体验了灵动课堂的功能，接下来可将灵动课堂 [集成](/zh-CN/solutions/flexible-classroom/get-started/integrate.javascript) 到你自己的项目中。
