...
```
   ...
   示例代码中需要传入 `token`。你可以使用[临时 Token](/zh-CN/solutions/flexible-classroom/build/create-classroom/enable#生成临时-token)，也可以参考[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/build/create-classroom/generate-token)从服务器生成 Token。

:::warning[注意]
声网提供的客户端临时 Token 生成器仅适用于 App 的快速测试。在正式环境中，为确保安全，你必须参考[使用 Token 鉴权文档](/zh-CN/solutions/flexible-classroom/build/create-classroom/generate-token)，在服务端部署并生成 Token。生成的 Token 中传入的 `userId` 需要与 `launch` 方法中传入的参数 `userUuid` 保持一致，否则该 Token 无效。
:::
...
4. 在终端中进入到工程目录，执行 `pod install` 命令。
   ...
   安装完成后，参照[自定义课堂 UI](/zh-CN/solutions/flexible-classroom/build/customize-classroom-experience/custom-ui) 了解灵动课堂的开源层设计思路，就可以修改源代码来自定义场景了。
...
示例代码中需要传入 `rtmToken`。你可以使用[临时 Token](/zh-CN/solutions/flexible-classroom/build/create-classroom/enable#生成临时-token)，也可以参考[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/build/create-classroom/generate-token)从服务器生成 Token。
...
3. 成功拉取代码后，根据你的实际需求，参考[自定义课堂 UI 文档](/zh-CN/solutions/flexible-classroom/build/customize-classroom-experience/custom-ui)修改代码。
...
示例代码中 `rtmToken` 的填写方式：你可以使用[临时 Token](/zh-CN/solutions/flexible-classroom/build/create-classroom/enable#生成临时-token)，也可以参考[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/build/create-classroom/generate-token)从服务器生成 Token。
