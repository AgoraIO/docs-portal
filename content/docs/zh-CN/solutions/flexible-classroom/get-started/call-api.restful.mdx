...
以[创建课堂 API](/zh-CN/solutions/flexible-classroom/api/api-classroom.restful#创建课堂) 为例，请求的 URL 示例如下：
...
以下是[创建课堂](/zh-CN/solutions/flexible-classroom/api/api-classroom.restful#创建课堂)的请求示例：
...
- `Authorization`：声网 RESTful API 要求 HTTP Token 认证。每次发送 HTTP 请求时，都必须在请求 Header 中填入 `Authorization` 字段。关于如何生成该字段的值，请参考[实现 HTTP Token 认证](/zh-CN/solutions/flexible-classroom/basic-features/http-token-auth.restful)。
...
响应的 HTTP 状态码分 200 和非 200 两种情况，详情可参考[响应状态码](/zh-CN/solutions/flexible-classroom/response-code.restful)。
...
响应 Body 包含了服务器反馈的数据。例如，[查询课堂](/zh-CN/solutions/flexible-classroom/api/api-classroom.restful#查询课堂)的响应 Body 包括标识课堂房间的 UUID、课堂名称、创建时间、房间属性等信息。具体以每个 API 的响应为准。

以下是[查询课堂](/zh-CN/solutions/flexible-classroom/api/api-classroom.restful#查询课堂)的响应示例：
...
当你遇到问题时，可以使用开发者工具查看请求和响应，结合[响应状态码](/zh-CN/solutions/flexible-classroom/response-code.restful)进行排查。如未能解决问题，可[联系技术支持](https://ticket.shengwang.cn/)。
