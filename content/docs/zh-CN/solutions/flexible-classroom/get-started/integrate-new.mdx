...
:::warning[注意]
使用云课堂前需要先调用服务端接口创建房间，否则部分功能无法使用（例如，学生无法自行打开摄像头麦克风等），详见[创建云课堂房间](/zh-CN/solutions/flexible-classroom/api/api-classroom#创建课堂)。
:::
...
示例代码中 `token` 的填写方式：你可以使用[临时 Token](/zh-CN/solutions/flexible-classroom/get-started/enable#生成临时-token)，也可以参考[使用 Token 鉴权](/zh-CN/solutions/flexible-classroom/basic-features/generate-token) 从服务器生成 Token。
