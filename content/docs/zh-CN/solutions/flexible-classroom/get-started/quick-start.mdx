...
:::warning[注意]
示例项目中使用了临时 Token，仅适用于运行 App 模块快速测试。在正式环境中，为确保安全，你必须参考[使用 Token 鉴权文档](/zh-CN/solutions/flexible-classroom/basic-features/generate-token)，在服务端部署并生成 Token。生成的 Token 对应的 `userId` 需要和 `launch` 方法中传入的参数 `userUuid` 保持一致，否则生成的 Token 无效。
:::
...
如果你想要升级灵动课堂代码的版本，参考[升级指南](/zh-CN/solutions/flexible-classroom/overview/migration)。
