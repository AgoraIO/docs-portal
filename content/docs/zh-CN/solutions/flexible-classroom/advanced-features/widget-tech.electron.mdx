...
灵动课堂内置插件中，白板插件和消息聊天插件挂载在固定位置，其余内置插件挂载在 Widget 容器内。你可以参考[内置插件](/zh-CN/solutions/flexible-classroom/advanced-features/widget.javascript.electron#内置插件)了解详情。
...
你可以参考 [rollbook-widget](/zh-CN/solutions/flexible-classroom/advanced-features/widget.javascript.electron#使用-agoraedutoolwidget) 来了解设置轨迹同步的详情。

### 实现多语言

你可以借助第三方开源库 `i18next` 提供的国际化能力，并提供多语言文本配置，来实现插件多语言。详细步骤可参考[实现插件多语言](/zh-CN/solutions/flexible-classroom/advanced-features/widget-previous.javascript.electron#实现插件多语言)。

## 后续步骤

了解技术原理后，你可以 [自定义插件](/zh-CN/solutions/flexible-classroom/advanced-features/widget.javascript.electron)。
