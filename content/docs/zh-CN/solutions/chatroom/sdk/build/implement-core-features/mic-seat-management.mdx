---
title: "管理语聊房麦位"
description: "了解语聊房在不同平台上的管理语聊房麦位方式，包括接入流程、关键配置和平台差异。"
---
<PlatformStructured platform="android">

本文介绍如何在语聊房中进行麦位管理。麦位管理包含如下操作：
...
![麦位管理功能示意](https://web-cdn.agora.io/docs-files/1689239633856)

## 示例项目

声网在 `agora-ent-scenarios` 仓库中提供麦位管理的源代码供你参考。
...
实现该进阶功能前，请确保你已实现基础的语聊房功能，例如登录即时通讯系统、获取房间列表、创建房间、加入房间。详见[实现语聊房](/zh-CN/solutions/chatroom/sdk/build/implement-core-features/quick-integration)。

### 注册回调

进行麦位管理前，你需要通过声网云服务中 `voiceServiceProtocol` 对象的 `subscribeEvent` 方法注册回调事件，监听房间内的回调事件。
...
![房主邀请上麦流程图](https://web-cdn.agora.io/docs-portal/img/chatroom/adv-host-invite.svg)
...
4. 如果听众接受上麦邀请，那么你需要让房内其他用户都收到麦位更新的通知。
    ...
    :::info[信息]
    房主邀请多个听众上麦时，如果多个听众同时点击接受上麦邀请，那么可能出现听众 A 刚上麦就被后面上麦的用户 B 踢下麦位。因此，为了避免多个听众同时主动修改麦位时造成互踢的冲突，声网推荐你在集成逻辑中控制上麦由房主决定，而不是听众决定。
    :::
...
![听众申请上麦流程图](https://web-cdn.agora.io/docs-portal/img/chatroom/adv-audience-request.svg)
...
不管是房主邀请，还是听众主动申请，在听众成为连麦听众后，你需要调用声网实时互动 SDK 中 [`setClientRole`](/api-ref/rtc/android/API/toc_channel#api_irtcengine_setclientrole) 方法将其角色从听众（`AUDIENCE`）切换成主播（`BROADCASTER`），以让连麦听众拥有发送音频流的权限。
...
不管是连麦听众主动下麦还是被踢下麦，在连麦听众成为普通听众后，你需要调用声网实时互动 SDK 中 [`setClientRole`](/api-ref/rtc/android/API/toc_channel#api_irtcengine_setclientrole) 方法将其角色从主播（`BROADCASTER`）切换成听众（`AUDIENCE`），以让其失去发送音频流的权限。
...
标记麦位静音或取消静音后，通过声网实时互动 SDK 中 [`muteLocalAudioStream`](/api-ref/rtc/android/API/toc_publishnsubscribe#api_irtcengine_mutelocalaudiostream) 方法停止发送或恢复发送麦位上的连麦听众的音频流，以达到静音或取消静音的效果。
...
换麦指将连麦听众从当前麦位更换到另一个空闲麦位。

1. 调用 `voiceServiceProtocol` 对象的 `changeMic` 方法更换麦位。
...
不管是房主邀请，还是听众主动申请，在听众成为连麦听众后，你需要调用声网实时互动 SDK 中 [`setClientRole`](/api-ref/rtc/ios/API/toc_channel#api_irtcengine_setclientrole) 方法将其角色从听众（`audience`）切换成主播（`broadcaster`），以让连麦听众拥有发送音频流的权限。
...
不管是连麦听众主动下麦还是被踢下麦，在连麦听众成为普通听众后，你需要调用声网实时互动 SDK 中 [`setClientRole`](/api-ref/rtc/ios/API/toc_channel#api_irtcengine_setclientrole) 方法将其角色从主播（`broadcaster`）切换成听众（`audience`），以让其失去发送音频流的权限。
...
标记麦位静音或取消静音后，通过声网实时互动 SDK 中 [`muteLocalAudioStream`](/api-ref/rtc/ios/API/toc_publishnsubscribe#api_irtcengine_mutelocalaudiostream) 方法停止发送或恢复发送麦位上的连麦听众的音频流，以达到静音或取消静音的效果。
...
1. 调用 `ChatRoomServiceImp` 类的 `changeMic` 方法更换麦位。
