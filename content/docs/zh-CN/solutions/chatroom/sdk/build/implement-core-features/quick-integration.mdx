...
本文介绍如何实现语聊房，包含登录系统、初始化引擎、创建房间、麦位管理、退出房间。声网在 `agora-ent-scenarios` 仓库中提供实现语聊房的源代码供你参考。
...
![进出房间流程](https://web-cdn.agora.io/docs-portal/img/chatroom/flow1-enter-room.svg)

### 房主邀请听众上麦

下图展示房主邀请听众上麦的流程。在这个流程中，房主发起上麦邀请。如果听众接受邀请，房主会收到通知并修改麦位信息，让听众上麦。听众随后发布自己的音频流。

![房主邀请听众上麦流程](https://web-cdn.agora.io/docs-portal/img/chatroom/flow2-host-invite.svg)

### 听众申请上麦

下图展示听众向房主申请上麦的流程。在这个流程中，听众主动发起上麦申请，如果房主接受申请，房主修改麦位信息以让听众上麦。听众收到上麦消息后，发布自己的音频流。

![听众申请上麦流程](https://web-cdn.agora.io/docs-portal/img/chatroom/flow3-audience-request.svg)
...
- 参考[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)创建声网开发者账号和声网项目，并获取项目的 App ID 和临时 Token，以及登录即时通讯系统所需的用户名 (ID) 和 Token。
...
1. 根据[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)在声网控制台创建声网项目后，复制界面的 App ID。
2. 调用声网 RTC SDK 中的 [`create`](/zh-CN/api-reference/rtc/android/initialize#api_irtcengine_initialize) 方法创建并初始化 `RtcEngine`。
...
3. 语聊房里需要有消息聊天和语音聊天，因此你还需进行如下操作：
    ...
    - 调用声网 RTC SDK 中 `RtcEngine` 类的 `joinChannel` 加入 RTC 频道以实现房间内的实时音频通话。方法中的参数含义和支持取值请参考 [`joinChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_joinchannel2)。在这一步里需要填写声网 RTC Token。你可以使用[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)中获取的临时 Token，也可以参考[使用 Token 鉴权](/zh-CN/realtime-media/rtc/build/setup-and-access/token-authentication)获取正式用途的声网 RTC Token。临时 Token 的有效期为 24 小时，建议你仅在测试用途下使用。

      你还可以调用 [`setChannelProfile`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_setchannelprofile)、[`setAudioProfile`](/zh-CN/api-reference/rtc/android/audio/audio-basic#api_irtcengine_setaudioprofile2)、[`setAudioScenario`](/zh-CN/api-reference/rtc/android/audio/audio-basic#api_irtcengine_setaudioscenario)、[`setParameters`](/zh-CN/api-reference/rtc/android/network#api_irtcengine_setparameters) 为不同业务场景（语聊社交、KTV、游戏陪玩、专业音频直播场景）配置语聊房的推荐音效参数。
...
具体步骤详见[麦位管理](/zh-CN/solutions/chatroom/sdk/build/implement-core-features/mic-seat-management)。
...
2. 调用 `RtcEngine` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_leavechannel) 方法离开 RTC 频道。

    ```kotlin
    rtcEngine?.leaveChannel()
    ```

3. 调用 [`leaveChatRoom`](https://docs-preprod.agora.io/cn/agora-chat/API%20Reference/im_java/v1.1.0/classio_1_1agora_1_1chat_1_1_chat_room_manager.html#a4f2c430782206971480c73a49311b1437) 离开聊天室，调用 [`asyncDestroyChatRoom`](https://docs-preprod.agora.io/cn/agora-chat/API%20Reference/im_java/v1.1.0/classio_1_1agora_1_1chat_1_1_chat_room_manager.html?transId=194274c0-bee4-11ed-b167-a39cc89dc96e#a0d50d2e34d9992fc3f8ea1c82e2db1a) 销毁聊天室。调用 [`destroy`](/zh-CN/api-reference/rtc/android/initialize#api_irtcengine_release) 销毁 RTC 引擎。
...
![API 时序](https://web-cdn.agora.io/docs-portal/img/chatroom/integ-and.svg)
...
- 有效的苹果开发者账号。
- 可以访问互联网的计算机。确保你的网络环境没有部署防火墙，否则无法正常使用声网服务。
- 参考[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)创建声网开发者账号和声网项目，并获取项目的 App ID 和临时 Token。
...
1. 根据[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)在声网控制台创建声网项目后，复制界面的 App ID。
2. 调用声网 RTC SDK 中的 [`sharedEngineWithAppId`](/zh-CN/api-reference/rtc/ios/initialize#api_irtcengine_create) 方法创建并初始化 `AgoraRtcEngineKit`。
...
3. 语聊房里需要有消息聊天和语音聊天，因此你还需进行如下操作：
    ...
    - 调用声网 RTC SDK 中 `AgoraRtcEngineKit` 类的 `joinChannelByToken` 加入 RTC 频道以实现房间内的实时音频通话。方法中的参数含义和支持取值请参考 [`joinChannelByToken`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_joinchannel1)。在这一步里需要填写声网 RTC Token。你可以使用[开通语聊房 SDK 相关服务](/zh-CN/solutions/chatroom/sdk/build/setup-and-access/enable-service)中获取的临时 Token，也可以参考[使用 Token 鉴权](/zh-CN/realtime-media/rtc/build/setup-and-access/token-authentication)获取正式用途的声网 RTC Token。临时 Token 的有效期为 24 小时，建议你仅在测试用途下使用。

      你还可以调用 [`setChannelProfile`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_setchannelprofile)、[`setAudioProfile`](/zh-CN/api-reference/rtc/ios/audio/audio-basic#api_irtcengine_setaudioprofile2)、[`setAudioScenario`](/zh-CN/api-reference/rtc/ios/audio/audio-basic#api_irtcengine_setaudioscenario)、[`setParameters`](/zh-CN/api-reference/rtc/ios/network#api_irtcengine_setparameters) 来为不同业务场景（语聊社交、KTV、游戏陪玩、专业音频直播场景）设置语聊房的最佳音效。
...
2. 调用 `AgoraRtcEngineKit` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_leavechannel) 方法离开 RTC 频道。

    ```swift
    rtcKit.leaveChannel(nil)
    ```

3. 调用 [`leaveChatroom:completion:`](https://docs-preprod.agora.io/cn/agora-chat/API%20Reference/im_oc/v1.1.0/protocol_i_agora_chatroom_manager-p.html#ae9b671ece1a36ad8ac26375d91755467) 离开聊天室，调用 [`destroyChatroom:`](https://docs-preprod.agora.io/cn/agora-chat/API%20Reference/im_oc/v1.1.0/protocol_i_agora_chatroom_manager-p.html#a1e3a6bacf44a96ff37b34d2068c84386) 销毁聊天室。调用 [`destroy`](/zh-CN/api-reference/rtc/ios/initialize#api_irtcengine_release) 销毁 RTC 引擎。
...
![API 时序](https://web-cdn.agora.io/docs-portal/img/chatroom/integ-ios.svg)
