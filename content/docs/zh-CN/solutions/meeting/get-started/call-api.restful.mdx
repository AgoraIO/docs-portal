...
在调用 RESTful API 之前，确保已经[开通服务](/zh-CN/solutions/meeting/get-started/enable-service)并获取 App ID 和临时 Token。

## 配置 URL

以[创建会议 API](/zh-CN/solutions/meeting/api/create-room.restful) 为例，请求的 URL 示例如下：
...
- `api.sd-rtn.com`：声网服务端域名。详见[保障 REST 服务高可用](/zh-CN/solutions/meeting/best-practices/high-availability.restful)。
...
- 如果 HTTP 状态码不为 `200`，参考[响应状态码](/zh-CN/solutions/meeting/response-code.restful)排查问题。
...
当你遇到问题时，可以使用开发者工具查看请求和响应，结合[响应状态码](/zh-CN/solutions/meeting/response-code.restful)进行排查。如未能解决问题，可[联系技术支持](https://ticket.shengwang.cn/)。
