...
调用[创建房间](/zh-CN/solutions/meeting/reference/create-room) API 时，通过 `roomProperties.webhookConfig` 字段设置接收 Webhook 事件的服务器地址和订阅事件列表后，你可以在这些事件发生后收到回调通知。
...
|`data`|Object|事件的具体数据，不同事件会包含不同数据，详见[事件类型](#事件类型)。|
