...
你需要在设备端集成 [RTSA Lite SDK](/zh-CN/realtime-media/rtsa/resources.c)，将音视频帧发送到客户端。

设备端调用 RTSA Lite SDK 中的 `agora_rtc_join_channel` 加入 RTC 频道。收到 `on_join_channel_success` 回调后，就可以开始发送音视频数据。下面的示例代码展示了设备端 RTSA Lite SDK 的调用逻辑。设备端与客户端必须使用同一个[声网 App ID](./enable-service/#获取-app-id) 且加入同一个 RTC 频道才能互通。
...
你需要在设备端集成 [RTSA Lite SDK](/zh-CN/realtime-media/rtsa/resources.c)，接收客户端发送的音视频帧。
...
你需要在 Android 设备端集成 [RTC SDK](/zh-CN/realtime-media/rtc/resources.android)，直接实现音视频采集、编码、传输、解码、渲染等功能。详见[实现音视频互动](/zh-CN/realtime-media/rtc/get-started/quick-start.android)。
...
你需要在客户端集成 [RTC SDK](/zh-CN/realtime-media/rtc/resources.android)，直接实现音视频采集、编码、传输、解码、渲染等功能。详见[实现音视频互动](/zh-CN/realtime-media/rtc/get-started/quick-start.android)。

在传输指定格式的音视频数据时，RTSA Lite SDK 与 RTC SDK 可以实现互通。详见[与 RTC SDK 互通音视频数据](/zh-CN/realtime-media/rtsa/best-practices/interoperate-rtc)。
