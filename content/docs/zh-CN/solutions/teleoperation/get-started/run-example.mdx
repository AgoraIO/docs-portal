---
title: "跑通远程控制示例项目"
description: "了解如何快速接入远程控制，完成环境准备、参数配置、示例运行和核心流程验证。"
---
...
:::info[信息]
如需体验其他设备端和操控端的示例项目，请联系 sales@shengwang.cn 获取，并参照 `README.md` 的步骤跑通。
:::
...
x86-64。如需支持其他架构，请联系 sales@shengwang.cn。
...
联系销售 sales@shengwang.cn 获取平行操控 Linux SDK。
...
:::info[信息]
本章节仅演示 YUV 原始视频数据的采集和收发。如需体验收发编码后的 H.264 格式的视频数据，请先运行 `./sync-data.sh` 命令获取相应的音频和视频文件。
:::
...
| 参数 | 描述 | 数据类型 | 是否必须 |
| --- | --- | --- | --- |
| `token` | 传入 App ID 或 Token。如果项目已开启 App 证书，则必须传 Token。详见[开通远程控制服务](/zh-CN/solutions/teleoperation/build/setup-and-access/enable-service)。 | String | **必填** |
| `channelId` | 频道名。 | String | **必填** |
| `userId` | <Slot name="run-example-markdown-0-3-1" /> | String | 选填 |
...
<Slot for="run-example-markdown-0-3-1">

设备端的用户 ID，默认值为 0，表示声网随机分配一个用户 ID。如需自行指定，需满足以下条件：
...
| `userId` | <Slot name="run-example-markdown-1-3-1" /> | String | 选填 |
...
<Slot for="run-example-markdown-1-3-1">

操控端的用户 ID，默认值为 0，表示声网随机分配一个用户 ID。如需自行指定，需满足以下条件：
...
| `userId` | 当前用户的用户 ID，仅支持传入内容为数值的字符串。 | String| **必填** |
| `remoteUserId` | 远端用户的用户 ID，仅支持传入内容为数值的字符串。 | String| **必填** |
...
| `userId` | 当前用户的用户 ID，即发送端的 `remoteUserId`。 | String| **必填** |
| `remoteUserId` | 远端用户的用户 ID，即发送端的 `userId`。 | String| **必填** |
