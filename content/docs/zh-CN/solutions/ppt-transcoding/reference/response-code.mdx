...
title: "响应状态码"
---
在调用 PPT 转码服务 RESTful API 过程中，你可能收到如下 HTTP 状态码：

- 如果状态码为 `200` 或 `201`，表示请求成功。

- 如果状态码不为 `200` 或 `201`，则请求失败。响应包体中可能包含 `error.code`、`error.title` 等字段，用于描述失败原因。
...
调用 PPT 转码 RESTful API 后收到的 HTTP 状态码为 `400 Invalid request` 时，在响应包体中会返回如下错误码 (`code`) 和错误消息 (`message`)：
...
| 2010203 | the number of tasks exceeds limit                            | 转换队列中的任务过多。                                       | 调用[取消指定的转码任务](/doc/conversion-ppt/restful/restful-ppt/operations/delete-v5-projector-tasks-uuid)以减少队列中的任务数量，或等待已有转换任务完成后再发起转换。 |
