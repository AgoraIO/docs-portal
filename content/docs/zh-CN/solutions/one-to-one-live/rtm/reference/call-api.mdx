---
title: "CallAPI 参考"
description: "查看 1v1 私密房 RTM 方案的 CallAPI 参考，了解关键概念、参数说明、使用限制和接入注意事项。"
---
...
| `Rtm` | `110`  | 信令服务的消息错误。如果使用默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/build/troubleshooting#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。          |
...
| `normal` | `0` | 业务类型的错误，暂无。         |
| `rtc` | `100`   | RTC 服务错误，你可以搭配 `onCallError` 中反馈的 `errorCode` 查看 [RTC 错误码文档](/zh-CN/realtime-media/rtc/reference/error-code)了解报错原因和解决办法。      |
| `message`| `110`  | 信令服务的消息错误。使用如果的默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/build/troubleshooting#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。         |
