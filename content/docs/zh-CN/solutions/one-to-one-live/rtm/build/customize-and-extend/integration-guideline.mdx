---
title: "优化 RTM 信令方案集成逻辑"
description: "了解 1v1 私密房 RTM 方案在不同平台上的优化 RTM 信令方案集成逻辑方式，包括接入流程、关键配置和平台差异。"
---
...
其中，当前 RTC 通话频道的 ID（`roomId` 参数）可以在 `onCallStateChanged` 的 `state` 为 `calling` 时从 `eventInfo` 里解析获取。
...
#### 信令 Token
    ...
    1. 监听 RTM Token 是否过期。通过[添加并监听 CallRtmManager 状态回调](/zh-CN/solutions/one-to-one-live/rtm/build/implement-core-features/start-one-to-one#3-添加并监听-callrtmmanager-状态回调) 监听 `ICallRtmManagerListener` 的 `onTokenPrivilegeWillExpire`。
        ...
        - 监听进行通话之前过期的 Token。
          ...
          val listener = object : ICallApiListener {
              override fun onCallError(
                  ...
                  if (
                      errorEvent == CallErrorEvent.RtcOccurError &&
                      errorType == CallErrorCodeType.Rtc &&
                      errorCode == ErrorCode.TokenExpired.value
                  ) {
...
:::info[信息]
建议给 RTC 和 RTM 配置相同的 Token 过期时间，然后通过[信令 Token](/zh-CN/solutions/one-to-one-live/rtm/build/customize-and-extend/integration-guideline) 方式，借助 RTM 来检测过期并同时更新。
:::
