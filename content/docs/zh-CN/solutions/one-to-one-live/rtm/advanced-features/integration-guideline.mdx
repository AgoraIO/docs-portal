...
#### 信令 Token
    ...
    1. 监听 RTM Token 是否过期。通过[添加并监听 CallRtmManager 状态回调](/zh-CN/solutions/one-to-one-live/rtm/basic-features/start-one-to-one#3-添加并监听-callrtmmanager-状态回调) 监听 `ICallRtmManagerListener` 的 `onTokenPrivilegeWillExpire`。
