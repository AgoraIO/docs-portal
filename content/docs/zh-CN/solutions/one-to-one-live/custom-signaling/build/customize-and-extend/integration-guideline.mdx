---
title: "进阶集成指引"
description: "了解 1v1 私密房三方信令方案在不同平台上的优化自定义信令方案集成逻辑方式，包括接入流程、关键配置和平台差异。"
---
<PlatformStructured platform="android">

## 使用已初始化过的 rtmClient

在集成 1v1 私密房方案过程中，你可以参考本文对 API 的调用时机、逻辑、参数设置等进行调优。声网 1v1 私密房的 `CallAPI` 中已经实现了对所需 RTM 服务的封装。如果在集成 1v1 私密房之前，你的项目中已经有 RTM 实例 `rtmClient` 了，则可以直接使用初始化过的 RTM 实例，然后调用相关功能。
...
1. 监听 RTM Token 是否过期。通过[添加并监听 CallRtmManager 状态回调](/zh-CN/solutions/one-to-one-live/custom-signaling/build/implement-core-features/start-one-to-one) 监听 `ICallRtmManagerListener` 的 `onTokenPrivilegeWillExpire`。
...
:::info[信息]
建议给 RTC 和 RTM 配置相同的 Token 过期时间，然后通过[信令 Token](/zh-CN/solutions/one-to-one-live/custom-signaling/build/customize-and-extend/integration-guideline) 方式，借助 RTM 来检测过期并同时更新。
:::
