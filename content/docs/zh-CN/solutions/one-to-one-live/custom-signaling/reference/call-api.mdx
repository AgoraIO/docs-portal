---
title: "CallAPI 参考"
description: "查看 1v1 私密房三方信令方案的 CallAPI 参考，了解关键概念、参数说明、使用限制和接入注意事项。"
---
<PlatformStructured platform="android">

声网为 1v1 私密房解决方案提供 `CallAPI`，本文提供相应的 API 参考。除了 `CallAPI`，你还需要使用 RTC API 来实现初始化等功能。你可以参考 [RTC API](/api-ref/rtc/android/API/rtc_api_overview) 来了解详细的 RTC API 用法。
...
| 参数        | 类型     | 含义                 |
| ---------- | ------- | -------------------- |
| `rtcToken`      | String | RTC 服务 Token，详见[使用 RTC Token 鉴权](/zh-CN/realtime-media/rtc/build/setup-and-access/token-authentication)。 |
...
:::info[信息]
如果你的信令 Token 与 RTC Token 过期时间一致，收到该回调即意味着信令 Token 也已过期。收到该回调后，你可以同步生成并更新信令 Token。例如，如果你使用的是声网的 RTM SDK 实现的信令通道，则可以重新[生成 RTM Token](/zh-CN/realtime-media/rtm/build/security-and-auth/token-generation)，然后调用 `AgoraRtmClientKit.revewToken` 方法更新信令 Token。
:::
...
| `autoJoinRTC` | Bool | <Slot name="call-api-android-html-3-5-2" /> |
...
RTC Token。
        ...
        :::warning[注意]
        - 你需要使用通配频道名的 Token，即在生成 Token 时将 `channelName` 设为通配符或空字符串。详见[使用通配 Token](/zh-CN/realtime-media/rtc/build/security-and-auth/wildcard-token)。
...
|------|--------------|---|
| `Normal` | `0` | 业务类型的错误，暂无。         |
| `Rtc` | `100`   | RTC 服务错误，你可以搭配 `onCallError` 中反馈的 `errorCode` 查看 [RTC 错误码文档](/zh-CN/realtime-media/rtc/reference/error-code)了解报错原因和解决办法。      |
| `Rtm` | `110`  | 信令服务的消息错误。使用如果的默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/build/troubleshooting#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。          |
...
| 参数        | 类型     | 含义                 |
| ---------- | ------- | -------------------- |
| `rtcToken`      | String | RTC 服务 Token，需要设为使用通配频道名的 Token，即在生成 Token 时将 `channelName` 设为通配符或空字符串。详见[使用通配 Token](/zh-CN/realtime-media/rtc/build/security-and-auth/wildcard-token)。 |
...
:::info[信息]
如果你的信令 Token 与 RTC Token 过期时间一致，收到该回调即意味着信令 Token 也已过期。收到该回调后，你可以同步生成并更新信令 Token。例如，如果你使用的是声网的 RTM SDK 实现的信令通道，则可以重新[生成 RTM Token](/zh-CN/realtime-media/rtm/build/security-and-auth/token-generation)，然后调用 `AgoraRtmClientKit.renewToken` 方法更新信令 Token。
:::
...
| `remoteAccepted` | `103` | 远端用户接受呼叫邀请。 |
| `localAccepted` | `104` | 本地用户接受呼叫邀请。 |
| `localHangup` | `105` | 本地用户挂断。 |
| `remoteHangup` | `106` | 远端用户挂断。 |
| `remoteJoined` | `107` | 远端用户加入 RTC 频道。 |
...
| `normal` | `0` | 业务类型的错误，暂无。         |
| `rtc` | `100`   | RTC 服务错误，你可以搭配 `onCallError` 中反馈的 `errorCode` 查看 [RTC 错误码文档](/zh-CN/realtime-media/rtc/reference/error-code)了解报错原因和解决办法。      |
| `message`| `110`  | 信令服务的消息错误。如果使用默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/build/troubleshooting#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。         |
