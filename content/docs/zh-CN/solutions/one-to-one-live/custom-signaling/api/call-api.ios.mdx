...
| 参数        | 类型     | 含义                 |
| ---------- | ------- | -------------------- |
| `rtcToken`      | String | RTC 服务 Token，需要设为使用通配频道名的 Token，即在生成 Token 时将 `channelName` 设为通配符或空字符串。详见[使用通配 Token](/zh-CN/realtime-media/rtc/best-practice/wildcard-token.android.ios)。 |
...
RTC Token。
        ...
        :::warning[注意]
        - 你需要使用通配频道名的 Token，即在生成 Token 时将 `channelName` 设为通配符或空字符串。详见[使用通配 Token](/zh-CN/realtime-media/rtc/best-practice/wildcard-token.android.ios)。
...
| `message`| `110`  | 信令服务的消息错误。如果使用默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/error-codes.android.ios#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。         |
