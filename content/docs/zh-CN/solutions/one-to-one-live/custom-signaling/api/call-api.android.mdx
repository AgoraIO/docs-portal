...
| `Rtm` | `110`  | 信令服务的消息错误。使用如果的默认的信令通道（声网 RTM），可以使用 [`AgoraRtmErrorCode`](/zh-CN/realtime-media/rtm/error-codes.android.android#错误码对照表) 查看报错信息；如果是自定义信令（如环信），则可以参考对应信令服务的错误信息。          |
