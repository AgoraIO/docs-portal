...
调用 [`create`](/api-ref/rtc/android/API/toc_initialize#api_irtcengine_initialize) 创建 `RtcEngine`。调用 `IMetaService` 类的 [`create`](/zh-CN/solutions/meta-world/reference/meta-api#create) 和 [`initialize`](/zh-CN/solutions/meta-world/reference/meta-api#initialize) 创建并初始化 `IMetaService`。
