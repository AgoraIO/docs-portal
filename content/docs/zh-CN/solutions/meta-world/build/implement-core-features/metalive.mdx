---
title: "实现元直播"
description: "了解声网解决方案在不同平台上的元直播方式，包括接入流程、关键配置和平台差异。"
---
...
完成[集成声网 Meta SDK](/zh-CN/solutions/meta-world/get-started/integrate-sdk) 后，你可以参考本节实现元直播。

下图展示实现元直播的 API 调用时序：

![元直播 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/meta-live.svg)
...
1. 调用 `RtcEngine` 类的 [`joinChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_joinchannel2) 使主播加入 RTC 频道。
2. 调用 `IMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/reference/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
1. 调用 `RtcEngine` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_leavechannel) 离开直播频道。
2. 调用 `IMetaService` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/reference/meta-api#leavescene) 离开场景。
3. 通过 `IMetaSceneEventHandler` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/reference/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`release`](/zh-CN/solutions/meta-world/reference/meta-api#release) 释放 `IMetaScene`。
4. 通过 `IMetaSceneEventHandler` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/reference/meta-api#onleavesceneresult) 回调监听 `IMetaScene` 是否释放成功。
...
调用 `AgoraMetaServiceKit` 类的 [`createScene`](/zh-CN/solutions/meta-world/reference/meta-api#createscene) 创建 `AgoraMetaScene`，并在 `AgoraMetaSceneConfig` 中设置场景配置信息。你需要在 `AgoraMetaSceneConfig` 中设置 `enableFaceCapture` 为 `true`，并在 `faceCaptureAppId` 和 `faceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。
...
2. 调用 `AgoraMetaLocalUserAvatar` 类的 `ILocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/reference/meta-api#setextrainfo) 设置用户的捏脸、换装信息。
...
1. 调用 `AgoraRtcEngineKit` 类的 [`joinChannelByToken`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_joinchannel2) 使主播加入 RTC 频道。
2. 调用 `AgoraMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/reference/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
1. 调用 `AgoraRtcEngineKit` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_leavechannel) 离开直播频道。
2. 调用 `AgoraMetaScene` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/reference/meta-api#leavescene) 离开场景。
3. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/reference/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`destroy`](/zh-CN/solutions/meta-world/reference/meta-api#destroy) 释放 `AgoraMetaScene`。
4. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/reference/meta-api#onreleasedscene) 回调监听 `AgoraMetaScene` 是否释放成功。
