---
title: "空间音频"
description: "了解声网解决方案在不同平台上的空间音频方式，包括接入流程、关键配置和平台差异。"
---
...
![用户空间音频 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/spatial-audio-user-android.svg)

#### 1 创建和初始化空间音频引擎

使用空间音频前，你需要调用 [`create`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#create) 和 [`initialize`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#initialize) 创建和初始化空间音频引擎。
...
调用 [`setAudioRecvRange`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#setaudiorecvrange) 设置空间音频接收范围，当远端用户相对本地用户的距离超出这个范围，本地用户就会听不到远端用户的声音。
...
通过 [`onUserPositionChanged`](/zh-CN/solutions/meta-world/reference/meta-api#onuserpositionchanged) 回调监听用户的位置变化。如果当前用户是本地用户，调用 [`updateSelfPosition`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#updateselfposition) 更新本地用户位置信息；如果当前用户是远端用户，调用 [`updateRemotePosition`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#updateremoteposition) 更新远端用户的信息位置。
...
不需要使用空间音频时，调用 [`destroy`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#destroy) 销毁空间音频引擎。
...
![音乐空间音频 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/spatial-audio-media-android.svg)
...
参考[媒体播放器](/zh-CN/realtime-media/rtc/build/audio/media-player)功能指南创建一个 `IMediaPlayer` 对象，然后打开、播放媒体资源。
...
处理 Unity 场景的回调信息，解析得到物体的三维坐标位置和朝向，并将这些信息传入 [`updatePlayerPositionInfo`](/zh-CN/api-reference/rtc/android/audio/audio-prenpost/spatial-audio#updateplayerpositioninfo) 方法，设置播放器的空间音频。
...
![用户空间音频 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/spatial-audio-user-ios.svg)

#### 1. 创建和初始化空间音频引擎

使用空间音频前，你需要调用 [`sharedLocalSpatialAudioWithConfig`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#sharedlocalspatialaudiowithconfig) 创建和初始化空间音频引擎。
...
调用 [`setAudioRecvRange`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#setaudiorecvrange) 设置空间音频接收范围，当远端用户相对本地用户的距离超出这个范围，本地用户就会听不到远端用户的声音。
...
通过 [`onUserPositionChanged`](/zh-CN/solutions/meta-world/reference/meta-api#onuerpositionchanged) 回调监听用户的位置变化。如果当前用户是本地用户，调用 [`updateSelfPosition`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#api_ibasespatialaudioengine_updateselfposition) 更新本地用户位置信息；如果当前用户是远端用户，调用 [`updateRemotePosition`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#api_ilocalspatialaudioengine_updateremoteposition) 更新远端用户的信息位置。
...
不需要使用空间音频时，调用 [`destroy`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#destroy) 销毁空间音频引擎。
...
![音乐空间音频 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/spatial-audio-media-ios.svg)

由于空间音频是基于 NPC 或物体的位置驱动，因此在进入 Unity 场景后，无论 NPC 或物体是否移动，都需要 Unity 脚本主动向 App 发送一次 NPC 或物体的位置信息。这样可以确保空间音频引擎始终基于最新的位置数据提供空间音频。

#### 1 实现媒体播放器

参考[媒体播放器](/zh-CN/realtime-media/rtc/build/audio/media-player)功能指南创建一个 `AgoraMediaPlayerProtocol` 对象，然后打开、播放媒体资源。
...
处理 Unity 场景的回调信息，解析得到物体的三维坐标位置和朝向，并将这些信息传入 [`updatePlayerPositionInfo`](/zh-CN/api-reference/rtc/ios/audio/audio-prenpost/spatial-audio#api_ibasespatialaudioengine_updateplayerpositioninfo) 方法，设置播放器的空间音频。
