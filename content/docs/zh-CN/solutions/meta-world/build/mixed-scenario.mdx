---
title: "实现融合场景"
description: "了解声网解决方案在不同平台上的融合场景方式，包括接入流程、关键配置和平台差异。"
---
...
![融合场景 API 时序图](https://web-cdn.agora.io/docs-portal/img/meta-world/meta-mixed.svg)
...
调用 `RtcEngine` 类的 [`create`](/zh-CN/api-reference/rtc/android/initialize#api_irtcengine_initialize) 创建 `RtcEngine`。调用 `IMetaService` 类的 [`create`](/zh-CN/solutions/meta-world/reference/meta-api#create) 和 [`initialize`](/zh-CN/solutions/meta-world/reference/meta-api#initialize) 创建并初始化 `IMetaService`。

初始化 `IMetaService` 时，需要在 `MetaServiceConfig` 里设置如下重要的字段：

- `mRtcEngine`：通过 `create` 方法创建的 `RtcEngine` 实例。
- `mAppId`：在声网控制台获取的 App ID。详见[开通元语聊服务](/zh-CN/solutions/meta-world/build/setup-and-access/enable-service)。
- `mUserId`：登录声网 RTM 系统的用户 ID。推荐取值详见 [`MetaServiceConfig`](/zh-CN/solutions/meta-world/reference/meta-api#metaserviceconfig)。
- `mRtmToken`：用于登录声网 RTM 系统的动态密钥。开启动态鉴权后可用。详见[生成 Token](/zh-CN/realtime-media/rtm/build/authentication-and-connection/token-generation)。
- `mLocalDownloadPath`：场景资源下载到本地的保存路径。
- `mEventHandler`：`IMetaService` 的回调事件句柄。

声网项目有两种 Token 和 UID，请不要搞混淆：

- RTC UID：用于在实时音视频通讯中标志用户身份的用户 ID。推荐取值详见 [joinChannel 的参数解释](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_joinchannel2)。
...
- RTC Token：用于保障实时音视频通讯安全的动态密钥。详见[如何生成 RTC Token 进行鉴权](/zh-CN/realtime-media/rtc/build/setup-and-access/token-authentication)。
- RTM Token：用于保障云信令系统安全的动态密钥。详见[如何生成 RTM Token 进行鉴权](/zh-CN/realtime-media/rtm/build/authentication-and-connection/user-authentication)。
...
1. 调用 `RtcEngine` 类的 [`setupLocalVideo`](/zh-CN/api-reference/rtc/android/video/video-rendering#api_irtcengine_setuplocalvideo) 初始化本地视图，用于本地预览。
2. 调用 `RtcEngine` 类的 [`joinChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_joinchannel2) 使主播加入 RTC 频道。
...
3. 调用 `RtcEngine` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_leavechannel) 离开直播频道。
...
调用 [`sharedEngineWithConfig`](/zh-CN/api-reference/rtc/ios/initialize#api_irtcengine_initialize) 创建 `AgoraRtcEngineKit`。调用 [`sharedMetaServiceWithConfig`](/zh-CN/solutions/meta-world/reference/meta-api#sharedmetaservicewithconfig) 创建并初始化 `AgoraMetaServiceKit`。
...
- `appId`：在声网控制台获取的 App ID。详见[开通元语聊服务](/zh-CN/solutions/meta-world/build/setup-and-access/enable-service)。
...
- `rtmToken`：用于登录声网 RTM 系统的动态密钥。开启动态鉴权后可用。详见[生成 Token](/zh-CN/realtime-media/rtm/build/authentication-and-connection/token-generation)。
- `localDownloadPath`：场景资源下载到本地的保存路径。
- `delegate`：`AgoraMetaServiceKit` 的回调事件。

声网项目有两种 Token 和 UID，请不要搞混淆：

- RTC UID：用于在实时音视频通讯中标志用户身份的用户 ID。推荐取值详见 [joinChannelByToken 的参数解释](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_joinchannel2)。
...
1. 调用 `AgoraRtcEngineKit` 类的 [`setupLocalVideo`](/zh-CN/api-reference/rtc/ios/video/video-rendering#api_irtcengine_setuplocalvideo) 初始化本地视图，用于摄像头画面本地预览。
2. 调用 `AgoraRtcEngineKit` 类的 [`joinChannelByToken`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_joinchannel2) 使主播加入 RTC 频道。
...
3. 调用 `AgoraRtcEngineKit` 类的 [`leaveChannel`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_leavechannel) 离开直播频道。
