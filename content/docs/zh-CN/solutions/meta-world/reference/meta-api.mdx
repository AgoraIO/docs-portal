---
title: "Meta SDK API"
description: "查看声网解决方案的 Meta SDK API，了解关键概念、参数说明、使用限制和接入注意事项。"
---
...
Meta SDK 是基于声网视频 SDK 开发的元系列特殊版 SDK。相比[视频 SDK](/zh-CN/realtime-media/rtc)，Meta SDK 中新增元系列相关的 API。
...
- `frame`：原始视频帧。详见 RTC SDK API 参考中的 [`AgoraVideoFrame`](/zh-CN/api-reference/rtc/android/class-externalvideoframe)。
...
该方法用于开启或关闭场景渲染画面捕获。通过与 `RtcEngine` 的 [`joinChannel` [2/2]](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_joinchannel2) 方法配合使用，可以将场景渲染的画面发布到 RTC 频道中。
...
- `mRtcEngine`：`RtcEngine` 实例，创建方法详见 [`create` [1/2]](/zh-CN/api-reference/rtc/android/initialize#api_irtcengine_create)。
...
Meta SDK 是基于声网视频 SDK 开发的元系列特殊版 SDK。相比[视频 SDK](/zh-CN/api-reference/rtc/ios/rtc-api-overview)，Meta SDK 中新增元系列相关的 API。
...
- `displayId`：指定显示器的 ID。ID 需要由你的 Native 开发人员和 Unity 开发人员协商后规定。
- `frame`：视频帧。详见 RTC SDK API 参考中的 [`AgoraVideoFrame`](/zh-CN/api-reference/rtc/ios/class-externalvideoframe)。
...
该方法用于开启或关闭场景渲染画面捕获。通过与 `AgoraRtcEngineKit` 的 [`joinChannelByToken` [1/2]](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_joinchannel1) 方法配合使用，可以将场景渲染的画面发布到 RTC 频道中。
...
- `rtcEngine`：`AgoraRtcEngineKit` 实例，创建方法详见 [`sharedEngineWithConfig`](/zh-CN/api-reference/rtc/ios/initialize#api_irtcengine_initialize)。
...
<a id="agormetasceneinfo"></a>

<a id="agorametaceneinfo"></a>
