...
2. 调用 `IMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
2. 调用 `IMetaService` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#leavescene) 离开场景。
3. 通过 `IMetaSceneEventHandler` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`release`](/zh-CN/solutions/meta-world/api-ref/meta-api#release) 释放 `IMetaScene`。
4. 通过 `IMetaSceneEventHandler` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调监听 `IMetaScene` 是否释放成功。
...
调用 `AgoraMetaServiceKit` 类的 [`createScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#createscene) 创建 `AgoraMetaScene`，并在 `AgoraMetaSceneConfig` 中设置场景配置信息。你需要在 `AgoraMetaSceneConfig` 中设置 `enableFaceCapture` 为 `true`，并在 `faceCaptureAppId` 和 `faceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。
...
2. 调用 `AgoraMetaLocalUserAvatar` 类的 `ILocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setextrainfo) 设置用户的捏脸、换装信息。
...
2. 调用 `AgoraMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
2. 调用 `AgoraMetaScene` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#leavescene) 离开场景。
3. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`destroy`](/zh-CN/solutions/meta-world/api-ref/meta-api#destroy) 释放 `AgoraMetaScene`。
4. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#onreleasedscene) 回调监听 `AgoraMetaScene` 是否释放成功。
