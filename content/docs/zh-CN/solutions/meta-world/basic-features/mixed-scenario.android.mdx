...
完成[集成声网 Meta SDK](/zh-CN/solutions/meta-world/get-started/integrate-sdk.android) 后，你可以参考本节实现元语聊和元直播的融合场景。
...
调用 `RtcEngine` 类的 [`create`](/api-ref/rtc/android/API/toc_initialize#api_irtcengine_initialize) 创建 `RtcEngine`。调用 `IMetaService` 类的 [`create`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#create) 和 [`initialize`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#initialize) 创建并初始化 `IMetaService`。
...
- `mUserId`：登录声网 RTM 系统的用户 ID。推荐取值详见 [`MetaServiceConfig`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#metaserviceconfig)。
...
- RTM UID：用于在云信令系统中标志用户身份的用户 ID。推荐取值详见 [MetaServiceConfig 的字段解释](/zh-CN/solutions/meta-world/api-ref/meta-api.android#metaserviceconfig)。
- RTC Token：用于保障实时音视频通讯安全的动态密钥。详见[如何生成 RTC Token 进行鉴权](/zh-CN/realtime-media/rtc/basic-features/token-authentication.android)。
- RTM Token：用于保障云信令系统安全的动态密钥。详见[如何生成 RTM Token 进行鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication.android.android)。
...
调用 `IMetaService` 类的 [`getSceneAssetsInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#getsceneassetsinfo) 获取场景资源，并通过 `IMetaServiceEventHandler` 类的 [`getSceneAssetsInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#ongetsceneassetsinforesult) 回调监听获取场景资源时的事件。

调用 `IMetaService` 类的 [`downloadScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#downloadscene) 获取场景资源，并通过 `IMetaServiceEventHandler` 类的 [`onDownloadSceneAssetsProgress`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#ondownloadsceneassetsprogress) 回调监听获取场景资源时的事件。
...
调用 `IMetaService` 类 [`createScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#createscene) 创建 `IMetaScene`，并在 `MetaSceneConfig` 中设置场景配置信息。为增加直播趣味性，声网推荐你开启面部捕捉，使用同步人脸表情的 Avatar 形象。你需要在 `MetaSceneConfig` 中设置 `mEnableFaceCapture` 为 `true`，并在 `mFaceCaptureAppId` 和 `mFaceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。

通过 `IMetaServiceEventHandler` 类的 [`onCreateSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#oncreatesceneresult) 和 [`onConnectionStateChanged`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onconnectionstatechanged) 回调监听创建场景和连接状态的事件。
...
1. 调用 `ILocalUserAvatar` 类的 [`setUserInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#setuserinfo) 和 [`setModelInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#setmodelinfo) 设置用户的基本信息和虚拟形象（Avatar）的模型信息。
2. 调用 `ILocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#setextrainfo) 设置用户的捏脸、换装信息。
3. 调用 `IMetaScene` 类的 [`enterScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#enterscene) 进入场景，并通过 `config` 设置配置信息。
4. 通过 `IMetaSceneEventHandler` 类的 [`onEnterSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onentersceneresult) 回调监听进入场景的结果。
...
:::warning[注意]
- 本小节展示在**进入场景前**设置用户和模型信息，如需在**进入场景后**变更设置，需调用 [`applyInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#applyinfo) 使设置生效。
- 声网 Meta SDK 支持自定义装扮捏脸功能，详见[换装和捏脸](/zh-CN/solutions/meta-world/advanced-features/dress-face.android)。
:::
...
3. 调用 `IMetaScene` 类的 [`addSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#addsceneview) 在场景中添加一个视图，用于在频道内发布主播的 Avatar 形象。
4. 通过 `IMetaSceneEventHandler` 类的 [`onAddSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onaddsceneviewresult) 回调监听添加场景显示视图的结果。
5. 调用 `IMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
1. 调用 `IMetaService` 类的 [`removeSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#removesceneview) 将 Avatar 直播画面从场景中移除。
2. 通过 `IMetaSceneEventHandler` 类的 [`onRemoveSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onremovesceneviewresult) 回调监听移除场景显示视图的结果。
3. 调用 `RtcEngine` 类的 [`leaveChannel`](/api-ref/rtc/android/API/toc_channel#api_irtcengine_leavechannel) 离开直播频道。
4. 调用 `IMetaService` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#leavescene) 离开场景。
5. 通过 `IMetaSceneEventHandler` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onleavesceneresult) 回调得知成功离开场景后，调用 [`release`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#release) 释放 `IMetaScene`。
6. 通过 `IMetaSceneEventHandler` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onleavesceneresult) 回调监听 `IMetaScene` 是否释放成功。
