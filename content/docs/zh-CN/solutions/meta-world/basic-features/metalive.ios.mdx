...
完成[集成声网 Meta SDK](/zh-CN/solutions/meta-world/get-started/integrate-sdk.ios) 后，你可以参考本节实现元直播。
...
调用 `AgoraMetaServiceKit` 类的 [`createScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#createscene) 创建 `AgoraMetaScene`，并在 `AgoraMetaSceneConfig` 中设置场景配置信息。你需要在 `AgoraMetaSceneConfig` 中设置 `enableFaceCapture` 为 `true`，并在 `faceCaptureAppId` 和 `faceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。
...
2. 调用 `AgoraMetaLocalUserAvatar` 类的 `ILocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#setextrainfo) 设置用户的捏脸、换装信息。
...
2. 调用 `AgoraMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
2. 调用 `AgoraMetaScene` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#leavescene) 离开场景。
3. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#onleavesceneresult) 回调得知成功离开场景后，调用 [`destroy`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#destroy) 释放 `AgoraMetaScene`。
4. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#onreleasedscene) 回调监听 `AgoraMetaScene` 是否释放成功。
