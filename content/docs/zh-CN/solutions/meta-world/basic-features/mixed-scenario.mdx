...
调用 `RtcEngine` 类的 [`create`](/api-ref/rtc/android/API/toc_initialize#api_irtcengine_initialize) 创建 `RtcEngine`。调用 `IMetaService` 类的 [`create`](/zh-CN/solutions/meta-world/api-ref/meta-api#create) 和 [`initialize`](/zh-CN/solutions/meta-world/api-ref/meta-api#initialize) 创建并初始化 `IMetaService`。

初始化 `IMetaService` 时，需要在 `MetaServiceConfig` 里设置如下重要的字段：

- `mRtcEngine`：通过 `create` 方法创建的 `RtcEngine` 实例。
- `mAppId`：在声网控制台获取的 App ID。详见[开通服务](/zh-CN/solutions/meta-world/get-started/enable-service)。
- `mUserId`：登录声网 RTM 系统的用户 ID。推荐取值详见 [`MetaServiceConfig`](/zh-CN/solutions/meta-world/api-ref/meta-api#metaserviceconfig)。
- `mRtmToken`：用于登录声网 RTM 系统的动态密钥。开启动态鉴权后可用。详见[生成 Token](/zh-CN/realtime-media/rtm/user-guide/token/token-generation)。
...
- RTM UID：用于在云信令系统中标志用户身份的用户 ID。推荐取值详见 [MetaServiceConfig 的字段解释](/zh-CN/solutions/meta-world/api-ref/meta-api#metaserviceconfig)。
- RTC Token：用于保障实时音视频通讯安全的动态密钥。详见[如何生成 RTC Token 进行鉴权](/zh-CN/realtime-media/rtc/basic-features/token-authentication)。
- RTM Token：用于保障云信令系统安全的动态密钥。详见[如何生成 RTM Token 进行鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication)。
...
调用 `IMetaService` 类的 [`getSceneAssetsInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#getsceneassetsinfo) 获取场景资源，并通过 `IMetaServiceEventHandler` 类的 [`getSceneAssetsInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#ongetsceneassetsinforesult) 回调监听获取场景资源时的事件。

调用 `IMetaService` 类的 [`downloadScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#downloadscene) 获取场景资源，并通过 `IMetaServiceEventHandler` 类的 [`onDownloadSceneAssetsProgress`](/zh-CN/solutions/meta-world/api-ref/meta-api#ondownloadsceneassetsprogress) 回调监听获取场景资源时的事件。
...
调用 `IMetaService` 类 [`createScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#createscene) 创建 `IMetaScene`，并在 `MetaSceneConfig` 中设置场景配置信息。为增加直播趣味性，声网推荐你开启面部捕捉，使用同步人脸表情的 Avatar 形象。你需要在 `MetaSceneConfig` 中设置 `mEnableFaceCapture` 为 `true`，并在 `mFaceCaptureAppId` 和 `mFaceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。

通过 `IMetaServiceEventHandler` 类的 [`onCreateSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#oncreatesceneresult) 和 [`onConnectionStateChanged`](/zh-CN/solutions/meta-world/api-ref/meta-api#onconnectionstatechanged) 回调监听创建场景和连接状态的事件。
...
1. 调用 `ILocalUserAvatar` 类的 [`setUserInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setuserinfo) 和 [`setModelInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setmodelinfo) 设置用户的基本信息和虚拟形象（Avatar）的模型信息。
2. 调用 `ILocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setextrainfo) 设置用户的捏脸、换装信息。
3. 调用 `IMetaScene` 类的 [`enterScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#enterscene) 进入场景，并通过 `config` 设置配置信息。
4. 通过 `IMetaSceneEventHandler` 类的 [`onEnterSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onentersceneresult) 回调监听进入场景的结果。
...
:::warning[注意]
- 本小节展示在**进入场景前**设置用户和模型信息，如需在**进入场景后**变更设置，需调用 [`applyInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#applyinfo) 使设置生效。
- 声网 Meta SDK 支持自定义装扮捏脸功能，详见[换装和捏脸](/zh-CN/solutions/meta-world/advanced-features/dress-face)。
:::
...
3. 调用 `IMetaScene` 类的 [`addSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api#addsceneview) 在场景中添加一个视图，用于在频道内发布主播的 Avatar 形象。
4. 通过 `IMetaSceneEventHandler` 类的 [`onAddSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onaddsceneviewresult) 回调监听添加场景显示视图的结果。
5. 调用 `IMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
1. 调用 `IMetaService` 类的 [`removeSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api#removesceneview) 将 Avatar 直播画面从场景中移除。
2. 通过 `IMetaSceneEventHandler` 类的 [`onRemoveSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onremovesceneviewresult) 回调监听移除场景显示视图的结果。
3. 调用 `RtcEngine` 类的 [`leaveChannel`](/api-ref/rtc/android/API/toc_channel#api_irtcengine_leavechannel) 离开直播频道。
4. 调用 `IMetaService` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#leavescene) 离开场景。
5. 通过 `IMetaSceneEventHandler` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`release`](/zh-CN/solutions/meta-world/api-ref/meta-api#release) 释放 `IMetaScene`。
6. 通过 `IMetaSceneEventHandler` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调监听 `IMetaScene` 是否释放成功。
...
调用 [`sharedEngineWithConfig`](/api-ref/rtc/ios/API/toc_initialize#api_irtcengine_initialize) 创建 `AgoraRtcEngineKit`。调用 [`sharedMetaServiceWithConfig`](/zh-CN/solutions/meta-world/api-ref/meta-api#sharedmetaservicewithconfig) 创建并初始化 `AgoraMetaServiceKit`。
...
- `appId`：在声网控制台获取的 App ID。详见[开通服务](/zh-CN/solutions/meta-world/get-started/enable-service)。
- `userId`：登录声网 RTM 系统的用户 ID。推荐取值详见 [AgoraMetaServiceConfig](/zh-CN/solutions/meta-world/api-ref/meta-api#agorametaserviceconfig)。
- `rtmToken`：用于登录声网 RTM 系统的动态密钥。开启动态鉴权后可用。详见[生成 Token](/zh-CN/realtime-media/rtm/user-guide/token/token-generation)。
...
- RTM UID：用于在云信令系统中标志用户身份的用户 ID。推荐取值详见 [AgoraMetaServiceConfig 的字段解释](/zh-CN/solutions/meta-world/api-ref/meta-api#agorametaserviceconfig)。
- RTC Token：用于保障实时音视频通讯安全的动态密钥。详见[如何生成 RTC Token 进行鉴权](/zh-CN/realtime-media/rtc/basic-features/token-authentication)。
- RTM Token：用于保障云信令系统安全的动态密钥。详见[如何生成 RTM Token 进行鉴权](/zh-CN/realtime-media/rtm/user-guide/token/user-authentication)。
...
调用 `AgoraMetaServiceKit` 类的 [`getSceneAssetsInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#getsceneassetsinfo) 获取场景资源，并通过 `AgoraMetaEventDelegate` 类的 [`onGetSceneAssetsInfoResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#ongetsceneassetsinforesult) 回调监听获取场景资源时的事件。

调用 `AgoraMetaServiceKit` 类的 [`downloadSceneAssets`](/zh-CN/solutions/meta-world/api-ref/meta-api#downloadsceneassets) 获取场景资源，并通过 `AgoraMetaEventDelegate` 类的 [`onDownloadSceneAssetsProgress`](/zh-CN/solutions/meta-world/api-ref/meta-api#ondownloadsceneassetsprogress) 回调监听获取场景资源时的事件。
...
调用 `AgoraMetaServiceKit` 类的 [`createScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#createscene) 创建 `AgoraMetaScene`，并在 `AgoraMetaSceneConfig` 中设置场景配置信息。为增加直播趣味性，声网推荐你开启面部捕捉，使用同步人脸表情的 Avatar 形象。你需要在 `AgoraMetaSceneConfig` 中设置 `enableFaceCapture` 为 `true`，并在 `faceCaptureAppId` 和 `faceCaptureCertificate` 中传入面部捕捉插件的 ID 和 Key。

通过 `AgoraMetaEventDelegate` 类的 [`onCreateSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#oncreatesceneresult) 和 [`onConnectionStateChanged`](/zh-CN/solutions/meta-world/api-ref/meta-api#onconnectionstatechanged) 回调监听创建场景和连接状态的事件。
...
1. 调用 `AgoraMetaLocalUserAvatar` 类的 [`setUserInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setuserinfo) 和 [`setModelInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#getmodelinfo) 设置用户的基本信息和虚拟形象（Avatar）的模型信息。
2. 调用 `AgoraMetaLocalUserAvatar` 类的 [`setExtraInfo`](/zh-CN/solutions/meta-world/api-ref/meta-api#setextrainfo) 设置用户的捏脸、换装信息。
3. 调用 `AgoraMetaScene` 类的 [`enterScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#enterscene) 进入场景，并通过 `config` 设置配置信息。
4. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onEnterSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onentersceneresult) 回调监听进入场景的结果。
...
3. 调用 `AgoraMetaScene` 类的 [`addSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api#addsceneview) 在场景中添加一个视图，用于在频道内发布主播的 Avatar 形象。
4. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onAddSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onaddsceneviewresult) 回调监听添加场景显示视图的结果。
5. 调用 `AgoraMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
1. 调用 `AgoraMetaScene` 类的 [`removeSceneView`](/zh-CN/solutions/meta-world/api-ref/meta-api#removesceneview) 将 Avatar 直播画面从场景中移除。
2. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onRemoveSceneViewResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onremovesceneviewresult) 回调监听移除场景显示视图的结果。
3. 调用 `AgoraRtcEngineKit` 类的 [`leaveChannel`](/api-ref/rtc/ios/API/toc_channel#api_irtcengine_leavechannel) 离开直播频道。
4. 调用 `AgoraMetaScene` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#leavescene) 离开场景。
5. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api#onleavesceneresult) 回调得知成功离开场景后，调用 [`destroy`](/zh-CN/solutions/meta-world/api-ref/meta-api#destroy) 释放 `AgoraMetaScene`。
6. 通过 `AgoraMetaSceneEventDelegate` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api#onreleasedscene) 回调监听 `AgoraMetaScene` 是否释放成功。
...
使用 3D 场景的过程中，不能销毁调用 [`createRenderView`](/zh-CN/solutions/meta-world/api-ref/meta-api#createrenderview) 创建的视图。该视图只有退出整个 App 时才能销毁。
