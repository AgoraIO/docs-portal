...
完成[集成声网 Meta SDK](/zh-CN/solutions/meta-world/get-started/integrate-sdk.android) 后，你可以参考本节实现元直播。
...
2. 调用 `IMetaScene` 类的 [`enableSceneVideoCapture`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#enablescenevideocapture) 并将 `enable` 设置为 `true` 开启场景渲染画面捕获，发布主播的 Avatar 形象到 RTC 频道。
...
2. 调用 `IMetaService` 类的 [`leaveScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#leavescene) 离开场景。
3. 通过 `IMetaSceneEventHandler` 类的 [`onLeaveSceneResult`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onleavesceneresult) 回调得知成功离开场景后，调用 [`release`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#release) 释放 `IMetaScene`。
4. 通过 `IMetaSceneEventHandler` 类的 [`onReleasedScene`](/zh-CN/solutions/meta-world/api-ref/meta-api.android#onleavesceneresult) 回调监听 `IMetaScene` 是否释放成功。
