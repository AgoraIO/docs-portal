...
实现换装和捏脸前，请确保你已实现基础的元语聊或元直播功能，如创建 Meta 服务、获取并下载场景资源、创建场景、设置虚拟形象的信息并进入场景。详见[基础功能](/zh-CN/solutions/meta-world/basic-features/metachat.ios)。
...
通过 [`sendSceneMessage`](/zh-CN/solutions/meta-world/api-ref/meta-api.ios#sendscenemessage) 方法向虚拟场景中发送用户虚拟角色的换装和捏脸消息。
