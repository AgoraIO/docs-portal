...
- `<#kUserKey#>` 和 `<#kUserSecret#>` 分别指客户 ID 和客户密钥。声网 RESTful API 使用客户 ID 和客户密钥进行 HTTP 基本认证。你可以参考[获取客户 ID 和客户密钥](/zh-CN/solutions/meta-world/build/setup-and-access/enable-service#获取客户-id-与客户密钥)获取。
