...
- 参考[开通元语聊服务](/zh-CN/solutions/meta-world/build/enable-service)注册并登录[声网控制台](http://console.shengwang.cn)，创建声网项目，获取项目的 App ID、App 证书
...
- `<#kUserKey#>` 和 `<#kUserSecret#>` 分别指客户 ID 和客户密钥。声网 RESTful API 使用客户 ID 和客户密钥进行 HTTP 基本认证。你可以参考[获取客户 ID 和客户密钥](/zh-CN/solutions/meta-world/build/enable-service#获取客户-id-与客户密钥)获取。
