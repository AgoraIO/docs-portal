...
| `licenseKey` | String | <Slot name="license-html-1-2-2" /> |
| `appid` | String | App ID，声网分配给每个项目的唯一标识。 |

<Slot for="license-html-1-2-2">
...
| --- | --- | --- |
| `pid` | String | 由 SKU、有效期、品类定义的 License 激活标识。 |
| `license` | String | <Slot name="license-html-2-2-2" /> |
...
| `skuView` | Array | <Slot name="license-html-2-6-2" /> |
...
| `expiringLicenses` | Int | <Slot name="license-html-3-3-2" /> |

<Slot for="license-html-3-3-2">
...
| `licenseInfos` | []object | <Slot name="license-html-4-2-2" /> |

<Slot for="license-html-4-2-2">
...
| `license` | String | <Slot name="license-html-5-3-2" /> |

<Slot for="license-html-5-3-2">
...
| --- | --- | --- |
| `count` | Int | 续期订单数量。 |
| `list` | []object | <Slot name="license-html-8-2-2" /> |

<Slot for="license-html-8-2-2">
...
声网控制台 RESTful API 仅支持 HTTPS 协议。发送请求时，你需要使用声网提供的客户 ID 和客户密钥生成一个 Base64 算法编码的凭证，并填入 HTTP 请求 Header 的 `Authorization` 字段中。详见[实现 HTTP 基本认证](/zh-CN/realtime-media/rtc/user-guides/http-basic-auth.restful)。
