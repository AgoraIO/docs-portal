...
- 如需实现推送和查看带视频和视频封面图片的告警消息，你需要先参考[云存储服务](/zh-CN/solutions/iot-apaas/advanced-features/cloud-storage.android)开通并使用云存储服务。
...
- 同时查询所有告警内容：你可以通过 `IAlarmMgr` 类中的 `queryById` 方法同时查询告警消息、告警图片和告警视频，并通过 <PlatformInline platform="android">
onAlarmInfoQueryDone 回调
</PlatformInline><PlatformInline platform="ios">
`queryById` 的返回值
