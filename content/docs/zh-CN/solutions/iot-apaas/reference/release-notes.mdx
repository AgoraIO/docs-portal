---
title: "发版说明"
description: "查看灵隼物联网云平台的版本更新、问题修复、行为变更和兼容性说明，评估升级影响。"
---
### 已知问题和局限性
...
:::warning[注意]
只有在 `agora_iot_push_video_frame` 方法中设置 `push_type` 参数为 `AGO_AV_PUSH_TYPE_MASK_OSS` 时，`ago_video_frame_t` 中的 `fps` 参数才有效。
:::
...
该版本于 2023 年 2 月  16 日发布，新增了对云存储视频文件的加密。

## 1.2.0 版

该版本于 2022 年 11 月  23 日发布，主要新增如下功能：

- 推送和查询告警消息、图片、视频。详见[告警服务](/zh-CN/solutions/iot-apaas/build/customize-and-extend/alarm)。
- 通过云存储服务录制告警视频。详见[云存储服务](/zh-CN/solutions/iot-apaas/build/customize-and-extend/cloud-storage)。

## 1.1.0 版

该版本于 2022 年 9 月 19 日发布，主要新增如下功能：

- [通过 OTA 升级设备端固件](/zh-CN/solutions/iot-apaas/build/customize-and-extend/upgrade-ota)。
- [使用 RESTful API 对接第三方用户账号](/zh-CN/solutions/iot-apaas/reference/restful#对接自研或第三方账户系统)。
