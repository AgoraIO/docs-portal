...
1. 运行秀场直播项目前，你需要在 `gradle.properties` 文件中设置如下参数：
    ...
    你可以在[创建声网项目](/zh-CN/solutions/showroom/build/enable-service#创建声网项目)后，从控制台获取这些参数的值，详情如下：

    | 参数 | 描述   | 获取方式 |
    |----|----|----|
    | `AGORA_APP_ID`    | 声网项目的 App ID     | [获取 App ID](/zh-CN/solutions/showroom/build/enable-service#获取-app-id)  |
    | `AGORA_APP_CERTIFICATE` | 声网项目的 App 证书 |[获取 App 证书](/zh-CN/solutions/showroom/build/enable-service#获取-app-证书)   |
    | `CLOUD_PLAYER_KEY` | 声网输入在线媒体流 RESTful 服务所需的客户 ID       | [获取客户 ID 和密钥](/zh-CN/solutions/showroom/build/enable-service#获取客户-id-与客户密钥)    |
    | `CLOUD_PLAYER_SECRET` | 声网输入在线媒体流 RESTful 服务所需的客户密钥   | [获取客户 ID 和密钥](/zh-CN/solutions/showroom/build/enable-service#获取客户-id-与客户密钥)   |
...
1. 运行秀场直播项目前，你需要在 `iOS/AgoraEntScenarios/KeyCenter.swift` 文件中设置如下参数：
    ...
    | 参数 | 描述   | 获取方式 |
    |----|----|----|
    | `AppId`    | 声网项目的 App ID     | [获取 App ID](/zh-CN/solutions/showroom/build/enable-service#获取-app-id)  |
    | `Certificate` | 声网项目的 App 证书 |[获取 App 证书](/zh-CN/solutions/showroom/build/enable-service#获取-app-证书)   |
    | `CloudPlayerKey` | 声网输入在线媒体流 RESTful 服务所需的客户 ID       | [生成客户 ID 和密钥](/zh-CN/solutions/showroom/build/enable-service#获取客户-id-与客户密钥)    |
    | `CloudPlayerSecret` | 声网输入在线媒体流 RESTful 服务所需的客户密钥   | [生成客户 ID 和密钥](/zh-CN/solutions/showroom/build/enable-service#获取客户-id-与客户密钥)   |
