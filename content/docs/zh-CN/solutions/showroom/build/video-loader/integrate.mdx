...
秒开和秒切可以优化直播体验，让观众能够更快速地进入直播间并切换直播间，提升观看的连贯性和流畅度，详见[什么是秒开秒切](/zh-CN/solutions/showroom/build/video-loader/overview)。本文介绍如何通过声网秒开秒切场景化 API（VideoLoader API）集成秒开秒切功能到实时音视频互动中。
...
请确保已准备好符合要求的开发环境，详见[准备开发环境](/zh-CN/solutions/showroom/build/integrate-showroom#准备开发环境)。
...
1. 参考[克隆仓库](/zh-CN/solutions/showroom/build/video-loader/run-project#克隆仓库)下载 `VideoLoaderAPI` 文件夹。将其中的如下文件夹和文件复制到与你的 `*.xcodeproj` 文件同层级的位置：
