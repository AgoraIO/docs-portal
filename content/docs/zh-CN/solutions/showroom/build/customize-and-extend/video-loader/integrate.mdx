...
调用 RTC SDK 中的 [`create`](/zh-CN/api-reference/rtc/android/initialize#create-22) 创建并初始化 `RtcEngine` 对象。
...
`OnRoomListScrollEventHandler` 对象监听到直播间列表的滑动事件后，会驱动 `OnRoomListScrollEventHandler` 类内部封装的最佳实践，并对屏幕内出现的直播间进行频道预加载（[`preloadChannel`](/zh-CN/api-reference/rtc/android/channel#api_irtcengine_preloadchannel)）。
...
在项目工程文件默认的 `ViewController` 里，调用声网 RTC SDK 中的 [`sharedEngineWithConfig`](/zh-CN/api-reference/rtc/ios/initialize#sharedEngineWithConfig) 创建并初始化 `AgoraRtcEngineKit` 对象，并初始化 VideoLoader。
...
`AGCollectionLoadingDelegateHandler` 对象监听到直播间列表的滑动事件后，会驱动 `AGCollectionLoadingDelegateHandler` 类内部封装的最佳实践，并对屏幕内出现的直播间进行频道预加载（[`preloadChannel`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengine_preloadchannel)）。
