---
title: "实现秀场直播"
description: "了解秀场直播在不同平台上的秀场直播方式，包括接入流程、关键配置和平台差异。"
---
<PlatformStructured platform="android">

## 示例项目

本文介绍如何实现秀场直播。声网在 `agora-ent-scenarios` 仓库中提供秀场直播源代码供你参考：
...
![秀场创建直播间业务流程](https://web-cdn.agora.io/docs-portal/img/showroom/create-room.png)
...
![秀场加入离开直播间业务流程](https://web-cdn.agora.io/docs-portal/img/showroom/join-leave-room.png)

### 主播 PK 连麦

下图展示主播 PK 连麦的流程。在这个流程中，房主邀请另一个房间的房主开始 PK 连麦。两个房间内的观众都可以看到两个房主 PK 连麦直播的画面。

![秀场 PK 连麦业务流程](https://web-cdn.agora.io/docs-portal/img/showroom/pk-host.png)
...
![秀场观众连麦业务流程](https://web-cdn.agora.io/docs-portal/img/showroom/co-host.png)
...
- [开通秀场直播服务](/zh-CN/solutions/showroom/build/setup-and-access/enable-service)并从声网控制台获取 App ID 和临时 Token。
...
1. [创建项目](/zh-CN/realtime-media/rtc/get-started/quick-start#创建项目)：新建一个 Android 项目。
2. [集成 SDK](/zh-CN/realtime-media/rtc/get-started/quick-start#集成-sdk)：将实时互动 RTC SDK 集成到项目中。
...
你需要调用 [`create`](/zh-CN/api-reference/rtc/android/initialize#create-22) 方法创建 RTC 引擎，并在 `config` 参数中配置上下文 Context、项目的 App ID、注册事件回调。调用 [`enableVideo`](/zh-CN/api-reference/rtc/android/video/video-basic#enablevideo) 开启视频。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and-1.svg)
...
调用 [`joinChannelEx`](/zh-CN/api-reference/rtc/android/channel#joinchannelex) 加入频道。频道用于传输直播间中的音视频流，应用业务服务用于传输直播间中的信令消息和存储数据。用户在频道内可以进行实时音视频互动。频道内的用户有两种角色：

- 主播：可以发送和接收音视频流。直播间的房主即为主播。
- 观众：只可以接收音视频流。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and-2.svg)
...
进入直播间时，你需要在主播和观众端都设置并渲染主播视频，再加入频道。本节展示调用 [`setupLocalVideo`](/zh-CN/api-reference/rtc/android/video/video-rendering#setuplocalvideo) 在主播端设置并渲染主播视频的示例代码。
...
进入直播间时，你需要在主播和观众端都设置并渲染主播视频，再加入频道。本节展示调用 [`setupRemoteVideoEx`](/zh-CN/api-reference/rtc/android/video/video-rendering#setupremotevideoex) 在观众端渲染远端视频（即主播的视频）的示例代码。
...
- 房间 A：
    - 房主 A 通过 [`joinChannelEx`](/zh-CN/api-reference/rtc/android/channel#joinchannelex) 加入频道 B，并且设置订阅频道 B 内音视频流，但不发送音视频流。同时通过 [`setupRemoteVideoEx`](/zh-CN/api-reference/rtc/android/video/video-rendering#setupremotevideoex) 渲染频道 B 中主播的视频。
...
结束 PK 连麦时，房间内用户都需要调用 [`leaveChannelEx`](/zh-CN/api-reference/rtc/android/channel#leavechannelex-12) 离开对方频道。

本节展示用户加入对方频道和离开对方频道的示例代码，如需查阅 `setupRemoteVideoEx` 方法调用逻辑，请参考 [GitHub 示例项目](#示例项目)。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and-3.svg)
...
本节展示观众连麦和结束连麦时更新频道媒体选项的示例代码。通过 [`updateChannelMediaOptionsEx`](/zh-CN/api-reference/rtc/android/channel#updatechannelmediaoptionsex) 方法在观众加入频道后更新频道媒体选项，例如是否开启本地音频采集，是否发布本地音频流等。观众的用户角色为 `CLIENT_ROLE_AUDIENCE`，因此无法在频道内发布音频流。如果观众想与主播连麦，需要将用户角色修改为 `CLIENT_ROLE_BROADCASTER`。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and-4.svg)
...
本节展示调用 [`destroy`](/zh-CN/api-reference/rtc/android/initialize#destroy) 销毁 RTC 引擎的示例代码。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and-5.svg)
...
![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-and.svg)

## 开发注意事项

如果你需要在秀场直播中实现美颜功能，可以参考[集成第三方美颜](/zh-CN/solutions/showroom/build/customize-and-extend/beauty/overview)。
...
1. [创建项目](/zh-CN/realtime-media/rtc/get-started/quick-start#创建项目)：新建一个 iOS 项目。
2. [集成 SDK](/zh-CN/realtime-media/rtc/get-started/quick-start#集成-sdk)：将实时互动 RTC SDK 集成到项目中。
...
- **声网 RTC SDK**：承担实时音视频的业务。本节会详细介绍如何调用 RTC SDK 的 API 完成这些音视频逻辑。
- **应用业务服务器**：承担信令消息和数据存储的业务。你需要自行实现相关功能，具体实现时可参考 [API 时序图](#api-时序图)和[示例项目](https://github.com/AgoraIO-Usecase/agora-ent-scenarios/tree/main/iOS/AgoraEntScenarios/Scenes/Show)。

### 1. 创建直播间

创建直播间时，你需要初始化 RTC 引擎、为主播设置本地视图并进行预览。本节展示调用 [`sharedEngineWithConfig`](/zh-CN/api-reference/rtc/ios/initialize#sharedEngineWithConfig) 初始化 RTC 引擎的示例代码。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios-1.svg)
...
调用 [`joinChannelExByToken`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengineex_joinchannelex) 加入频道。频道用于传输直播间中的音视频流，应用业务服务用于传输直播间中的信令消息和存储数据。用户在频道内可以进行实时音视频互动。频道内的用户有两种角色：

- 主播：可以发送和接收音视频流。直播间的房主即为主播。
- 观众：只可以接收音视频流。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios-2.svg)
...
进入直播间时，你需要在主播和观众端都设置并渲染主播视频，再加入频道。本节展示调用 [`setupLocalVideo`](/zh-CN/api-reference/rtc/ios/video/video-rendering#setuplocalvideo) 在主播端设置并渲染主播视频的示例代码。
...
进入直播间时，你需要在主播和观众端都设置并渲染主播视频，再加入频道。本节展示调用 [`setupRemoteVideoEx`](/zh-CN/api-reference/rtc/ios/video/video-rendering#api_irtcengineex_setupremotevideoex) 在观众端渲染远端视频（即主播的视频）的示例代码。
...
- 房间 A：
    - 房主 A 通过 [`joinChannelExByToken`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengineex_joinchannelex) 加入频道 B，并且设置订阅频道 B 内音视频流，但不发送音视频流。同时通过 [`setupRemoteVideoEx`](/zh-CN/api-reference/rtc/ios/video/video-rendering#api_irtcengineex_setupremotevideoex) 渲染频道 B 中主播的视频。
    - 观众通过 `joinChannelEx` 加入频道 B，并且设置订阅频道 B 内音视频流，但不发送音视频流。同时通过 `setupRemoteVideoEx` 渲染频道 B 中主播的视频。
- 房间 B：
    - 房主 B 通过 `joinChannelEx` 加入频道 A，并且设置订阅频道 A 内音视频流，但不发送音视频流。同时通过 `setupRemoteVideoEx` 渲染频道 A 中主播的视频。
    - 观众通过 `joinChannelEx` 加入频道 A，并且设置订阅频道 A 内音视频流，但不发送音视频流。同时通过 `setupRemoteVideoEx` 渲染频道 A 中主播的视频。
...
结束 PK 连麦时，房间内用户都需要调用 [`leaveChannelEx`](/zh-CN/api-reference/rtc/ios/channel#api_irtcengineex_leavechannelex) 离开对方频道。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios-3.svg)
...
本节展示观众连麦和结束连麦时更新频道媒体选项、设置视图的示例代码。通过 [`updateChannelExWithMediaOptions`](/zh-CN/api-reference/rtc/ios/channel#updateChannelExWithMediaOptions) 方法在观众加入频道后更新频道媒体选项，例如是否开启本地音频采集，是否发布本地音频流等。观众的用户角色为 `audience`，因此无法在频道内发布音频流。如果观众想与主播连麦，需要将用户角色修改为 `broadcaster`。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios-4.svg)
...
本节展示调用 [`destroy`](/zh-CN/api-reference/rtc/ios/initialize#destroy) 销毁 RTC 引擎的示例代码。

![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios-5.svg)
...
![](https://web-cdn.agora.io/docs-portal/img/showroom/integ-ios.svg)
