...
请确保已准备好符合要求的开发环境，详见[准备开发环境](/zh-CN/solutions/showroom/basic-features/integrate-showroom#准备开发环境)。
...
为加快用户加入频道的速度，你可以使用通配 Token，详见[使用通配 Token 最佳实践](/zh-CN/realtime-media/rtc/best-practice/wildcard-token)。
...
1. 参考[克隆仓库](/zh-CN/solutions/showroom/advanced-features/video-loader/run-project#克隆仓库)下载 `VideoLoaderAPI` 文件夹。将其中的如下文件夹和文件复制到与你的 `*.xcodeproj` 文件同层级的位置：
