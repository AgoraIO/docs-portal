...
- `token`：用于鉴权的动态密钥。如果在[创建声网项目](/zh-CN/solutions/showroom/get-started/enable-service#创建声网项目)时启用**调试模式**，那么将 `token` 参数传空。如果启用**安全模式**，那么你先参考[使用 Token 鉴权](/zh-CN/realtime-media/rtc/basic-features/token-authentication)在你的业务服务端生成 Token，然后将生成的 Token 传入该参数。
- `channelId`：频道名。
- `options`：频道媒体设置选项。
