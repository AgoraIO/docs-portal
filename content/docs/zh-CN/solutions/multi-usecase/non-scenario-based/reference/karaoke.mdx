---
title: "非场景化 API 方案"
description: "查看声网解决方案的非场景化 API 方案，了解关键概念、参数说明、使用限制和接入注意事项。"
---
<PlatformStructured platform="android">

本文介绍如何通过声网实时互动 SDK v4.x 在你的 Android 项目里实现在线 K 歌房的主要功能。

:::info[信息]
如果你想简化集成步骤，请参考[场景化 API 解决方案](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/get-music)。
:::
...
3. 用户可以点歌，从[声网内容中心](/zh-CN/api-reference/online-ktv)获取歌单，并下载歌曲文件和歌词文件，成为主唱。
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
7. 主唱通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。

### 合唱者实现流程

下图展示在线 K 歌房场景的合唱者的实现流程：
...
5. 合唱者通过 `onStreamMessage` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
...
根据下表提供的链接，下载对应平台的 SDK 并集成到你的项目中，并[开通内容中心服务](/zh-CN/api-reference/online-ktv)。
...
| 第三方云服务 | [第三方云服务 SDK](https://leancloud.cn/docs/sdk_down.html) | <Slot name="karaoke-android-markdown-1-2-2" /> |
| 声网歌词打分组件 | [源码](https://github.com/AgoraIO-Community/LyricsView/tree/main) | [歌词组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring) |

<Slot for="karaoke-android-markdown-1-2-2">
...
| API | 实现功能 |
| --- | --- |
| `setClientRole` | <Slot name="karaoke-android-markdown-2-1-1" /> |

<Slot for="karaoke-android-markdown-2-1-1">

设置用户角色。
加入歌房时，需要将主唱的用户角色设为 `BROADCASTER`、听众的用户角色设为 `AUDIENCE`。听众成功上麦后，需要先调用该方法将用户角色切换为 `BROADCASTER`，才能在房间里发布音频流。
...
4. 获取音乐榜单列表或搜索音乐资源。
   ...
   你还可以通过 [searchMusic[2/2]](/zh-CN/api-reference/rtc/android/play/drm#searchmusic-22) 和 [getMusicCollectionByMusicChartId[2/2]](/zh-CN/api-reference/rtc/android/play/drm#getmusiccollectionbymusicchartid-22) 中的 `jsonOption` 字段来筛选有副歌片段、支持打分的音乐资源。`jsonOption` 字段的具体说明见对应方法的 API 文档。
...
5. 预加载音乐资源。
   ...
   如果你需要预加载的是某一音乐资源独立的副歌片段，在预加载之前你还需要调用 [getInternalSongCode](/zh-CN/api-reference/rtc/android/play/drm#getinternalsongcode) 来获取该片段的资源编号（`internalSongCode`）。
...
7. 销毁播放器及引擎。
   ...
   1. 调用 `IAgoraMusicPlayer` 类下的 `destroy` 销毁音乐播放器；
   2. 调用 `IAgoraMusicContentCenter` 类下的 `destroy` 销毁音乐内容中心实例；
   3. 调用 `RtcEngine` 类下的 `destroy` 销毁引擎。
...
你可以参考[版权音乐 API 文档](/zh-CN/api-reference/rtc/android/play/drm)了解更多详细信息，音乐播放器的参数调节、推流设置、事件回调等请参考[媒体播放器 API 文档](/zh-CN/api-reference/rtc/android/play/mediaplayer/mediaplayer-initialize)。
...
| 音乐资源类型 | 切换原唱\伴奏方式 |
| --- | --- |
| 左声道伴奏、右声道原唱的单音轨歌曲 | <Slot name="karaoke-android-markdown-3-1-1" /> |
| 多音轨的歌曲 | <Slot name="karaoke-android-markdown-3-2-1" /> |

<Slot for="karaoke-android-markdown-3-1-1">

切换至原唱：调用 `setAudioDualMonoMode`，将 `mode` 设为 `AUDIO_DUAL_MONO_R`(2)。
切换至伴奏：调用 `setAudioDualMonoMode`，将 `mode` 设为 `AUDIO_DUAL_MONO_L`(1)。
:::info[信息]
...
<Slot for="karaoke-android-markdown-3-2-1">
...
调用 `setAudioEffectPreset` 方法，在不改变原声的性别特征的前提下，设置人声音效。设置音效后，频道内所有用户都能听到该效果。详见[设置人声效果](/zh-CN/realtime-media/rtc/build/audio/voice-changer)。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `onAudioVolumeIndication` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。

### 相关文档

- [声网内容中心](/zh-CN/api-reference/online-ktv)
- [歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)
- [实时互动 API 参考](/zh-CN/api-reference/rtc/android/rtc-api-overview)
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
7. 主唱通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
5. 合唱者通过 `receiveStreamMessageFromUid` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
6. 合唱者通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
| 声网歌词组件 | [源码](https://github.com/AgoraIO-Community/LrcView-iOS/tree/feature/1.1.1) | [歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring) |
...
| API | 实现功能 |
| --- | --- |
| `setClientRole` | <Slot name="karaoke-ios-markdown-2-1-1" /> |

<Slot for="karaoke-ios-markdown-2-1-1">
...
```
   ...
   你还可以通过 [searchMusicWithKeyWord](/zh-CN/api-reference/rtc/ios/play/drm) 和 [getMusicCollectionWithMusicChartId](/zh-CN/api-reference/rtc/ios/play/drm) 中的 `jsonOption` 字段来筛选有副歌片段、支持打分的音乐资源。`jsonOption` 字段的具体说明见对应方法的 API 文档。
...
| 音乐资源类型 | 切换原唱\伴奏方式 |
| --- | --- |
| 左声道伴奏、右声道原唱的单音轨歌曲 | <Slot name="karaoke-ios-markdown-3-1-1" /> |
| 多音轨的歌曲（第一个音轨为原唱，第二个音轨为伴奏） | <Slot name="karaoke-ios-markdown-3-2-1" /> |

<Slot for="karaoke-ios-markdown-3-1-1">
...
<Slot for="karaoke-ios-markdown-3-2-1">
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `reportAudioVolumeIndicationOfSpeakers` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。

### 相关文档

- [声网内容中心](/zh-CN/api-reference/online-ktv)
- [歌词打分组件](/zh-CN/solutions/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)
- [实时互动 API 参考](/zh-CN/api-reference/rtc/ios/rtc-api-overview)
