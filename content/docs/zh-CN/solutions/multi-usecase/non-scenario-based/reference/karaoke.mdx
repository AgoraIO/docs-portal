...
:::info[信息]
如果你想简化集成步骤，请参考[场景化 API 解决方案](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/get-music)。
:::
...
3. 用户可以点歌，从[声网内容中心](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/music-content-center)获取歌单，并下载歌曲文件和歌词文件，成为主唱。
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
...
5. 合唱者通过 `onStreamMessage` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
...
根据下表提供的链接，下载对应平台的 SDK 并集成到你的项目中，并[开通内容中心服务](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/music-content-center#开通服务)。
...
| 声网歌词打分组件 | [源码](https://github.com/AgoraIO-Community/LyricsView/tree/main) | [歌词组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring) |
...
4. 获取音乐榜单列表或搜索音乐资源。
   ...
   你还可以通过 [searchMusic[2/2]](/api-ref/rtc/android/API/toc_drm#searchMusic%20[2/2]) 和 [getMusicCollectionByMusicChartId[2/2]](/api-ref/rtc/android/API/toc_drm#getMusicCollectionByMusicChartId%20[2/2]) 中的 `jsonOption` 字段来筛选有副歌片段、支持打分的音乐资源。`jsonOption` 字段的具体说明见对应方法的 API 文档。
...
5. 预加载音乐资源。
   ...
   如果你需要预加载的是某一音乐资源独立的副歌片段，在预加载之前你还需要调用 [getInternalSongCode](/api-ref/rtc/android/API/toc_drm#getInternalSongCode) 来获取该片段的资源编号（`internalSongCode`）。
...
你可以参考[版权音乐 API 文档](/api-ref/rtc/android/API/toc_drm)了解更多详细信息，音乐播放器的参数调节、推流设置、事件回调等请参考[媒体播放器 API 文档](/api-ref/rtc/android/API/toc_mediaplayer_initialize)。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `onAudioVolumeIndication` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。

### 相关文档

- [声网内容中心](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/music-content-center)
- [歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)
- [实时互动 API 参考](/api-ref/rtc/android/API/rtc_api_overview)
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
7. 主唱通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
5. 合唱者通过 `receiveStreamMessageFromUid` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。
6. 合唱者通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
| 声网歌词组件 | [源码](https://github.com/AgoraIO-Community/LrcView-iOS/tree/feature/1.1.1) | [歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring) |
...
```
   ...
   你还可以通过 [searchMusicWithKeyWord](/api-ref/rtc/ios/API/toc_drm#searchMusicWithKeyWord) 和 [getMusicCollectionWithMusicChartId](/api-ref/rtc/ios/API/toc_drm#getMusicCollectionWithMusicChartId) 中的 `jsonOption` 字段来筛选有副歌片段、支持打分的音乐资源。`jsonOption` 字段的具体说明见对应方法的 API 文档。
...
```
   ...
   如果你需要预加载的是某一音乐资源独立的副歌片段，在预加载之前你还需要调用 [getInternalSongCode](/api-ref/rtc/ios/API/toc_drm#getInternalSongCode) 来获取该片段的资源编号（`internalSongCode`）。
...
你可以参考[版权音乐 API 文档](/api-ref/rtc/ios/API/toc_drm)了解更多详细信息，音乐播放器的参数调节、推流设置、事件回调等请参考[内置媒体播放器 API 文档](/api-ref/rtc/ios/API/toc_mediaplayer_initialize)。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `reportAudioVolumeIndicationOfSpeakers` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)。

### 相关文档

- [声网内容中心](/zh-CN/realtime-media/online-ktv/ktv-scenario/reference/music-content-center)
- [歌词打分组件](/zh-CN/realtime-media/online-ktv/ktv-scenario/build/extend-karaoke/lyrics-scoring)
- [实时互动 API 参考](/api-ref/rtc/ios/API/rtc_api_overview)
