...
:::info[信息]
如果你想简化集成步骤，请参考[场景化 API 解决方案](./get-music)。
:::
...
3. 用户可以点歌，从[声网内容中心](../music-content-center)获取歌单，并下载歌曲文件和歌词文件，成为主唱。
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](../lyrics-scoring)。
...
5. 合唱者通过 `onStreamMessage` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词组件](../lyrics-scoring)。
...
根据下表提供的链接，下载对应平台的 SDK 并集成到你的项目中，并[开通内容中心服务](../music-content-center#开通服务)。

| 产品 | SDK 下载 | 集成文档 |
| --- | --- | --- |
| 声网实时互动 SDK v4.x | [声网实时互动 SDK v4.x ](/zh-CN/realtime-media/rtc/resources) | [实现纯语音互动](/zh-CN/realtime-media/rtc/basic-features/audio-quick-start) |
| 第三方云服务 | [第三方云服务 SDK](https://leancloud.cn/docs/sdk_down.html) | <Slot name="karaoke-android-markdown-1-2-2" /> |
| 声网歌词打分组件 | [源码](https://github.com/AgoraIO-Community/LyricsView/tree/main) | [歌词组件](../lyrics-scoring) |
...
调用 `setAudioEffectPreset` 方法，在不改变原声的性别特征的前提下，设置人声音效。设置音效后，频道内所有用户都能听到该效果。详见[设置人声效果](/zh-CN/realtime-media/rtc/advanced-features/voice-changer)。
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `onAudioVolumeIndication` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](../lyrics-scoring)。

### 相关文档

- [声网内容中心](../music-content-center)
...
6. 主唱在唱歌的同时，调用 `sendStreamMessage` 方法获将当前歌曲播放进度同步到房间内所有用户。合唱者和听众可以结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](../lyrics-scoring)。
7. 主唱通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
5. 合唱者通过 `receiveStreamMessageFromUid` 回调接收主唱发送的数据流，实现歌曲同步。同时结合歌词组件实现歌词同步显示，并自定义歌词界面，详见[歌词打分组件](../lyrics-scoring)。
6. 合唱者通过第三方云服务发起下麦指令，并调用 `leaveChannel` 方法离开歌房。
...
| 声网歌词组件 | [源码](https://github.com/AgoraIO-Community/LrcView-iOS/tree/feature/1.1.1) | [歌词打分组件](../lyrics-scoring) |
...
调用 `enableAudioVolumeIndication` 方法开启歌唱评分功能，并通过 `reportAudioVolumeIndicationOfSpeakers` 回调，结合声网歌词组件实现歌唱评分效果，详见[歌词打分组件](../lyrics-scoring)。
