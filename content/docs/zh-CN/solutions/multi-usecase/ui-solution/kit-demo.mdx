...
参考[开通服务](/zh-CN/solutions/multi-usecase/ui-solution/enable-service)获取你的声网项目的 App ID、证书，以及环信项目的 APPKEY、Client ID 以及 Client Secret，参考[部署后端服务](./run-github-project-backend)获取你的项目的后端服务的 Host URL，然后将上述信息填入项目的 `local.peoperties` 文件。
...
2. 参考[开通服务](/zh-CN/solutions/multi-usecase/ui-solution/enable-service)获取你的声网项目的 App ID、证书，以及环信项目的 APPKEY、Client ID 以及 Client Secret，参考[部署后端服务](./run-github-project-backend)获取你的项目的后端服务的 Host URL，然后将上述信息填入项目的 `KeyCenter.swift` 文件并保存。
