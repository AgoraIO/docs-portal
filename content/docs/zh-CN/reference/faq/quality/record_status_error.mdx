...
大多数情况为录制超时退出，详见[`idleLimitSec`](/zh-CN/api-reference/local-server-recording/cpp/legacy/structagora-1-1recording-1-1-recording-config#aca9710dfdb0596c88f26e3c1c3daf48b)。检查 `recording_sys.log`，是否有 "No users in channel" 的关键字即可确认。
