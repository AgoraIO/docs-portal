...
- Occurs when ...，表示该回调的触发时机。大部分回调的简述都可以使用这种句式。例如：
  - 中文：成功加入频道回调。
  - 英文：Occurs when a user joins a channel.

- Reports ...，表示该回调用来报告哪些数据。大部分上报质量数据的回调可以使用这种句式。例如：
  - 中文：通话中远端视频流的统计信息回调。
  - 英文：Reports the statistics of the video stream sent by each remote users.
