...
- [单流录制和合流录制（本地服务端录制）](/zh-CN/realtime-media/local-server-recording/build/configure-recording-output/recording-mode)
...
如果你在创建项目时选择了**调试模式**，后续也可通过启用 App 证书来升级为 Token 鉴权。详见[如何升级到 Token 鉴权方案？](/zh-CN/reference/faq/integration/appid_to_token)。
...
:::info[相关链接]
- [控制台概览](/zh-CN/introduction/quickstart)
...
一个频道同一时间连麦主播的数量限制，详见[声网 SDK 最多支持多少人同时在线](/zh-CN/reference/faq/product/capacity)。如果连麦主播数量过多时，可能会出现音画不同步和信息丢失的问题。
