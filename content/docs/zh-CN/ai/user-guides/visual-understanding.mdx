---
title: "视觉理解能力"
description: "了解如何在对话式 AI 引擎中视觉理解能力，包括适用场景、配置步骤、实现要点和注意事项。"
---
声网对话式 AI 引擎支持视觉理解能力。

开启后，智能体可以在推理时结合用户当前画面或用户上传的图片理解问题，并基于图像内容生成回复。
...
- 如果需要让用户上传图片，已阅读[发送图片消息](/zh-CN/ai/build/send-multimodal-messages)并完成 RTM 侧的收发消息集成。
...
调用 [POST 创建对话式智能体](/zh-CN/api-reference/api-ref/conversational-ai/join) 接口时，在 `properties.llm.input_modalities` 中包含 `"image"` 即可开启视觉能力。
...
1. 调用 [POST 创建对话式智能体](/zh-CN/api-reference/api-ref/conversational-ai/join)，将 `llm.input_modalities` 配置为 `["text", "image"]`。
2. 客户端加入 RTC 频道并发布视频流。
3. 用户发起语音提问，或业务侧调用 [POST 发送自定义指令](/zh-CN/api-reference/api-ref/conversational-ai/think)。
...
2. 客户端通过 RTM 发送图片消息上传图片，详见[发送图片消息](/zh-CN/ai/build/send-multimodal-messages)。
3. 用户发起语音提问，或业务侧调用 [POST 发送自定义指令](/zh-CN/api-reference/api-ref/conversational-ai/think)。
...
| :--- | :--- | :--- |
| 用户语音说话 | 是 | 是 |
| [POST 发送自定义指令](/zh-CN/api-reference/api-ref/conversational-ai/think) | 是 | 是 |
| 静默超时后触发模型思考 | 是 | 是 |
| 智能体问候语 | 否 | 否 |
| [POST 播报自定义消息](/zh-CN/api-reference/api-ref/conversational-ai/speak) | 否 | 否 |
...
| 上传图片后智能体仍未使用图片回答 | 图片未发送成功；图片消息格式不符合要求；图片过大 | 对照[发送图片消息](/zh-CN/ai/build/send-multimodal-messages)检查消息发送流程；确认图片可正常访问且大小不超过 5 MB |
| 智能体问候语没有结合画面内容 | 问候语不会触发大模型推理 | 如需首次开场就结合画面，可在问候语后由用户提问，或调用 [POST 发送自定义指令](/zh-CN/api-reference/api-ref/conversational-ai/think) 触发一次推理 |

### 相关文档

- [发送图片消息](/zh-CN/ai/build/send-multimodal-messages)
- [自定义大模型](/zh-CN/ai/build/custom-model-integration/custom-llm)
- [使用音频模态输出](/zh-CN/ai/build/custom-model-integration/audio-output)
