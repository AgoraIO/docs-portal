...
- 完成 RTM SDK 的基础集成，并具备基础的 RTM 登录、订阅和消息监听能力。可参考[实现收发消息](/zh-CN/realtime-media/rtm/get-started/quick-start.javascript)。
...
调用 [POST 创建对话式智能体](/doc/convoai/restful/convoai/operations/start-agent) 时，先开启 RTM，再根据你的业务场景决定是手动控制 SoS、手动控制 EoS，还是两者都手动控制。
...
- [手动控制说话开始和结束](/zh-CN/ai/user-guides/manual-turn-control)
