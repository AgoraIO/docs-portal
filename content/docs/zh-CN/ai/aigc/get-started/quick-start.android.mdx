...
1. 采集本地用户发送的音频数据。如果你选用的是声网 RTC SDK，可以参考[音频原始数据](/zh-CN/realtime-media/rtc/advanced-features/raw-audio-data.android)，从 `onRecordAudioFrame` 回调中获取。
...
:::info[信息]
如果你选用的是声网 RTC SDK，可以参考[音频原始数据](/zh-CN/realtime-media/rtc/advanced-features/raw-audio-data.android)，使用 `onPlaybackAudioFrame` 播放。
:::
...
<Cards>
  <Card title="原始音频数据" href="/zh-CN/realtime-media/rtc/advanced-features/raw-audio-data.android" />
</Cards>
