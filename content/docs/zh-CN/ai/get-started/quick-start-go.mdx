...
- LLM 参数配置：
  ...
  - `llmModel`：大语言模型名。
- TTS 参数配置：示例代码中以火山 TTS 为例，你可以根据实际情况选择其他 TTS 供应商，并参考 [`JoinPropertiesTTSBody`](/api-ref/convoai/go/go-api/struct#joinpropertiesttsbody) 了解具体配置方式。

### 创建并初始化 Client

在 `main` 函数中初始化声网配置，调用 [`NewClient`](/api-ref/convoai/go/go-api/client#newclient) 创建对话式 AI 引擎 REST Client，并设置日志输出格式：
...
使用 [`Join`](/api-ref/convoai/go/go-api/client#join) 方法创建对话式智能体，并传入 LLM 和 TTS 的配置参数（[`JoinPropertiesCustomLLMBody`](/api-ref/convoai/go/go-api/struct#joinpropertiescustomllmbody) 和 [`JoinPropertiesTTSBody`](/api-ref/convoai/go/go-api/struct#joinpropertiesttsbody)）：
...
使用 [`List`](/api-ref/convoai/go/go-api/client#list) 方法获取当前运行的对话式智能体列表：
