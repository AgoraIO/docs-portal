...
<Accordions>
<Accordion title="main.go 完整示例代码">
...
- LLM 参数配置：
  ...
  - `llmModel`：大语言模型名。
- TTS 参数配置：示例代码中以火山 TTS 为例，你可以根据实际情况选择其他 TTS 供应商，并参考 [`JoinPropertiesTTSBody`](/zh-CN/api-reference/conversational-ai/restclient-go/struct#joinpropertiesttsbody) 了解具体配置方式。

### 创建并初始化 Client

在 `main` 函数中初始化声网配置，调用 [`NewClient`](/zh-CN/api-reference/conversational-ai/restclient-go/client.go#newclient) 创建对话式 AI 引擎 REST Client，并设置日志输出格式：
...
使用 [`Join`](/zh-CN/api-reference/conversational-ai/restclient-go/client.go#join) 方法创建对话式智能体，并传入 LLM 和 TTS 的配置参数（[`JoinPropertiesCustomLLMBody`](/zh-CN/api-reference/conversational-ai/restclient-go/struct#joinpropertiescustomllmbody) 和 [`JoinPropertiesTTSBody`](/zh-CN/api-reference/conversational-ai/restclient-go/struct#joinpropertiesttsbody)）：
...
使用 [`List`](/zh-CN/api-reference/conversational-ai/restclient-go/client.go#list) 方法获取当前运行的对话式智能体列表：
...
<Cards>
  <Card title="agora-rest-client-go" href="https://gitee.com/agoraio-community/agora-rest-client-go/tree/main/examples/convoai" />
  <Card title="agora-rest-client-go" href="https://github.com/AgoraIO-Community/agora-rest-client-go/tree/main/examples/convoai" />
</Cards>
