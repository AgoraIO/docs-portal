...
以下是一次实际对话中三个对话轮次的延迟数据，来自声网消息通知服务的 [111 agent metrics](/zh-CN/ai/webhook/ncs-events#111-agent-metrics) 事件：
...
详细的集成步骤和 API 参考请见[监听智能体事件](/zh-CN/ai/user-guides/listen-agent-events)。

### 方式二：通过消息通知服务（NCS）

如果你开通了[声网消息通知服务](/zh-CN/ai/webhook/enable-ncs)，可以通过接收 `eventType` 为 `111` 的 `agent metrics` 事件来获取智能体性能指标。
...
详细的事件字段说明请见[事件类型 - 111 agent metrics](/zh-CN/ai/webhook/ncs-events#111-agent-metrics)。
...
详细的优化音频设置请见[优化音频设置](/zh-CN/ai/best-practice/audio-settings)。
...
- [优化音频设置](/zh-CN/ai/best-practice/audio-settings)
- [消息通知服务 - agent metrics 事件](/zh-CN/ai/webhook/ncs-events#111-agent-metrics)
