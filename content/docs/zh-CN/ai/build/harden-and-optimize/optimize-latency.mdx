---
title: "优化端到端对话延迟"
description: "了解如何在对话式 AI 引擎中优化端到端对话延迟，包括适用场景、配置步骤、实现要点和注意事项。"
---
在与智能体对话的场景中，延迟是影响用户体验的关键因素。

本文帮助你理解和优化与对话式智能体的端到端对话延迟，从而提升用户体验。
...
| **LLM** | `llm_ttfb` / `llm_ttfs` | <Slot name="opt-latency-markdown-0-4-2" /> | 250-1000 ms |
...
<Slot for="opt-latency-markdown-0-4-2">
...
以下是一次实际对话中三个对话轮次的延迟数据，来自声网消息通知服务的 [111 agent metrics](/zh-CN/ai/reference/event-types#111-agent-metrics) 事件：
...
详细的集成步骤和 API 参考请见[监听智能体事件](/zh-CN/ai/build/handle-runtime-events/event-notifications)。

### 方式二：通过消息通知服务（NCS）

如果你开通了[声网消息通知服务](/zh-CN/ai/build/handle-runtime-events/webhooks)，可以通过接收 `eventType` 为 `111` 的 `agent metrics` 事件来获取智能体性能指标。
...
详细的事件字段说明请见[事件类型 - 111 agent metrics](/zh-CN/ai/reference/event-types#111-agent-metrics)。
...
RTC 延迟包括音频采集、编码、网络传输、解码、播放等环节。优化 RTC 延迟需要在客户端配置适合 AI 对话的音频场景和音频参数。
...
你可以通过以下任一方式配置 AI 对话场景：

<Tabs defaultValue="Android" groupId="rtc-latency-platform" persist>
<TabsList>
...
详细的音频参数、插件集成和版本兼容说明请见[优化音频设置](/zh-CN/ai/build/harden-and-optimize/audio-setup)。
...
- [优化音频设置](/zh-CN/ai/build/harden-and-optimize/audio-setup)
- [消息通知服务 - agent metrics 事件](/zh-CN/ai/reference/event-types#111-agent-metrics)
