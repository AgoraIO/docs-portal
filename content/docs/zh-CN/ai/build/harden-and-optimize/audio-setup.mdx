---
title: "优化音频设置"
description: "了解对话式 AI 引擎中优化音频设置相关内容的适用场景、配置步骤、实现要点和注意事项。"
---
...
2. **设置音频相关参数**：在加入频道前和音频路由发生变化（触发 [`rtcEngine:didAudioRouteChanged:`](https://doc.shengwang.cn/zh-CN/api-reference/rtc/ios/device-management/audio-route#rtcEngine:didAudioRouteChanged:) 回调）时，调用 [`setParameters`](/zh-CN/api-reference/rtc/ios/network#api_irtcengine_setparameters) 方法设置音频相关参数。主要包括音频 3A（回声消除（AEC）、降噪（ANS）和增益控制（AGC））插件的开关，以及音频采样率、音频处理模式等设置。各参数推荐设定的值参考示例代码。
...
- [`rtcEngine:didAudioRouteChanged:`](https://doc.shengwang.cn/zh-CN/api-reference/rtc/ios/device-management/audio-route#rtcEngine:didAudioRouteChanged:)
