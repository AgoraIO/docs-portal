...
If your account balance is negative after the deduction, Agora sends you an email, reminding you to [top up your account](./pricing.mdx#top-up-packages) at your earliest convenience and avoid account suspension.
...
2. Purchase a [pre-paid monthly package](../subscription-packages.mdx) or [top-up package](../subscription-packages.mdx#top-up-package). This option is highly recommended.

## Free-of-charge policy

:::info
Agora offers a new [pricing](./pricing.mdx#subscription-packages) model. If your account uses this model, you receive a monthly free usage package by default instead of the 10,000 free minutes described below.
:::
...
Agora calculates service minutes **by the number of users**. Service minutes are also calculated based on aggregate video resolution. For details, see [Aggregate resolution](./pricing.mdx#aggregate-resolution).
