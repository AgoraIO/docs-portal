...
![Spatial audio comparison](https://assets-docs.agora.io/images/video-sdk/spatial-audio-comparison.png)
...
Use an instance of `ILocalSpatialAudioEngine` to implement 3D Spatial Audio. Call `updateSelfPosition` and `updateRemotePosition` to specify the spatial coordinates of local and remote users in the channel. RTC SDK calculates the relative positions of local and remote users. This enables local users to experience the 3D Spatial Audio of remote users.

![SpatialAudio](https://assets-docs.agora.io/images/video-sdk/spatial-audio-effect-user.svg)
...
![SpatialAudio](https://assets-docs.agora.io/images/video-sdk/spatial-audio-effect-player.svg)
...
![Spatial audio steps](https://assets-docs.agora.io/images/video-sdk/spatial-audio-steps.svg)
...
<div style={{ width: '33%' , height : '33%' }}>
 ![Social Chat](https://assets-docs.agora.io/images/video-sdk/voice-chatroom-social-spatial.png)
</div>
...
- **Audio Blurring**: Enable audio blurring for specific users or media.
...
- **Range Audio**: Set audio reception range based on the scene. The farther the sound travels, the fainter it becomes. Adjust the attenuation factor for different effects:
  - **Jungle scene:** Set to 0.9 for fast sound decay.
  - **Plain scene:** Set to 0.1 for slower sound decay over a longer distance.

- **Ultra-Realistic Space**: Assign virtual characters 3D coordinates for a realistic spatial experience, including:
  - Avatar coordinates (x, y, z). The coordinates of the avatar in 3D space.
  - Virtual character face orientation coordinates (x, y, z): The face orientation coordinates of the virtual character in the scene.
  - Virtual character head top orientation coordinates (x, y, z): Coordinates of the virtual character's head top orientation, which can be combined with the face orientation coordinates to determine the virtual character's actions and postures. For example, when the virtual character lies flat, the face is facing the sky and the top of the head is facing forward.
  - Coordinate system (left or right-handed). Whether the rectangular coordinate system uses a left-handed coordinate system or a right-handed coordinate system. Only one coordinate system can be used in a scene.

- **Sound Insulation Area**: Create sound-insulated zones with customized sound attenuation. When a receiver outside the area listens to the sound source in the area, they experience the attenuation effect of the sound in the real environment on encountering the building partition.
...
![Games & Metaverse](https://assets-docs.agora.io/images/video-sdk/gmes-scenario-spatial.png)
...
![Online Meetings](https://assets-docs.agora.io/images/video-sdk/online-spatial-scenario.png)
