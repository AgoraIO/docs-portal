...
![Virtual human girl effect](https://web-cdn.agora.io/doc-cms/uploads/1706688158041-avatar_girl_6s.gif)

**Animoji (dog)**

![Animoji dog effect](https://web-cdn.agora.io/doc-cms/uploads/1706773052008-dog_480x1038_6s.gif)
...
- Integrated the v4.2.x or v4.3.x of the RTC SDK and implemented basic real-time audio and video functions in your app. See [SDK quickstart](/en/realtime-media/rtc/get-started-sdk).
...
- A computer that can access the Internet. If your network environment has a firewall deployed, refer to [Firewall requirements](/en/realtime-media/rtc/reference/firewall) to use the Agora services normally.

## Project setup

To implement MetaKit effects in your app, open the [SDK quickstart for Video Calling](/en/realtime-media/rtc/get-started-sdk) project and take the steps described below.
...
1. Call `addExtension` with `AgoraFaceCapturePlugin` (`agora_face_capture_extension`) and `MetaKitPlugin` (`agora_metakit_extension`). Then, implement the `IMediaExtensionObserver` callback interface and register it to receive extension events such as `onEvent`.
...
2. In the callback, specify `provider` as `agora_video_filters_metakit` and `extension` as `metakit` to filter events from the MetaKit extension. The `onEvent` callback transparently passes engine state events such as `unityLoadFinish` (Unity environment loading completed) and `loadSceneResp` (scene resource loading completed).
...
<CodeBlockTab value="java">
    ...
    mRtcEngine.setExtensionProperty(
      ...
      "{\"company_id\":\"agoraDemo\", \"license\":\"xxxxxxxxxx\"}",
...
To implement the capabilities of the MetaKit extension, use the [setExtensionProperty](https://api-ref.agora.io/en/video-sdk/android/4.x/API/class_irtcengine.html#api_irtcengine_setextensionproperty) method provided by the Agora RTC SDK v4.x. Pass in the specified `key` and `value` as follows:
...
- `value`: Object. Contains the following field:
  ...
  1. **Set 1**
     ...
     ```
    ![Avatar dressup set 1](https://web-cdn.agora.io/doc-cms/uploads/1708415768849-dress1.png)
  ...
  1. **Set 2**
     ...
     ```
    ![Avatar dressup set 2](https://web-cdn.agora.io/doc-cms/uploads/1708415915330-dress2.png)

#### Animoji

The MetaKit extension allows you to switch the image of Animoji. To experience Animoji-related functions, set `avatarMode` to `1` when [enabling texture request](#enable-texture-request) or [adding scene view](#add-scene-view).
...
<video controls width="30%" height="auto">
  <source src="https://web-cdn.agora.io/doc-cms/uploads/1706758897677-Face_脸颊.mp4" type="video/mp4" />
</video>
...
<video controls width="30%" height="auto">
  <source src="https://web-cdn.agora.io/doc-cms/uploads/1706759936570-Eyebrow_眉毛长短.mp4" type="video/mp4" />
</video>
...
<video controls width="30%" height="auto">
  <source src="https://web-cdn.agora.io/doc-cms/uploads/1706761070968-Eye_眼睛大小.mp4" type="video/mp4" />
</video>
...
<video controls width="30%" height="auto">
  <source src="https://web-cdn.agora.io/doc-cms/uploads/1706761125372-Nose_鼻头整体大小.mp4" type="video/mp4" />
</video>
...
<video controls width="30%" height="auto">
  <source src="https://web-cdn.agora.io/doc-cms/uploads/1706761188082-Mouse_上下偏移.mp4" type="video/mp4" />
</video>
...
1. Initialize `AgoraRtcEngineKit` by calling `sharedEngineWithConfig`. In `AgoraRtcEngineConfig`, create the `AgoraMediaFilterEventDelegate` callback interface for the extension and register extension event callbacks such as `onEvent`.
...
2. In the callback, specify `provider` as `agora_video_filters_metakit` and `extension` as `metakit` to filter events from the MetaKit extension. `onEvent` passes through engine states such as `unityLoadFinish` (Unity environment loaded) and `addSceneViewResp` (scene view loaded).
...
Call `registerExtensionWithVendor` to register the MetaKit extension with the extension service provider name (`agora_video_filters_metakit`) and extension name (`metakit`).
...
To implement the capabilities of the `MetaKit` extension, use the [setExtensionPropertyWithVendor](https://api-ref.agora.io/en/video-sdk/ios/4.x/documentation/agorartckit/setextensionpropertywithvendor(_:extension:extensioninfo:key:value:)) method provided by Agora RTC SDK v4.x. Pass in the specified `key` and `value` as follows:
