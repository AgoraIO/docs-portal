...
2. Implement client-side logic
   ...
   1. To manually add the encryption library to your project:
      ...
      1. Copy the `AgoraRtcCryptoLoader.framework` file from the Agora RTC SDK package to your project folder.
...
RTC SDK supports the following encryption modes:
...
2. Implement client-side logic
  ...
  1. When you call `setEncryptionConfig`, the web client:
     ...
     * Converts the key from Hex to ASCII, before passing it to the RTC SDK.

     * Converts the salt from Base64 to Uint8Array, before passing it to the RTC SDK.
