...
In some use-cases, raw audio captured through the microphone must be processed to enhance the user experience or achieve the desired functionality RTC SDK enables you to pre-process and post-process the captured audio for implementation of custom playback effects.
...
For use-cases that require self-processing of audio data, Agora RTC SDK provides raw data processing functionality. You can perform pre-processing to modify the captured audio signal before sending the data to the encoder, or post-process data to modify the received audio signal after sending the data to the decoder.
...
:::info
RTC SDK uses a synchronous callback mechanism for processing raw audio data. When you save or rewrite data using the callbacks, consider the following best practices:
...
For use-cases that require self-processing of audio data, use the [Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API) with Agora RTC SDK to enable raw audio data processing. The Web Audio API allows you to modify the captured audio signal before transmitting it to the channel.
...
- [`registerAudioFrameObserver`](https://api-ref.agora.io/en/video-sdk/unreal-engine/4.x/API/class_imediaplayer.html#api_imediaplayer_registeraudioframeobserver)
...
- [`onRecordAudioFrame`](https://api-ref.agora.io/en/video-sdk/unreal-engine/4.x/API/class_iaudioframeobserverbase.html#callback_iaudioframeobserverbase_onrecordaudioframe)
- [`onPlaybackAudioFrame`](https://api-ref.agora.io/en/video-sdk/unreal-engine/4.x/API/class_iaudioframeobserverbase.html#callback_iaudioframeobserverbase_onplaybackaudioframe)
- [`onplaybackaudioframebeforemixing`](https://api-ref.agora.io/en/video-sdk/unreal-engine/4.x/API/class_iaudioframeobserver.html#class_iaudioframeobserver)
- [`onMixedAudioFrame`](https://api-ref.agora.io/en/video-sdk/unreal-engine/4.x/API/class_iaudioframeobserverbase.html#callback_iaudioframeobserverbase_onmixedaudioframe)
</PlatformStructured>
