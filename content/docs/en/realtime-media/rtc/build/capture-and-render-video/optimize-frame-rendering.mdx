...
### Set up a RTC SDK instance

Creating and initializing the RTC SDK engine takes time. To reduce first-frame display time, Agora recommends initializing the engine when the module is loaded, not when SDK functions are first called.
