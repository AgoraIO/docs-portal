...
Screen sharing in the Agora RTC SDK is implemented through a plug-in. You can automatically integrate the plug-in through Maven Central or manual import of the **aar** file.
...
- After turning on screen sharing, Agora uses the resolution of the screen sharing video stream as the billing standard. Please see [Pricing](/en/realtime-media/rtc/reference/pricing) for details. The default resolution is 1280 × 720, but you can adjust it according to your business needs.
...
- The Agora RTC SDK only supports screen sharing in the following desktop browsers:
  - Chrome 58 or above.
  - Firefox 56 or above.
...
RTC SDK for Web supports screen sharing on Edge 80 and higher on the Windows 10+ platform.
...
RTC SDK for Web supports screen sharing on Firefox 56 and higher. For Firefox, you specify the type of screen sharing by setting `screenSourceType`.
...
RTC SDK for Web supports screen sharing on Safari 13 and higher on the macOS platform.
...
When enabling screen sharing on Android, you only need to call the `startScreenCapture` method. Refer to the `screen_sharing.dart` file in the Agora RTC SDK to implement screen sharing.
