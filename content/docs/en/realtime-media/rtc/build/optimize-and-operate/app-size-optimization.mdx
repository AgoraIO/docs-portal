...
Since version 4.4.0, Agora offers a Lite SDK that provides basic audio and video call functions in a smaller package size. If you only need to implement basic audio and video call functions, integrate the Lite SDK. For details see [SDKs](../../../sdks.md).
...
### Use the Voice SDK
RTC SDK supports both audio and video functions, and the package size is large. If you only need to use audio features, best practice is to integrate the [Voice SDK](../../../sdks.md). For details, see [Voice SDK Quickstart](/en/realtime-media/rtc/voice-quickstart).

### Integrate the FFmpeg plugin as needed

Since v4.7.0, the Voice SDK provides FFmpeg as an optional plugin, `libagora_ffmpeg_extension.so`. The SDK package includes this plugin by default.

The FFmpeg plugin extends the audio file playback capabilities of the media player and depends on `libaosl.so`. If your app does not need the playback capabilities that FFmpeg provides, remove the plugin or do not link it when you integrate the SDK, to reduce your app size. After you remove it, the media player uses the system's native playback capability, and the audio file formats it supports depend on the container formats and audio codecs that the system supports. To play audio formats that the system does not support natively, keep the FFmpeg plugin integrated.

### Remove unnecessary extensions
RTC SDK provides optional extension dynamic libraries. The name of the extension is suffixed with `extension`. See the [extension list](#extension-list) for details on the function and size of the extensions. Refer to the following ways to exclude these extensions to reduce the size of the app.
...
<Accordion title="Perceptual Video Coding (PVC)">
...
<Accordion title="Virtual Background">
...
<Accordion title="Copyright Music">
...
The Screen Sharing extension enables a local user to share their screen contents with other users to improve communication efficiency. For details, see [Screen Sharing](/en/realtime-media/rtc/build/capture-and-render-video/screen-sharing).
:::warning
...
<Accordion title="Video Quality Analyzer">
...
<Accordion title="Video Encoding">
...
<Accordion title="AV1 Stream Encoding">
...
<Accordion title="AV1 Stream Decoding">
...
- `libagora-rtc-sdk.so`
- `libagora-fdkaac.so`
- `libagora_ffmpeg_extension.so` (since v4.7.0)
- `libagora-soundtouch.so`
- `libagora-core.so` (renamed to `libaosl.so` since v4.3.0; dependency since v4.5.0)
- SDK header files
...
Since v4.7.0, the Voice SDK provides FFmpeg as an optional plugin, `AgoraffmpegExtension.xcframework`. The SDK package includes this plugin by default.

The FFmpeg plugin extends the audio file playback capabilities of the media player and depends on `aosl.xcframework`. If your audio-only use case does not need the playback capabilities that FFmpeg provides, remove the plugin or do not link it when you integrate the SDK, to reduce your app size. After you remove it, the media player uses the system's native playback capability, and the audio file formats it supports depend on the container formats and audio codecs that the system supports. To play audio formats that the system does not support natively, keep the FFmpeg plugin integrated.
...
<CodeBlockTabsList>
  <CodeBlockTabsTrigger value="all">Use all extensions</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="none">Don't use any extensions</CodeBlockTabsTrigger>
  <CodeBlockTabsTrigger value="some">Use some extensions</CodeBlockTabsTrigger>
</CodeBlockTabsList>
...
- `video_dec.xcframework` (since v4.6.2)
...
- `Agorafdkaac.xcframework`
- `AgoraSoundTouch.xcframework`
- `AgoraffmpegExtension.xcframework` (since v4.7.0)
- `AgoraCore.xcframework` (renamed to `aosl.xcframework` since v4.3.0; dependency since v4.5.0, see [AgoraInfra_iOS](https://cocoapods.org/pods/AgoraInfra_iOS))
...
When integrating the iOS or macOS SDK through CocoaPods, use the `subspecs` field in the Podfile to specify the dynamic libraries you need to integrate. This excludes extensions you do not need. For details about the corresponding relationship between dynamic libraries and `subspecs` field, see [subspecs field](#integrating-extensions-into-your-project-2).
...
In versions before v4.19.0, the `AgoraRTC` module is the entrance to all callable methods in the Web RTC SDK. Even if you only use a part of the RTC SDK's functionality, you still need to load the complete SDK, which may result in the app containing a lot of unused code.

Starting from version 4.19.0, the RTC SDK for Web has integrated [tree shaking](https://webpack.js.org/guides/tree-shaking/) support. Using tree shaking has the following advantages:
...
1. Starting from version 4.23.0, you can integrate the functional modules you need as follows:
    ...
    <CodeBlockTabs defaultValue="plan-b">
    <CodeBlockTabsList>
      <CodeBlockTabsTrigger value="plan-b">Plan B</CodeBlockTabsTrigger>
      <CodeBlockTabsTrigger value="content-moderation">Content moderation</CodeBlockTabsTrigger>
      <CodeBlockTabsTrigger value="relay">Channel media relay</CodeBlockTabsTrigger>
      <CodeBlockTabsTrigger value="live">Live streaming</CodeBlockTabsTrigger>
    </CodeBlockTabsList>

    <CodeBlockTab value="plan-b">
    ...
    <CodeBlockTab value="content-moderation">
...
This page shows you how to optimize the size of games with integrated RTC SDK.
