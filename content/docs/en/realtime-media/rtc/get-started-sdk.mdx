---
title: "Quickstart"
description: "Build a basic voice calling, video calling, or streaming app with the Agora RTC SDK."
---
...
<TabsList>
  ...
  <TabsContent value="maven">
    ...
    1. Open the `settings.gradle` file in the project's root directory and add the Agora Maven CDN repository to `dependencyResolutionManagement.repositories`.
        ...
        - Groovy `settings.gradle`
          ...
          dependencyResolutionManagement {
              repositories {
                  google()
                  mavenCentral()
                  maven { url = 'https://download.agora.io/maven/' }
              }
          }
          ```
        ...
        :::note
        If you cannot find the `dependencyResolutionManagement` field in `settings.gradle` or `settings.gradle.kts`, your Android Gradle plugin version might be earlier than 7.1.0.
    ...
    1. To integrate the RTC SDK into your Android project, add one of the following to the `dependencies` block in your project module `build.gradle` file:
        ...
        :::info[Note]
        To get the latest version number, check the [Release notes](/en/realtime-media/rtc/reference/release-notes).
        :::

    1. Prevent code obfuscation

        If your app does not enable R8/ProGuard shrinking and obfuscation, no additional configuration is required. If your app enables R8/ProGuard shrinking and obfuscation, choose one of the following configurations:

        - Quick configuration: Open the `/app/proguard-rules.pro` file and add the following rules to keep all RTC SDK classes:
        ...
        - Fine-grained configuration: If you have stricter requirements for app size, code shrinking, or obfuscation, use the `consumer-proguard-rules.pro` file provided with the SDK. This file keeps only the Java classes, members, and methods that the native layer accesses through JNI, so the rest of the SDK Java code can still be shrunk, optimized, and obfuscated by R8/ProGuard.

            `consumer-proguard-rules.pro` is included in the AAR and takes effect automatically when you build your app. You do not need to copy it manually.
  ...
  <TabsContent value="manual">
    ...
    1. Prevent code obfuscation
        ...
        - Fine-grained configuration: If you have stricter requirements for app size, code shrinking, or obfuscation, use the `consumer-proguard-rules.pro` file provided with the SDK. This file keeps only the Java classes, members, and methods that the native layer accesses through JNI, so the rest of the SDK Java code can still be shrunk, optimized, and obfuscated by R8/ProGuard.
            ...
            Copy `consumer-proguard-rules.pro` into your project and add it to `build.gradle`:
...
1. Use the following code to handle runtime permissions in your Android app. The logic ensures that the necessary permissions are granted before starting Realtime Communication. In your `MainActivity` file, add the following code:
...
1. When a user closes the client, or switches the client to the background, call `stopPreview` to stop the video preview and then call `leaveChannel` to leave the current channel and release all session-related resources.
...
Call `sharedEngine(withAppId:delegate:)` to create and initialize an `AgoraRtcEngineKit` instance. Provide your [App ID](/en/introduction/account#get-the-app-id-and-app-certificate) and an `AgoraRtcEngineDelegate` implementation to [handle SDK events](#subscribe-to-video-sdk-events).
...
1. Allow the app to access the device's microphone and camera.
...
1. On a second device, repeat the previous steps to install and launch the client. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
The RTC SDK provides the `PrivacyInfo.xcprivacy` file that contains the required reasons for the APIs used by the SDK. To add the privacy manifest to your app in Xcode, follow these steps:
...
To add Realtime Communication to your existing project, take the following steps:
...
<Accordions>
<Accordion title="Quick start sequence">
![Realtime Communication quick start sequence](https://assets-docs.agora.io/images/video-sdk/quick-start-sequence-web.svg)
</Accordion>
</Accordions>
...
Set the `mode`, `role`, and audience latency `level` according to your use case:

| Use case | Mode | Role | Latency level |
|----------|------|------|----------------|
| Voice calling | `rtc` | N/A | N/A |
| Video calling | `rtc` | N/A | N/A |
| Interactive live streaming | `live` | `host` or `audience` | `2` (ultra-low, default) |
| Broadcast streaming | `live` | `host` or `audience` | `1` (low) |

:::info[Note]

- In `rtc` mode, every user can publish and subscribe. In `live` mode, you assign each user a role: hosts publish and subscribe, audience members only subscribe.
- The latency `level` only applies to the audience role, set through `setClientRole`, and affects your [pricing](/en/realtime-media/rtc/reference/pricing#subscription-packages) tier.

:::

The following example shows the configuration for interactive live streaming with the host role:
...
function initializeClient() {
    // Set mode to "live" for streaming use cases, or "rtc" for calling use cases (see table above)
    // Set role to "host" or "audience" according to your use case
...
- **App ID**: The [App ID](/en/introduction/account#get-the-app-id-and-app-certificate) for your project.
...
The `mode` and `role` you set when you [Initialize an instance of `AgoraRTCClient`](#initialize-an-instance-of-agorartcclient) determine what a user can do after joining.
...
For the audience role in `live` mode, change the role and latency level after joining by calling `setClientRole`:

```javascript
// Low audience latency level; use 2 for ultra-low latency
...
// Initialize the AgoraRTC client
function initializeClient() {
    // Set mode to "live" for streaming use cases, or "rtc" for calling use cases
    // Set role to "host" or "audience" according to your use case
    client = AgoraRTC.createClient({ mode: "live", codec: "vp8", role: "host" });
    setupEventListeners();
}
...
1. On a second device, repeat the previous steps to install and launch the client. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) or clone the [sample project on Github](https://github.com/AgoraIO/API-Examples-Web) to join the same channel and test the following use-cases:
...
When you use `npm` to install the Web SDK, you can enable tree shaking to reduce the size of the app after integration. For details, see [Using tree shaking](/en/realtime-media/rtc/build/optimize-and-operate/app-size-optimization#use-tree-shaking).
...
* Download the RTC SDK package locally, save the `.js` files in the SDK package to the project directory, and then add the following tag to the project HTML file:
  ...
  ```html
  <script src="./AgoraRTC_N-4.24.8.js"></script>
  ```
...
For real-time communication, initialize an `IRtcEngine` instance and set up event handlers to manage user interactions within the channel. Use `RtcEngineContext` to specify the [App ID](/en/introduction/account#get-the-app-id-and-app-certificate), and custom [event handler](#subscribe-to-video-sdk-events), then call `initialize` to initialize the engine, enabling further channel operations. Add the following code to your header and `.cpp` files:
...
1. Use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
For real-time communication, create an `IRtcEngine` object by calling `createAgoraRtcEngine` and then call `initialize` with `RtcEngineContext` to specify the [App ID](/en/introduction/account#get-the-app-id-and-app-certificate). Call `registerEventHandler` to register a custom [event handler](#subscribe-to-video-sdk-events) for managing user interactions within the channel.
...
1. To start Realtime Communication, [initialize the engine](#initialize-the-engine), [display the local video](#display-the-local-video) and [join a channel](#join-a-channel).

2. When Realtime Communication ends, leave the channel and clean up resources:
...
window.onload = () => {
    ...
    // Join the channel using a temporary token
    rtcEngine.joinChannel(token, channel, uid, {
        // Set the channel profile according to your use case (see the table in Join a channel)
        channelProfile: ChannelProfileType.ChannelProfileLiveBroadcasting,
        // Set the user role to ClientRoleBroadcaster (host) or ClientRoleAudience according to your use case
        clientRoleType: ClientRoleType.ClientRoleBroadcaster,
        // Only takes effect when clientRoleType is ClientRoleAudience
...
1. Invite a friend to run the demo app on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
For real-time communication, initialize an `RtcEngine` instance. Use `RtcEngineContext` to specify the [App ID](/en/introduction/account#get-the-app-id-and-app-certificate), and other configuration parameters. In your dart file, add the following code:
...
1. On a second target device, repeat the previous steps to install and launch the client. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
### Initialize the engine
For real-time communication, call `createAgoraRtcEngine` to create an `RtcEngine` instance, and then `initialize` the engine using `RtcEngineContext` to specify the [App ID](/en/introduction/account#get-the-app-id-and-app-certificate).
...
1. Invite a friend to install and run the client on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel. Once your friend joins the channel, you can see and hear each other.
...
- **App ID**: [`appid`](/en/introduction/account#get-the-app-id-and-app-certificate) identifies the project you created in Agora Console.
...
1. To start Realtime Communication, initialize the engine and set up the event handler.
...
1. To clean up all session-related resources when a user exits the client, call the `Dispose` method of the `IRtcEngine`.
    ...
    After calling `Dispose`, you can no longer use any methods or callbacks of the SDK. To use Realtime Communication features again, create a new engine instance.
...
1. Invite a friend to run the demo client on a second device. Use the same `_appID_`, `_token`, and `_channelName` to join. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
1. Download the latest version of Agora Unreal RTC SDK from [Download SDKs](/en/api-reference/sdks) and unzip it.
...
For real-time communication, initialize an `IRtcEngine` instance and set up an event handler to manage user interactions within the channel. Use `RtcEngineContext` to specify [App ID](/en/introduction/account#get-the-app-id-and-app-certificate), and custom [event handler](#subscribe-to-video-sdk-events), then call `RtcEngineProxy->initialize(RtcEngineContext)` to initialize the engine, enabling further channel operations.
...
1. Refer to [How to add the permissions required for real-time interaction to an Unreal Engine project?](/en/realtime-media/rtc/)
...
1. Create a **Level Blueprint** and associate it with the created **Widget Blueprint**.
    ...
    1. Right-click and enter **Create Widget** in the search box. Select the created **AgoraWidget**, then create **Event BeginPlay** and **Add to Viewport** in the same way. Connect them as follows:
...
1. Invite a friend to run the demo client on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
To learn about Agora Unreal Blueprint development, refer to the [Unreal (Blueprint) Quickstart](/en/realtime-media/rtc/get-started-sdk?platform=blueprint).
...
3. When a remote user joins the channel, the `onUserJoined` callback is triggered to create a remote view. `uid` is the uid sent from the remote end, and the video source type is `VIDEO_SOURCE_REMOTE` (remote video obtained from the network):
