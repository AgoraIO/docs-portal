...
<Tabs>
<TabsList>
  <TabsTrigger value="new-project">Create a new project</TabsTrigger>
  <TabsTrigger value="existing-project">Add to an existing project</TabsTrigger>
</TabsList>
...
<TabsList>
  ...
  <TabsTrigger value="manual-integration">Manual integration</TabsTrigger>
</TabsList>
...
1. On a second Android device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
This iOS quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your iOS project and install the Agora RTC SDK.
...
Use one of the following methods to install RTC SDK.

<Tabs>
<TabsList>
  <TabsTrigger value="swift-package-manager">Swift Package Manager</TabsTrigger>
...
1. Run the `pod install` command in the terminal to install the RTC SDK. After successful installation, the terminal shows **Pod installation complete!**.
...
Call `sharedEngine(withAppId:delegate:)` to create and initialize an `AgoraRtcEngineKit` instance. Provide your [App ID](/en/introduction/account) and an `AgoraRtcEngineDelegate` implementation to [handle SDK events](#subscribe-to-rtc-sdk-events).
...
1. On a second iOS device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
### Add a privacy manifest file
 ...
 The Agora RTC SDK for iOS provides the `PrivacyInfo.xcprivacy` file that contains the required reasons for the APIs used by the SDK. To add the privacy manifest to your app in Xcode, follow these steps:
...
2. Add the items in `PrivacyInfo.xcprivacy` file of the RTC SDK to `PrivacyInfo.xcprivacy` of the app using the following source code:
...
This macOS quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your macOS project and install the Agora RTC SDK.
...
4. On a second macOS device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
- **App ID**: The [App ID](/en/introduction/account) for your project.
...
This Windows quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your Windows project and install the Agora RTC SDK.
...
- [`IRtcEngineEventHandler`](https://api-ref.agora.io/en/video-sdk/cpp/4.x/API/class_irtcengineeventhandler.html#class_irtcengineeventhandler)
...
<Tabs>
<TabsList>
  <TabsTrigger value="npm-integration">npm integration</TabsTrigger>
</TabsList>
...
For real-time communication, create an `IRtcEngine` object by calling `createAgoraRtcEngine` and then call `initialize` with `RtcEngineContext` to specify the [App ID](/en/introduction/account). Call `registerEventHandler` to register a custom [event handler](#subscribe-to-rtc-sdk-events) for managing user interactions within the channel.
...
- [`IRtcEngineEventHandler`](https://api-ref.agora.io/en/video-sdk/electron/4.x/API/class_irtcengineeventhandler.html#class_irtcengineeventhandler)
...
1. Add a new file to the `lib` folder and name it `agora_logic.dart`.
...
For real-time communication, initialize an `RtcEngine` instance. Use `RtcEngineContext` to specify the [App ID](/en/introduction/account), and other configuration parameters. In your dart file, add the following code:
...
This React Native quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your React Native project and install the Agora RTC SDK.
...
### Initialize the engine
For real-time communication, call `createAgoraRtcEngine` to create an `RtcEngine` instance, and then `initialize` the engine using `RtcEngineContext` to specify the [App ID](/en/introduction/account).
...
4. Invite a friend to install and run the app on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel. Once your friend joins the channel, you can talk to each other.
...
This JavaScript quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your ReactJS project and install the Agora RTC SDK.
...
- **App ID**: [`appid`](/en/introduction/account) identifies the project you created in Agora Console.

- **Channel name**: The name of the `channel` to join. Clients that pass the same channel name join the same channel. If a channel with the specified name does not exist, it is created when the first user joins.

- **Authentication token**: A `token` is a dynamic key that authenticates a user when the client joins a channel. In a production environment, you obtain a token from a [token server](build/authenticate-users/deploy-token-server.mdx) in your security infrastructure. For the purpose of this guide [Generate a temporary token](/en/introduction/account).
...
This Unity quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your Unity project and install the Agora RTC SDK.
...
1. Invite a friend to run the demo game on a second device. Use the same `_appID_`, `_token`, and `_channelName` to join. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
This Unreal Engine quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your Unreal Engine project and install the Agora RTC SDK.
...
1. Add the Agora dependency library
...
Take the following steps to add the Unreal Engine RTC SDK to your project:

1. Download the latest version of Agora Unreal RTC SDK from [Download SDKs](/en/api-reference/sdks?product=video&platform=unreal-engine) and unzip it.
...
For real-time communication, initialize an `IRtcEngine` instance and set up an event handler to manage user interactions within the channel. Use `RtcEngineContext` to specify [App ID](/en/introduction/account), and custom [event handler](#subscribe-to-rtc-sdk-events), then call `RtcEngineProxy->initialize(RtcEngineContext)` to initialize the engine, enabling further channel operations.
...
**iOS and macOS**
1. Refer to [How to add the permissions required for real-time interaction to an Unreal Engine project?](/en/realtime-media/rtc/voice-quickstart)
...
1. Invite a friend to run the demo game on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
1. Invite a friend to run the demo game on a second device. Use the same app ID, channel name, and token to join. After your friend joins successfully, you can hear and see each other.
...
This Python quickstart shows you how to create a basic Voice Calling app using the Agora RTC SDK.
...
This section shows you how to set up your Python project and install the Agora RTC SDK.
...
- [`rtc.py`](https://api-ref.agora.io/en/video-sdk/python/rtc-py-api.html)
</PlatformStructured>
