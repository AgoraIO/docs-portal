...
- Added a `['RtmKit']` subspec version to resolve integration issues when co-integrating with the RTC SDK. For details, see [Handle issues when integrating the Signaling SDK and RTC SDK simultaneously](/en/api-reference/faq/integration/rtm2_rtc_integration_issue).
