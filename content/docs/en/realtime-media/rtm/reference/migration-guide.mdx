...
```java
// 2.x
HashMap<String, String> stateItems = new HashMap<>();
...
// Add a Metadata Item
metadata.getItems().add(new MetadataItem("Apple", "100", 174298200));
// Add another Metadata Item
metadata.getItems().add(new MetadataItem("Banana", "200", 174298100));
...
- For `2.x`:
    ...
    // Create and initialize the RTM instance in a single call
    int errorCode = 0;
    IRtmClient* rtmClient = createAgoraRtmClient(config, errorCode);
    if (!rtmClient || errorCode != RTM_ERROR_OK) {
        // Handle initialization errors
    }
    ```
...
```cpp
    ...
    uint64_t requestId;
    rtmClient->getPresence()->getOnlineUsers("channelName", RTM_CHANNEL_TYPE_STREAM, options, requestId);
...
```cpp
    ...
    uint64_t requestId;
    rtmClient->getPresence()->setState("channelName", RTM_CHANNEL_TYPE_STREAM, stateItems.data(), stateItems.size(), requestId);
...
```cpp
    ...
    uint64_t requestId;
    rtmClient->getStorage()->setChannelMetadata("channelName", RTM_CHANNEL_TYPE_STREAM, metadata, options, "lockName", requestId);
