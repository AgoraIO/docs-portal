...
```java
    dependencies {
        ...
        // For v2.2.7 and later:
        implementation 'io.agora.rtm:rtm-sdk:x.y.z'
        // For v2.2.6 and earlier:
        // implementation 'io.agora:agora-rtm:x.y.z'
    }
```
:::info[Info]
Signaling `1.x` uses the package name `rtm-sdk` (`io.agora.rtm:rtm-sdk`). Signaling `2.x` originally used a different package name, `agora-rtm` (`io.agora:agora-rtm`), for versions up to `2.2.6`. Starting from `2.2.7`, `2.x` reuses the `rtm-sdk` package name (`io.agora.rtm:rtm-sdk`).
:::
