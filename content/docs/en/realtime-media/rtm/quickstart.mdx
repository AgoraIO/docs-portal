...
1. Maven doesn't distribute the native libraries the SDK depends on, so [download](/en/api-reference/sdks?product=signaling&platform=linux) the SDK package separately, unzip it, and copy the `*.so` files to the project's `lib` folder.
