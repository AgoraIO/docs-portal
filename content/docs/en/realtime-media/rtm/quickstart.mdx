...
1. Use the [Token Builder](https://agora-token-generator-demo.vercel.app/) to generate a Signaling token:
    ...
    1. Fill in your app ID and app certificate from [Agora Console](https://console.agora.io/). Leave the remaining fields blank.
...
<Tabs defaultValue="swift" groupId="ios-language" persist>
<TabsList>
...
<Accordions>
<Accordion title="Complete sample code for real-time Signaling">
...
<Accordions>
<Accordion title="Complete sample code">
...
- A device running Ubuntu 18.04 or Debian 9.9, x86-64 architecture.
- Java 8 or above.
- Maven.
...
To integrate the Linux Java Signaling SDK into your project:

1. [Download](/en/api-reference/sdks?product=signaling&platform=linux) the latest version of the SDK and unzip it.

1. Copy the `*.so` files and the `*.jar` file from the SDK package to the project's `lib` folder.

1. Create a `pom.xml` file in the `RTM_quickstart` folder with the following content, to build the project with Maven:
...
1. In your code, replace `<Your App ID>` with your app ID from Agora Console and `<Your token>` with the generated token. Make sure Signaling is activated for your project in [Agora Console](https://console.agora.io/).

1. The SDK isn't published to a public Maven repository, so register the `agora-rtm-sdk.jar` file you copied into the `lib` folder as a local dependency:
    ...
    Use the same `x.y.z` version number as in your `pom.xml`.

1. In the terminal, run the following commands to compile the project:
...
1. Set the library path and run the app:
...
1. Follow the prompts to log in and **subscribe** to a channel.
...
:::note[Best practice]
To continuously monitor the network connection state of the client, best practice is to continue to listen for `onLinkStateEvent` event notifications throughout the life cycle of the application. For further details, see [Event listeners](/en/realtime-media/rtm/build/send-and-receive-messages/add-event-listener).
:::
...
1. In your code, replace `<Your App ID>` with your app ID from Agora Console and `<Your token>` with the generated token. Save the project. Make sure Signaling is activated for your project in [Agora Console](https://console.agora.io/).
