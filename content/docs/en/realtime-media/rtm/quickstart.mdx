...
class ChatViewModel: NSObject, ObservableObject, AgoraRtmClientDelegate {
    ...
    func login() {
        ...
        // The user ID is part of the client configuration, so create the
        // client after the user enters a name.
        initializeEngine()
...
1. Maven doesn't distribute the native libraries the SDK depends on, so [download](/en/api-reference/sdks?product=signaling&platform=linux-java) the SDK package separately, unzip it, and copy the `*.so` files to the project's `lib` folder.
...
1. [Download](/en/api-reference/sdks?product=signaling&platform=linux-cpp) the latest version of the SDK and unzip it.
