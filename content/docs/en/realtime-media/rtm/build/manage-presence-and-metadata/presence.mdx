...
Signaling does not permanently save the status data. When a user unsubscribes from a channel, times out, or exits a channel, the data is deleted. To save user data permanently, use the [Store user metadata](storage/store-user-metadata.mdx) feature.
...
A presence event notification returns the [PresenceEvent](/en/api-reference/api-ref/signaling) data structure, which includes the [RtmPresenceEventType](/en/api-reference/api-ref/signaling) parameter.

To receive presence event notifications, implement an event listener. See [event listeners](/en/api-reference/api-ref/signaling) for details. In addition, set the `withPresence` parameter to `true` when subscribing to or joining a channel.
...
- [`getOnlineUsers​`](/en/api-reference/api-ref/signaling)
- [`getUserChannels`](/en/api-reference/api-ref/signaling)
- [`setState`](/en/api-reference/api-ref/signaling)
- [`getState`](/en/api-reference/api-ref/signaling)
- [`removeState`](/en/api-reference/api-ref/signaling)
