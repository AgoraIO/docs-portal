...
rtm.addEventListener("presence", event => {
    ...
    const totalOccupancy = interval?.totalOccupancy; // Total occupancy of the channel, only for the interval event
...
rtm.addEventListener("linkState", event => {
    const currentState = event.currentState;          // Current link state
    const previousState = event.previousState;        // Previous link state
    const serviceType = event.serviceType;            // Service type
    const operation = event.operation;                // Operation that triggered this event
    const reason = event.reason;                      // Reason for the state change
    const reasonCode = event.reasonCode;               // Reason code for the state change
    const affectedChannels = event.affectedChannels;  // Affected channels
    const timestamp = event.timestamp;                // Event timestamp
    const isResumed = event.isResumed;                // Whether the connection resumed
});
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/web#event-listeners) in the API reference.
...
// Configure RtmClient and specify the event listener
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/android#event-listeners) in the API reference.
...
- (void)rtmKit:(AgoraRtmClientKit *)rtmKit didReceivePresenceEvent:(AgoraRtmPresenceEvent *)event {
    ...
    if (event_type  == AgoraRtmPresenceEventTypeInterval) {
        ...
        NSDictionary<NSString *, NSString *> *states = event.states;
    }
}
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/ios#event-listeners) in the API reference.
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/macos#event-listeners) in the API reference.
...
Signaling uses the `addListener` method to process messages and event notifications. Each message and event notification has a corresponding named parameter, where you pass a callback that implements your own processing logic. Refer to the following code to register event handlers:

```dart
rtmClient.addListener(
    // Handle message event
    message: (MessageEvent event) {
        final channelType = event.channelType;      // The channel type, `stream` `message`, or `user`
        final channelName = event.channelName;      // The name of the channel that this message came from
        final topicName = event.channelTopic;       // (Stream channel only) The name of the topic that this message came from
        final messageType = event.messageType;      // The message type, `string` or `binary`
        final customType = event.customType;        // User defined type
        final publisher = event.publisher;          // The message publisher
        final message = event.message;              // The message payload
        final timestamp = event.timestamp;          // Event timestamp
    },
    // Handle presence event
    presence: (PresenceEvent event) {
        final action = event.type;                  // The event type, should be one of `snapshot`, `interval`, `remoteJoinChannel`, `remoteLeaveChannel`, `remoteTimeout`, `remoteStateChanged`, `errorOutOfService`.
        final channelType = event.channelType;      // The channel type, should be `stream`, `message` or `user`
        final channelName = event.channelName;      // The name of the channel this event came from
        final publisher = event.publisher;          // The user who triggered this event
        final states = event.stateItems;            // User state payload, only for the stateChanged event
        final interval = event.interval;            // Interval payload, only for the interval event
        final snapshot = event.snapshot;            // Snapshot payload, only for the snapshot event
        final timestamp = event.timestamp;          // Event timestamp
    },
   // Handle topic event
    topic: (TopicEvent event) {
        final action = event.type;                  // The event type, should be one of `snapshot`, `remoteJoinTopic`, `remoteLeaveTopic`
        final channelName = event.channelName;      // The name of the channel this event came from
        final publisher = event.publisher;          // The user who triggered this event
        final topicInfos = event.topicInfos;        // Topic information payload
        final timestamp = event.timestamp;          // Event timestamp
    },
    // Handle storage event
    storage: (StorageEvent event) {
        final channelType = event.channelType;      // The channel type, should be `stream`, `message` or `user`
        final target = event.target;                // Which channel or user triggered this event
        final storageType = event.storageType;      // The storage type, should be `user`, `channel`
        final action = event.eventType;             // The event type, should be one of `snapshot`, `set`, `remove`, `update` or `none`
        final data = event.data;                    // USER_METADATA or CHANNEL_METADATA payload
        final timestamp = event.timestamp;          // Event timestamp
    },
    // Handle lock event
    lock: (LockEvent event) {
        final channelType = event.channelType;      // The channel type, should be `stream`, `message` or `user`
        final channelName = event.channelName;      // The name of the channel this event came from
        final action = event.eventType;             // The action type, should be one of `lockSet`, `lockRemoved`, `lockAcquired`, `lockReleased`, `lockExpired`, `snapshot`
        final lockDetailList = event.lockDetailList;    // Lock event payload
        final timestamp = event.timestamp;          // Event timestamp
    },
    // Handle link state event
    linkState: (LinkStateEvent event) {
        final currentState = event.currentState;    // The current link state
        final previousState = event.previousState;  // The previous link state
        final serviceType = event.serviceType;      // The service type, Should be "stream" or "message".
        final operation = event.operation;          // Which operation triggered this state changed
        final reason = event.reason;                // The reason for this state change
        final affectedChannels = event.affectedChannels;  // Which channels are affected
        final unrestoredChannels = event.unrestoredChannels;    // Which channels are unrestored
        final isResumed = event.isResumed;
        final timestamp = event.timestamp;          // Event timestamp
    },
    // Handle token event
    token : (TokenEvent event) {
        final channelName = event.channelName;      // The channel for which the token will expire
    });
```
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/flutter#event-listeners) in the API reference.
...
int errorCode = 0;
IRtmClient* rtm_client = createAgoraRtmClient(cfg, errorCode);
```
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/windows-cpp#event-listeners) in the API reference.
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/linux-cpp#event-listeners) in the API reference.
...
// Configure RtmClient
// Specify the event listener when building the client configuration
RtmConfig rtmConfig = new RtmConfig.Builder("your_appId", "your_userId")
    .eventListener(eventListener)
    .build();
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/linux-java#event-listeners) in the API reference.
...
rtmClient.OnStorageEvent += newStorageEvent =>
{
    ...
    var storageType = newStorageEvent.storageType;
    var eventType = newStorageEvent.eventType;
    var target = newStorageEvent.target;
...
For details on the parameters passed in by each event, refer to [Event listeners](/en/api-reference/api-ref/signaling/unity#event-listeners) in the API reference.
