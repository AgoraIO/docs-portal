...
[_kit getMessages:@"channel_name"
    ...
    if (response != nil) {
        ...
        for (AgoraRtmHistoryMessage *msg in response.messageList) {
...
To store a message on the server, set `storeInHistory` to `true` in the `PublishOptions` you pass to `publish()`. The following example shows the minimum code to store a message published to a message channel:
...
rtmClient_->publish(userId.c_str(), message.c_str(), message.size(), options,
            requestId);
```
