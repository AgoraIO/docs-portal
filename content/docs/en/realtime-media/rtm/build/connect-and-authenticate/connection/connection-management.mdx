...
join_option.features = [.presence, .metadata]
...
join_opt.features = AgoraRtmJoinChannelFeaturePresence | AgoraRtmJoinChannelFeatureMetadata;
...
var rtmConfig = RtmConfig( heartbeatInterval:30 );

try {
    var (status,client) = await RTM(appId, userId, config:rtmConfig);
    if (status.error == true) {
        print(status);
    } else {
        rtmClient = client;
...
```cpp
uint64_t requestId;
stream_channel->leave(requestId);
```
