...
proxyConfig.setProxyType(RtmProxyType.CLOUD_TCP);
...
val proxyConfig = RtmProxyConfig().apply {
    proxyType = RtmProxyType.CLOUD_TCP
}
...
proxyConfig.proxyType = .cloudTcp
...
proxy_config.proxyType = AgoraRtmProxyTypeCloudTcp;
...
```dart
const proxyConfig = RtmProxyConfig( proxyType: RtmProxyType.cloudTcp );
...
const client = createAgoraRtmClient(
 new RtmConfig({
   ...
   appId: 'your_appId',
   useStringUserId: true,
   proxyConfig: { proxyType: RtmProxyType.cloudTcp }, // Enable cloud proxy
 })
);
```
...
config.proxyConfig.proxyType = RTM_PROXY_TYPE_CLOUD_TCP;
```
