...
description: "Upgrade to the latest version of Voice Calling."
---
...
| API | v3.x | v4.x |
| --- | --- | --- |
| `setDefaultAudioRouteToSpeakerphone` | 1. You can only set the audio route before joining a channel. 2. This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of `setDefaultAudioRouteToSpeakerphone` to `true` or `false`, calling `setEnableSpeakerphone(false)` changes the audio route to the earpiece. | 1. You can set the audio route either before or after joining a channel. 2. This method is a steady API and can change the default audio route of the system. For example, after calling `setDefaultAudioRouteToSpeakerphone(true)` to set the initial audio route to the speakerphone, calling `setEnableSpeakerphone(false)` cannot change the audio route to the earpiece. |
...
See [Virtual Background](https://docs.agora.io/en/video-calling/advanced-features/virtual-background_android) to update the code in your app.
...
This section introduces the main changes of v4.x compared to v3.7.x in the following categories. You need to update the code of your app according to your business use-case.
  ...
  :::note
  If the version before upgrading is a version before v3.7.0, please check the [v3.x release notes](https://docs.agora.io/en/3.x/video-calling/introduction/release-notes_ios.md) to obtain the changes between this version and v3.7.0.
  :::
...
| API | v3.x | v4.x |
| --- | --- | --- |
| `setDefaultAudioRouteToSpeakerphone` | 1. You can only set the audio route before joining a channel. 2. This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of `setDefaultAudioRouteToSpeakerphone` to `YES` or `NO`, calling `setEnableSpeakerphone(NO)` changes the audio route to the earpiece. | 1. You can set the audio route either before or after joining a channel. 2. This method is a steady API and can change the default audio route of the system. For example, after calling `setDefaultAudioRouteToSpeakerphone(YES)` to set the initial audio route to the speakerphone, calling `setEnableSpeakerphone(NO)` cannot change the audio route to the earpiece. |
| `setEnableSpeakerphone` | After connecting external playback devices such as Bluetooth and wired headphones, calling `setEnableSpeakerphone` cannot switch the audio route to the speakerphone or earpiece. | Not recommended. |
...
-  Remove the callback in [Client.setClientRole]({{global.API_REF_WEB_ROOT}}/interfaces/iagorartcclient.html#setclientrole) and return a promise. In the Voice Calling v4.x, setting the user role as `audience` does not unpublish, and when calling `publish`, the SDK does not automatically setting the user role as `host`
...
-  Enable/disable a local track: The Voice Calling v4.x replaces the `Stream.muteAudio` and `Stream.muteVideo` methods with [ILocalTrack.setEnabled]({{global.API_REF_WEB_ROOT}}/interfaces/ilocaltrack.html#setenabled) for enabling or disabling a local audio or video track. This replacement changes the following SDK behaviors:
...
-  The Voice Calling v4.x replaces `Stream.switchDevice` with [IMicrophoneAudioTrack.setDevice]({{global.API_REF_WEB_ROOT}}/interfaces/imicrophoneaudiotrack.html#setdevice) and [ICameraVideoTrack.setDevice]({{global.API_REF_WEB_ROOT}}/interfaces/icameravideotrack.html#setdevice) for setting the media input device.

-  The Voice Calling v4.x replaces `Stream.setAudioOutput` with [IRemoteAudioTrack.setPlaybackDevice]({{global.API_REF_WEB_ROOT}}/interfaces/iremoteaudiotrack.html#setplaybackdevice) for setting the audio output device.
</PlatformStructured>
...
See [Get started](../get-started/get-started-sdk#project-setup) for more information about integrating the `agora_rtc_engine` 6.x SDK into your project.
...
Agora SDK v4.0.0 is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see see [Benefits and features](../product-overview.md).
