...
- After a network outage, the `onNetworkQuality` callback sometimes reported inaccurate network quality.
- If the receiver used the `AUDIO_SCENARIO_AI_CLIENT` audio scenario and the sender frequently stopped and resumed sending audio, the receiver occasionally heard no audio when network conditions were poor.
- Audio quality occasionally degraded when using a wired headset in certain audio routing configurations.
- Echo occasionally occurred in some Android emulator scenarios.
- After Google security hardening was enabled, the app occasionally crashed or became unresponsive (ANR) during audio interactions.
- On Android watches connected to Bluetooth headphones, the audio route could switch to the speaker.
- The app occasionally crashed during audio interactions.
- With Media Player caching enabled, playing a cached resource after a network disconnection could be delayed by about 10 seconds and return an error.
- When an app joined a channel in communication mode (`CHANNEL_PROFILE_COMMUNICATION`) and then called `updateChannelMediaOptions` to update channel media options, the channel information returned by NCS and RESTful APIs was incorrectly identified as interactive live streaming mode.
...
- After a network outage, the `networkQuality` callback sometimes reported inaccurate network quality.
- If the receiver used the `AUDIO_SCENARIO_AI_CLIENT` audio scenario and the sender frequently stopped and resumed sending audio, the receiver occasionally heard no audio when network conditions were poor.
- Audio quality occasionally degraded when using a wired headset in certain audio routing configurations.
- With Media Player caching enabled, playing a cached resource after a network disconnection could be delayed by about 10 seconds and return an error.
- When an app joined a channel in communication mode (`CHANNEL_PROFILE_COMMUNICATION`) and then called `updateChannelMediaOptions` to update channel media options, the channel information returned by NCS and RESTful APIs was incorrectly identified as interactive live streaming mode.
...
<include>../../video/reference/release-notes.mdx#web-sdk-v4-24-8</include>

<include>../../video/reference/release-notes.mdx#web-sdk-v4-24-7</include>

<include>../../video/reference/release-notes.mdx#web-sdk-v4-24-6</include>
