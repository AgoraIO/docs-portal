...
- After joining a channel with the `CHANNEL_PROFILE_COMMUNICATION` channel profile, calling `updateChannelMediaOptions` to update channel media options could unexpectedly switch the channel profile to interactive live streaming.
