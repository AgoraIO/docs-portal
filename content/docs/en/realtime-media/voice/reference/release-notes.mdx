...
:::note
Agora charges separately for this function. See [AI Noise Suppression unit pricing](pricing.md).
:::
...
When using the channel preload feature, Agora recommends sending the host's media stream immediately when calling the method to join the channel. This reduces the first frame rendering time. For details, see [Preload channels](https://docs.agora.io/en/voice-calling/best-practices/preload-channels). With this setting, users will start pulling streams as soon as they join the channel, which will generate charges. For details on how Agora charges for the services, see [Pricing](pricing.md).
