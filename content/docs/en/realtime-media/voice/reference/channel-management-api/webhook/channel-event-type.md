...
| `Agora-Signature` | The signature value generated by Agora using the customer key and the HMAC/SHA1 algorithm. You need to use the customer key and the HMAC/SHA1 algorithm to verify the signed value. See [Add signature verification](../../../build/receive-notifications.mdx) for details. |
| `Agora-Signature-V2` | The signature value generated by Agora using the customer key and the HMAC/SHA256 algorithm. You need to use the customer key and the HMAC/SHA256 algorithm to verify the signed value. See [Add signature verification](../../../build/receive-notifications.mdx) for details. |
...
|`channelName` |	String	|The name of the channel. |
|`ts`	| Number | The Unix timestamp(s) indicating when this event occurred on the Agora business server. |
|`lastUid`|Number| The ID of the last user to leave the channel. Note: If multiple users leave the channel at the same time, the Notifications service may return several `lastUid`s. 
...
| `clientSeq` | Number | Serial number identifying the order in which the events occur on the app client. It can be used to sort events for the same user. For details, see [Implement online user status tracking](../../../build/receive-notifications.mdx). |
