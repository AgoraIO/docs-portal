...
4. To test the server, follow the steps given in the [Enable Webhook notifications](#enable-webhook-notifications) section.
...
2. To test signature verification, follow the steps given in the [Enable Webhook notifications](#enable-webhook-notifications) section.
...
| `clientType` | Number | The type of services used by the host on Linux. Common return values include `3`: On-premise Recording, `10`: Cloud Recording. This field is only returned when platform is `6`. |
| `clientSeq` | Number | The sequence number, which is used to identify the order in which events occur on the app client. You can use the value of this field to sort the events of a user into chronological order. |
| `reason` | Number | The reason why the host leaves the channel. See the reason codes below. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
| `duration` | Number | The length of time (s) that the host stays in the channel. |

The `reason` field can be one of the following:
...
| `clientSeq` | Number | The sequence number, which is used to identify the order in which events occur on the app client. You can use the value of this field to sort the events of a user into chronological order. |
| `reason` | Number | The reason why the audience member leaves the channel. See the reason codes below. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
...
| `clientSeq` | Number | The sequence number, which is used to identify the order in which events occur on the app client. You can use the value of this field to sort the events of a user into chronological order. |
| `reason` | Number | The reason why a user leaves the channel. See the reason codes below. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
