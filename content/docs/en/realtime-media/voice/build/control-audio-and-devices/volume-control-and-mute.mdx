...
- [Custom video source](https://docs.agora.io/en/voice-calling/advanced-features/custom-video)
</PlatformStructured>
...
* To mute or unmute the local audio stream, call `muteLocalAudioStream`:
  ...
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="objective-c">objc</CodeBlockTabsTrigger>
...
The Voice SDK provides the ability to monitor the volume level of users using the `useVolumeLevel` hook. To get the audio volume level, pass the respective audio track (local or remote) to the hook:
