...
The token used by the subscribing bot for channel authentication. Required only when your project has [App Certificate](/en/realtime-media/video/core-concepts#app-certificate) enabled. Generate this token on your token server. For details, see [Token authentication](/en/realtime-media/video/build/deploy-token-server).
...
The target languages for translation. You can specify a maximum of 5 target languages for each source language. Refer to [Supported Languages](/en/realtime-media/speech-to-text/reference/supported-languages#real-time-translation) for details.
