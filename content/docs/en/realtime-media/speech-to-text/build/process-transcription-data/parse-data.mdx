...
:::info
When using the [ARES](/en/ai/models/asr/ares) provider, a single Protobuf `Text` message can carry multiple segments. For example, the `words[]` array in a transcription message or the `trans[]` array in a translation message can have a length greater than `1`. Do not assume that a single data stream message contains only one transcription or translation segment.
:::
...
When using the [ARES](/en/ai/models/asr/ares) provider, the JSON protocol uses `results[]` to explicitly carry one or more segments. For a single segment, `results[]` has a length of `1`. For a mixed result, a single message can contain both a stabilized prefix segment and a segment that may still change, in which case `results[]` has a length greater than `1`.
