...
Inject online media stream refers to injecting an online media stream in an ILS channel to share the stream with all users in the channel. The Agora RTC SDK provides a method for developers to inject an online mixed audio and video stream or an audio only stream to a channel.

### ILS

Enabled by either Agora’s RTC SDK or Voice SDK, ILS gives you full control over the streaming experience from a standard one-to-many stream to a highly-interactive live event.
...
The mediaplayer kit is a plug-in of the RTC SDK to play local and online media resources and publish the media streams to other users in an ILS channel.
