...
Enabled by either Agora’s Video SDK or Voice SDK, ILS gives you full control over the streaming experience from a standard one-to-many stream to a highly-interactive live event.
...
A token, also known as a dynamic key, is used for authentication when an app user joins a channel or logs onto Agora Signaling.
