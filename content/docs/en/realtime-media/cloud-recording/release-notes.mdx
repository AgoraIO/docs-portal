---
title: "Release notes"
description: "New features, improvements and resolved issues for Cloud Recording."
---
...
In order to meet the diverse recording needs of users, this release adds the [`streamMode`](/en/realtime-media/cloud-recording/reference/stream-mode) parameter in the `start` method to support [Individual Audio Non-transcoding Recording](/en/realtime-media/cloud-recording/build/individual-nontranscoding).

**Postpone Audio Mixing**
For voice chat use-cases, this release adds the [`appsCollection`](/en/api-reference/cloud-recording/restful#appscollection) and [`transcodeOptions`](/en/api-reference/cloud-recording/restful#transcodeoptions) parameters in the `start` method to support [Postpone Audio Mixing](../develop/individual-nontranscoding#implement-an-postpone-audio-mixing). If you enable this function, the recording service merges and transcodes the recorded files of all user IDs in the specified channel within 24 hours after the recording ends to generate an MP3/M4A/AAC file, and then uploads the recording file to your specified third-party cloud storage.

**A Combined audio and video file after individual recording**

When calling `start`, set the `streamMode` field of the [`recordingConfig`](/en/api-reference/cloud-recording/restful#recordingconfig) parameter to `standard`, you can get a M3U8 audio index file, a M3U8 video index file and a combined M3U8 audio and video index file. If VP9 encoding is used on the web side, a combined MPD audio and video index file are generated.
...
This release adds the [`extensionParams`](/en/api-reference/cloud-recording/restful#extensionparams) field to support the third-party cloud storage service to encrypt and tag uploaded recording files. This feature is applicable to Amazon S3 and Alibaba Cloud OSS.
### 2021.12.09
...
Adds support for converting the content and audio of a web page into a media stream and pushing it to the CDN during the web page recording. See the [`start`](/en/api-reference/cloud-recording/restful#start), [`update`](/en/api-reference/cloud-recording/restful#update) and [`query`](/en/api-reference/cloud-recording/restful#query) methods of the Agora Cloud Recording RESTful API for details.
...
In order to ensure the validity and completeness of page recording, this version supports page load timeout detection function. See [Web Page Load Timeout Detection](/en/realtime-media/cloud-recording/build/webpage-load-timeout).

**Pause the page recording**

This release adds the `onhold` parameter to the [`start`](/en/api-reference/cloud-recording/restful#start) and [`update`](/en/api-reference/cloud-recording/restful#update) methods, which is used to pause the recording when the web page recording starts or is in progress.

**Capture screenshots and recording in a recording process**

This release adds support for recording and taking screenshots in a recording process. Only the recording fee is charged. See [Capture Screenshots](/en/realtime-media/cloud-recording/build/screen-capture).
...
- The [`update`](/en/api-reference/cloud-recording/restful#update) method adds the `webRecordingConfig` parameter, which is used to update related parameters during a web page recording.
...
This release adds `maxVideoDuration` to the [`start`](/en/api-reference/cloud-recording/restful#start) method to set the maximum duration of the MP4 slice file generated by the web page recording.
...
This release adds the `region` parameter to the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method so you can geofence your cloud recording service. If you specify a region, the cloud recording server connects only to the Agora servers within that region. The following regions are supported:
...
- Adds the `region` parameter in the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method.
...
- Adds enumerators `5` and `6` for `decryptionMode` in the [`start`](/en/api-reference/cloud-recording/restful#start) method.
...
- Adds the `backgroundImage`, `defaultUserBackgroundImage` and `backgroundConfig` fields in the `transcodingConfig` parameter of the [`start`](/en/api-reference/cloud-recording/restful#start) method.
- Adds the `backgroundImage`, `defaultUserBackgroundImage` and `backgroundConfig` fields in the `clientRequest` parameter of the [`updateLayout`](/en/api-reference/cloud-recording/restful#updateLayout) method.
- Adds the `fileList` field in the [`web_recorder_stopped`](../reference/rest-api-overview#71-web_recorder_stopped) callback.

### 2021.02.05
This release raises the maximum resolution of video sources for [web page recording](/en/realtime-media/cloud-recording/build/webpage-mode). The recorded web page can now contain video that has a resolution of up to of 1920 × 1080.
...
Adds [web page recording](/en/realtime-media/cloud-recording/build/webpage-mode) mode. In this mode, the content and audio of a specified web page are recorded in a single file.
...
- Adds `"web_recorder_service"` as an extension service in the `extensionServiceConfig` parameter of the [`start`](/en/api-reference/cloud-recording/restful#start) method.
...
**API changes**
- [`start`](/en/api-reference/cloud-recording/restful#start) adds the `extensionServiceConfig` parameter for specifying the configurations of ApsaraVideo for VoD.
...
- In [`start`](/en/api-reference/cloud-recording/restful#start) adds the `unSubscribeVideoUids` and `unSubscribeAudioUids` parameters for setting up subscription blacklists.
- Adds [`update`](/en/api-reference/cloud-recording/restful#update) for updating the subscription lists during the cloud recording.
...
### 2020.04.17
Cloud Recording now supports taking screenshots of the video streams in individual recording mode. See [Capture Screenshots](/en/realtime-media/cloud-recording/build/screen-capture) for details.
...
In the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method, adds the `resourceExpiredHour` parameter for setting the time limit for all cloud recording method calls.
...
The RESTful API adds individual recording mode, which supports recording the audio and video of each user ID separately. See [Individual Recording](/en/realtime-media/cloud-recording/build/individual-mode) for details. Meanwhile, Agora provides the Audio & Video File Merging script, which you can use to merge the audio and video files generated in individual mode. See [Merge Audio and Video Files](/en/realtime-media/cloud-recording/build/merge-files) for details.
...
- [RESTful API Reference](/en/api-reference/cloud-recording/restful): Details of the RESTful API methods.
