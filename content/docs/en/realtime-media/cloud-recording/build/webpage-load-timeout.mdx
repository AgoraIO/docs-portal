...
The Agora web page load timeout detection function can only ensure that page elements are loaded correctly. If you need more comprehensive protection for web page recording, please refer to [Best Practices in Integrating Web Page Recording](/en/realtime-media/cloud-recording/build/webpage-best-practices).

## Implementation

You need to implement the web page load timeout detection function in two steps:

1. When calling the [`start`](/en/api-reference/cloud-recording/restful#start) method, set the web page load timeout limit through the `readyTimeout` parameter.
...
You need to set the web page load timeout limit through the `readyTimeout` parameter when calling the [`start`](/en/api-reference/cloud-recording/restful#start) method. The parameter settings are as follows:
...
- [`start`](/en/api-reference/cloud-recording/restful#start)
