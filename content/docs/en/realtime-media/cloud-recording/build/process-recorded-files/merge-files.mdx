...
description: "Merge each user's audio and video files into one file."
---
...
| `-p`   | Set the frame rate of the transcoded video. The default rate is 15 fps. The value range is from 5 fps to 120 fps. If `-p` is less than 5 fps, the video will be transcoded with the frame rate of 5 fps. For more details, see [How do I set the video profile of the recorded video?](/en/realtime-media/cloud-recording/build/customize-the-recording/recording-video-profile)      | NO              |
| `-r`   | Set the resolution of the transcoded video in the format of “width height”, for example, `-r 640 360`. For more details, see [How do I set the video profile of the recorded video?](/en/realtime-media/cloud-recording/build/customize-the-recording/recording-video-profile)         | NO              |
...
- Switching between different [AudioProfile](/en/api-reference/api-ref/cloud-recording#transcodingconfig) configuration at the streaming sender during a recording is not supported.
...
| :--------------- | :--- |
| Linux | <Slot name="linux" /> |
| Windows | <Slot name="windows" /> |
| macOS | <Slot name="macos" /> |
| iOS | <Slot name="ios" /> |
