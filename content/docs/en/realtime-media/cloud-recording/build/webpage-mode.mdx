...
Before recording, call the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method to apply for a resource ID. Note that you must set `scene` as `1`.
...
To enable [web page recording mode](/en/realtime-media/cloud-recording/build/webpage-mode), set `mode` to `web` when calling  [`start`](/en/api-reference/cloud-recording/restful#start). Use `extensionServiceConfig` to configure web page recording, and use `storageConfig` to configure your third-party cloud storage.
...
Before starting the process, call the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method to apply for a resource ID. Note that you must set `scene` as `1`.
...
When a recording and stream pushing finishes, call [`stop`](/en/api-reference/cloud-recording/restful#stop) to leave the channel and stop recording. To use Agora Cloud Web Page Recording again, you need to call the [`acquire`](/en/api-reference/cloud-recording/restful#acquire) method for a new resource ID.
