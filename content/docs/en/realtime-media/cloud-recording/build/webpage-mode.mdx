...
To enable [web page recording mode](/en/realtime-media/cloud-recording/build/webpage-mode), set `mode` to `web` when calling  [`start`](/en/api-reference/api-ref/cloud-recording#start). Use `extensionServiceConfig` to configure web page recording, and use `storageConfig` to configure your third-party cloud storage.
