...
1. Call `setDefaultAudioRoutetoSpeakerphone(true)` in the app. Audio routing is changed to speakerphone.
...
Video SDK provides APIs for managing input and output devices. Use these APIs to switch devices dynamically during Video Calling and implement hot-swap logic when devices are added or removed.
...
1. Call `SetDefaultAudioRouteToSpeakerphone(true)` in the game. Audio routing is changed to speakerphone.
