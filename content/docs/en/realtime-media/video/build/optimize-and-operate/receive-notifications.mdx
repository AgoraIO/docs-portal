...
| `Agora-Signature` | The signature value generated by Agora using the customer key and the `HMAC/SHA1` algorithm. |
| `Agora-Signature-V2` | The signature value generated by Agora using the customer key and the `HMAC/SHA256` algorithm. |

The request body includes the following fields:

| Field name | Type | Description |
| --- | --- | --- |
| `noticeId` | String | The ID of the notification callback. |
| `productId` | Number | The product ID. |
| `eventType` | Number | The type of event. |
| `notifyMs` | Number | The Unix timestamp (ms) when Agora sends the callback. |
...
| `clientType` | Number | The type of services used by the host on Linux. This field is only returned when `platform` is `6`. |
| `clientSeq` | Number | The sequence number used to identify the order in which events occur on the app client. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
...
This event type indicates that a host leaves the channel in the streaming profile.

#### 105 audience join channel

This event type indicates that an audience member joins the channel in the streaming profile.

#### 106 audience leave channel

This event type indicates that an audience member leaves the channel in the streaming profile.

#### 107 user join channel with communication mode

This event type indicates that a user joins the channel in the communication profile.

#### 108 user leave channel with communication mode

This event type indicates that a user leaves the channel in the communication profile.

#### 111 client role change to broadcaster

This event type indicates that an audience member switches their role to host.

#### 112 client role change to audience

This event type indicates that a host switches their role to audience member.
...
* For improved reliability of Notifications, your server may receive more than one notification callback for a single event.
