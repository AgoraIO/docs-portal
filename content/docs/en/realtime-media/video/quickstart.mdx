...
1. Download the latest version of Voice SDK from the [SDKs](../../../sdks.md) page and unzip it.
...
<Accordions>
  <Accordion title="Voice Calling workflow">

    ![Voice Calling workflow](/images/voice-sdk/get-started-sdk-voice.svg)
...
<Accordions>
  <Accordion title="Quick start sequence">
    ...
    ![Voice Calling call sequence](/images/video-sdk/quickstart-voice-call-sequence.svg)
...
Call `sharedEngine(withAppId:delegate:)` to create and initialize an `AgoraRtcEngineKit` instance. Provide your [App ID](manage-agora-account.mdx) and an `AgoraRtcEngineDelegate` implementation to [handle SDK events](#subscribe-to--events).
