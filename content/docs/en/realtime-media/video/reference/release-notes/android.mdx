---
title: Video SDK release notes (Android)
---
...
<section id="android-sdk-known-issues">
...
#### Android SDK v4.2.3 [#android-sdk-v423]

**Android 14 screen sharing issue**

On Android 14 devices such as the OnePlus 11, screen sharing might not be available when `targetSdkVersion` is set to `34`. For example, half of the shared screen might be black. Agora recommends setting `targetSdkVersion` to 34 or earlier. This can cause the screen sharing process to be interrupted when switching between portrait and landscape orientation. If the device asks whether to start recording the screen, confirm the prompt to resume screen sharing.

:::warning[Library conflict]
Starting from v4.5.0, both Video SDK and Signaling SDK v2.2.0 and later include the `libaosl.so` library. If you manually integrate Video SDK through a CDN and also use Signaling SDK, delete the earlier version of `libaosl.so` to avoid conflicts.
:::
...
3. **Channel media relay**
   ...
   | ----------------------------------------------------- | ---------------------------------- |
   | <Slot name="release-channel-relay-4-4-0-deleted" />     | `startOrUpdateChannelMediaRelay`   |
   | <Slot name="release-channel-relay-4-4-0-deleted-ex" /> | `startOrUpdateChannelMediaRelayEx` |
   | `onChannelMediaRelayEvent`                            | `onChannelMediaRelayStateChanged`  |

   <Slot for="release-channel-relay-4-4-0-deleted">
   ...
   <Slot for="release-channel-relay-4-4-0-deleted-ex">
...
<Accordions type="multiple" defaultValue="v220-1">

<Accordion title="v2.2.0" id="v220-1" headingLevel={3} value="v220-1">
...
<Accordion title="v2.1.0" id="v210-2" headingLevel={3} value="v210-2">
...
<Accordion title="v2.0.0" id="v200-3" headingLevel={3} value="v200-3">
...
<Accordions type="multiple" defaultValue="v120-1">

<Accordion title="v1.2.0" id="v120-1" headingLevel={3} value="v120-1">
