---
title: Video SDK release notes (Web)
---
...
<section id="web-sdk-known-issues">
...
- On Windows, when Chrome 122 or later uses the Web SDK to share an application window, switching the shared content to full-screen slide show mode does not update the receiving end to show the full-screen slide show. **Solution:** Share the entire screen instead of a specific window.
- Starting from Chrome 117, the `getStats()` method is no longer supported in SDK v4.9.3 and earlier. Calling this method returns `0` for all values because Chrome 117 removed the old `getStats()` WebRTC API, as described in the [Chrome documentation](https://developer.chrome.com/blog/getstats-migration). **Solution:** Upgrade to a later SDK version.
- On all AMD-based and some Intel-based Windows devices, the video transmission bitrate may be lower than the configured value when Chrome uses the H.264 codec. **Solution:** Configure the browser to use VP9, or disable hardware acceleration.
- Chrome 84 on macOS has a known issue where WebRTC frame rate can suddenly drop when using H.264. For details, see [Chromium issue 1203206](https://bugs.chromium.org/p/chromium/issues/detail?id=1203206&can=2&q=webrtc%20drop%20component%3ABlink%3EWebRTC%3EVideo) and [WebRTC issue 12704](https://bugs.chromium.org/p/webrtc/issues/detail?id=12704#c3). **Solution:** Use VP9 instead of H.264 on Chrome 84 and later on macOS.
- Acoustic Echo Cancellation (AEC) can fail when extra processing, such as spatial audio, is applied to the received remote audio stream because AEC cannot get its reference signal. For details, see [Chromium issue 687574](https://bugs.chromium.org/p/chromium/issues/detail?id=687574). **Solution:** Agora recommends that users wear headphones. To use speakers in this scenario, contact [technical support](mailto:support@agora.io).
- Chrome plans to deprecate and remove the Plan B dialect in WebRTC. For details, see the [Chrome feature description](https://www.chromestatus.com/feature/5823036655665152). If you use one of the following Video SDK versions, upgrade to the latest version:

  | Version | Impact |
  | --- | --- |
  | v2.5.0 to v2.9.0 or v3.0.0 to v3.6.1 | Calling `switchDevice` after publishing a stream causes video to freeze or audio to stop. |
  | v4.0.0 to v4.0.1 | The `publish` and `subscribe` methods have no effect. |

</Accordion>
<Accordion title="Safari">

- On Safari 17.2, the SDK cannot properly switch to the low-quality stream.
- On Safari on some Intel-based Mac devices, calling `enableDualStream` to enable H.264-encoded dual-stream mode can cause system lag when the small-stream resolution is less than one quarter of the large-stream resolution.
- On Safari 17, constraints passed to `getDisplayMedia` for a screen-sharing video stream do not take effect, resulting in blurry video.
- On macOS Ventura, the screen-sharing video resolution configured by the SDK can differ from the actual capture resolution in Safari. If the browser does not support the configured resolution, billing uses the actual capture resolution, which can be higher or lower because of browser restrictions.
- On Safari 15.4, setting the local audio track sample rate to 32,000 Hz ends the track and triggers the `AgoraRTCClient.on('track-ended')` event.
- On Safari 15.1, setting [`MediaStreamTrack.enabled`](https://developer.mozilla.org/en-US/docs/Web/API/MediaStreamTrack/enabled) to `false` while using H.264 crashes the browser tab.
- On Safari 14.0.1, audio can stutter.
- Safari 13 users might not hear other users.
- Safari 12.1 and earlier only support the H.264 codec.
- Safari 11 only supports video resolutions of 480p and higher.
- Safari does not expose output device information, so it does not support the `getPlayoutDevices` and `setAudioOutput` methods.
- On Safari, some properties returned by quality statistics APIs are `0`. For example, `sendPacketsLost` is `0` in the local audio track statistics returned by `getLocalAudioStats`.
- If **Auto-Play** is not enabled in Safari, stream playback has no audio. Call `navigator.mediaDevices.getUserMedia` to obtain device permissions before playing a stream.

  ![Safari Auto-Play setting](https://assets-docs.agora.io/images/video-sdk/safari-auto-play.png)

- On Safari 16.2 and 16.3 on macOS Ventura, after creating a screen-sharing video track with `createScreenVideoTrack`, calling `setEncoderConfiguration` to change the track resolution can produce a green screen. This is caused by a WebKit regression described in [WebKit bug 254652](https://bugs.webkit.org/show_bug.cgi?id=254652). **Solution:** Upgrade to macOS Ventura 13.3 and Safari 16.4.

</Accordion>
<Accordion title="Firefox">

- On Firefox 138, the receiver can see a black screen instead of video during real-time audio and video interaction. This is caused by a browser regression that prevents video stream decoding. For details, see [Bugzilla issue 1965831](https://bugzilla.mozilla.org/show_bug.cgi?id=1965831). **Workaround:** Agora has deployed a temporary configuration fix. The fix might not take effect in private browsing mode, where integration changes can be required. Contact [technical support](mailto:support@agora.io) for assistance.
- The Web SDK default minimum video bitrate does not take effect on Firefox.
- Firefox does not support sending SEI data in H.264 video streams.
- On Firefox 98 on Windows, H.264 keyframes might not be received immediately after subscribing to video, which delays first-frame rendering.
- When the Web SDK on Firefox communicates with the SDK on some devices, video on Firefox is rotated.
- Firefox does not support changing the frame rate, which is 30 fps by default.
- Firefox does not support setting the audio encoding bitrate.
- Setting the video profile on Firefox does not take effect on the following devices:

  - MacBook Pro (13-inch, 2016, two Thunderbolt 3 ports)
  - Windows 10 (MI)

- On Macs with an Apple M1 chip, Firefox does not support H.264. For details, see [Bugzilla issue 1686470](https://bugzilla.mozilla.org/show_bug.cgi?id=1686470).
- Firefox does not support getting `CodecType`, `sendFrameRate`, or `captureFrameRate`.

##### Other desktop restrictions [#other-desktop-restrictions]

- On Chrome 81 or later, Safari, and Firefox, device IDs are available only after the user grants media device permission. This restriction affects the `getMicrophones`, `getCameras`, and `getPlaybackDevices` methods.
- The `setPlaybackDevice` method is supported only on Chrome. Calling it on another browser throws the `NOT_SUPPORTED` error.
- The `setOptimizationMode` method and the `optimizationMode` parameter in `CameraVideoTrackInitConfig`, `ScreenVideoTrackInitConfig`, and `CustomVideoTrackInitConfig` are supported only on Chrome.
- Calling `createScreenVideoTrack` to enable screen sharing has the following restrictions:

  - Chrome 58 or later supports screen sharing. Before Chrome 72, install the [Google Chrome Extension for Screen Sharing](/en/realtime-media/video/build/capture-and-render-video/screen-sharing), obtain its `extensionId`, and pass that value when creating the screen-sharing video track. On Chrome 72 or later, call `createScreenVideoTrack` directly.
  - Firefox 56 or later supports screen sharing. Firefox on Windows does not support `application` mode.
  - Edge 80 or later supports screen sharing on Windows 10 and later.
  - Safari 13 or later supports screen sharing on macOS. Users can share only the entire screen.
  - Chrome 74 or later supports sharing local playback audio while sharing a screen. Set the `withAudio` parameter when calling `createScreenVideoTrack`. On Windows, audio can be shared when sharing the entire screen or a Chrome tab, but not an application window. On macOS, audio can be shared only with a Chrome tab.

- The `setBeautyEffect` method is supported only on Chrome 65 or later, Safari 12 or later, and Firefox 70.0.1 or later.
...
##### iOS known issues [#ios-known-issues]

<Accordions type="multiple">
<Accordion title="iOS 16.0: Video stretches after the screen orientation changes">

**Impact:** All browsers and apps that use WKWebView on iOS 16.0, including Safari and Chrome.

**Reason:** A regression in the WebKit WebRTC module on iOS 16.0 prevents RTP streaming with coordination of video orientation (CVO) from working properly.

**Workaround:** Add a prompt to warn users about this issue.

</Accordion>
<Accordion title="iOS 15.1.x: The browser crashes when sending H.264 video">

**Impact:** All browsers and apps that use WKWebView on iOS 15.1.x, including Safari and Chrome.

**Details:** If `codec` is set to `'h264'` when calling `createClient`, the browser crashes after sending the video stream.

**Reason:** A regression in the WebKit video encoder on iOS 15.1.x. For details, see [WebKit bug 231505](https://bugs.webkit.org/show_bug.cgi?id=231505).

**Workaround:** Use VP9 for video encoding:
...
<Accordion title="iOS 15.x: Actual H.264 sending bitrate is lower than expected">

**Impact:** Safari on iOS 15.x.

**Reason:** **WebRTC H264 LowLatency encoder** is enabled by default in Safari on iOS 15.x. For details, see [WebKit bug 238366](https://bugs.webkit.org/show_bug.cgi?id=238366).

**Workaround:** Use either of the following approaches:

- Use VP9 for video encoding:

  ```javascript
  createClient({ codec: 'vp9', mode });
  ```

- On the iOS device, go to **Settings** > **Safari** > **Advanced** > **Experimental Features**, then disable **WebRTC H264 LowLatency encoder**.

</Accordion>
<Accordion title="iOS 15.0 to 15.3: Low audio volume">

**Impact:** All browsers and apps that use WKWebView on iOS 15.0 to 15.3, including Safari and Chrome.

**Details:** After the local user subscribes to and plays a `RemoteAudioTrack`, audio can be routed to the earpiece instead of the speaker and be difficult to hear.

**Reason:** A regression in the WebKit audio module on iOS 15.0 to 15.3. For details, see [WebKit bug 230902](https://bugs.webkit.org/show_bug.cgi?id=230902).

**Workaround:** Use `WebAudio` to play the audio and `GainNode` to increase its volume:

1. Upgrade to Web SDK v4.9.0 or later.
2. Set the private SDK parameter `REMOTE_AUDIO_TRACK_USES_WEB_AUDIO` to `true`:

   ```javascript
   function isIOS15(ua) {
     // Determine whether the iOS version is 15 from the user agent.
   }

   if (isIOS15(navigator.userAgent)) {
     // @ts-ignore: setParameter is a private SDK API.
     AgoraRTC.setParameter('REMOTE_AUDIO_TRACK_USES_WEB_AUDIO', true);
   }
   ```

</Accordion>
<Accordion title="iOS 15.x: Video playback goes black">

**Impact:** All browsers and apps that use WKWebView on iOS 15.x, including Safari and Chrome.

**Details:** If video is played in a DOM node and CSS properties such as `transform` or `animation` are applied to the `video` element or one of its parents, redrawing the video rendering area can make the video go black.

**Reason:** A regression in the WebKit video renderer on iOS 15.x. For details, see [WebKit bug 230902](https://bugs.webkit.org/show_bug.cgi?id=230902).

**Workaround:** Upgrade to Web SDK v4.7.3 or later and minimize changes to the CSS properties of the `video` element and its parent elements.

</Accordion>
<Accordion title="iOS 15.x: Bluetooth headset audio can be significantly distorted">

**Impact:** All browsers and apps that use WKWebView on iOS 15.x, including Safari and Chrome.

**Reason:** A regression in the WebKit audio playback module on iOS 15.x. For details, see [WebKit bug 231422](https://bugs.webkit.org/show_bug.cgi?id=231422).

**Workaround:** Add a prompt to warn headset users about possible audio distortion.

</Accordion>
<Accordion title="Backgrounding the browser or app stops audio streaming">

**Impact:** Safari on iOS 15.x, and browsers and apps that use WKWebView on iOS 14.4 to 15.x.

**Reason:**

- In Safari, [WebKit bug 231105](https://bugs.webkit.org/show_bug.cgi?id=231105) causes the `AudioContext` to stop processing audio after the browser enters the background.
- In browsers and apps that use WKWebView, iOS prohibits microphone use while the app is in the background. For details, see [Microphone gets muted in background](https://developer.apple.com/forums/thread/689182).

**Workaround:** There is no workaround for browsers and apps that use WKWebView. For Safari on iOS 15.x:

1. Upgrade to Web SDK v4.7.3 or later.
2. Set `bypassWebAudio` to `true` when calling `createMicrophoneAudioTrack`:
   ...
   This workaround prevents the SDK from processing the local audio stream through `WebAudio`, and disables audio mixing with `MixingAudioTrack`.

</Accordion>
<Accordion title="iOS 15.x: Media playback might not resume after a system interruption">

**Impact:** All browsers and apps that use WKWebView on iOS 15.x, including Safari and Chrome.

**Reason:** A phone call, another real-time interaction app, Siri, or an alarm can set `video` and `audio` elements to `paused`. After the interruption, calling `HTMLMediaElement.play` might not resume playback. For details, see [WebKit bug 232599](https://bugs.webkit.org/show_bug.cgi?id=232599) and [WebKit bug 226698](https://bugs.webkit.org/show_bug.cgi?id=226698).

**Workaround:** Upgrade to Web SDK v4.7.3 or later. The SDK attempts to resume playback after the interruption. Agora also recommends prompting users to refresh the page.

</Accordion>
<Accordion title="iOS 15.x and 16.x: Noise after joining a channel on certain devices">

**Impact:** Chrome and Safari on the following iPad and iPhone models:
...
- Audio routing can change randomly. Audio can be routed to the speaker when a headset is connected, or to the earpiece when no headset is connected.
- If `getUserMedia` is called twice to get two tracks of the same media type, the first track can become muted or black.
- After a user switches to another app that uses the microphone or camera, such as Siri or Skype, and then switches back, audio sampling or video capture can fail.

</Accordion>
</Accordions>

##### iOS known limitations [#ios-known-limitations]

<Accordions type="multiple">
<Accordion title="createScreenVideoTrack is not supported">

iOS Safari and WKWebView do not support `mediaDevices.getDisplayMedia`.

</Accordion>
<Accordion title="setBeautyEffect is not supported">

WebGL support on iOS Safari and WKWebView is insufficient, and the image enhancement algorithm can reduce system performance below acceptable levels.

</Accordion>
<Accordion title="IBufferSourceAudioTrack.seekAudioBuffer is not supported">

`WebAudio` on iOS does not support this method.

</Accordion>
<Accordion title="H.264 video at 1080p or higher cannot be sent">

The Web SDK uses the H.264 Baseline Profile for negotiation, so iOS does not support encoding and sending video at 1080p or higher.

</Accordion>
<Accordion title="Low-quality stream bitrate and resolution are restricted">

When sending a low-quality stream on iOS Safari, `LowStreamParameter.bitrate` cannot be set, and the low-quality stream resolution must be proportional to the high-quality stream resolution. iOS Safari and WKWebView do not support setting the frame rate with `RTCRTPSender.setParameters`. After the resolution is compressed with `scaleResolutionDownBy`, the low-quality stream remains proportional to the high-quality stream.

</Accordion>
<Accordion title="encodeDelay is not supported">

The `encodeDelay` property cannot be calculated through the WebRTC `getStats` method on iOS.

</Accordion>
<Accordion title="Backgrounding the browser or app makes the user's video black">

iOS `AVCaptureSession` prohibits camera use while the app is in the background. For details, see [`AVCaptureSessionInterruptionReason`](https://developer.apple.com/documentation/avfoundation/avcapturesessioninterruptionreason/avcapturesessioninterruptionreasonvideodevicenotavailableinbackground?preferredLanguage=occ) and [WebRTC issue 4294](https://bugs.chromium.org/p/webrtc/issues/detail?id=4294).

</Accordion>
<Accordion title="RemoteAudioTrack.setVolume does not change playback volume">

iOS and iOS WebView do not support changing media volume through `HTMLMediaElement.volume`.
...
##### Android known issues [#android-known-issues]

<Accordions type="multiple">
<Accordion title="Android 12: Video distortion on Chrome with hardware acceleration enabled">

**Impact:** Chrome or a Chromium-based browser version 97 or earlier on certain Android 12 devices, including Pixel 3 and Pixel 4.

**Details:** Video distortion can occur if the browser enables WebRTC H.264 or VP9 hardware acceleration by default.

**Reason:** A regression in the Chromium WebRTC video encoder. For details, see [Chromium issue 1237677](https://bugs.chromium.org/p/chromium/issues/detail?id=1237677).

**Workaround:** Upgrade to Chrome 97 or later.

</Accordion>
<Accordion title="Video sending bitrate does not reach the configured value">

**Impact:** Certain Android devices, including Xiaomi and OnePlus devices.

**Reason:** At some video frame rates, the hardware encoder does not reach the configured bitrate.

**Workaround:** Try setting the frame rate to 30 fps. The bitrate is often lower at 15 fps, but using 30 fps can affect performance.

</Accordion>
<Accordion title="WeChat blocks autoplay of inaudible media">

**Impact:** The WeChat browser using the Chromium 89 engine.

**Details:** Autoplay of inaudible media remains blocked even after the user interacts with the page to resume video playback.

**Reason:** The WeChat browser implements a custom autoplay policy.

**Workaround:**

1. Upgrade to Web SDK v4.6.0 or later.
2. Listen for the `AgoraRTC.onAutoplayFailed` event and ask the user to click the page to resume playback:

   ```javascript
   AgoraRTC.onAutoplayFailed = () => {
     window.alert('Please click the page to resume playback');
   };
   ```
...
**Impact:** Certain Xiaomi and OnePlus devices.

**Details:** When the Bluetooth headset starts capturing local audio, remote audio can be lost.

**Reason:** A Chromium audio issue related to Bluetooth profile switching.

**Workaround:** Add a prompt to warn Bluetooth headset users about possible remote audio loss.

</Accordion>
<Accordion title="Switching the audio output device while sending audio has no effect">

**Impact:** Chromium-based browsers on Android devices.

**Details:** If the user switches from the speaker to a Bluetooth headset while sending local audio, remote audio can continue to play from the speaker.

**Reason:** Android controls audio routing at the operating system level, so Chromium cannot correct the behavior. For details, see [Chromium issue 1317548](https://bugs.chromium.org/p/chromium/issues/detail?id=1317548&q=bluetooth&can=2).

</Accordion>
<Accordion title="Issues on specific Android devices">

- On devices with MediaTek chips, the Web SDK cannot encode and send H.264 video streams.
- On devices with Huawei HiSilicon Kirin chips, Chrome versions earlier than 88 cannot encode and send H.264 video streams.
- On a OnePlus 6 receiving video in Chrome, the video can freeze when the screen turns off.
- HarmonyOS does not support sending a 180p video stream.

</Accordion>
<Accordion title="Other Android known issues">

- On some Android devices, device labels might not be available.
- On some Android devices, tracks can end when audio and video are interrupted by a system phone call or another calling app. To resume the call, recapture the audio and video.
- On Android Chrome, the Web SDK cannot send both high-quality and low-quality streams in H.264.
- On Android Chrome earlier than 90, `getVolumeLevel` returns `0` even though the user can hear audio.
- On Android System WebView versions 55 to 75, `decodeFrameRate` remains `0`.
- On certain OPPO devices running Android 11, including Reno4 and A11, users experience black video and no audio because WebRTC is not supported.

</Accordion>
</Accordions>

##### Android known limitations [#android-known-limitations]

<Accordions type="multiple">
<Accordion title="createScreenVideoTrack is not supported">

Mobile browsers and WKWebView do not implement `mediaDevices.getDisplayMedia`.

</Accordion>
<Accordion title="setBeautyEffect is not supported">

The image enhancement algorithm can reduce mobile device performance below acceptable levels.
...
<section id="web-sdk-v4-24-6">

<Accordion title="v4.24.6" id="v4246" headingLevel={3} value="v4246">

Released on July 13, 2026.

This release adds automatic receiving and playback of remote audio and improves audio and connection stability in Safari and iOS environments.

#### New features [#new-features-47]

- **Automatically receive and play remote audio after joining a channel**

  This release adds the `autoReceiveAndPlayAudio` join option. When enabled, the SDK automatically receives and attempts to play remote audio after a user joins a channel, reducing the delay before users hear remote audio.

  This feature is useful for voice chat rooms and online meetings where users need to hear audio as soon as possible after joining, audience clients in live streaming that receive channel audio by default, and scenarios where remote audio first-frame latency is critical.
...
Upgrade to v4.24.6 if your app:

- Uses Safari or iOS browsers for real-time audio or video
- Uses Web Audio and cannot resume audio after autoplay is blocked
- Needs users to hear remote audio sooner after joining a channel
...
#### Upgrade considerations [#upgrade-considerations-1]
...
- **Domain allowlist change**
  ...
  If you upgrade to v4.24.5, update your firewall allowlist so the SDK continues to work in restricted-network environments. See [Firewall requirements](/en/realtime-media/video/reference/firewall).
...
As of v4.13.0, the SDK supports using media processing extensions on remote audio and video tracks (extension support for local audio and video tracks was added in [v4.10.0](#v410-4)).
...
:::info
Agora charges additionally for this extension. See [Pricing](/en/realtime-media/video/reference/pricing-legacy#ai-noise-suppression-pricing).
:::
...
<Accordions type="multiple" defaultValue="v100-beta2">

<Accordion title="v1.0.0-beta2" id="v100-beta2" headingLevel={3} value="v100-beta2">
...
<Accordions type="multiple" defaultValue="v210-1">
...
<Accordions type="multiple" defaultValue="v100-2">

<Accordion title="v1.0.0" id="v100-2" headingLevel={3} value="v100-2">
...
<Accordions type="multiple" defaultValue="v100-beta">

<Accordion title="v1.0.0-beta" id="v100-beta" headingLevel={3} value="v100-beta">
...
<Accordions type="multiple" defaultValue="v102-beta">

<Accordion title="v1.0.2-beta" id="v102-beta" headingLevel={3} value="v102-beta">
