...
You can implement different types of Media Push depending on the live streaming use-case requires transcoded or non-transcoded streaming. Generally speaking, in the case of multiple-hosts live streaming, you need to enable transcoding when using Media Push RESTful APIs; in the case of single-host live streaming, only one media stream is pushed to the CDN, so you do not need to enable transcoding. For details, see [Media Push Features](../index#key-features).
...
| transcodeOptions.audioOptions                              | (Optional) JSON Object | The audio transcoding configuration of the Converter. See [audioOptions](../reference/restful-type-definition#audiooptions) for details.
