...
| `volumes` |  (Optional) JSON Array | Set the volume of users participating in the mixing.<li>This field takes effect only when mixing user IDs are specified (set `rtcStreamUids`).</li><li>You can call the [`Update`](../build/restful-api#update-update-the-converter) method to adjust the volume of the specified user. Each time the `Update` request is successful, the volume of other users in the mixing is reseted to the default value (`100`).</li> |
