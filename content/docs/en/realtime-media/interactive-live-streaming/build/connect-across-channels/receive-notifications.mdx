...
4. In the `main.go` file, replace your imports with the following:
...
5. Add the following code after the list of imports:
...
6. In `main.go`, add the following code after you read the request body:
...
7. To test the server, follow the steps given in the [Enable notifications](#enable-notifications) section.

8. When you receive an event from the console, and if the signature matches, the event details are displayed in your browser.
...
| `channelName` | String | The name of the channel. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
| `lastUid` | Number | The ID of the last user to leave the channel. Note: If multiple people leave the channel at the same time, Agora message notification may return different `lastUid`s. You can choose any one of them. |
...
| `platform` | Number | <Slot name="platform" /> |
| `clientType` | Number | <Slot name="clienttype" /> |
...
| `clientType` | Number | <Slot name="clienttype" /> |
| `clientSeq` | Number | The sequence number, which is used to identify the order in which events occur on the app client. You can use the value of this field to sort the events of a user into chronological order. |
| `reason` | Number | <Slot name="reason" /> |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
...
The reason why the host leaves the channel:

- 1: The host quits the call.
- 2: The connection between the app client and the Agora RTC server times out, which occurs when the Agora SDRTN® does not receive any data packets from the app client for more than 10 seconds.
...
The reason why the audience member leaves the channel:
...
The reason why a user leaves the channel:
...
A webhook is a user-defined callback over HTTPS that allows your game or back-end system to receive notifications when certain events occur. Agora calls your webhook endpoint from its servers to send notifications about Interactive Live Streaming events. With Notifications, you can subscribe to Interactive Live Streaming events and receive notifications in real time.
