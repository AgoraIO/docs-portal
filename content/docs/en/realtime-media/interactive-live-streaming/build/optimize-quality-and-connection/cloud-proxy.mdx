...
1. Configure the enterprise firewall to allow traffic from the IP address and port ranges in the [Agora Allowed IP List](../../reference/cloud-proxy-allowed-ips.md).
...
2. To stop using Cloud proxy, call `setCloudProxy` and set `proxyType` to `TRANSPORT_TYPE_NONE_PROXY`.

3. To update the current `proxyType`, call `setCloudProxy(TRANSPORT_TYPE_NONE_PROXY)` first, then call `setCloudProxy` again with the new `proxyType`.
...
2. To stop using Cloud proxy, call `setCloudProxy` and set `proxyType` to `noneProxy`.

3. To update the current cloud proxy type, call `setCloudProxy(noneProxy)` first, then call `setCloudProxy` again and set the cloud proxy type.
...
3. Video SDK provides a `client.on("is-using-cloud-proxy")` event that triggers the callback after the media stream is successfully published locally, to inform whether the current media stream is forwarded by the cloud proxy service.
...
4. To stop using the proxy server, call `stopProxyServer` after leaving the channel.
...
2. To stop using Cloud proxy, call `setCloudProxy` and set `proxyType` to `NONE_PROXY`.

3. To update the current cloud proxy type, call `setCloudProxy(NONE_PROXY)` first, then call `setCloudProxy` again and set the cloud proxy type.
