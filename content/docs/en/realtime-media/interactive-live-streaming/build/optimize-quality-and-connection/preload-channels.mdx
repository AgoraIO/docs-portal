...
To speed up the subsequent joining of the channel, use a [wildcard token](/en/realtime-media/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens) in the `token` parameter of the `preload` method.
...
- Before entering the channel
    ...
    To speed up joining the channel, use a [wildcard token](/en/realtime-media/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens) in the `token` parameter of the `join` method.
...
- Entering the channel
    ...
    Joining a channel incurs additional fees due to the transmission of media streams. See [Pricing](/en/realtime-media/interactive-live-streaming/reference/pricing#cost-calculation) for details.
...
2. Use a wildcard token.
    ...
    To speed up the process of users joining a channel, use a [wildcard token](/en/realtime-media/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens). Generate the token on your server and pass it to the client for authentication.
