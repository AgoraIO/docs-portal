...
Call `sharedEngine(withAppId:delegate:)` to create and initialize an `AgoraRtcEngineKit` instance. Provide your [App ID](/en/introduction/account) and an `AgoraRtcEngineDelegate` implementation to [handle SDK events](#subscribe-to--events).
...
<Accordions>
<Accordion title="Quick start sequence">
![Quick start sequence](https://assets-docs.agora.io/images/video-sdk/quick-start-sequence-macos.svg)
</Accordion>
</Accordions>
...
![image](https://assets-docs.agora.io/images/video-sdk/ils-quick-start-sequence-web.svg)
...
3. Open your browser and navigate to the URL displayed in your terminal, for example, `http://localhost:5173`.
  ...
  ![image](https://assets-docs.agora.io/images/video-sdk/quickstart-ui-web-ils.png)

4. On a second device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) or clone the [sample project on Github](https://github.com/AgoraIO/API-Examples-Web) to join the same channel and test the following use-cases:
...
3. Use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
For real-time communication, create an `IRtcEngine` object by calling `createAgoraRtcEngine` and then call `initialize` with `RtcEngineContext` to specify the [App ID](/en/introduction/account). Call `registerEventHandler` to register a custom [event handler](#subscribe-to--events) for managing user interactions within the channel.
...
3. Invite a friend to run the demo app on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
1. On a second target device, repeat the previous steps to install and launch the app. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel and test the following use-cases:
...
4. Invite a friend to install and run the app on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel. Once your friend joins the channel, you can see and hear each other.
...
![image](https://assets-docs.agora.io/images/video-sdk/video-call-ui-unity.png)
...
5. Invite a friend to run the demo game on a second device. Use the same `_appID_`, `_token`, and `_channelName` to join. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
For real-time communication, initialize an `IRtcEngine` instance and set up an event handler to manage user interactions within the channel. Use `RtcEngineContext` to specify [App ID](/en/introduction/account), and custom [event handler](#subscribe-to--events), then call `RtcEngineProxy->initialize(RtcEngineContext)` to initialize the engine, enabling further channel operations.
...
4. Invite a friend to run the demo game on a second device. Alternatively, use the [Web demo](https://webdemo-global.agora.io/example/quickStart/videoAndVoiceCalling/index.html) to join the same channel.
...
| :--- | :----- | :----- | :------------ |
| 1 | **Set Show Mouse Cursor** | Native* | (Optional) Set whether to display the mouse cursor. Check to display it. Note: This node is available on Windows and macOS only. If the node is not retrieved at creation time, uncheck **Context Sensitive**. ![image](https://assets-docs.agora.io/images/video-sdk/quickstart-implementation-blueprint-3.png) |
