...
Since v1.9.4, IoT SDK establishes RDT connections automatically (`auto_connect_rdt` defaults to `true`), so you only need to set `enable_rdt` to `true` in `rtc_channel_options_t` before you call `agora_rtc_join_channel` to use RDT. Set `enable_rdt` to `false` if you don't want to use RDT at all.
