...
The Internet of things (IoT) connects physical objects with sensors, processors, and software to enable data exchange with other devices. In related applications, such as apps for smart doorbells and remote monitoring, IoT devices send or receive audio and video streams, and exchange signaling messages, to perform their function. Agora IoT SDK gives you the ability to enable live voice and video streams, and reliable signaling, on IoT devices on a variety of platforms and for many use cases, while also offering a compact SDK size, low memory usage, and low power consumption.

This page shows you how to integrate IoT SDK into your own project from scratch: initialize the SDK, transmit media streams over RTC, transmit signaling messages over RTM, and release resources when you are done. If you only want to see IoT SDK working before you integrate it, use the [Quickstart](quickstart.mdx) instead.
...
* Applied for and activated a device [license](build/authenticate-and-secure-channels/license.mdx). IoT SDK automatically verifies the license when a device joins a channel.
* A development machine, which can be a computer or an embedded device. This page uses a computer running Ubuntu 18.04 as an example. For the full list of supported operating systems and toolchains, see [Supported platforms](reference/supported-platforms.mdx).
* An Agora [account](/en/introduction/account) and [project](/en/introduction/account).
* A [temporary RTC token](/en/introduction/account#generate-temporary-tokens) and a temporary RTM token generated for your project. RTC tokens and RTM tokens use different generation mechanisms and scopes, so you cannot use one in place of the other.

## Project setup

[Download](/en/api-reference/sdks?platform=linux) the SDK package that matches your development environment, and link the IoT SDK library into your own C project. See [Quickstart](quickstart.mdx) for the download and extraction commands if you have not already done this.

## Initialize the SDK

Call `agora_rtc_init` once, before any other IoT SDK call, to initialize the engine. The call takes your App ID, an event handler, and a service options struct:
...
If your device only sends and receives signaling messages and does not transmit media, you can pass `0` instead of an event handler. To transmit media streams, populate the event handler with callbacks as shown in [Listen for events](#listen-for-events).
...
Replace `DEFAULT_AREA_CODE`, `DEFAULT_SDK_LOG_PATH`, and `AGORA_APP_ID_FOR_TEST` with values for your own project. For details on `area_code`, see [Geofencing](build/manage-connections-and-quality/geofencing.mdx).

### Create a connection and join a channel

Call `agora_rtc_create_connection` to create a connection, then configure `rtc_channel_options_t` and call `agora_rtc_join_channel` to join a channel on that connection:
...
A single connection can join multiple RTC channels. Every channel the connection has joined receives the audio and video you send on that connection. IoT SDK triggers `on_join_channel_success` when the join succeeds.
...
:::info
The RTSA Lite SDK does not capture or encode audio and video itself. Your device firmware must interface with the microphone and camera hardware, capture raw data, and encode it before sending. See [Codec](build/configure-media/codec.mdx) for supported formats.
:::
...
Call `agora_rtc_login_rtm` to log in to the RTM system on the device, passing an RTM user ID, an RTM token, and an `agora_rtm_handler_t` populated with your signaling callbacks:
...
:::info
RTM tokens and RTC tokens use different generation mechanisms and scopes. You cannot use an RTC token to log in to RTM, or an RTM token to join an RTC channel.
:::

### Send and receive signaling messages

Implement the RTM callbacks to receive messages and monitor login and send status:
...
When you are done sending signaling messages, log out to release RTM resources:
