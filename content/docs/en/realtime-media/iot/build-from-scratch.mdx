...
You can join multiple RTC channels using a single connection. When you send audio or video on a connection, IoT SDK delivers it to every channel joined on that connection. IoT SDK triggers `on_join_channel_success` when the join succeeds.
...
:::info
IoT SDK does not capture or encode audio and video itself. Your device firmware must interface with the microphone and camera hardware, capture raw data, and encode it before sending. See [Codec](build/configure-media/codec.mdx) for supported formats.
:::
...
Call `agora_rtc_login_rtm` to log in to Signaling on the device, passing an RTM user ID, an RTM token, and an `agora_rtm_handler_t` populated with your signaling callbacks:
...
:::info
RTM tokens and RTC tokens use different generation mechanisms and scopes. You cannot use an RTC token to log in to Signaling, or an RTM token to join an RTC channel.
:::
