...
| Feature | Description |
|---|---|
| RTSA SDK interoperability | Supports intercommunication between IoT SDKs using multiple audio and video encoding formats. See `audio_data_type_e` and `video_data_type_e`. |
| RTC SDK interoperability | Supports bidirectional intercommunication between IoT SDK and RTC Server SDK, RTC Native SDK, RTC third-party framework SDKs, and RTC Web SDK. See [Interoperate with RTC SDK](build/communicate-with-other-sdks/interoperate-with-rtc-sdk.mdx). |
| Uplink bandwidth prediction and bitrate adaptation | Proposes target bitrate recommendations based on a bandwidth prediction algorithm, and reports whether to increase or decrease bitrate to boost throughput and avoid network congestion. See [Bitrate adaptation](build/manage-connections-and-quality/bitrate-adaptation.mdx). |
| Key frame | Supports requesting local key frames to improve encoding and decoding efficiency and reduce data storage footprint. See [Key frame](build/manage-connections-and-quality/key-frame.mdx). |
