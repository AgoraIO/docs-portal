...
The following figure illustrates the workflow when Notifications is enabled for the specific Broadcast Streaming events you subscribe to:
...
2. To test signature verification, follow the steps given in the [Enable notifications](#enable-notifications) section.
...
| `channelName` | String | The name of the channel. |
| `ts` | Number | The Unix timestamp (s) when the event occurs on the Agora RTC server. |
| `lastUid` | Number | <Slot name="lastuid" /> |

<Slot for="lastuid">

The ID of the last user to leave the channel.

:::info
If multiple people leave the channel at the same time, Agora message notification may return different `lastUid`s. You can choose any one of them.
:::
