...
1. Enable the plugin
  ...
  2. Inject the plugin into the audio processing pipeline by calling the SDK's <Link to = "{{global.API_REF_WEB_ROOT}}/interfaces/imicrophoneaudiotrack.html#pipe">`pipe`</Link> method and specify the `processorDestination` property.
