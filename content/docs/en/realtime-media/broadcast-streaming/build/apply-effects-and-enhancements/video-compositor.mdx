...
description: "Integrate the Video Compositor extension to combine multiple local video streams"
---
...
Try out the [online demo](https://webdemo-global.agora.io/example/plugin/videoCompositor/index.html) to experience the Video Compositor extension.
...
![Video compositor extension technology](https://assets-docs.agora.io/images/extensions-marketplace/video-compositor-tech.png)
...
2. Connect the pipeline between each video track and its corresponding input layer, injecting the video stream into the input layer. The compositor combines all input layers.
3. Connect the pipeline between the compositor and the local video track, then output the combined video to the SDK.

## Prerequisites

Ensure that you have implemented the [SDK quickstart](../../index) in your project using the Web SDK version 4.12.0 or later.
...
![Video compositor use case](https://assets-docs.agora.io/images/extensions-marketplace/video-compositor-usecase.png)
...
- Two local images.
- Source video track 1: Created from the video stream captured by the camera, with the background removed using the [Virtual Background extension](/en/realtime-media/video/build/apply-video-effects/virtual-background).
...
1. Integrate the [Virtual Background](/en/realtime-media/broadcast-streaming/build/apply-effects-and-enhancements/virtual-background#integrate-the-virtual-background-extension) extension. Ensure that you understand the [considerations](/en/realtime-media/broadcast-streaming/build/apply-effects-and-enhancements/virtual-background#considerations).

2. Run the following command to integrate the [Video Compositor extension](https://www.npmjs.com/package/agora-extension-video-compositor) into your project using npm:

   ```bash
   npm install agora-extension-video-compositor
   ```

3. Import the Video Compositor extension in either of the following ways:
...
2. Create the input layers of the image and video tracks in order, from bottom to top. The layers created later overlay the earlier ones. In the following code, the screen-sharing image layer is at the bottom, and the image of the source video track 2 is at the top layer:
...
- For a working example, check out the [Video Compositor extension](https://webdemo.agora.io/example/plugin/videoCompositor/index.html?_gl=1*1437x8t*_gcl_au*MTM4NzU5ODkyNy4xNzE5NTgxMTUy*_ga*MjA2MzYxMjY4Mi4xNzAzMDczMjA1*_ga_BFVGG7E02W*MTcxOTY1MDQ3NS4zNTQuMC4xNzE5NjUwNDc1LjAuMC4w).
