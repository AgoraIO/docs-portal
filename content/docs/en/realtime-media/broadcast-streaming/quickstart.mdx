...
1. Use the following code to handle runtime permissions in your Android app. The logic ensures that the necessary permissions are granted before starting Broadcast Streaming. In your `MainActivity` file, add the following code:
...
**Complete sample code for real-time Broadcast Streaming**
...
1. For basic Broadcast Streaming, select **RtcBasic**.
...
**Complete sample code for Broadcast Streaming**
...
1. To start Broadcast Streaming, initialize the engine and set up the event handler.
...
|:--|:-----|:-----|:------------|
| 1 | **Set Show Mouse Cursor** | Native* | <Slot name="1" /> |
...
| 7 | **Check Permission** | Custom | <Slot name="7" /> |
...
:::info
This node is available on Windows and macOS only. If the node is not retrieved at creation time, uncheck **Context Sensitive**. ![image](/images/video-sdk/quickstart-implementation-blueprint-3.png)
:::
