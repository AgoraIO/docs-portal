...
- A valid Agora account. If you have a Netless account, you must complete the account migration first. See [Migrate from Netless to Agora](migration-guide.md).
...
- **Access Key (AK)** and **Secret Key (SK)**: A pair of keys you use to generate whiteboard tokens. See [Generate token using an app server](generate-token-app-server.md).
- **SDK Token**: An SDK token is a dynamic key. You can generate an [SDK Token](authentication-workflow.md) here for development and testing purposes. In a production environment, you generate an SDK Token at your app server either by using the [code samples](generate-token-app-server.md) or calling the [RESTful API](generate-token-rest.md).
