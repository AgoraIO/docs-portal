---
title: "Fastboard API"
description: "The API reference guide for the Fastboard SDK across Android, iOS, and Web."
---

Use the platform tabs below to switch between Android, iOS, and Web Fastboard API references in one place.

<Tabs defaultValue="web" groupId="whiteboard-uikit-sdk-platform" persist>
<TabsList>
...
- `appId`: String. App Identifier for the interactive whiteboard project. For details, see [Get security credentials for your whiteboard project](../../../build/enable-whiteboard.md#get-security-credentials-for-your-whiteboard-project).
- `uuid`: String. The UUID of the room, which is the unique identifier of the room. For details, see [Create a room (POST)](../../rest-api/room-management#create-a-room-post) for the value of the `uuid` parameter in the response package body after the request is successful.
- `token`: String. Room Token of the room, used for user authentication when joining the room. It can be obtained in the following ways:
  - Call the [Generate a room token (POST)](../../../build/generate-token-rest.md#generate-a-room-token-post) RESTful API.
    - Build a token generator at your app server. See [Generate a token at app server](../../../build/generate-token-app-server.md).
  ...
  - `fastRegion`: The data center, which must be the same as the data center you chose when [creating the whiteboard room](../../rest-api/room-management#create-a-room-post). See [FastRegion](#fastregion).
...
- `converterType`: enumeration. The version of the document conversion service, the values are as follows:
   - `Projector`: new version. For details, see [New version of document conversion service](../../../build/file-conversion-overview.md).
 ...
 **Parameters**
 ...
 - `appIdentifier`: String. App Identifier for the interactive whiteboard project. For details, see [Get security credentials for your whiteboard project](../../../build/enable-whiteboard.md#get-security-credentials-for-your-whiteboard-project).
 - `roomUUID`: String. The UUID of the room, which is the unique identifier of the room. For details, see [Create a room (POST)](../../rest-api/room-management#create-a-room-post) The value of the `uuid` parameter in the response package body after the request is successful.
 - `roomToken`: String. Room Token of the room, used for user authentication when joining the room. It can be obtained in the following ways:
    - Call the [Generate a room token (POST)](../../../build/generate-token-rest.md#generate-a-room-token-post) RESTful API.
    - Use code to generate it on the app server. For details, see [Generate a Token at App Server](../../../build/generate-token-app-server.md).
 ...
 **Parameters**
 ...
 Dynamic documents refer to HTML web pages converted from PPTX format files through [Angora Interactive Whiteboard Document Conversion Service](../../../build/file-conversion-overview.md). The converted file will retain the animation effects in the source file.
 ...
 **Parameters**
 ...
 Static documents refer to files converted from PPT, PPTX, DOC, DOCX, and PDF formats through [Agora File Conversion Service](../../../build/file-conversion-overview.md) into still images in the PNG or JPG/JPEG format.
...
Inserts and displays a document in the whiteboard sub-window. This method applies to the [new file conversion service](../../../build/file-conversion-overview.md).

After successfully launching a file conversion task and calling the [Query file conversion progress API](../../rest-api/file-conversion#query-the-progress-of-a-file-conversion-task) to get the file conversion result, you can call this method and pass in the obtained file conversion result. After a successful call, the SDK automatically creates a sub-window to insert and display the converted document per page.

**Parameters**

- `title`: The title of the sub-window.
- `response`: The response body returned when the call of the [Query file conversion progress API](../../rest-api/file-conversion#query-the-progress-of-a-file-conversion-task) succeeds.
...
After successfully [launching a file conversion task](../../rest-api/file-conversion#start-file-conversion), you can call this method and pass in the parameters of the converted file. After a successful call, the SDK automatically creates a sub-window to insert and display the converted file per page.

**Note**
Compared with `insertDocs [1/2]`, `insertDocs [2/2]` requires passing in parameters manually instead of the response body acquired by calling the [Query file conversion progress API](../../rest-api/file-conversion#query-the-progress-of-a-file-conversion-task).
...
- `sdkConfig`: (Required) Object. Configurations for the [WhiteWebSdk](/en/api-reference/whiteboard/web) object, in which you must pass the following parameters:
  - `appIdentifier`: String. The App Identifier of your Interactive Whiteboard project issued by Agora. See [Get security credentials for your whiteboard project](../../../build/enable-whiteboard.md#get-security-credentials-for-your-whiteboard-project).
  - `region`: The data center, which must be the same as the data center you chose when [creating the whiteboard room](../../rest-api/room-management#create-a-room-post). The supported values are as follows:
...
- `joinRoom`: (Required) Object. Parameters for joining a room:
  - `uid`: String. The unique identifier of a user in string format. The maximum length is 1,024 bytes. Ensure that the `uid` of each user in the same room is unique.
  - `uuid`: String. The room UUID, that is, the unique identifier of a room. See the value of the `uuid` parameter in the response body after a successful call of [Create a room (POST)](../../rest-api/room-management#create-a-room-post).
...
- `scenePath`: String. The path of the whiteboard scene. Agora suggests you use "`/pdf/uuid`", where `uuid` is the UUID of the file conversion task. You can get `uuid` from the response body when the [Start file conversion (POST) API](../../rest-api/file-conversion#start-file-conversion) call succeeds.
- `scenes`: SceneDefinition[]. The content to be displayed on the whiteboard scene, that is, the converted file. The `SceneDefinition[]` consists of multiple `SceneDefinition` objects. Each `SceneDefinition` object represents a page of file and must contain the following parameters:
  ...
  - `ppt`: String. A JSON object containing the following:
    ...
    - `src`: String. The URL address of each page of file, which you can get from the field in the response body when the [Query file conversion progress](../../rest-api/file-conversion#query-the-progress-of-a-file-conversion-task) API call succeeds.
...
- `scenePath`: String. The path of the whiteboard scene. Agora suggests that you use "`/pptx/uuid`", where `uuid` is the UUID of the file conversion task. You can get `uuid` from the response body when the [Start file conversion API](../../rest-api/file-conversion#start-file-conversion) call succeeds.
- `taskId`: String. The UUID of the file conversion task. You can get `uuid` from the response body when the [Start file conversion API](../../rest-api/file-conversion#start-file-conversion) call succeeds.
- `title`: (Optional) String. The title of the sub-window.
