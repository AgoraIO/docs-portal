...
- Request body:
  ...
  :::info
To configure the `vendor` and `region` parameters, see [Third-party cloud storage regions](/en/realtime-media/recording/reference/region-vendor).
:::
...
For file naming rules, see [Manage recording files](../develop/manage-files). You can play M3U8 audio files, or you can use the [Agora Format Converter Script](https://download.agora.io/acrsdk/release/cloud_recording_tools.v3.8.0.69-202302061216-release-prod.tar.gz) to merge and convert all TS files of the specified user ID into one MP3/M4A/AAC audio file for playback. See [Merge audio and video files after an individual recording](../develop/merge-files) for details.
