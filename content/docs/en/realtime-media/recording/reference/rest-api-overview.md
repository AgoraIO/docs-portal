...
For details on setting up a NCS webhook server, see [Create your 
...
| [12](#12-session_failover)   | 0 (cloud recording service)   | The cloud recording service has enabled the [high availability mechanism](../overview).                   |
...
- `warnCode`: Number. The warning code. 
  - If the warning occurs in the recorder, see [warning code](/en/api-reference/local-server-recording/restful).
  - If the warning occurs in the uploader, see [upload warning code](#uploadwarn).

### 3 cloud_recording_status_update {#3}
...
### 4 cloud_recording_file_infos {#4}
...
### 43 recorder_audio_stream_state_changed {#43}
...
### 44 recorder_video_stream_state_changed {#44}
...
### 72 web_recorder_capability_limit {#72}
...
### Error codes related to uploading {#uploaderr}
...
### Warning codes related to uploading {#uploadwarn}
...
### Error codes for the Agora Cloud Recording service {#clouderr}
...
### Status codes for the Agora Cloud Recording service {#state}
