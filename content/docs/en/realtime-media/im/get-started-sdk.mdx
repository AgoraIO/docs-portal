...
1. Users log in to Chat using the App ID, their user ID, and token.
...
- A Windows or macOS computer that meets the following requirements:
  - A browser supported by the Agora Chat SDK:
    ...
    - Chrome 54 or later
    - Safari 6 or later
  - Access to the Internet. If your network has a firewall, follow the instructions in [Firewall requirements](/en/introduction/firewall) to access Agora services.
...
![image](https://assets-docs.agora.io/images/im/chat-call-logic-flutter.svg)
</details>
...
![Android React Native quickstart app interface](https://web-cdn.agora.io/doc-cms/uploads/1742458543456-android_rn_quickstart.png)

- iOS platform

![iOS React Native quickstart app interface](https://web-cdn.agora.io/doc-cms/uploads/1742458548797-ios_rn_quickstart.png)
...
1. **Instantiate the `SDKClient`**
    ...
    void setupChatSDK()
    {
        if (appId == "")
        {
            Debug.Log("You should set your App ID first!");
            return;
        }
        // Initialize the Agora Chat SDK
        Options options = Options.InitOptionsWithAppId(appId);
...
![image](https://assets-docs.agora.io/images/im/chat-call-logic-windows.svg)
</details>
...
1. **Declare connection variables**
    ...
    private string token = "<Your authentication token>";
    private readonly string app_id = "<App ID from Agora console>";
    private bool isJoined = false;
...
1. **Initialize the `SDKClient`**
    ...
    private void setupChatClient()
    {
        var options = Options.InitOptionsWithAppId(app_id);
        options.UsingHttpsOnly = true;
...
1. Create an app instance for the first user:
    ...
    1. In the `MainActivity` class, update `userId`, `token`, and `appId` with values from Agora Console.  To get your App ID, see [Get Chat project information](./get-started/enable#get-chat-project-information).
...
1. Create an app instance for the second user:
    ...
    1. In the `MainActivity` class, update `userId` and `token` with values for the second user. Make sure you use the same `appId` as for the first user.

    1. Run the modified app on a device emulator or a second physical Android device.

1. On each device, click **Join** to log in to Agora Chat.
...
1. Press **Leave** to log out of Agora Chat.
...
1. Create an app instance for the first user:
    ...
    1. In the `ViewController` class, update `userId`, `token`, and `appId` with values from Agora Console.  To get your App ID, see [Get Chat project information](./get-started/enable#get-chat-project-information).
...
1. Create an app instance for the second user:
    ...
    1. In the `ViewController` class, update `userId` and `token` with values for the second user. Make sure you use the same `appId` as for the first user.
...
1. Create an app instance for the first user:
    ...
    1. In the `_MyHomePageState` class, update `appId`, `token`, and `userId` with values from Agora Console.  To get your App ID, see [Get Chat project information](./get-started/enable#get-chat-project-information).
...
1. Create an app instance for the second user:
    ...
    1. In the `_MyHomePageState` class, update `userId` and `token` with values for the second user. Make sure you use the same `appId` as for the first user.
...
1. Log in
a. In the [`App.js`](#sign-in) file, replace the placeholders of `appId`, `username`, and `chatToken` with the App ID, user ID, and Agora token of the sender.
...
a. After signing out, change the values of `appId`, `username`, and `chatToken` to the user ID, Agora token, and App ID of the receiver in the [`App.js`](#sign-in) file.
...
1. In the `NewBehaviourScript` class, update `userId`, `token`, and `appId` with values from Agora Console. To get your App ID, see [Get Chat project information](./get-started/enable#get-chat-project-information).

1. In Unity Editor, click **Play**. A moment later you see the project installed on your development device.

1. Click **Join** to log in to Agora Chat.
...
1. [Register the recipient user](./get-started/enable#register-a-user) in Agora Console and [Generate a user token](./get-started/enable#generate-a-user-token).

1. In the `NewBehaviourScript` class, update `userId` and `token` with values you generated for recipient. Make sure you use the same `appId` as for both users.
...
1. Create an app instance for the first user:
    ...
    1. In the `MainWindow` class, update `userId`, `token`, and `appId` with values from Agora Console.  To get your App ID, see [Get Chat project information](./get-started/enable#get-chat-project-information).

    1. Click the **Start** button. You see the following interface:

        ![image](https://assets-docs.agora.io/images/im/chat-windows-interface.png)

1. Create an app instance for the second user:

   1. Register a second user in Agora Console and generate a user token.

    1. Build a second instance of your app after updating `userId` and `token` with values for the second user. Make sure you use the same `appId` as for the first user.
...
1. Type a message in the **Message** box of either instance and press **`>>`**. The message is sent and appears on the other app.
    ...
    ![image](https://assets-docs.agora.io/images/im/chat-windows-communication.png)
...
- [`ChatClient.init`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatClient/init.html)

- [`ChatClient.loginWithToken`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatClient/loginWithToken.html)

- [`ChatClient.logout`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatClient/logout.html)

- [`ChatManager.sendMessage`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatManager/sendMessage.html)

- [`ChatManager`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatManager-class.html)

- [`ChatMessage`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatMessage-class.html)

- [`ConnectionEventHandler`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ConnectionEventHandler-class.html)

- [`ChatEventHandler`](https://api-ref.agora.io/en/chat-sdk/flutter/1.x/agora_chat_sdk/ChatEventHandler-class.html)
