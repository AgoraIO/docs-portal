...
- You have an Agora project with Chat [enabled](../get-started/enable.md).
- You understand the API call frequency limit as described in [Limitations](../reference/limitations.md).
- The HTTP callback feature is not enabled by default. To use this feature, you need to subscribe to the **Pro** or **Enterprise** [pricing plan](../reference/pricing-plan-details.md) and enable it in [Agora Console](https://console.agora.io/v2).
:::info
You must contact [support@agora.io](mailto:support@agora.io) to disable this feature as this operation will delete all the relevant configurations.
:::
...
- To receive notifications for other events, configure callbacks under **Chat > Features > Callbacks** for your project in [Agora Console](https://console.agora.io/). See [Callbacks and events](../reference/callbacks-events.md) for details.
...
| `payload`    | JSON     | The message content which matches the [content of the `payload` parameter in historical messages retrieved with the REST API](../restful-api/message-management.md#content-of-historical-messages). |
...
In order to improve the security of the project, Agora uses a token (dynamic key) to authenticate users before they log into the chat system. The Agora Chat RESTful API only supports authenticating users using app tokens. For details, see [Secure authentication with tokens](./authentication.md).
...
| :--------- | :----- | :----------------------------------------------------------- | :------- |
| `host` | String | The domain name assigned by the Chat service to access RESTful APIs. For how to get the domain name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
| `org_name` | String | The unique identifier assigned to each company (organization) by the Chat service. For how to get the org name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
| `app_name` | String | The unique identifier assigned to each app by the Chat service. For how to get the app name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
...
| Parameter | Type | Description | Required |
| :--------- | :----- | :----------------------------------------------------------- | :------- |
| `host` | String | The domain name assigned by the Agora Chat service to access RESTful APIs. For how to get the domain name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
| `org_name` | String | The unique identifier assigned to each company (organization) by the Agora Chat service. For how to get the organization name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
| `app_name` | String | The unique identifier assigned to each app by the Agora Chat service. For how to get the app name, see [Get Chat project information](../get-started/enable.md#get-chat-project-information). | Yes |
...
| `action`       | String | The request method.      |
| `data`         | Boolean   | Whether the callback data is successfully resent: `success`: The data is successfully sent. `failure`: The data fails to be resent.    |
| `duration`     | Number  |The duration (ms) from when the HTTP request is sent to the time the response is received. |
