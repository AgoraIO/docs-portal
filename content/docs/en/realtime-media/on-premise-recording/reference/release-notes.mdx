...
You can either configure watermark settings in the `setVideoMixingLayout` method, or add, update, and remove watermark settings with the `updateWatermarkConfigs` method. See [Watermark](/en/realtime-media/on-premise-recording/build/watermark) for details.
...
Supports the cloud proxy service. See [Use Cloud Proxy](/en/realtime-media/on-premise-recording/build/cloud-proxy) for details.
