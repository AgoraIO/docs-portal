...
|--------------------------|------------|---------------|
| Create and initialize a recording instance | - `AgoraService`
 - `AgoraService.initialize`
 - `createAgoraMediaComponentFactory`
 - `createMediaRtcRecorder`
 - `AgoraMediaRtcRecorder.initialize`          | `RecordingSDK`|
| Join a channel          | `joinChannel`
If the String UID function is enabled, you can directly use the String UID to pass parameters when calling `joinChannel`, without calling other APIs.           |  - `createChannel`
 - `createChannelWithUserAccount`
 - `getUidByUserAccount`
 - `getUserAccountByUid`  |
| Implement subscription logic        | - `subscribeAllAudio`
 - `unsubscribeAllAudio`
 - `subscribeAllVideo`
 - `unsubscribeAllVideo`
 - `subscribeAudio`
 - `unsubscribeAudio`
 - `subscribeVideo`
 - `unsubscribeVideo`     | The following fields of the `RecordingConfig` object in the `create` method:
 - `autoSubscribe`
 - `subscribeVideoUids`
 - `subscribeAudioUids`|
| Start or stop recording| **Individual recording:**
...
**Composite recording:**
 ...
 - `stopRecording`
 - `setRecordingConfig`
Use with `enableMix` set to `true` when calling `initialize` |  - `startService`
 - `stopService`
Use with `isMixingEnabled` in `RecordingConfig`       |
| Watermark | - `enableAndUpdateVideoWatermarks`
 - `disableVideoWatermarks`
 - `enableAndUpdateVideoWatermarksByUid`
 - `disableVideoWatermarksByUid`       | `updateWatermarkConfigs`    |
</TabsContent>
...
|--------------------------|------------|---------------|
| Create and initialize a recording instance | - `createAgoraService`
- `IAgoraService::initialize`
- `createAgoraMediaComponentFactory`
- `createMediaRtcRecorder`
- `IAgoraMediaRtcRecorder::initialize` | `createAgoraRecordingEngine`                                                             |
| Join a channel                         | `joinChannel`
If the String UID function is enabled, you can directly use the String UID to pass parameters when calling `joinChannel`, without calling other APIs.         | - `joinChannel`
...
- `unsubscribeAudio`
- `subscribeVideo`
- `unsubscribeVideo` | Use the following fields in `RecordingConfig` when calling `joinChannel`:
- `autoSubscribe`
- `subscribeVideoUids`
- `subscribeAudioUids` |
| Start or stop recording                | **Individual recording:**
...
- `stopRecording`
- `setRecordingConfig`
Use with `enable_mix` set in `initialize` | - `startService`
- `stopService`
Use with `isMixingEnabled` in `RecordingConfig` |
| Watermark                              | - `enableAndUpdateVideoWatermarks`
- `disableVideoWatermarks`
- `enableAndUpdateVideoWatermarksByUid`
- `disableVideoWatermarksByUid` | Use `updateWatermarkConfigs` with `wm_num` and `wm_configs` in `VideoMixingLayout`       |
</TabsContent>
</Tabs>
