...
To use Agora SDKs, create an audio and video project in the Agora Console first. See [Agora account management](/en/realtime-media/marketplace/manage-agora-account) for details.
...
To use your App certificate for setting up a token server, see [Deploy a token server](/video-calling/token-authentication/deploy-token-server).
