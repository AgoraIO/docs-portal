...
- Your project integrates the [SDK quickstart](https://docs-md.agora.io/en/video-calling/get-started/get-started-sdk.md) using Web SDK v4.15.0 or later.
...
1. Register the plugin
  ...
  Call the [`AgoraRTC.registerExtensions`]({{global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#registerextensions) method and pass the created `VADExtension` instance to register the VAD plugin. You can also listen for events when the Wasm file fails to load.
...
1. Enable the plugin
  ...
  1. Inject the plugin into the audio processing pipeline by calling the SDK's [`pipe`]({{global.API_REF_WEB_ROOT}}/interfaces/imicrophoneaudiotrack.html#pipe) method and specify the `processorDestination` property.
