...
To speed up the subsequent joining of the channel, use a [wildcard token](/en/solutions/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens) in the `token` parameter of the `preload` method.
...
- Before entering the channel
    ...
    To speed up joining the channel, use a [wildcard token](/en/solutions/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens) in the `token` parameter of the `join` method.
...
- Entering the channel
    ...
    | Before | After |
    |--------|-------|
    |1. The user joins the channel.<br />2. The channel sends the host/media information.<br />2. The user subscribes to the media stream in the channel.<br />2. The Agora backend starts sending the media stream in the channel.<br />2. The user waits for the Agora backend to send the media stream, receives it, and renders it locally.|1. The user joins the channel.<br />2. The Agora backend immediately sends the media stream in the channel.<br />2. The channel sends the host/media information.<br />2. After the user subscribes to the media stream, they can immediately receive and render the media stream locally.|

    ![first-frame-rendring](/images/video-sdk/first-frame-rendering.png)

    :::info[Note]

    Joining a channel incurs additional fees due to the transmission of media streams. See [Pricing](/en/solutions/interactive-live-streaming/reference/pricing#cost-calculation) for details.
...
2. Use a wildcard token.
    ...
    To speed up the process of users joining a channel, use a [wildcard token](/en/solutions/interactive-live-streaming/build/authenticate-users/deploy-token-server#generate-wildcard-tokens). Generate the token on your server and pass it to the client for authentication.
