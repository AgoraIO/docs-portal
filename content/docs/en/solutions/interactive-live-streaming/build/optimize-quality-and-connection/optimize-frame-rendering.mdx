...
Complete the steps in the [SDK Quickstart](../../quickstart) to build a basic Interactive Live Streaming app.
...
If you can’t retrieve a token early, consider using a [wildcard token](../authenticate-users/deploy-token-server#generate-wildcard-tokens).
