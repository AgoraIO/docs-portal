...
**Supported platforms:** Android, iOS, Web, React Native, and Unity.
...
- Example 2:
  4. A user joins a voice call channel. Audio is routed to the speakerphone.
  5. App calls `{props.setEnableSpeakerphoneAPI}(true)`. Audio routing is changed to speakerphone.
  6. User plugs in headset. Audio routing changes to headset.
  7. User unplugs the headset. Audio routing changes to earpiece.
      :::note
...
Video SDK provides APIs for managing input and output devices. Use these APIs to switch devices dynamically during Interactive Live Streaming and implement hot-swap logic when devices are added or removed.
...
* Dynamically switch devices during Interactive Live Streaming.
