...
1. Enable the plugin
  ...
  2. Inject the plugin into the audio processing pipeline by calling the SDK's [`pipe`]({{global.API_REF_WEB_ROOT}}/interfaces/imicrophoneaudiotrack.html#pipe) method and specify the `processorDestination` property.
