...
**Manual integration**
4. Download the latest version of Video SDK from the the [SDKs](../../../sdks.md) page and unzip it.

5. Open the unzipped file and copy the following files or subfolders to your project path.
...
6. Select the file `/app/libs/agora-rtc-sdk.jar` in the left navigation bar of Android Studio project files, right-click, and select **add as a library** from the drop-down menu.
...
3. In Android Studio, click ![image](/images/video-sdk/icon_android_gradle_sync.png) **Sync Project with Gradle Files** to resolve project dependencies and update the configuration.

4. After synchronization is successful, click ![image](/images/video-sdk/icon_android_run.png) **Run app**. Android Studio starts compilation. After a few moments, the app is installed on your Android device.
...
**CocoaPods**
6. Go to the project root directory in the terminal and run the `pod init` command. A text file named `Podfile` is generated in the project folder.

7. Open `Podfile` and modify the content as follows. Replace `Your App` with your target name.
...
8. Run the `pod install` command in the terminal to install the Video SDK. After successful installation, the terminal shows **Pod installation complete!**.

9. After successful installation, a file with the suffix `.xcworkspace` is generated in the project folder. Open the file through Xcode for subsequent operations.

**Manual integration**
10. Download the latest version of the SDK from [SDKs download](../../../sdks.md) and extract the contents.

11. Copy the files in the `libs` folder of the SDK package to your project directory.

12. Open Xcode and [add the corresponding dynamic library](https://help.apple.com/xcode/mac/current/#/dev51a648b07). Make sure the **Embed** property of the added dynamic library is set to **Embed & Sign**.
...
**iOS and macOS**
4. Refer to [How to add the permissions required for real-time interaction to an Unreal Engine project?](https://docs-md.agora.io/en/interactive-live-streaming/get-started.md)

5. In `Project/Source/Project.Target.cs`, add the following code:
...
3. Invite a friend to run the demo game on a second device. Use the same same app ID, channel name, and token to join. After your friend joins successfully, you can hear and see each other.
