...
description: "Upgrade to the latest version of Interactive Live Streaming."
---
...
- [What's changed](#what-has-changed) between  Interactive Live Streaming v2.x and v4.x.


## Migration steps

This section introduces the main steps to upgrade the SDK from v2.x or v3.7.0 to v4.x.

1.  Integrate the SDK

    See [Quickstart](/en/solutions/interactive-live-streaming/quickstart) for more information about integrating the v4.x SDK into your project.
...
3.  Update the Agora code in your app
    ...
    The 4.x SDK has optimized or modified the implementation of some functions, resulting in incompatibility with the v3.7.0 SDK. In order to retain Agora functionality in your app, update the code in your app according to [What has changed](#what-has-changed).

## What has changed

This section introduces the main changes of 4.x compared to v3.7.0 in the following categories. You need to update the code of your app according to your business use-case.
...
-   [Function gaps](#function-gaps): Introduces functions that were supported in v3.7.0 but are not supported in 4.x. However, these functions are intended to be added in a future release.

-   [Removed APIs](#removed-apis): Introduces APIs that were supported in v3.7.0 but removed in 4.x. Most of these APIs have alternatives in 4.x. Modifying the related implementation should require less time.
...
After upgrading from v3.7.0 to 4.x, the way the APIs implement some functions is different. This section introduces compatibility changes for these APIs and the logic for updating the code of your app.
...
In v3.7.0, the SDK uses the `publishLocalAudio` and `publishLocalVideo` members in `ChannelMediaOptions` to control the audio and video publishing state in the channel.
...
In v3.7.0, the SDK returns warning codes through the `onWarning` callback.
...
In v3.7.0, when the SDK creates multiple log files, the earlier files are named in a agorasdk\_x.log format, such as agorasdk\_1.log. 4.x modified the naming format to agorasdk.x.log, such as agorasdk.1.log. Additionally, 4.x adds the `agoraapi.log` file to record API logs.

#### Fast channel switching

In v3.7.0, you need to call `switchChannel` to quickly switch a channel.

In 4.x, you can achieve the same switching speed as `switchChannel` in v3.7.0 by switching a channel through `leaveChannel` and `joinChannel`. Therefore, 4.x removes `switchChannel`. If you call `switchChannel` to quickly switch a channel in v3.7.0, see the [VideoQuickSwitch](https://github.com/AgoraIO/API-Examples/tree/main/Android/APIExample/app/src/main/java/io/agora/api/example/examples/advanced/VideoQuickSwitch.java) sample project to update the code in your app.
...
There are differences in the behavior of the SDK when setting channel media options while joining a channel between v3.7.0 and 4.x:
...
This section introduces functions that were supported in v3.7.0 but are no longer supported or behave inconsistently in 4.x. Plans exist to support them or make them consistent in a future release, however.

#### Audio application use-cases.

4.x reconstructs the audio application scenarios, which can replace most of the audio application scenarios of v3.7.0. The following table shows the correspondence of audio application scenarios in the two releases:
...
The following table shows the differences in the behavior of APIs related to the audio route between v3.7.0 and 4.x:
...
<table>
    ...
    <tr class="odd">
        ...
        <td><p>You can only set the audio route before joining a channel.</p>; <p>This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of `setDefaultAudioRouteToSpeakerphone` to `true` or `false`, calling `setEnableSpeakerphone(false)` changes the audio route to the earpiece.</p></td>
        <td><p>You can set the audio route either before or after joining a channel.</p>; <p>This method is a steady API and can change the default audio route of the system. For example, after calling `setDefaultAudioRouteToSpeakerphone(true)` to set the initial audio route to the speakerphone, calling `setEnableSpeakerphone(false)` cannot change the audio route to the earpiece.</p></td>
    </tr>
    <tr class="even">
...
-   In v3.7.0, the audio route changes as follows (in terms of priority): The external device connected next to last (if any) &gt; … &gt; The external device connected first &gt; `setEnableSpeakerphone` &gt; `setDefaultAudioRoutetoSpeakerphone` &gt; The default audio route.
...
In v3.7.0, if you set the video bitrate in VideoEncoderConfiguration as `STANDARD_BITRATE`, the default video bitrate in the `CHANNEL_PROFILE_LIVE_BROADCASTING` profile is twice that of the `CHANNEL_PROFILE_COMMUNICATION` profile.
...
See [Virtual Background](/en/solutions/interactive-live-streaming/build/apply-effects-and-enhancements/virtual-background) to update the code in your app.
...
Compared to v3.7.0, some features are not supported or only partially supported in 4.x. This section shows the APIs currently unsupported but for which support is planned for a future release.
...
-   The following enumerations of audio mixing: Rarely used in v3.7.0.
...
-   The `info` parameter in `joinChannel` [2/2]: This parameter is optional and rarely used in v3.7.0.
...
This section introduces the main steps to upgrade the SDK from v2.x or 3.7.0 to v4.x.
...
2.  Update the Agora code in your app
    ...
    The v4.x SDK has optimized or modified the implementation of some functions, resulting in incompatibility with the v3.7.0 SDK. In order to retain Agora functionality in your app, update the code in your app according to the **What has changed** section below.

## What has changed

This section introduces the main changes of v4.x compared to v3.7.0 in the following categories. You need to update the code of your app according to your business use-case.


:::info[Notice]
If the version before upgrading is a version before v3.7.0, check the legacy v3.x release notes to obtain the changes between this version and v3.7.0.
:::
...
After upgrading from v3.7.0 to v4.x, the way the APIs implement some functions is different. This section introduces compatibility changes for these APIs and the logic for updating the code of your app.
...
In v3.7.0, the SDK uses the `publishLocalAudio` and `publishLocalVideo` members in `AgoraRtcChannelMediaOptions` to control the audio and video publishing state in the channel.
...
See the [JoinChannelVideo](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Basic/JoinChannelVideo) sample project to update the code in your app.
...
-   Custom video source: [CustomVideoSourcePush](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Advanced/CustomVideoSourcePush)

-   Custom video renderer: [CustomVideoRender](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Advanced/CustomVideoRender)

-   Switching video source: [ScreenShare](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Advanced/ScreenShare)

#### Error codes and warning codes

In v3.7.0, the SDK returns error codes and warning codes through the `didOccurError` and `didOccurWarning` callbacks.
...
In v3.7.0, when the SDK creates multiple log files, the earlier files are named in a `agorasdk_x.log` format, such as `agorasdk_1.log`. v4.x modified the naming format to `agorasdk.x.log`, such as `agorasdk.1.log`. Additionally, v4.x adds the `agoraapi.log` file to record API logs.

#### Fast channel switching

In v3.7.0, you need to call `switchChannelByToken` to quickly switch a channel.

In v4.x, you can achieve the same switching speed as `switchChannelByToken` in v3.7.0 by switching a channel through `leaveChannel` and `joinChannelByToken`. Therefore, v4.x removes `switchChannelByToken`. If you call `switchChannelByToken` to quickly switch a channel in v3.7.0, see the [QuickSwitchChannel](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Advanced/QuickSwitchChannel) sample project to update the code in your app.
...
This section introduces functions that were supported in v3.7.0 but are no longer supported or behave inconsistently in v4.x. Plans exist to support them or make them consistent in a future release, however.

#### Audio application scenarios

v4.x reconstructs the audio application scenarios, which can replace most of the audio application scenarios of v3.7.0. The following table shows the correspondence of audio application scenarios in the two releases:
...
The following table shows the differences in the behavior of APIs related to the audio route between v3.7.0 and v4.x:
...
<tr class="odd">
<td><p>`setDefaultAudioRouteToSpeakerphone`</p></td>
<td><p>You can only set the audio route before joining a channel.</p>; <p>This method only controls the initial state of the audio route and does not change the default audio route of the system. For example, regardless of whether you set the parameter of `setDefaultAudioRouteToSpeakerphone` to `YES` or `NO`, calling `setEnableSpeakerphone(NO)` changes the audio route to the earpiece.</p></td>
<td><p>You can set the audio route either before or after joining a channel.</p>; <p>This method is a steady API and can change the default audio route of the system. For example, after calling `setDefaultAudioRouteToSpeakerphone(YES)` to set the initial audio route to the speakerphone, calling `setEnableSpeakerphone(NO)` cannot change the audio route to the earpiece.</p></td>
</tr>
<tr class="even">
<td><p>`setEnableSpeakerphone`</p></td>
<td><p>After connecting external playback devices such as Bluetooth and wired headphones, calling `setEnableSpeakerphone` cannot switch the audio route to the speakerphone or earpiece.</p></td>
...
In v3.7.0, if you set the video bitrate in `AgoraVideoEncoderConfiguration` as `AgoraVideoBitrateStandard`, the default video bitrate in the `AgoraChannelProfileLiveBroadcasting` profile is twice that of the `AgoraChannelProfileCommunication` profile.
...
See the [VideoProcess](https://github.com/AgoraIO/API-Examples/tree/main/iOS/APIExample/Examples/Advanced/VideoProcess) sample project to update the code in your app.

#### Unsupported functions

Compared to v3.7.0, some features are not supported or only partially supported in v4.x. This section shows the APIs currently unsupported but for which support is planned for a future release.
...
-   The `info` parameter in `joinChannelByToken` [2/2]: This parameter is optional and rarely used in v3.7.0.
...
Agora SDK v4.0.0 is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Benefits and features](/en/solutions/interactive-live-streaming/product-overview).
...
See [Quickstart](/en/solutions/interactive-live-streaming/quickstart) for more information about integrating the v4.0.0 SDK into your project.
...
`AgoraRtcChannel` and `AgoraRtcEngineKit` of v3.7.0 are partially duplicated and overlap in their functionality, so v4.0.0 hides the `AgoraRtcChannel` and `AgoraRtcChannelDelegate` classes. See [Join multiple channels](/en/solutions/interactive-live-streaming/build/connect-across-channels/join-multiple-channels) for more details on how to replace `AgoraRtcChannel` with `joinChannelExByToken` and `AgoraRtcChannelMediaOptions`. The expected migration cost is one day or less.
...
-   If your channel profile is https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/clientconfig.html#mode[live broadcast], the Interactive Live Streaming v3.x automatically sets the user role as `host` when publishing the stream. However, in the Interactive Live Streaming v4.x, you need to call `setClientRole` to set the user role as `host`.
...
-   Enable/disable a local track: The Interactive Live Streaming v4.x replaces the `Stream.muteAudio` and `Stream.muteVideo` methods with [ILocalTrack.setEnabled](https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/ilocaltrack.html#setenabled) for enabling or disabling a local audio or video track. This replacement changes the following SDK behaviors:
...
-   The Interactive Live Streaming v4.x replaces `Stream.switchDevice` with [IMicrophoneAudioTrack.setDevice](https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/imicrophoneaudiotrack.html#setdevice) and [ICameraVideoTrack.setDevice](https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/icameravideotrack.html#setdevice) for setting the media input device.

-   The Interactive Live Streaming v4.x replaces `Stream.setAudioOutput` with [IRemoteAudioTrack.setPlaybackDevice](https://api-ref.agora.io/en/video-sdk/web/4.x/interfaces/iremoteaudiotrack.html#setplaybackdevice) for setting the audio output device.
...
|-----------------------------------------------------------|---------------------------|
|`enableVirtualBackground`                                      | Virtual background extension |
|`setBeautyEffectOption`; `setVideoDenoiserOptions`; `setLowlightEnhanceOptions`; `setColorEnhanceOptions`| Video enhancement extension  |
| `enableRemoteSuperResolution`                                  | Super resolution extension   |
| `setAudioEffectPreset`; `setVoiceBeautifierPreset`; `setVoiceConversionPreset`| Voice beautifier extension   |
| `enableSpatialAudio`                                           | Spatial audio extension      |
| `enableContentInspect`                                         | Content moderation extension |
...
Agora SDK v4.x is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Product overview](/en/solutions/interactive-live-streaming/product-overview).

This page introduces the main steps to upgrade the SDK from v3.7.0 to v4.x, as well as the related changes.

## Migration steps

This section introduces the main steps to upgrade the SDK from v3.7.0  to v4.x.

1. **Integrate the SDK**

    See Project setup for more information about integrating the v4.x SDK into your project.

2. **Update the Agora code in your app**

    The v4.x SDK has optimized or modified the implementation of some functions, resulting in incompatibility with the v3.7.0  SDK. In order to retain Agora functionality in your app, update the code in your app according to [What has changed](#what-has-changed).

## What has changed

This section is based on v3.7.0  and introduces the main changes of v4.x compared to v3.7.0 in the following categories. You need to update the code of your app according to your business use-case.
    ...
    * _Function gaps_: Introduces functions that were supported in v3.7.0 but are not supported in v4.x. However, these functions are intended to be added in a future release.
    * _Removed APIs_: Introduces APIs that were supported in v3.7.0 but removed in v4.x. Most of these APIs have alternatives in v4.x. Modifying the related implementation should require less time.
...
After updating from v4.0.0 Beta to v4.0.0, the SDK package name has changed from `electron-agora-rtc-ng` to `agora-electron-sdk`. See [Quickstart](/en/solutions/interactive-live-streaming/quickstart) to integrate the SDK.
...
| :----------------------------------------------------------- | :--------------------------- |
| `enableVirtualBackground`                                      | Virtual background extension |
  | `setBeautyEffectOption`; `setVideoDenoiserOptions`; `setLowlightEnhanceOptions`; `setColorEnhanceOptions`|
  Video enhancement extension  |
| `enableRemoteSuperResolution`                                  | Super resolution extension   |
  | `setAudioEffectPreset`; `setVoiceBeautifierPreset`; `setVoiceConversionPreset`| Voice beautifier
  extension   |
| `enableSpatialAudio`                                           | Spatial audio extension      |
| `enableContentInspect`                                         | Content moderation extension |
...
-  [Migration steps](#migration-steps) to upgrade your  app to Interactive Live Streaming 6.x.

- [What's changed](#what-has-changed) between Interactive Live Streaming 5.x and 6.x.
...
See [Quickstart](/en/solutions/interactive-live-streaming/quickstart) for more information about integrating the `agora_rtc_engine` 6.x SDK into your project.
...
This section introduces functions that were supported in ``agora_rtc_engine``: Interactive Live Streaming 5.x but are no longer supported or behave inconsistently in `agora_rtc_engine` 6.x. Plans exist to support them or make them consistent in a future release.
...
See the [quickstart](/en/solutions/interactive-live-streaming/quickstart) for more information about integrating the v4.0.0 SDK into your project.
...
After upgrading from v4.0.0 Beta to v4.0.0, the SDK package name has changed from `react-native-agora-rtc-ng` to `react-native-agora`. See [Quickstart](/en/solutions/interactive-live-streaming/quickstart) to integrate the SDK.
...
Refer to the [quickstart](/en/solutions/interactive-live-streaming/quickstart) to integrate the v4.0.0 SDK into your project.
...
To integrate Video SDK for Unreal Engine v4.2.1 into your project, see [Quickstart](/en/solutions/interactive-live-streaming/quickstart).
...
Video SDK for Unreal Engine v4.x is based on Video SDK for C++ v4.x. First follow the Windows steps in this migration guide.
