...
Agora creates independent and isolated classrooms for audio, video, or messaging data transmission with the room ID specified by the app developer. All classrooms are logically separated and only authenticated users from the same Agora [App ID](../../reference/glossary.md) can join the same classroom.
...
The app developer can enable token authentication (App Certificate) in [Agora Console](https://console.agora.io/v2). When enabled, all user’s request to join a classroom must be done with a valid Signaling token.

- For more information on how to enable Signaling token authentication, see [Secure authentication with tokens](../set-up-your-account-and-authentication/authentication-workflow.md).
- For how to generate a Signaling token on the app backend, see [Secure authentication with tokens](../set-up-your-account-and-authentication/authentication-workflow.md).
...
1. Enable token authentication in [Agora Console](https://console.agora.io/v2).
...
3. [Deploy a Signaling token server](../set-up-your-account-and-authentication/authentication-workflow.md) in your backend services.
