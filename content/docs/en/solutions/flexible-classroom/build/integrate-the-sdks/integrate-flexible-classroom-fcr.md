...
The sample code requires passing in `rtmToken`. You can refer to [Secure authentication with tokens](../set-up-your-account-and-authentication/authentication-workflow.md) to learn what a Signaling token is, how to get a temporary Signaling token for testing purposes, and how to generate a Signaling token from the server. The generated token passed in `userId` must be consistent with the `userUuid` parameters passed in the `launch` method; otherwise, the generated token will be invalid.
...
:::note
Make sure you have set up a development environment. See [Configure Flexible Classroom](../set-up-your-account-and-authentication/enable-flexible-classroom.md).
:::
...
1. Modify the code according to your needs. See [FcrUIScene SDK customization guide](../customize-the-ui-and-plugins/customize-ui-scene-sdk.md) for details.
