...
|:------|:------|:------|
| `type` | String | <Slot name="type" /> |
| `region` | String | <Slot name="region" /> |

<Slot for="type">

Required. The domain name type:
...
Required when the domain name type is set as `"publish"`. This parameter only applies to stream-pushing domain names. The region of the Agora server used for the stream-pushing domain name:
