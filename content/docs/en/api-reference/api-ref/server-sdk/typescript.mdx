...
| `HumeAITTS` | `key`, `voiceName`, `provider`, `configId?`, `baseUrl?`, `speed?`, `trailingSilence?` |
...
| `SarvamTTS` | `key`, `speaker`, `targetLanguageCode`, `pitch?`, `pace?`, `loudness?`, `model?`, `speechSampleRate?`, `enablePreprocessing?` |
