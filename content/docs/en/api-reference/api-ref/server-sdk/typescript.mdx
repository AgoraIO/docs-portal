...
| `withTools(enabled)` | `boolean` | Enable or disable custom tool and MCP tool invocation. In v2.12, this also governs custom tools declared via the LLM vendor's `tools` option, in addition to MCP tools |
...
Filler word type aliases: `FillerWordsConfig`, `FillerWordsTrigger`, `FillerWordsTriggerFixedTimeConfig`, `FillerWordsContent`, `FillerWordsContentStaticConfig`, and `FillerWordsContentGeneratedConfig` (its `llm_provider` field takes a plain object with `base_url`, `api_key`, and `params`). See [Talking while waiting](/en/ai/build/shape-the-conversation/filler-words) for usage.
...
| `tools` | `Record<string, unknown>[]` | No | Custom tool definitions the LLM can call |
