...
| `WithTools(enabled)` | `bool` | Enable or disable custom tool and MCP tool invocation. In v2.12, this also governs custom tools declared via the LLM vendor's `Tools` field, in addition to MCP tools |
...
Filler word type aliases: `agentkit.FillerWordsConfig`, `agentkit.FillerWordsTrigger`, `agentkit.FillerWordsTriggerFixedTimeConfig`, `agentkit.FillerWordsContent`, `agentkit.FillerWordsContentStaticConfig`, `agentkit.FillerWordsContentGeneratedConfig`, and `agentkit.FillerWordsGeneratedLlmProvider`. See [Talking while waiting](/en/ai/build/shape-the-conversation/filler-words) for usage.
...
In v2.12, `ThinkOnListeningAction`, `ThinkOnThinkingAction`, and `ThinkOnSpeakingAction` also accept `append`, available as `ThinkOnListeningActionAppend`, `ThinkOnThinkingActionAppend`, and `ThinkOnSpeakingActionAppend`. It waits for the current turn's LLM output to finish, then appends the instruction as a separate user message and starts a new turn, without interrupting the turn in progress.
...
| `Tools` | `[]*Agora.LlmTool` | No | — | Custom tool definitions the LLM can call |
...
Custom tool type aliases: `Agora.LlmTool`, `Agora.LlmToolFunction`, `Agora.LlmToolFunctionParameters`, `Agora.LlmToolExecution`, `Agora.LlmToolServer`, and the `Agora.LlmToolServerMethodGet`/`Agora.LlmToolServerMethodPost` constants. See [Call custom tools](/en/ai/build/custom-model-integration/custom-tools) for usage.
