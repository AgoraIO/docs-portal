...
The `Tools` field in LLM vendor options declares custom tools the LLM can choose to call. Each tool includes a model-visible function definition and the server configuration for a synchronous `GET` or `POST` request. `WithTools(true)` applies to both `Tools` and `McpServers`. For more information, see [Call custom tools](/en/ai/build/custom-model-integration/custom-tools).
