...
Panics if `Key`, `VoiceName`, or `Provider` is empty.

| Field | Type | Required | Description |
|---|---|---|---|
| `Key` | `string` | Yes | Hume AI API key |
| `VoiceName` | `string` | Yes | Hume AI voice name |
...
| `Model` | `string` | No | TTS model, for example `"bulbul:v2"` |
| `SpeechSampleRate` | `*int` | No | Audio sample rate in Hz |
| `EnablePreprocessing` | `*bool` | No | Normalizes English words and numeric entities before synthesis |
| `SkipPatterns` | `[]int` | No | Patterns to skip |
