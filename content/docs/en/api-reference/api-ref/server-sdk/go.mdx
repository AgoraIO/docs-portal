...
`FillerWordsConfig` supports static and generated filler words. In generated mode, `GeneratedConfig`, `Prompt`, and the static fallback phrases are optional. If you omit `GeneratedConfig`, the service uses its default generation settings. Agora hosts the generation service, so you can't configure its model endpoint, API key, or model parameters. The only supported fallback strategy is `static`. For the field structure, see [Configure generated filler words](/en/ai/build/shape-the-conversation/filler-words#configure-generated-filler-words).
