...
| `with_tools(enabled)` | `bool` | Enable or disable custom tool and MCP tool invocation. In v2.12, this also governs custom tools declared via the LLM vendor's `tools` parameter, in addition to MCP tools |
...
Filler word type aliases: `FillerWordsConfig`, `FillerWordsTrigger`, `FillerWordsTriggerFixedTimeConfig`, `FillerWordsContent`, `FillerWordsContentStaticConfig`, `FillerWordsContentGeneratedConfig`, and `FillerWordsContentGeneratedConfigLlmProvider`. See [Talking while waiting](/en/ai/build/shape-the-conversation/filler-words) for usage.
...
Think value constants: `ThinkOnListeningActionInject`, `ThinkOnListeningActionInterrupt`, `ThinkOnListeningActionAppend`, `ThinkOnListeningActionIgnore`, `ThinkOnThinkingActionInterrupt`, `ThinkOnThinkingActionAppend`, `ThinkOnThinkingActionIgnore`, `ThinkOnSpeakingActionInterrupt`, `ThinkOnSpeakingActionAppend`, `ThinkOnSpeakingActionIgnore`.
...
In API v2.12, `on_listening_action`, `on_thinking_action`, and `on_speaking_action` also accept `append`. It waits for the current turn's LLM output to finish, then appends the instruction as a separate user message and starts a new turn, without interrupting the turn in progress.
...
| `tools` | `List[Dict]` | No | `None` | Custom tool definitions the LLM can call |
