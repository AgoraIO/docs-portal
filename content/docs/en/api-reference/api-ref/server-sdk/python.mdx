...
| `voice_name` | `str` | Yes | — | Hume AI voice name |
...
| `model` | `str` | No | `None` | TTS model, for example `'bulbul:v2'` |
| `speech_sample_rate` | `int` | No | `None` | Audio sample rate in Hz |
| `enable_preprocessing` | `bool` | No | `None` | Normalizes English words and numeric entities before synthesis |
| `skip_patterns` | `List[int]` | No | `None` | Skip patterns for bracketed content |
