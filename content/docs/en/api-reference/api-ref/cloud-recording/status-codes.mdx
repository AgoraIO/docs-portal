...
description: Error codes and status codes returned by the Cloud Recording RESTful API.
---
...
| `401 Unauthorized` | `"Unauthorized"` | Authorization header is missing. | Ensure you are using [HTTP Basic Auth](authentication) and that the `Authorization` field is included in the request header. |
