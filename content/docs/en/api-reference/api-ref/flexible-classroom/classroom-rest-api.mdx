...
Flexible Classroom Cloud Service uses tokens for authentication. You need to put the corresponding information into the `Authorization: agora token=` field when sending your HTTP request. For details, see [Secure authentication with tokens](/en/solutions/flexible-classroom/build/authentication-workflow).
...
(Required) The region for connection. For details, see [Network geofencing](/en/solutions/flexible-classroom/build/classroom-security#network-geofencing). Flexible Classroom supports the following regions:
