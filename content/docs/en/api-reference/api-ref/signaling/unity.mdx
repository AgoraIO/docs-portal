...
| `areaCode` | `RTM_AREA_CODE` | Optional | `GLOB` | Service area code, you can choose according to the region where your business is deployed. See [`RTM_AREA_CODE`](#rtm_area_code). |
...
| `level` | `RTM_LOG_LEVEL` | Optional | `INFO` | Output level of log information. See [`RTM_LOG_LEVEL`](#rtm_log_level). |
...
| Properties | Type | Required | Default | Description |
|:-----------:|:----------------:|:--------:|:---------------------:| ------------------------------------------------------------------- |
| `proxyType` | `RTM_PROXY_TYPE` | Optional | `NONE` | Proxy protocol type. See [`RTM_PROXY_TYPE`](#rtm_proxy_type). |
...
| Properties | Type | Required | Default | Description |
|:-------------------:|:---------------------:|:--------:|:--------------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `encryptionMode` | `RTM_ENCRYPTION_MODE` | Optional | `NONE` | Encryption mode. See [`RTM_ENCRYPTION_MODE`](#rtm_encryption_mode). |
| `encryptionKey` | string | Optional | - | User-defined encryption key, unlimited length. Agora recommends using a 32-byte key. |
| `encryptionSalt` | `byte[]` | Optional | `null` | User-defined encryption salt, length is 32 bytes. Agora recommends using OpenSSL to generate salt on the server side. |
...
// Set your salt. It must be a 32-byte array.
byte[] salt = new byte[32];
encryptionConfig.encryptionSalt = salt;
...
rtmClient.OnTopicEvent += (TopicEvent @event) =>
{
    ...
    Debug.Log(string.Format("Received topic event {0} from userId:{1} at channel:{2}. ", eventType, publisher, channelName));
    if (topicInfos != null)
    {
        // your logic
    }
...
{
    var channelType = @event.channelType;
    var storageType = @event.storageType;
    var eventType = @event.eventType;
    var target = @event.target;
    var data = @event.data;
    Debug.Log(string.Format("Received storage event {0} for {1} with channel type of {2}. ", eventType, target, channelType));
    if (data != null)
    {
        // your logic
    }
...
rtmClient.OnLockEvent += (LockEvent @event) =>
{
    ...
    Debug.Log(string.Format("Received lock event {0} at channel:{1} with channel type of {2}. ", eventType, channelName, channelType));
    if( LockDetail != null )
    {
        // your logic
    }
...
|:------------------:|:-------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `messageType` | `RTM_MESSAGE_TYPE` | Message types. See [`RTM_MESSAGE_TYPE`](#rtm_message_type). |
...
| Properties | Type | Description |
|:------------------:|:-------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `type` | `RTM_PRESENCE_EVENT_TYPE` | Presence event type. See [`RTM_PRESENCE_EVENT_TYPE`](#rtm_presence_event_type). |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `channelName` | string | Channel name. |
| `publisher` | string | User ID of the message publisher. |
...
| Properties | Type | Description |
|:------------------:|:-------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `type` | `RTM_TOPIC_EVENT_TYPE` | Topic event type. See [`RTM_TOPIC_EVENT_TYPE`](#rtm_topic_event_type). |
| `channelName` | string | Channel name. |
...
|:------------------:|:-------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `storageType` | `RTM_STORAGE_TYPE` | Storage type. See [`RTM_STORAGE_TYPE`](#rtm_storage_type). |
| `eventType` | `RTM_STORAGE_EVENT_TYPE` | Storage event type. See [`RTM_STORAGE_EVENT_TYPE`](#rtm_storage_event_type). |
...
|:------------------:|:-------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `eventType` | `RTM_LOCK_EVENT_TYPE` | Lock event type. See [`RTM_LOCK_EVENT_TYPE`](#rtm_lock_event_type). |
| `channelName` | string | Channel name. |
...
var result = await rtmClient.SubscribeAsync("Chat_room", options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode:{1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Subscribe channel success! at Channel:{0}", result.Response.ChannelName));
}
```
...
{
    Debug.Log(string.Format("User:{0} Join stream channel success! at Channel:{1}", result.Response.UserId, result.Response.ChannelName));
}
```
...
| Properties | Type | Required | Default | Description |
| :-------------: | :------------------------------------------------------: | :------: | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `qos` | `RTM_MESSAGE_QOS` | Optional | `UNORDERED` | Whether the data transmitted in the topic is ordered. See [`RTM_MESSAGE_QOS`](#rtm_message_qos). |
| `priority` | `RTM_MESSAGE_PRIORITY` | Optional | `NORMAL` | The priority of data transmission in the topic compared to other topics in the same channel. See [`RTM_MESSAGE_PRIORITY`](#rtm_message_priority). |
...
var result = await streamChannel.JoinTopicAsync(topic, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, because of: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
else
{
    Debug.Log(string.Format("User:{0} Join Topic:{1} success! at Channel:{2}", result.Response.UserId, result.Response.Topic, result.Response.ChannelName));
}
```
...
var result = await streamChannel.PublishTopicMessageAsync(topic, message, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
```csharp
var result = await streamChannel.LeaveTopicAsync("Motion");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
var result = await streamChannel.SubscribeTopicAsync(subtopic, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
var result = await streamChannel.UnsubscribeTopicAsync(subtopic, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
| Properties | Type | Required | Default | Description |
| :----------: | :----: | :------: | :----: | ------------------------------------------------ |
| `channelType` | `RTM_CHANNEL_TYPE` | Required | - | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
...
var result = await rtmClient.PublishAsync(channelName, message, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
By calling the `GetOnlineUsersAsync` method, you can query real-time information about the number of online users, the list of online users, and the temporary state of online users in a specified channel.

#### Method

You can call the `GetOnlineUsersAsync` method as follows:
...
| Parameters | Type | Required | Default | Description |
| :--------------: | :-------------------------------------------------------: | :------: | :----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `channelName` | string | Required | - | Channel name. If you do not fill in the parameter, the `GetOnlineUsersAsync` method returns in all online users information in the channel which is accordance with the `RTM_CHANNEL_TYPE` conditions. |
| `channelType` | `RTM_CHANNEL_TYPE` | Required | - | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
...
#### Basic usage
```csharp
var options = new GetOnlineUsersOptions();
options.includeState = true;

var result = await rtmClient.GetPresence().GetOnlineUsersAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("You have got {0} users information ", result.Response.TotalOccupancy));
    if (result.Response.NextPage != null)
    {
...
The `GetOnlineUsersAsync` operation returns a `RtmResult&lt;GetOnlineUsersResult&gt;` data type, including the following properties:

| Properties | Type | Description |
| :--------: | :-----------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------- |
| `Status` | `RtmStatus` | No matter whether the operation is successful, this property returns a `RtmStatus` data type, including the operation of the state. |
| `Response` | `GetOnlineUsersResult` | After the operation succeeds, this property returns a `GetOnlineUsersResult` data type. |
...
The `GetOnlineUsersResult` data type contains the following properties:

| Properties | Type | Description |
| :---------------: | :--------------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NextPage` | string | Page number of the next page. Confirm whether there is a next page:<br />- A null value indicates that next page does not exist.<br />- A non-null value indicates that there is a next page. You can fill this value in the `page` property of the `GetOnlineUsersAsync` method to query the next page results. |
...
In use-cases such as statistic analytics and debugging, you may need to know all the channels that a specified user has subscribed to or joined. Call the `GetUserChannelsAsync` method to get the list of channels where the specified user is in real time.

#### Method

You can call the `GetUserChannelsAsync` method as follows:
...
```csharp
var result = await rtmClient.GetPresence().GetUserChannelsAsync("Tony");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("User Tony is now in {0} channels ", result.Response.Channels.Length));
    if (result.Response.Channels.Length > 0)
    {
...
The `GetUserChannelsAsync` operation returns a `RtmResult&lt;GetUserChannelsResult&gt;` data type, including the following properties:

| Properties | Type | Description |
| :--------: | :-------------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------- |
| `Status` | `RtmStatus` | No matter whether the operation is successful, this property returns a `RtmStatus` data type, including the operation of the state. |
| `Response` | `GetUserChannelsResult` | After the operation succeeds, this property returns a `GetUserChannelsResult` data type. |
...
The `GetUserChannelsResult` data type contains the following properties:
...
var result = await rtmClient.GetPresence().SetStateAsync(channelName, RTM_CHANNEL_TYPE.MESSAGE, stateItems);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
```csharp
var result = await rtmClient.GetPresence().GetStateAsync(channelName, RTM_CHANNEL_TYPE.MESSAGE, userId);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("User:{0}, have stateCount:{1} states",result.Response.State.userId, result.Response.State.states.Length));
}
```
...
var result = await rtmClient.GetPresence().RemoveStateAsync(channelName, RTM_CHANNEL_TYPE.MESSAGE, keys);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
var result = await rtmClient.GetStorage().SetChannelMetadataAsync("channel_name", RTM_CHANNEL_TYPE.MESSAGE, metadata, metadataOptions, lockName);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Set Channel :{0} metadata success! Channel Type is :{1}! ", result.Response.ChannelName, result.Response.ChannelType));
}
```
...
```csharp
var result = await rtmClient.GetStorage().GetChannelMetadataAsync("channel_name", RTM_CHANNEL_TYPE.MESSAGE);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Get Channel :{0} metadata success! Channel Type is :{1}! ", result.Response.ChannelName, result.Response.ChannelType));
    var data =  result.Response.Data;
    if (data.metadataItemsSize != 0)
    {
        Debug.Log(string.Format("Channel Metadata Major Revision is :{0} ! ", data.majorRevision));
        for (int i = 0; i < (int)data.metadataItemsSize; i++)
        {
...
var result = await rtmClient.GetStorage().RemoveChannelMetadataAsync("channel_name", RTM_CHANNEL_TYPE.MESSAGE, metadata, options, "");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Remove Channel :{0} metadata success! Channel Type is :{1}! ", result.Response.ChannelName, result.Response.ChannelType));
}
```
...
var result = await rtmClient.GetStorage().UpdateChannelMetadataAsync("channel_name", RTM_CHANNEL_TYPE.MESSAGE, metadata, options, lockName);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Update Channel :{0} metadata success! Channel Type is :{1}! ", result.Response.ChannelName, result.Response.ChannelType));
}
```
...
var result = await rtmClient.GetStorage().SetUserMetadataAsync("Tony", metadata, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Set user :{0} metadata success! ", result.Response.UserId));
}
```
...
var result = await rtmClient.GetStorage().RemoveUserMetadataAsync("Tony", metadata, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Remove User :{0} metadata success! ", result.Response.UserId));
}
```
...
var result = await rtmClient.GetStorage().UpdateUserMetadataAsync("Tony", metadata, options);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Update User :{0} metadata success! ", result.Response.UserId));
}
```
...
```csharp
var result = await rtmClient.GetLock().SetLockAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, "lock1", 30);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode {1}, because of: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Set Lock at Channel:{0} with Channel Type {1} success! Lock name is {2}", result.Response.ChannelName, result.Response.ChannelType, result.Response.LockName));
}
```
#### Return value
...
```csharp
var result = await rtmClient.GetLock().AcquireLockAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, "lock1", false);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Acquire Lock at Channel:{0} with Channel Type {1} success! Lock name is {2}", result.Response.ChannelName, result.Response.ChannelType, result.Response.LockName));
}
```
...
```csharp
var result = await rtmClient.GetLock().ReleaseLockAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, "lock1");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Release Lock at Channel:{0} with Channel Type {1} success! Lock name is {2}", result.Response.ChannelName, result.Response.ChannelType, result.Response.LockName));
}
```
...
```csharp
var result = await rtmClient.GetLock().RevokeLockAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, "lock1","Tony");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, ErrorCode: {1}, due to: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("RevokeLock at Channel:{0} with Channel Type {1} success! Lock name is {2}", result.Response.ChannelName, result.Response.ChannelType, result.Response.LockName));
}
```
...
```csharp
var result = await rtmClient.GetLock().GetLocksAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE);
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, The error code is {1}, because of: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Get Lock at Channel:{0} with Channel Type {1} success! ", result.Response.ChannelName, result.Response.ChannelType));
}
```
...
```csharp
var result = await rtmClient.GetLock().RemoveLockAsync("Chat_room", RTM_CHANNEL_TYPE.MESSAGE, "lock1");
if (result.Status.Error)
{
    Debug.Log(string.Format("{0} is failed, The error code is {1}, because of: {2}", result.Status.Operation, result.Status.ErrorCode, result.Status.Reason));
}
...
{
    Debug.Log(string.Format("Remove Lock{0} at Channel:{1} with Channel Type {2} success! ",result.Response.LockName, result.Response.ChannelName, result.Response.ChannelType));
}
```
