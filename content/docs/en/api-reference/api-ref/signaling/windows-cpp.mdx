---
title: Windows C++
description: API reference for the Agora Signaling Windows C++ SDK.
---

:::info
The Windows C++ SDK uses the same Signaling C++ API surface as the Linux C++ SDK.
:::
...
config.areaCode = static_cast<RTM_AREA_CODE>(RTM_AREA_CODE_CN | RTM_AREA_CODE_AS);
...
| `areaCode` | `RTM_AREA_CODE` | Optional | `RTM_AREA_CODE_GLOB` | Service area code, you can choose according to the region where your business is deployed. See [`RTM_AREA_CODE`](#rtm_area_code). |
...
| `reconnectTimeout` | uint32_t | Optional | `0` | SDK connection timeout in seconds. The value range is [15,3600]. The default value is `0` which means no timeout; the SDK keeps retrying until successful. This parameter applies to both the initial login to the Signaling service and reconnection after disconnection: <br />- If the initial login times out, the SDK triggers the `onLoginResult` callback with `errorCode` as `RTM_ERROR_LOGIN_TIMEOUT`, and also triggers the `onLinkStateChanged` callback with `currentState` as `RTM_LINK_STATE_FAILED` and `reason` as `RTM_LINK_STATE_CHANGE_REASON_LOGIN_TIMEOUT`. <br />- If reconnection times out, the SDK triggers the `onLinkStateChanged` callback with `currentState` as `RTM_LINK_STATE_FAILED` and `reason` as `RTM_LINK_STATE_CHANGE_REASON_RECONNECT_TIMEOUT`. |
...
| `level` | `RTM_LOG_LEVEL` | Optional | `RTM_LOG_LEVEL_INFO` | Output level of log information. See [`RTM_LOG_LEVEL`](#rtm_log_level). |
...
| `proxyType` | `RTM_PROXY_TYPE` | Optional | `RTM_PROXY_TYPE_NONE` | Proxy protocol type. See [`RTM_PROXY_TYPE`](#rtm_proxy_type). |
...
| Properties | Type | Required | Default | Description |
|:-------------------:|:---------------------:|:--------:|:--------------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `encryptionMode` | `RTM_ENCRYPTION_MODE` | Optional | `RTM_ENCRYPTION_MODE_NONE` | Encryption mode. See [`RTM_ENCRYPTION_MODE`](#rtm_encryption_mode). |
| `encryptionKey` | const char* | Optional | - | User-defined encryption key, unlimited length. Agora recommends using a 32-byte key. |
| `encryptionSalt` | uint8_t[32] | Optional | - | User-defined encryption salt, length is 32 bytes. Agora recommends using OpenSSL to generate salt on the server side. |
...
| Properties | Type | Required | Default | Description |
|:-------------------:|:---------------------:|:--------:|:--------------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `serviceType` | `RTM_SERVICE_TYPE` | Optional | - | Service type. See [`RTM_SERVICE_TYPE`](#rtm_service_type). |
...
config.privateConfig.serviceType = static_cast<RTM_SERVICE_TYPE>(RTM_SERVICE_TYPE_MESSAGE | RTM_SERVICE_TYPE_STREAM);
```
...
|:----------:|------------------------------|
| `onMessageEvent` | Receive message notifications from all subscribed Message Channels or topic message notifications subscribed to, in all Stream Channels you join. See [`MessageEvent`](#messageevent) . |
| `onPresenceEvent` | Receive presence event notifications in subscribed message channels and joined stream channels. See [`PresenceEvent`](#presenceevent) |
| `onTopicEvent` | Receive all topic event notifications in joined stream channels. See [`TopicEvent`](#topicevent) |
| `onStorageEvent` | Receive channel metadata event notifications in subscribed message channels and joined stream channels, and the user metadata event notification of the subscribed users. See [`StorageEvent`](#storageevent) |
| `onLockEvent` | Receive lock event notifications in subscribed message channels and joined stream channels. See [`LockEvent`](#lockevent) |
| `onLinkStateEvent` | Receive event notifications when client connection status changes. See [`LinkStateEvent`](#linkstateevent). |
...
class RtmEventHandler: public IRtmEventHandler {
    ...
    void onPresenceEvent(const PresenceEvent &event) {}
...
| `messageType` | `RTM_MESSAGE_TYPE` | Message type. See [`RTM_MESSAGE_TYPE`](#rtm_message_type). |
...
| `totalOccupancy` | size_t | The total number of online users in the current channel. In large-channel optimization mode, this value represents the total number of online users in the channel and is not limited by the maximum of 512 user details that are continuously synchronized. This value may be delayed slightly. |
...
|:----------:|:-----:|:--------------------------- |
| `currentState` | `RTM_LINK_STATE` | The current link state. See [`RTM_LINK_STATE`](#rtm_link_state). |
| `previousState` | `RTM_LINK_STATE` | The previous link state. See [`RTM_LINK_STATE`](#rtm_link_state). |
| `serviceType` | `RTM_SERVICE_TYPE` | The network connection type. See [`RTM_SERVICE_TYPE`](#rtm_service_type). |
| `operation` | `RTM_LINK_OPERATION` | The operation that triggered the current state transition. See [`RTM_LINK_OPERATION`](#rtm_link_operation). |
| `reasonCode` | `RTM_LINK_STATE_CHANGE_REASON` | The reason for this state transition. See [`RTM_LINK_STATE_CHANGE_REASON`](#rtm_link_state_change_reason). |
...
| :--------------: | :--------------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `requestId` | const uint64_t | Request identifier. |
| `serverType` | `RTM_SERVICE_TYPE` | Service type. See [`RTM_SERVICE_TYPE`](#rtm_service_type). |
| `channelName` | const char* | Channel name. |
| `errorCode` | `RTM_ERROR_CODE` | [Error codes](#error-codes-table). |
...
```cpp
uint64_t requestId;
rtmClient->renewToken("your token", requestId);
```
...
| Properties | Type | Required | Default | Description |
| :-------------: | :------------------------------------------------------: | :------: | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `qos` | `RTM_MESSAGE_QOS` | Optional | `RTM_MESSAGE_QOS_UNORDERED` | Whether the data transmitted in the topic is ordered. See [`RTM_MESSAGE_QOS`](#rtm_message_qos). |
| `priority` | `RTM_MESSAGE_PRIORITY` | Optional | `RTM_MESSAGE_PRIORITY_NORMAL` | The priority of data transmission in the topic compared to other topics in the same channel. See [`RTM_MESSAGE_PRIORITY`](#rtm_message_priority). |
...
| Properties | Type | Required | Default | Description |
| :----------: | :----: | :------: | :----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `messageType` | `RTM_MESSAGE_TYPE` | Optional | `RTM_MESSAGE_TYPE_BINARY` | Message type. See [`RTM_MESSAGE_TYPE`](#rtm_message_type). |
...
uint64_t requestId;
streamChannel->publishTopicMessage("topic_name", message, 5, options, requestId);
```
...
| Properties | Type | Required | Default | Description |
| :----------: | :----: | :------: | :----: | ------------------------------------------------ |
| `channelType` | `RTM_CHANNEL_TYPE` | Optional | `RTM_CHANNEL_TYPE_MESSAGE` | Channel type. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `messageType` | `RTM_MESSAGE_TYPE` | Optional | `RTM_MESSAGE_TYPE_BINARY` | Message type. See [`RTM_MESSAGE_TYPE`](#rtm_message_type). |
| `customType` | const char* | Optional | `NULL` | A user-defined field. Only supports string type. |
...
| `channelName` | const char* | Required | - | Channel name. |
| `channelType` | `RTM_CHANNEL_TYPE` | Required | - | Channel types. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `data` | `const Metadata&` | Required | - | Metadata item. See [`Metadata`](#metadata). |
...
| `requestId` | const uint64_t | Request identifier. |
| `channelName` | const char* | Channel name. |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel type. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `errorCode` | `RTM_ERROR_CODE` | [Error codes](#error-codes-table). |
...
| `channelName` | const char* | Channel name. |
| `channelType` | `RTM_CHANNEL_TYPE` | Channel type. See [`RTM_CHANNEL_TYPE`](#rtm_channel_type). |
| `data` | `const Metadata&` | Metadata item. See [`Metadata`](#metadata). |
| `errorCode` | `RTM_ERROR_CODE` | [Error codes](#error-codes-table). |
...
| `-10001` | `RTM_ERROR_NOT_INITIALIZED` | The SDK is not initialized. Please initialize the `IRtmClient` instance by calling the `createAgoraRtmClient` method before performing other operations. |
...
| `-11040` | `RTM_ERROR_CHANNEL_SUBSCRIBE_CANCELED` | The channel subscription is canceled before it completes. |
...
| `-13014` | `RTM_ERROR_PRESENCE_INACTIVE` | The Presence service is not enabled or is currently unavailable. |
| `-13015` | `RTM_ERROR_PRESENCE_PENDING_REQUEST_CANCELLED` | The pending Presence request is canceled. |
| `-13016` | `RTM_ERROR_PRESENCE_PENDING_REQUEST_EXCEED_LIMITATION` | The number of pending Presence requests exceeds the limit. |
