...
| `cloudProxy` | boolean | Optional | `false` | Whether to enable the cloud proxy: <br />- `true`: Enable. <br />- `false`: Disable. Note: This feature applies to the Message channels and User channels only. |
...
| `status` | (Deprecated) Receive event notifications when client connection status changes. For details, see [SDK connection state](#deprecated-sdk-connection-states) and [SDK connection state change reason](#deprecated-reasons-for-the-sdk-connection-state-change). |
