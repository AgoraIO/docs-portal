...
`RTMConfig` is a plain object with the following shape. Pass it as the third argument when you create an `RTM` instance:

```javascript
const rtmConfig = {
    // Set any of the properties below.
};
```

| Property | Type | Required | Default | 描述 |
| :---------------: | :--------: | :------: | :-----: | ---------------------------------------- |
| `encryptionMode` | string | Optional | - | Encryption mode for end-to-end messages. If you do not set this property or set it as `NONE`, end-to-end encryption is disabled. For details, see [Encryption Mode](#end-to-end-encryption-modes). |
...
| `logLevel` | string | Optional | - | Set the output level of SDK log. For details, see [log output level](#log-output-levels). |
...
|:-------------------:|:---------------------:|:--------:|:--------------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `serviceType` | string[] | Optional | - | Service type. See [Service Types](#service-types). |
...
| `linkState` | Receive event notifications when client connection status changes. For details, see [SDK Link State Types](#sdk-link-state-types). |
...
try {
    const result = await stChannel.subscribeTopic( "Gesture", { users:UIDs } );
    console.log(result);
} catch (status) {
    console.log( status);
}
...
```javascript
try {
    const result = await stChannel.unsubscribeTopic( "Gesture", { users:["Tony","Bo"] });
    console.log("unsubscribe Topic success: ", result);
} catch (status) {
    console.log("unsubscribe Topic failed: ", status);
}
```
...
```javascript
try {
    const result = await stChannel.unsubscribeTopic("Gesture");
    console.log("unsubscribe topic success: ", result);
} catch (status) {
    console.log("unsubscribe topic failed: ", status);
}
...
```javascript
try{
    const result = await rtm.lock.revokeLock(
        "chat_room",
        "STREAM",
        "my_lock",
        "Tony"
...
If you no longer need a lock, you can remove the lock by calling the `removeLock` method. When the lock is successfully removed, all the users in the channel receives the `lock` event notification in the `REMOVED` type. For details, see [Event Listeners](#event-listeners).
...
```javascript
try{
    const result = await rtm.lock.removeLock(
        "chat_room",
        "STREAM",
        "my_lock"
    );
