...
| `encryptionSalt` | Uint8List | Optional | `null` | User-defined encryption salt, length is 32 bytes. Agora recommends using OpenSSL to generate salt on the server side. |
...
// Use a 32-byte salt generated on your server
final salt = Uint8List.fromList(List<int>.filled(32, 0));
...
| `config` | `RtmConfig` | Optional | - | Configuration parameters for the Signaling client. See [`RtmConfig`](#rtmconfig). |
...
var (status,rtmClient) = await RTM("myAppId", "Tony", config:rtmConfig);
...
| `data` | `Metadata` | Metadata of the channel or user. Contains a list of [`MetadataItem`](#metadataitem). |
| `timestamp` | int | The timestamp when the event occurs. |
...
| `reasonCode` | `RtmLinkStateChangeReason` | The reason code for the current state transition. See [`RtmLinkStateChangeReason`](#rtmlinkstatechangereason). |
| `reason` | String | A human-readable description of the reason for the current state transition. |
...
The methods in this section belong to the `RtmClient` instance.
...
The methods in this section belong to the `StreamChannel` instance.
...
```dart
var payload = Uint8List.fromList([1,2,3,4]);
...
| :--------------: | :--------------------------------------------------------: | :------: | :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `topic` | String | Required | - | Topic name. |
| `users` | `List&lt;String&gt;` | Optional | - | A list of user IDs of message publishers that you want to subscribe to. If you do not set this property, you can randomly subscribe to up to 64 users by default. |
...
| :---------: | :----: | :------: | :----: | ---------------------------------------------------------- |
| `topic` | String | Required | - | Topic name. |
| `users` | `List&lt;String&gt;` | Optional | - | A list of user IDs of message publishers that you want to unsubscribe from. If you do not set this property, you can randomly unsubscribe from up to 64 users. |
...
- Message Channel: The real-time channel. Messages are transmitted through the channel, and the channel is highly scalable. Local users can call the `publish` method, set the `channelType` parameter to `message`, and set the `channelName` parameter to the channel name to send messages in the channel. The remote users can call the `subscribeTopic` method to subscribe to the channel and receive messages.
...
You can directly call the `publish` method to send string messages to all online users subscribed to this channel. Even if you do not subscribe to the channel, you can still send messages in the channel.
...
| `customType` | String | Optional | - | A user-defined field. Only supports string type. |
| `storeInHistory` | bool | Optional | `false` | Whether to store the message in history. When set to `true`, the message is stored on the server and users can query historical messages after joining the channel. |
...
The methods in this section belong to the `RtmPresence` instance.
...
The methods in this section belong to the `RtmStorage` instance.
...
The methods in this section belong to the `RtmLock` instance.
...
| :------ | :------------------|:---------------------------|
| `0` | `ok` | Correct call |
| `-10001` | `notInitialized` | The SDK is not initialized. Please initialize the `RtmClient` instance by calling the `RTM()` method before performing other operations. |
...
| `-10007` | `initServiceFailed` | SDK initialization failed. Please reinitialize by calling the `RTM()` method. |
...
| `-10018` | `instanceAlreadyReleased` | Repeat `RtmClient` instantiation or `StreamChannel` instantiation. |
...
| `-11009` | `channelInvalidMessage` | Invalid message. Check whether the message type is legal, Signaling only supports `string`, `Uint8List` type messages. |
...
| `-11023` | `channelEncryptMessageFailed` | Message encryption failed: <br /> - Check that the `encryptionKey` is valid. <br /> - Check that the `encryptionSalt` is valid. <br /> - Check if `encryptionMode` mode matches the `encryptionKey` and `encryptionSalt`. |
