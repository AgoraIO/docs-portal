---
title: Linux Java
description: API reference for the Agora Signaling Linux Java SDK.
---
...
RtmEventListener eventListener = new RtmEventListener() {
 ...
 // Pass the listener to the Builder when you create the RtmConfig instance
RtmConfig rtmConfig = new RtmConfig.Builder("yourAppId", "Tony")
    .eventListener(eventListener)
    .build();
```
...
rtmClient.getPresence().getUserChannels("Tony", new ResultCallback<ArrayList<ChannelInfo>>() {
    ...
    public void onSuccess(ArrayList<ChannelInfo> channels) {
        log(CALLBACK, "get Tony's getUserChannels success");
...
rtmClient.getLock().revokeLock("chat_room", RtmChannelType.STREAM, "my_lock", lockOwner, new ResultCallback<Void>() {
    @Override
    public void onSuccess(Void responseInfo) {
        log(CALLBACK, "revoke lock(" + "my_lock" + ") success");
    }
