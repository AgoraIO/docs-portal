...
| `protocolType` | `RtmProtocolType` | Optional | `TCP_UDP` | Protocol types for message transmission. By default, Signaling utilizes one-way TCP and one-way UDP protocols for transmission. You have the flexibility to modify the protocol types based on your requirements. See [`RtmProtocolType`](#rtmprotocoltype). |
...
| `fileSize` | int | Optional | `1024` | Log file size in KB, with a value range of [128,1024].<br />- If the value you enter is less than 128, the SDK sets the value to 128.<br />- If the value you enter is greater than 1024, the SDK sets the value to 1024. |
...
| Properties | Type | Required | Default | Description |
|:-------------------:|:---------------------:|:--------:|:--------------------------:| ----------------------------------------------------------------------------------------------------------------------------------- |
| `serviceType` | `EnumSet<RtmServiceType>` | Optional | - | Service type. See [`RtmServiceType`](#rtmservicetype). |
...
| `publisherId` | String | User ID of the message publisher. |
...
| Properties | Type | Description |
|:------------------:|:-------------------:| ---------------------- |
| `data` | Object | Message content. Cast it to `String` or `byte[]` according to `type`. |
| `type` | `RtmMessageType` | Message type. See [`RtmMessageType`](#rtmmessagetype). |
...
| Properties | Type | Description |
|:------------------:|:-------------------:| ---------------------- |
| `eventType` | `RtmPresenceEventType` | Presence event type. See [`RtmPresenceEventType`](#rtmpresenceeventtype). |
| `channelType` | `RtmChannelType` | Channel types. See [`RtmChannelType`](#rtmchanneltype). |
| `channelName` | String | Channel name. |
| `publisherId` | String | User ID of the message publisher. |
| `stateItems` | `HashMap<String, String>` | Key-value pairs that identify the user's presence state. |
...
Each entry in the `stateItems` map contains the following:
...
| `data` | `Metadata` | Metadata item. See [`Metadata`](#metadata). |
| `timestamp` | long | The timestamp when the event occurs. |
...
| `reasonCode` | `RtmLinkStateChangeReason` | The reason for this state transition. See [`RtmLinkStateChangeReason`](#rtmlinkstatechangereason). |
...
| `errorReason` | String | Error reason for this operation. |
...
| Properties | Type | Required | Default | Description |
| :------------: | :-----: | :------: | :----: | ------------------------------------------------------ |
| `token` | String | Required | - | An RTC token for joining a stream channel.<br />- If you have enabled token authentication for your project, generate a [temporary RTC token](/en/introduction/account#generate-temporary-tokens) or obtain an RTC token generated by your [token server](/en/realtime-media/rtc/build/authenticate-users/deploy-token-server).<br />- If you have not enabled token authentication for your project, use an empty string or the App ID of your project that has RTC and Signaling services enabled. |
| `withPresence` | boolean | Optional | `true` | Whether to subscribe to presence event notifications in the channel. |
| `withMetadata` | boolean | Optional | `false` | Whether to subscribe to storage event notifications in the channel. |
| `withLock` | boolean | Optional | `false` | Whether to subscribe to lock event notifications in the channel. |
...
| Properties | Type | Required | Default | Description |
| :-------------: | :------------------------------------------------------: | :------: | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `messageQos` | `RtmMessageQos` | Optional | `UNORDERED` | Whether the data transmitted in the topic is ordered. See [`RtmMessageQos`](#rtmmessageqos). |
...
options.setUsers(new ArrayList<>(Arrays.asList("UserA","UserB","UserC")));
...
```java
TopicOptions options = new TopicOptions();
options.setUsers(new ArrayList<>(Arrays.asList("Tony","Bo")));
...
```java
String queryUserId = "Tony";
rtmClient.getPresence().getUserChannels(queryUserId, new ResultCallback<ArrayList<ChannelInfo>>() {
    @Override
...
Each entry in the `items` map contains the following:
...
| :--------------: | :-----------------: | :------: | :-----------: | ----------------------------------- |
| `channelName` | String | Required | - | Channel name. |
| `channelType` | `RtmChannelType` | Required | - | Channel type. See [`RtmChannelType`](#rtmchanneltype). |
| `data` | `Metadata` | Required | - | Metadata item. See [`Metadata`](#metadata). |
...
| `channelName` | String | Required | - | Channel name. |
| `channelType` | `RtmChannelType` | Required | - | Channel type. See [`RtmChannelType`](#rtmchanneltype). |
| `resultCallback` | `ResultCallback<Metadata>` | Required | - | Invocation result callback:<br />- Success: Executes the `onSuccess` method and return the [`Metadata `](#metadata) query result.<br />- Failure: Executes the `onFailure` method. |
...
rtmClient.getLock().revokeLock("chat_room", RtmChannelType.STREAM, lockName, lockOwner, new ResultCallback<Void>() {
    @Override
    public void onSuccess(Void responseInfo) {
...
rtmClient.getLock().getLocks("chat_room", RtmChannelType.STREAM, new ResultCallback<ArrayList<LockDetail>>() {
    ...
    public void onSuccess(ArrayList<LockDetail> details) {
        ...
        for (LockDetail detail : details) {
            log(INFO, "lock name: " + detail.getLockName() + ", lock owner: " + detail.getLockOwner() + ", ttl: " + detail.getTTL());
        }
    }
