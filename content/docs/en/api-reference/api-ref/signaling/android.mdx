...
| `totalOccupancy` | int | The total number of online users in the current channel. In large-channel optimization mode, this value indicates the number of all online users in the channel and is not limited by the maximum of 512 user details that are continuously synchronized. This value may have a short delay. |
