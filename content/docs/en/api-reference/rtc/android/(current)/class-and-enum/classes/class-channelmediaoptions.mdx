...
支持在同一个 [RtcConnection](/en/api-reference/rtc/android/class-and-enum/classes/class-rtcconnection) 中同时发布多个音频流和一个视频流。例如，可以同时启用 `publishMicrophoneTrack`、 `publishCustomAudioTrack` 和 `publishMediaPlayerAudioTrack`，但 `publishCameraTrack`、 `publishScreenCaptureVideo`、 `publishCustomVideoTrack` 和 `publishEncodedVideoTrack` 中只能有一个设置为 true。
...
要发布的自定义音频轨道的 ID。可通过 [createCustomAudioTrack](/en/api-reference/rtc/android/audio/audio-custom-capturenrendering#api_irtcengine_createcustomaudiotrack) 方法获取该 ID。
...
首选的传输路径类型。详见 [MultipathType](/en/api-reference/rtc/android/class-and-enum/enums/enum-multipathtype)。
