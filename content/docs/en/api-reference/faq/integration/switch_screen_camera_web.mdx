...
This page introduces three solutions for switching between the screen-sharing stream and the stream captured by the camera in a [screen-sharing](/en/3.x/interactive-live-streaming/basic-features/screensharing) scenario using the Agora Video SDK for Web.
...
Create two Client objects on the sender, which correspond to the screen-sharing stream and the camera stream. See [Share the Screen](/en/3.x/interactive-live-streaming/basic-features/screensharing) for details.
