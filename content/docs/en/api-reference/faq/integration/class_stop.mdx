...
3. To end the course without direct interaction, call Flexible Classroom backend RESTful interface [Set Classroom Status](/en/flexible-classroom/restful-api/classroom-api?platform=android#set-the-classroom-state). Set the class status to one of the following values:
