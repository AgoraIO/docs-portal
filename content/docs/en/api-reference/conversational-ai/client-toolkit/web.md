...
| `transcription` | `ISubtitleHelperItem>[]` | An array of transcripts of the conversation between the user and the agent. See [`ISubtitleHelperItem`](#isubtitlehelperitem). |
...
| `rtmEngine` | `RTMClient` | Agora RTM engine instance. See [`RTMClient`](https://doc.shengwang.cn/signaling/reference/api?platform=web#rtmclientag_platform). |
...
```javascript
export interface ISubtitleHelperItem {
 uid: string;
 stream_id: number;
 turn_id: number;
 _time: number;
