...
- **Custom tools for MLLM agents**
  ...
  MLLM agents can call your own HTTPS endpoints as tools. Tool definitions use the same format as for cascading LLM agents, so an existing tool works with either agent type. Available in the Agora Agents SDKs and the RESTful API. For details, see [Call custom tools](build/custom-model-integration/custom-tools).
...
- **MCP tool-use for MLLM agents in the Agora Agents SDKs**
  ...
  MLLM agents have supported MCP tool-use through the RESTful API since v2.12. This release adds the same support to the Agora Agents SDKs, which accept MCP servers on their MLLM vendor classes, so you can configure MCP tool-use without calling the RESTful API directly. For details, see [Use MCP tools](build/mcp-tools).
...
- **MCP tool-use for MLLM agents**
  ...
  MLLM agents now support MCP tool-use, in addition to cascading LLM agents. Available only in the RESTful API. For details, see [Use MCP tools](build/mcp-tools).
