...
- **New MLLM provider**
  ...
  - [OpenAI GPT-Live](models/mllm/openai-gpt-live), as an early access preview. Preview agents use a separate endpoint and request header, so see the vendor page before you start one.
...
- **Generated filler words use conversation context**
  ...
  This release improves generated filler words so that they reflect the recent exchange, not just the current turn. You can now set the conversation context for the generation service using a message count and a character budget. For details, see [Talking while waiting](build/shape-the-conversation/filler-words).

#### API changes

This release introduces the following changes for the new Smallest AI and OpenAI GPT-Live providers, Gemini ASR, and conversation context in generated filler words.
...
- **Updated behavior**
    ...
    - `prompt`: A custom prompt now fully replaces Agora's default prompt, and applies over the same conversation context window.
...
- Changes to [**Start a conversational AI agent**](/en/api-reference/api-ref/conversational-ai/join)
  ...
  - **Updated parameters**
    ...
    - [`filler_words.content.generated_config.prompt`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-filler-words-content-generated-config-prompt): A custom prompt now fully replaces Agora's default prompt, and applies over the same conversation context window.
