...
- **Generic HTTP TTS provider**
  ...
  Integrate a custom TTS service that implements the OpenAI TTS protocol using the new `generic_http` TTS vendor. Configure [`tts.url`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-tts-url), [`tts.headers`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-tts-headers), and `tts.params` when starting the agent. For details, see [Generic TTS](/en/ai/models/tts/generic-http) and [Connect your own TTS service](/en/ai/build/custom-model-integration/custom-tts).
