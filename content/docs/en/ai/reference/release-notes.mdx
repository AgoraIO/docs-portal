...
Released on July XX, 2026.

#### New features

Included in this release:

- **Generic HTTP TTS provider (Beta)**

  Integrate a custom TTS service that implements the OpenAI TTS protocol using the new `generic_http` TTS vendor. Configure `tts.url`, `tts.headers`, and `tts.params` when starting the agent. For details, see [Generic TTS](/en/ai/models/tts/generic-http).

- **Client toolkit support for manual turn control**

  The Android, iOS, and Web client toolkits now provide dedicated APIs for manual Start of Speech (SoS) and End of Speech (EoS) control. Call `manualSOS`/`manualEOS` to send requests, and register the new result callbacks to handle the server's response.  This capability is suitable for scenarios that require explicit control of user turn boundary from the client side, such as AI interviews, interactive quizzes, and walkie-talkie. For details, see [Manually control start and end of speech](/en/ai/best-practices/manual-turn-control).
...
- Changes to [**Start a conversational AI agent**](/en/api-reference/api-ref/conversational-ai/join)
  ...
  - **New parameters**
    ...
    - [`llm.headers`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-llm-headers): Allows you to pass custom headers in LLM requests, such as business-specific fields or tenant identifiers.
...
- Changes to [**Start a conversational AI agent**](/en/api-reference/api-ref/conversational-ai/join)
  ...
  - **AIVAD activation**
    ...
    The `advanced_features.enable_aivad` field is now deprecated; to configure AIVAD activation use [`turn_detection.config.end_of_speech.mode = "sematic"`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-turn-detection-config-end-of-speech-mode).
