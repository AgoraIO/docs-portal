...
[Voice AI quickstart](/en/ai/apps/get-started/quickstart) guide. 

> **Info**
> This project uses Agora-managed presets, so no vendor API keys are required to complete this tutorial. If you prefer to switch to your own vendor accounts, see the documentation for your chosen [ASR](/en/ai/apps/models/asr), [LLM](/en/ai/apps/models/llm), and [TTS](/en/ai/apps/models/tts) providers.
...
In the meantime, use the [Voice AI quickstart](/en/ai/apps/get-started/quickstart)
...
> **Coming soon**
> The Go backend walkthrough is not yet available. It will mirror the Python path: a standalone backend on port 8000, a Next.js web client on port 3000, and the same three HTTP endpoints. In the meantime, see the [Voice AI quickstart](/en/ai/apps/get-started/quickstart) guide to get started with Go.
...
> [Voice AI quickstart](/en/ai/apps/get-started/quickstart) guide in the meantime.
...
- [Integrate an MLLM](../../models/mllm): Replace the cascading STT → LLM → TTS pipeline with a single realtime model.
- [Transmit custom information](custom-information): Guide the agent with user-specific context to personalize responses.
- [Integrate short-term memory](short-term-memory): Help the agent maintain context across a conversation.
- [Receive webhook notifications](webhooks): Receive agent event notifications in real time.
- [Use filler words](filler-words): Reduce perceived latency by filling silence during LLM processing.
- [Optimize conversation latency](/en/ai/apps/build/harden-and-optimize/optimize-latency): Tune LLM, ASR, and TTS components for lower end-to-end latency.
