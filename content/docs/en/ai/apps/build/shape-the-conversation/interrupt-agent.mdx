...
To customize interruption behavior, configure the `turn_detection` parameters when calling [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join).

To enable AIVAD-based interruption, set [`turn_detection.config.end_of_speech.mode`](/en/api-reference/api-ref/conversational-ai/join#properties-turn-detection-config-end-of-speech-mode) to `"semantic"`.
...
- Integrate VSDK v4.5.1 or later and follow the [Quickstart](/en/introduction/realtime-audio-video) guide to implement basic real-time audio and video features.
- Enable the SIG service for your project in the Agora Console and follow the [SIG Quickstart](/en/realtime-media/rtm) to implement real-time messaging.
...
Call [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join) using the following parameter settings:
