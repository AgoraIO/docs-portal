...
> 信息
> - 优雅打断为增值服务，开启后会收取费用，详见[计费说明](../overview/billing)。
> - 当语义判定用户未结束发言时，会等待一段时间才最终判定对话结束，此时响应延时会有所增加。

优雅打断功能默认关闭，你需要在调用 [POST 创建对话式智能体](../operations/start-agent)时，设置 `turn_detection.config.end_of_speech.mode` 为 `semantic` 开启优雅打断。以 curl 为例：
...
- [打断智能体](./interrupt-agent)
- [实时字幕](./realtime-sub)
- [监听智能体相关事件](./listen-agent-events)
- [设置最佳音频参数](../best-practice/audio-settings) （仅支持 Android 和 iOS）
- [发送图片消息](./send-multimodal-message)
...
- 已参考[实现对话式智能体](../get-started/quick-start)实现与智能体对话的基本逻辑。
...
调用 [POST 创建对话式智能体](../operations/start-agent)接口，并完成以下参数设置：
...
- [POST 创建对话式智能体](../operations/start-agent)
- [POST 打断智能体](../operations/agent-interrupt)
