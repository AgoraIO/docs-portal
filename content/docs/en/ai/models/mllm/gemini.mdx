...
`gemini-3.5-live` and `gemini-3.5-live-thinking` are available as an early access preview. When using either model, send your [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join) request to the following preview endpoint instead of the standard endpoint, and include the required header:
