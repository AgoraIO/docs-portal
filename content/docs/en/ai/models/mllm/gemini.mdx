...
Set `params.model` to one of the following Gemini Live models:

| Model | Description |
| --- | --- |
| `gemini-3.1-flash-live-preview` | General-purpose Gemini Live model. |
| `walkie-talkie` | Preview Gemini Live model. See [walkie-talkie and clever-chatter](#walkie-talkie-and-clever-chatter). |
| `clever-chatter` | Preview Gemini Live model. Supports an additional `thinking_level` parameter. See [walkie-talkie and clever-chatter](#walkie-talkie-and-clever-chatter). |
...
### walkie-talkie and clever-chatter

`walkie-talkie` and `clever-chatter` are early access preview models, each with a dedicated SDK vendor class: `GeminiWalkieTalkie` and `GeminiCleverChatter`. Unlike the production Gemini Live vendor shown in [Sample configuration](#sample-configuration), these require the preview client (`AgoraPreview`, `AgoraPreviewClient`, or `NewAgoraPreviewClient`) instead of the standard client. The preview client points requests at the preview endpoint and attaches the required header automatically.

`clever-chatter` additionally supports a `thinking_level` parameter (`low`, `medium`, or `high`) that controls its reasoning budget. `walkie-talkie` does not support this parameter.
...
from agora_agent.agentkit.preview import AgoraPreview, GeminiCleverChatter
...
To use `walkie-talkie` instead, import `GeminiWalkieTalkie` and drop `thinking_level`, which it does not support.
...
```typescript
import { Agent, AgoraPreviewClient, Area, GeminiCleverChatter } from 'agora-agents';
...
const agent = new Agent({ client })
  .withMllm(new GeminiCleverChatter({
    apiKey: 'your-google-api-key',
    voice: 'Puck',
...
To use `walkie-talkie` instead, import `GeminiWalkieTalkie` and drop `thinkingLevel`, which it does not support.
...
agent := agentkit.NewAgent(client).WithMllm(
    vendors.NewGeminiCleverChatter(vendors.GeminiCleverChatterOptions{
        GeminiPreviewMLLMOptions: vendors.GeminiPreviewMLLMOptions{
            APIKey: "your-google-api-key",
            Voice:  "Puck",
        },
...
To use `walkie-talkie` instead, call `vendors.NewGeminiWalkieTalkie(vendors.GeminiPreviewMLLMOptions{...})`, which has no `ThinkingLevel` field.
...
:::info[Important]
`walkie-talkie` and `clever-chatter` are early access preview models. Send your [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join) request to the following preview endpoint instead of the standard endpoint, and include the required header:
...
Unlike the production Gemini Live payload, the preview payload repeats `api_key` inside `params`, always includes `params.language`, and defaults `url` to the Gemini Developer API host. Use the following `mllm` configuration in your request:
...
To use `walkie-talkie` instead, set `params.model` to `"walkie-talkie"` and remove `params.thinking_level`, which only `clever-chatter` supports.
...
<ParameterList title="mllm" required={true}>
  ...
  <Parameter name="params" type="object" required={true}>
    ...
    <Parameter name="model" type="string" required={true} possibleValues="gemini-3.1-flash-live-preview, walkie-talkie, clever-chatter">
      The Gemini Live model identifier. See [Supported models](#supported-models) for available options.
    </Parameter>
  ...
  <Parameter name="turn_detection" type="object" required={false}>
    Turn detection configuration for the MLLM module. For a full list of `turn_detection` parameters, see [`mllm.turn_detection`](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-mllm-turn-detection).
