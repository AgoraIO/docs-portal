...
| `gemini-3.5-live` | Gemini Live model. |
| `gemini-3.5-live-thinking` | Gemini Live thinking model. |
...
:::info[Important]
`gemini-3.5-live` and `gemini-3.5-live-thinking` are early access preview models. When using either model, send your [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join) request to the following preview endpoint instead of the standard endpoint, and include the required header:
...
<ParameterList title="mllm" required={true}>
  ...
  <Parameter name="params" type="object" required={true}>
    ...
    <Parameter name="model" type="string" required={true} possibleValues="gemini-3.1-flash-live-preview, gemini-3.5-live, gemini-3.5-live-thinking">
