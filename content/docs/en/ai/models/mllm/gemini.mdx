...
| `walkie-talkie` | Preview Gemini Live model. |
| `clever-chatter` | Preview Gemini Live model. |

:::info[Info]
`walkie-talkie` and `clever-chatter` are available as an early access preview. To use either model, send your [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join) request to the following preview endpoint instead of the standard endpoint, and include the required header:
