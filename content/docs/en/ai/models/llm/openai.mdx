...
- **Managed mode**
  ...
  To use an OpenAI LLM with an Agora managed key, specify one of the following presets in the [`preset`](/en/api-reference/conversational-ai/rest-api/agent/join#preset) field when starting an agent:
...
For complete `llm` configuration options, see [LLM parameters](/en/api-reference/conversational-ai/rest-api/agent/join#properties-llm). For OpenAI-specific options and model capabilities, see the [OpenAI API documentation](https://platform.openai.com/docs/api-reference/chat).
