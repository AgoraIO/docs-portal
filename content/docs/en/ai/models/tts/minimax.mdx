...
<ParameterList title="params">
  <Parameter name="key" type="string" optional>
    The MiniMax API key used to authenticate requests. Required when using your own API key (BYOK).
  </Parameter>
  <Parameter name="group_id" type="string" optional>
    The MiniMax group identifier. Required when using your own API key (BYOK).
  </Parameter>
  <Parameter name="model" type="string" optional>
    The TTS model to use. Required when using your own API key (BYOK). For example, `speech-02-turbo`.
  </Parameter>
  <Parameter name="voice_setting" type="object" required>
    Voice configuration object.

    <ParameterList>
      <Parameter name="voice_setting.voice_id" type="string" required>
        The voice style identifier to use. For example, `English_captivating_female1`.
      </Parameter>
      <Parameter name="voice_setting.speed" type="number" optional>
        Speaking rate multiplier. `1.0` is the normal speed.
      </Parameter>
    </ParameterList>
  </Parameter>
  <Parameter name="audio_setting" type="object" optional>
    Audio output configuration.

    <ParameterList>
      <Parameter name="audio_setting.sample_rate" type="number" optional>
