...
<ParameterList title="params" required={true}>
  ...
  <Parameter name="model" type="string" possibleValues="bulbul:v2, bulbul:v3" required={false}>
  The Sarvam TTS model version to use.
  </Parameter>
  <Parameter name="speech_sample_rate" type="number" defaultValue="24000" possibleValues="8000, 16000, 22050, 24000, 32000, 44100, 48000" required={false}>
  The audio sample rate in Hz.
  </Parameter>
  <Parameter name="enable_preprocessing" type="boolean" required={false}>
  Whether to normalize English words and numeric entities, such as dates and currency, before speech synthesis.
  </Parameter>
</ParameterList>
