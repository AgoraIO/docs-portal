...
- **MCP tool calls**: When the agent invokes tools through [MCP servers](../../api-reference/conversational-ai/rest-api/agent/join.md#properties-llm-mcp-servers), response times can increase significantly. Filler words bridge this gap while the agent waits for tool results.
...
- An Agora [account](../../api-reference/conversational-ai/rest-api/agent/join.md) and project with Conversational AI enabled. 
- A working Conversational AI Engine project. If you don't have one yet, follow the [quickstart](../get-started/quickstart.md) to set one up.

## Implementation

### Enable filler words

To enable filler words, add the `filler_words` object to `properties` when calling the [Start a conversational AI agent](../../api-reference/conversational-ai/rest-api/agent/join.md) API. Set `enable` to `true`:
...
Add the following `filler_words` object to `properties` in your [Start a conversational AI agent](../../api-reference/conversational-ai/rest-api/agent/join.md) request body. This example plays a random filler phrase if the LLM takes longer than 1.5 seconds to respond:
