...
- Firmware configuration, build, flash, and end-to-end verification
- Device network access through firmware configuration
- Optional BLE provisioning when you need to change Wi-Fi without rebuilding firmware

## Recommended order

1. [Run the demo server](run-the-demo-server)
2. [Build and flash firmware](build-and-flash-firmware)
3. Restart the device near the 2.4 GHz Wi-Fi network configured in firmware
4. Verify the device interaction using the checks below

Use [Optional BLE network provisioning](configure-device-network) only if you need to change Wi-Fi settings after flashing.
...
2. Restart or wake the device near the 2.4 GHz Wi-Fi network configured in the firmware.
...
- [Optional BLE network provisioning](configure-device-network)
