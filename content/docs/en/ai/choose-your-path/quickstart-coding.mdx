...
If the `agora` command is not found after installation, re-run with `--add-to-path` or manually add the install directory to your shell profile.
...
If the agent does not join or transcripts do not appear, run `agora project doctor` to check credential validity, feature enablement, and network reachability.

## Recipes

Use these task-focused recipes after your starter is running:

- [Start and stop an agent](../build/start-stop-agent): Create and end a voice agent session with the REST API.
- [Use presets](../build/presets): Start with Agora-managed ASR, LLM, and TTS providers.
...
- [Transmit custom information](../build/custom-information): Guide the agent with user-specific context to personalize responses.
- [Integrate short-term memory](../build/short-term-memory): Help the agent maintain context across a conversation.
- [Use filler words](../build/filler-words): Reduce perceived latency by filling silence during LLM processing.

## Reference

- [REST API reference](/en/api-reference/conversational-ai/rest-api)
- [Typescript Server SDK](../../api-reference/conversational-ai/server-sdk/typescript)
- [Python Server SDK](../../api-reference/conversational-ai/server-sdk/python)
- [Go Server SDK](../../api-reference/conversational-ai/server-sdk/go)

## Full-control path

- [Architecture](../build/architecture): Understand how the backend, client, RTC, RTM, and agent session fit together.
- [Build a backend and client from scratch](../build/build-server-client): Follow the full end-to-end tutorial if you want every implementation step.
