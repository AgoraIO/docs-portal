...
For the full client setup and platform examples, see [Listen to agent runtime events on software clients](event-notifications.md).
...
For the full webhook setup and signature verification steps, see [Receive agent runtime events on your server](webhooks.md).
...
- If you want to understand what the webhook event numbers mean, see [Notification event types](../reference/event-types.md).
- If you want to decide which events to use for state, errors, and performance, see [Monitor agent status, errors, and performance](monitor-agent-runtime.md).
- If you want to investigate failures, see [Debug agent failures with runtime events](debug-agent-failures.mdx).
- If you want post-session history and turn data, see [Retrieve conversation history after a session ends](retrieve-session-history.mdx).
