...
An `eventType` of `101` indicates that an agent has joined a channel. The `payload` contains the following fields:
    ...
    The agent name provided when calling [Start a conversational AI agent](../../api-reference/conversational-ai/rest-api/agent/join.md). Unique within a channel.
...
An `eventType` of `110` indicates that an agent has encountered an error. The `payload` contains the following fields:
    ...
    Transcript conversation turn. For details, see [Live Transcript API Reference](transcripts.md#api-reference).
