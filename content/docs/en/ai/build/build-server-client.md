...
[Voice AI quickstart](../get-started/quickstart.md) guide. 

> **Info**
> This project uses Agora-managed presets, so no vendor API keys are required to complete this tutorial. If you prefer to switch to your own vendor accounts, see the documentation for your chosen [ASR](../models/asr/index.md), [LLM](../models/llm/index.md), and [TTS](../models/tts/index.md) providers.
...
1. Write your Agora credentials to `.env.local`:
    ...
    cat > .env.local < server-python/.env < web-client/.env.local < **Coming soon**
...
In the meantime, use the [Voice AI quickstart](../get-started/quickstart.md)
...
> **Coming soon**
> The Go backend walkthrough is not yet available. It will mirror the Python path: a standalone backend on port 8000, a Next.js web client on port 3000, and the same three HTTP endpoints. In the meantime, see the [Voice AI quickstart](../get-started/quickstart.md) guide to get started with Go.
...
export type InviteResponse = { agentId: string; agentUid: number };
  ...
  token: (channel: string, uid: number) => post('/api/token', { channel, uid }),
  invite: (channel: string) => post('/api/invite-agent', { channel }),
...
> [Voice AI quickstart](../get-started/quickstart.md) guide in the meantime.
...
- [Transmit custom information](custom-information.md): Guide the agent with user-specific context to personalize responses.
- [Integrate short-term memory](short-term-memory.md): Help the agent maintain context across a conversation.
- [Receive webhook notifications](webhooks.md): Receive agent event notifications in real time.
- [Use filler words](../best-practices/filler-words.md): Reduce perceived latency by filling silence during LLM processing.
- [Optimize conversation latency](../best-practices/optimize-latency.md): Tune LLM, ASR, and TTS components for lower end-to-end latency.
