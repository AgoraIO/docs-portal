---
title: Call custom tools
description: Let the LLM call your own HTTPS endpoints as tools during a conversation.
---
You can give a Conversational AI agent access to custom tools backed by your own HTTPS endpoints. The LLM decides when to call a tool and generates its parameters from the conversation. The Conversational AI Engine then validates the parameters, makes one synchronous request to your endpoint, and returns the result to the LLM so it can continue generating a response. The HTTP response itself is never spoken directly; only the LLM's generated reply is sent to TTS.

Common use cases include:

- **Looking up business data**: Query order status, account balances, or inventory from your own systems.
- **Performing actions**: Create a support ticket, schedule an appointment, or update a record.
- **Grounding responses**: Pull current, business-specific information into the conversation without retraining the model.

## Understand the tech

Each custom tool declares a `function` block with a name, description, and a JSON Schema for its parameters. It also declares a `server` block that describes the HTTPS request the Conversational AI Engine sends once the LLM has generated valid parameters. Tool calling currently supports only synchronous `GET` and `POST` requests to a single endpoint per tool.

Custom tools, MCP tools, built-in tools, and model-native tools all share one tool namespace. Names are not case-sensitive. An agent can expose at most 32 tools to the LLM. If tool names conflict or the limit is exceeded, the agent fails to start.

The following aren't supported yet:

- `PUT`, `PATCH`, or `DELETE` requests, automatic retries, or following HTTP redirects.
- Asynchronous tools, callbacks, polling, or stored/reusable tool references.
- OAuth token exchange, dynamic request signing, or mTLS for authenticating to your endpoint.

## Prerequisites

- Implemented the basic logic for interacting with a conversational AI agent by following the [quickstart](../../get-started/quickstart).
- A standard text LLM that supports function calling. Custom LLMs, MLLMs, realtime voice models, and Dify aren't supported yet.
- An HTTPS endpoint your agent can call to fulfill the tool.

## Implementation

Take the following steps to add a custom tool to your agent.

### Configure a GET tool

The following example configures a `GET` tool that looks up order status:

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

```python
from agora_agent import OpenAI

# ... other agent configuration ...
.with_llm(OpenAI(
    api_key='your-llm-api-key',
    model='gpt-4o-mini',
    tools=[
        {
            'type': 'function',
            'function': {
                'name': 'getOrderStatus',
                'description': "Get the shipping status for a customer's order by order ID.",
                'parameters': {
                    'type': 'object',
                    'properties': {
                        'order_id': {
                            'type': 'string',
                            'description': 'The order ID, for example ORD-10293.',
                        },
                    },
                    'required': ['order_id'],
                    'additionalProperties': False,
                },
            },
            'server': {
                'method': 'GET',
                'url': 'https://api.example.com/orders/{{args.order_id}}/status',
                'headers': {
                    'Authorization': '{{template_variables.api_key}}',
                },
                'timeout_ms': 5000,
            },
        },
    ],
))
.with_tools(True)
# ... continue with .with_tts() and .create_session() ...
```

</TabsContent>

<TabsContent value="typescript">

```typescript
import { OpenAI } from 'agora-agents';

// ... other agent configuration ...
.withLlm(new OpenAI({
  apiKey: 'your-llm-api-key',
  model: 'gpt-4o-mini',
  tools: [
    {
      type: 'function',
      function: {
        name: 'getOrderStatus',
        description: "Get the shipping status for a customer's order by order ID.",
        parameters: {
          type: 'object',
          properties: {
            order_id: {
              type: 'string',
              description: 'The order ID, for example ORD-10293.',
            },
          },
          required: ['order_id'],
          additionalProperties: false,
        },
      },
      server: {
        method: 'GET',
        url: 'https://api.example.com/orders/{{args.order_id}}/status',
        headers: {
          Authorization: '{{template_variables.api_key}}',
        },
        timeout_ms: 5000,
      },
    },
  ],
}))
.withTools(true)
// ... continue with .withTts() and .createSession() ...
```

</TabsContent>

<TabsContent value="go">

```go
tool := &Agora.LlmTool{
    Function: &Agora.LlmToolFunction{
        Name:        "getOrderStatus",
        Description: Agora.String("Get the shipping status for a customer's order by order ID."),
        Parameters: &Agora.LlmToolFunctionParameters{
            Properties: map[string]interface{}{
                "order_id": map[string]interface{}{
                    "type":        "string",
                    "description": "The order ID, for example ORD-10293.",
                },
            },
            Required: []string{"order_id"},
        },
    },
    Execution: &Agora.LlmToolExecution{
        Mode: Agora.String("sync"),
    },
    Server: &Agora.LlmToolServer{
        Method: Agora.LlmToolServerMethodGet,
        URL:    "https://api.example.com/orders/{{args.order_id}}/status",
        Headers: map[string]string{
            "Authorization": "{{template_variables.api_key}}",
        },
        TimeoutMs: Agora.Int(5000),
    },
}

// ... other agent configuration ...
.WithLlm(
    vendors.NewOpenAI(vendors.OpenAIOptions{
        APIKey: "your-llm-api-key",
        Model:  "gpt-4o-mini",
        Tools:  []*Agora.LlmTool{tool},
    }),
)
.WithTools(true)
// ... continue with .WithTts() and .CreateSession() ...
```

</TabsContent>

<TabsContent value="rest-api">

```json
{
  "properties": {
    "advanced_features": {
      "enable_tools": true
    },
    "llm": {
      "url": "https://api.openai.com/v1/chat/completions",
      "api_key": "your-llm-api-key",
      "params": { "model": "gpt-4o-mini" },
      "template_variables": {
        "api_key": "Bearer your-order-service-key"
      },
      "tools": [
        {
          "type": "function",
          "function": {
            "name": "getOrderStatus",
            "description": "Get the shipping status for a customer's order by order ID.",
            "parameters": {
              "type": "object",
              "properties": {
                "order_id": {
                  "type": "string",
                  "description": "The order ID, for example ORD-10293."
                }
              },
              "required": ["order_id"],
              "additionalProperties": false
            }
          },
          "server": {
            "method": "GET",
            "url": "https://api.example.com/orders/{{args.order_id}}/status",
            "headers": {
              "Authorization": "{{template_variables.api_key}}"
            },
            "timeout_ms": 5000
          }
        }
      ]
    }
  }
}
```

</TabsContent>
</Tabs>

To create the agent, the REST API must set `properties.advanced_features.enable_tools` to `true`; the Agora Agents SDKs must call `with_tools(True)`, `withTools(true)`, or `WithTools(true)`. If tool calling isn't enabled, tools aren't exposed to the model and their endpoints are never called.

Once this is configured, a user can ask something like "What's the status of order ORD-10293?" After the model generates `order_id`, the engine sends:

```http
GET https://api.example.com/orders/ORD-10293/status
Authorization: Bearer your-order-service-key
```

#### Field reference

The field names differ slightly by language, since Python, TypeScript, and REST pass a raw object mirroring the JSON body, while Go uses typed structs.

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

Configure tools using the `tools` parameter on the LLM vendor class, and enable tool calling with `Agent.with_tools(True)`.

| Parameter | Required | Description |
|---|---|---|
| `OpenAI(tools=...)` | No | The custom tool array. |
| `Agent.with_tools(True)` | Yes | Enables custom tool and MCP tool invocation together. |
| `tools[].type` | Yes | Fixed as `function`. |
| `tools[].function` | Yes | The tool definition visible to the model. |
| `function.name` | Yes | Must start with a letter, contain only letters and numbers, and be no more than 64 characters long. Can't use the `mcp` prefix or the value `ragsearch_tool`. |
| `function.description` | Yes | Explains the tool's purpose, when to call it, and what it returns. Don't include secrets. |
| `function.parameters` | Yes | A JSON Schema object. The root `type` must be `object`. Passed using OpenAI's non-strict function-calling mode. |
| `execution.mode` | No | Defaults to `sync` when omitted. Currently only `sync` is supported. |
| `server.method` | Yes | Only `GET` and `POST` are supported. `GET` can't configure `body`. |
| `server.url` | Yes | An absolute HTTPS URL. The path and query parameters can use template variables. |
| `server.headers` | No | Up to 32 entries. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `server.body` | Optional for `POST` | `POST` only, supports nested JSON. Template placeholders must be complete, not concatenated with other text. |
| `server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="typescript">

Configure tools using the `tools` option on the LLM vendor class, and enable tool calling with `Agent.withTools(true)`.

| Parameter | Required | Description |
|---|---|---|
| `OpenAI({ tools: [...] })` | No | The custom tool array, of type `LlmTool[]`. |
| `Agent.withTools(true)` | Yes | Enables custom tool and MCP tool invocation together. |
| `tools[].type` | Yes | Fixed as `function`. |
| `tools[].function` | Yes | The tool definition visible to the model. |
| `function.name` | Yes | Must start with a letter, contain only letters and numbers, and be no more than 64 characters long. Can't use the `mcp` prefix or the value `ragsearch_tool`. |
| `function.description` | Yes | Explains the tool's purpose, when to call it, and what it returns. Don't include secrets. |
| `function.parameters` | Yes | A JSON Schema object. The root `type` must be `object`. Passed using OpenAI's non-strict function-calling mode. |
| `execution.mode` | No | Defaults to `sync` when omitted. Currently only `sync` is supported. |
| `server.method` | Yes | Only `GET` and `POST` are supported. `GET` can't configure `body`. |
| `server.url` | Yes | An absolute HTTPS URL. The path and query parameters can use template variables. |
| `server.headers` | No | Up to 32 entries. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `server.body` | Optional for `POST` | `POST` only, supports nested JSON. Template placeholders must be complete, not concatenated with other text. |
| `server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="go">

Configure tools using the `Tools` field on the LLM vendor options, and enable tool calling with `agentkit.WithTools(true)`.

| Parameter | Required | Description |
|---|---|---|
| `OpenAIOptions.Tools` | No | The custom tool array, of type `[]*Agora.LlmTool`. |
| `WithTools(true)` | Yes | Enables custom tool and MCP tool invocation together. |
| `Tool.Function` | Yes | The tool definition visible to the model. |
| `Function.Name` | Yes | Must start with a letter, contain only letters and numbers, and be no more than 64 characters long. Can't use the `mcp` prefix or the value `ragsearch_tool`. |
| `Function.Description` | Yes | Explains the tool's purpose, when to call it, and what it returns. Don't include secrets. |
| `Function.Parameters` | Yes | A JSON Schema object. The root type is serialized as `object` and passed using OpenAI's non-strict function-calling mode. |
| `Execution.Mode` | No | Defaults to `sync` when omitted. Currently only `sync` is supported. |
| `Server.Method` | Yes | Only `GET` and `POST` are supported. `GET` can't configure `Body`. |
| `Server.URL` | Yes | An absolute HTTPS URL. The path and query parameters can use template variables. |
| `Server.Headers` | No | Up to 32 entries. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `Server.Body` | Optional for `POST` | `POST` only, supports nested JSON. Template placeholders must be complete, not concatenated with other text. |
| `Server.TimeoutMs` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="rest-api">

Configure tools in the `properties.llm.tools` array, and set `properties.advanced_features.enable_tools` to `true`.

| Parameter | Required | Description |
|---|---|---|
| `properties.advanced_features.enable_tools` | Yes | Set to `true` so the agent can expose custom tools and MCP tools to the model. If omitted or `false`, the agent doesn't call any tool endpoints. |
| `properties.llm.tools` | No | The custom tool array. Custom tools, MCP tools, built-in tools, and model-native tools share one model-visible tool namespace. |
| `tools[].type` | Yes | Fixed as `function`. |
| `tools[].function` | Yes | The tool definition visible to the model, including its name, description, and parameter schema. |
| `tools[].function.name` | Yes | Must start with a letter, contain only letters and numbers, and be no more than 64 characters long. Can't use the `mcp` prefix or the value `ragsearch_tool`. |
| `tools[].function.description` | Yes | Explains the tool's purpose, when to call it, and what it returns. Don't include secrets. |
| `tools[].function.parameters` | Yes | A JSON Schema object. The root `type` must be `object`. Passed using OpenAI's non-strict function-calling mode. |
| `tools[].execution` | No | Defaults to `{ "mode": "sync" }` when omitted. Currently only `sync` is supported. |
| `tools[].server` | Yes | The actual HTTP request configuration. |
| `tools[].server.method` | Yes | Only `GET` and `POST` are supported. `GET` can't configure `body`. |
| `tools[].server.url` | Yes | An absolute HTTPS URL. The path and query parameters can use template variables. |
| `tools[].server.headers` | No | Up to 32 entries. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `tools[].server.body` | Optional for `POST` | `POST` only, supports nested JSON. Template placeholders must be complete, not concatenated with other text. |
| `tools[].server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>
</Tabs>

Even when tool calling isn't enabled, any tools you declare are still validated for field type, size, template syntax, and duplicate names.

### Configure a POST tool

Once your `GET` tool works, you can configure a `POST` tool for actions with side effects, such as creating a ticket or sending a message. `server.body` can only be used with `POST`; configuring it for a `GET` tool causes validation to fail. The following example sends a confirmed delivery address to a phone number:

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

```python
post_tool = {
    'type': 'function',
    'function': {
        'name': 'sendFollowUpSms',
        'description': "Send the customer's confirmed delivery address to a phone number. Only call this after the customer has confirmed both the address and the phone number.",
        'parameters': {
            'type': 'object',
            'properties': {
                'phone_number': {
                    'type': 'string',
                    'description': 'The recipient phone number, in E.164 format.',
                },
                'address': {
                    'type': 'string',
                    'description': "The customer's confirmed delivery address.",
                },
            },
            'required': ['phone_number', 'address'],
            'additionalProperties': False,
        },
    },
    'server': {
        'method': 'POST',
        'url': 'https://api.example.com/messages/sms',
        'headers': {
            'Authorization': '{{template_variables.api_key}}',
            'Content-Type': 'application/json',
        },
        'body': {
            'to': '{{args.phone_number}}',
            'message': '{{args.address}}',
            'idempotency_key': '{{tool_call_id}}',
        },
        'timeout_ms': 10000,
    },
}

# ... other agent configuration ...
.with_llm(OpenAI(
    api_key='your-llm-api-key',
    model='gpt-4o-mini',
    tools=[post_tool],
))
.with_tools(True)
# ... continue with .with_tts() and .create_session() ...
```

</TabsContent>

<TabsContent value="typescript">

```typescript
const postTool = {
  type: 'function' as const,
  function: {
    name: 'sendFollowUpSms',
    description:
      "Send the customer's confirmed delivery address to a phone number. Only call this after the customer has confirmed both the address and the phone number.",
    parameters: {
      type: 'object' as const,
      properties: {
        phone_number: {
          type: 'string' as const,
          description: 'The recipient phone number, in E.164 format.',
        },
        address: {
          type: 'string' as const,
          description: "The customer's confirmed delivery address.",
        },
      },
      required: ['phone_number', 'address'],
      additionalProperties: false,
    },
  },
  server: {
    method: 'POST' as const,
    url: 'https://api.example.com/messages/sms',
    headers: {
      Authorization: '{{template_variables.api_key}}',
      'Content-Type': 'application/json',
    },
    body: {
      to: '{{args.phone_number}}',
      message: '{{args.address}}',
      idempotency_key: '{{tool_call_id}}',
    },
    timeout_ms: 10000,
  },
};

// ... other agent configuration ...
.withLlm(new OpenAI({
  apiKey: 'your-llm-api-key',
  model: 'gpt-4o-mini',
  tools: [postTool],
}))
.withTools(true)
// ... continue with .withTts() and .createSession() ...
```

</TabsContent>

<TabsContent value="go">

```go
postTool := &Agora.LlmTool{
    Function: &Agora.LlmToolFunction{
        Name:        "sendFollowUpSms",
        Description: Agora.String("Send the customer's confirmed delivery address to a phone number. Only call this after the customer has confirmed both the address and the phone number."),
        Parameters: &Agora.LlmToolFunctionParameters{
            Properties: map[string]interface{}{
                "phone_number": map[string]interface{}{
                    "type":        "string",
                    "description": "The recipient phone number, in E.164 format.",
                },
                "address": map[string]interface{}{
                    "type":        "string",
                    "description": "The customer's confirmed delivery address.",
                },
            },
            Required: []string{"phone_number", "address"},
        },
    },
    Server: &Agora.LlmToolServer{
        Method: Agora.LlmToolServerMethodPost,
        URL:    "https://api.example.com/messages/sms",
        Headers: map[string]string{
            "Authorization": "{{template_variables.api_key}}",
            "Content-Type":  "application/json",
        },
        Body: map[string]interface{}{
            "to":              "{{args.phone_number}}",
            "message":         "{{args.address}}",
            "idempotency_key": "{{tool_call_id}}",
        },
        TimeoutMs: Agora.Int(10000),
    },
}

// ... other agent configuration ...
.WithLlm(
    vendors.NewOpenAI(vendors.OpenAIOptions{
        APIKey: "your-llm-api-key",
        Model:  "gpt-4o-mini",
        Tools:  []*Agora.LlmTool{postTool},
    }),
)
.WithTools(true)
// ... continue with .WithTts() and .CreateSession() ...
```

</TabsContent>

<TabsContent value="rest-api">

```json
{
  "type": "function",
  "function": {
    "name": "sendFollowUpSms",
    "description": "Send the customer's confirmed delivery address to a phone number. Only call this after the customer has confirmed both the address and the phone number.",
    "parameters": {
      "type": "object",
      "properties": {
        "phone_number": {
          "type": "string",
          "description": "The recipient phone number, in E.164 format."
        },
        "address": {
          "type": "string",
          "description": "The customer's confirmed delivery address."
        }
      },
      "required": ["phone_number", "address"],
      "additionalProperties": false
    }
  },
  "server": {
    "method": "POST",
    "url": "https://api.example.com/messages/sms",
    "headers": {
      "Authorization": "{{template_variables.api_key}}",
      "Content-Type": "application/json"
    },
    "body": {
      "to": "{{args.phone_number}}",
      "message": "{{args.address}}",
      "idempotency_key": "{{tool_call_id}}"
    },
    "timeout_ms": 10000
  }
}
```

</TabsContent>
</Tabs>

#### Field reference

`function` and `execution` are the same as for a `GET` tool. The following fields are specific to a `POST` tool's `server` block:

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

| Parameter | Required | Description |
|---|---|---|
| `server.method` | Yes | Fixed as `POST`. |
| `server.url` | Yes | An absolute HTTPS URL. The path or query parameters can use template variables. |
| `server.headers` | No | Configure authentication and `Content-Type`. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `server.body` | No | The `POST` request body. Supports nested JSON. Values can be constants or complete template placeholders, not concatenated text. |
| `server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="typescript">

| Parameter | Required | Description |
|---|---|---|
| `server.method` | Yes | Fixed as `POST`. |
| `server.url` | Yes | An absolute HTTPS URL. The path or query parameters can use template variables. |
| `server.headers` | No | Configure authentication and `Content-Type`. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `server.body` | No | The `POST` request body. Supports nested JSON. Values can be constants or complete template placeholders, not concatenated text. |
| `server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="go">

| Parameter | Required | Description |
|---|---|---|
| `Server.Method` | Yes | Fixed as `POST`. |
| `Server.URL` | Yes | An absolute HTTPS URL. The path or query parameters can use template variables. |
| `Server.Headers` | No | Configure authentication and `Content-Type`. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `Server.Body` | No | The `POST` request body. Supports nested JSON. Values can be constants or complete template placeholders, not concatenated text. |
| `Server.TimeoutMs` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>

<TabsContent value="rest-api">

| Parameter | Required | Description |
|---|---|---|
| `tools[].server.method` | Yes | Fixed as `POST`. |
| `tools[].server.url` | Yes | An absolute HTTPS URL. The path or query parameters can use template variables. |
| `tools[].server.headers` | No | Configure authentication and `Content-Type`. Can't use `{{args.<name>}}`; pass authentication only as a complete header value. |
| `tools[].server.body` | No | The `POST` request body. Supports nested JSON. Values can be constants or complete template placeholders, not concatenated text. |
| `tools[].server.timeout_ms` | No | Defaults to `10000`, in the range `1000` to `100000`. |

</TabsContent>
</Tabs>

`{{tool_call_id}}` is the ID correlating this tool call. You can pass it to your own service to correlate logs or as a hint for deduplication, but it isn't a business idempotency mechanism on its own, and Agora doesn't automatically retry requests. Your service is still responsible for handling duplicate requests and unknown outcomes.

For `POST` tools that have side effects:

- State the conditions for calling the tool in `function.description` and in your system prompt, such as requiring the user to confirm first.
- Perform the final authorization check on your own server. Don't rely on the LLM's judgment alone.
- Only have the agent tell the user the request succeeded after your endpoint returns a `2xx` status.
- If a request has already been sent, a user interruption doesn't guarantee the remote action is canceled or rolled back.

### Configure template variables

Template variables let you fill an HTTP request with either LLM-generated parameters or fixed values known when you create the agent. The following example puts a tenant ID in a header and a customer ID in the request body:

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

Configure fixed values in the LLM vendor's `template_variables` parameter, then reference them from a tool's `server.headers` or `server.body`:

```python
post_tool = {
    'type': 'function',
    'function': {
        'name': 'getCustomerProfile',
        'description': "Look up the current customer's profile.",
        'parameters': {
            'type': 'object',
            'properties': {},
            'additionalProperties': False,
        },
    },
    'server': {
        'method': 'POST',
        'url': 'https://api.example.com/customers/profile',
        'headers': {
            'X-Tenant-ID': '{{template_variables.tenant_id}}',
        },
        'body': {
            'customer_id': '{{template_variables.customer_id}}',
        },
    },
}

# ... other agent configuration ...
.with_llm(OpenAI(
    api_key='your-llm-api-key',
    model='gpt-4o-mini',
    template_variables={
        'tenant_id': 'acme-corp',
        'customer_id': 'cust-4471',
    },
    tools=[post_tool],
))
.with_tools(True)
# ... continue with .with_tts() and .create_session() ...
```

</TabsContent>

<TabsContent value="typescript">

Configure fixed values in the LLM vendor's `templateVariables` option, then reference them from a tool's `server.headers` or `server.body`:

```typescript
const postTool = {
  type: 'function' as const,
  function: {
    name: 'getCustomerProfile',
    description: "Look up the current customer's profile.",
    parameters: { type: 'object' as const, properties: {}, additionalProperties: false },
  },
  server: {
    method: 'POST' as const,
    url: 'https://api.example.com/customers/profile',
    headers: {
      'X-Tenant-ID': '{{template_variables.tenant_id}}',
    },
    body: {
      customer_id: '{{template_variables.customer_id}}',
    },
  },
};

// ... other agent configuration ...
.withLlm(new OpenAI({
  apiKey: 'your-llm-api-key',
  model: 'gpt-4o-mini',
  templateVariables: {
    tenant_id: 'acme-corp',
    customer_id: 'cust-4471',
  },
  tools: [postTool],
}))
.withTools(true)
// ... continue with .withTts() and .createSession() ...
```

</TabsContent>

<TabsContent value="go">

The Go SDK configures fixed values in the LLM vendor's `TemplateVariables`, then references them from `LlmToolServer.Headers` or `Body`:

```go
tool := &Agora.LlmTool{
    Function: &Agora.LlmToolFunction{
        Name:        "getCustomerProfile",
        Description: Agora.String("Look up the current customer's profile."),
        Parameters: &Agora.LlmToolFunctionParameters{
            Properties: map[string]interface{}{},
        },
    },
    Server: &Agora.LlmToolServer{
        Method: Agora.LlmToolServerMethodPost,
        URL:    "https://api.example.com/customers/profile",
        Headers: map[string]string{
            "X-Tenant-ID": "{{template_variables.tenant_id}}",
        },
        Body: map[string]interface{}{
            "customer_id": "{{template_variables.customer_id}}",
        },
    },
}

// ... other agent configuration ...
.WithLlm(
    vendors.NewOpenAI(vendors.OpenAIOptions{
        APIKey: "your-llm-api-key",
        Model:  "gpt-4o-mini",
        TemplateVariables: map[string]string{
            "tenant_id":   "acme-corp",
            "customer_id": "cust-4471",
        },
        Tools: []*Agora.LlmTool{tool},
    }),
)
.WithTools(true)
// ... continue with .WithTts() and .CreateSession() ...
```

</TabsContent>

<TabsContent value="rest-api">

```json
{
  "properties": {
    "llm": {
      "template_variables": {
        "tenant_id": "acme-corp",
        "customer_id": "cust-4471"
      },
      "tools": [
        {
          "type": "function",
          "function": {
            "name": "getCustomerProfile",
            "description": "Look up the current customer's profile.",
            "parameters": {
              "type": "object",
              "properties": {},
              "additionalProperties": false
            }
          },
          "server": {
            "method": "POST",
            "url": "https://api.example.com/customers/profile",
            "headers": {
              "X-Tenant-ID": "{{template_variables.tenant_id}}"
            },
            "body": {
              "customer_id": "{{template_variables.customer_id}}"
            }
          }
        }
      ]
    }
  }
}
```

</TabsContent>
</Tabs>

#### Field reference

The following fields configure and reference `template_variables` in each language.

<Tabs defaultValue="python" groupId="ai-sdk-language" persist>
<TabsList>
  <TabsTrigger value="python">Python SDK</TabsTrigger>
  <TabsTrigger value="typescript">TypeScript SDK</TabsTrigger>
  <TabsTrigger value="go">Go SDK</TabsTrigger>
  <TabsTrigger value="rest-api">REST API</TabsTrigger>
</TabsList>

<TabsContent value="python">

| Parameter | Description |
|---|---|
| `OpenAI(template_variables=...)` | Fixed string values known when the agent is created. |
| `OpenAI(tools=...)` | The custom tools. |
| `Agent.with_tools(True)` | Enables custom tool and MCP tool invocation. |
| `server.body` / `server.headers` | Read a fixed value with `{{template_variables.<name>}}`. Can also reference `args` or `tool_call_id`. |

</TabsContent>

<TabsContent value="typescript">

| Parameter | Description |
|---|---|
| `OpenAI({ templateVariables: ... })` | Fixed string values known when the agent is created. |
| `server.headers` / `server.body` | Read a fixed value with `{{template_variables.<name>}}`. |
| `server.body` | Can also use `{{args.<name>}}` and `{{tool_call_id}}`, neither of which need `templateVariables` configured. |

</TabsContent>

<TabsContent value="go">

| Parameter | Description |
|---|---|
| `OpenAIOptions.TemplateVariables` | Fixed string values known when the agent is created. |
| `LlmToolServer.Headers` / `Body` | Read a fixed value with `{{template_variables.<name>}}`. |
| `LlmToolServer.Body` | Can also use `{{args.<name>}}` and `{{tool_call_id}}`, neither of which need `TemplateVariables` configured. |

</TabsContent>

<TabsContent value="rest-api">

| Parameter | Description |
|---|---|
| `properties.llm.template_variables` | Fixed string values known when the agent is created. |
| `properties.llm.tools[].server.headers` / `body` | Read a fixed value with `{{template_variables.<name>}}`. |
| `properties.llm.tools[].server.body` | Can also use `{{args.<name>}}` and `{{tool_call_id}}`, neither of which need `template_variables` configured. |

</TabsContent>
</Tabs>

Keep the following template rules in mind:

- Templates are substituted once; they aren't expanded recursively.
- A template must reference a complete variable. Concatenating text, such as `order-{{args.order_id}}`, isn't supported.
- Nested fields, array indexes, and shorthand forms like `{{name}}` aren't supported.
- `{{args.<name>}}` can't be used in headers.
- Header names, the HTTP method, and the URL's scheme, host, and port can't be templated.
- Variables used in the URL path and query parameters are encoded for their respective part of the URL. A variable can't inject a new host, path segment, or query key.
- If a variable doesn't exist, is `null`, or has a type that doesn't fit its target location, the tool call fails without your endpoint being contacted.
- Pass authentication information only as a complete header value. Don't put it in the URL, request body, tool parameters, or tool description.

## Reference

The following reference material covers the tool parameter schema, response handling, configuration limits, and common issues.

### Parameter schema

`function.parameters` uses JSON Schema and is passed to the LLM using OpenAI's non-strict function-calling mode. The root must be `object`, with parameters typically declared under `properties`. For details, see [OpenAI's Function Calling guide](https://platform.openai.com/docs/guides/function-calling).

```json
{
  "type": "object",
  "properties": {
    "order_id": {
      "type": "string",
      "description": "The order ID."
    },
    "include_items": {
      "type": "boolean",
      "description": "Whether to also return line-item details."
    },
    "region": {
      "type": "string",
      "enum": ["us", "eu"],
      "description": "The order's region."
    }
  },
  "required": ["order_id"],
  "additionalProperties": false
}
```

`additionalProperties: false` in this example only demonstrates disallowing undeclared parameters. It isn't required, and setting it doesn't itself enable strict mode. To allow extra parameters, omit this field or set it to `true`.

Practical rules:

- Common JSON Schema types are supported: `object`, `array`, `string`, `integer`, `number`, `boolean`, and `null`.
- Standard JSON Schema keywords relevant to tool parameters are supported, such as `type`, `description`, `enum`, `properties`, `required`, `additionalProperties`, and `items`. The actual effect depends on your LLM's function-calling support.
- The root `type` must be `object`. Declare array elements with `items`.
- Don't add optional parameters to `required`. `additionalProperties` can be omitted, in which case extra properties are allowed per JSON Schema semantics; set it to `false` explicitly to disallow undeclared parameters.
- Don't rely on a specific model's support for advanced keywords such as `$ref`, `$defs`, `format`, `pattern`, range constraints, or composition schemas. Confirm compatibility with your LLM before using them.
- The serialized schema size is capped at 64 KiB.

### Response and runtime behavior

The Conversational AI Engine passes your endpoint's response back to the LLM. The exact behavior depends on the response status and content:

| Endpoint result | Tool result | Conversation behavior |
|---|---|---|
| Non-empty `2xx`, valid JSON | JSON is parsed, then serialized back to a string | The model generates a reply based on the full result |
| Non-empty `2xx`, UTF-8 text | The raw text is used | The model generates a reply based on the full result |
| Empty `2xx`, such as `204` | Reported as success, result is `[]` | The model continues based on the success result |
| Non-`2xx`, valid JSON | Reported as failed; JSON is parsed, then serialized back to a string | The full response is still returned to the model, but it shouldn't claim the action succeeded |
| Non-`2xx`, UTF-8 text | Reported as failed; the raw text is used | The full response is still returned to the model, but it shouldn't claim the action succeeded |
| Non-`2xx`, empty body | Reported as failed, result is `[]` | The model gets a failure result and shouldn't claim the action succeeded |
| `3xx` | Reported as failed, following the non-`2xx` rules | The engine doesn't follow `Location` or forward the request to the redirect target |
| Timeout, network error, or parameter/template validation failure | Reported as failed | The request isn't retried automatically |
| Response over 1 MiB or not valid, safe UTF-8 text | Reported as failed | The partial response isn't passed to the model |

A few other runtime behaviors to keep in mind:

- If a single model response submits multiple tool calls, the engine runs them concurrently. One call failing doesn't block the others.
- The engine waits for every submitted call to reach a final result or error before starting the next LLM turn, and writes the results back in the order the model returned them.
- If the user interrupts the agent, that stops the current playback but doesn't guarantee that an in-flight HTTP request is canceled or that the remote action is rolled back.
- While waiting for a tool call, the agent uses your existing [filler words](../shape-the-conversation/filler-words) configuration. Custom tools don't add a separate, tool-specific filler word trigger.
- Tool responses never go directly to TTS. Only the model's final generated text is spoken.

### Security and configuration limits

- `server.url` must be an absolute HTTPS URL. Usernames and passwords in the URL aren't supported.
- Header values are treated as sensitive: they aren't passed to the model, and aren't written to regular logs, subtitles, or TTS output. OAuth token exchange, dynamic request signing, mTLS, and reusable credential resources aren't provided yet.
- The final rendered URL is capped at 8192 UTF-8 bytes: each path variable at 1024 bytes, and each query variable at 4096 bytes.
- Headers are capped at 32 entries, totaling up to 16 KiB.
- `server.body` is `POST` only. Both the configured value and the rendered result are capped at 64 KiB.
- `server.timeout_ms` defaults to `10000`, in the range `1000` to `100000`.
- A non-empty response body can't exceed 1 MiB. Extracting specific fields, JSONPath, or custom truncation aren't supported.
- Custom tools, MCP tools, built-in tools, and model-native tools share one namespace. Tool names must start with a letter, contain only letters and numbers, and be no more than 64 characters, and comparisons aren't case-sensitive.
- Tool names can't use the `mcp` prefix or the value `ragsearch_tool`. After discovery and filtering, a maximum of 32 tools total can be exposed to the model.
- Stored tools, asynchronous tasks, callbacks, polling, automatic retries, `PUT`/`PATCH`/`DELETE`, and configurable serial tool scheduling aren't supported yet.

### Troubleshooting

If a custom tool isn't behaving as expected, start with these checks.

| Symptom | Check first |
|---|---|
| The model never calls the tool | Whether `enable_tools` is on; whether the tool description explains when to call it and what it returns; whether the current LLM supports function calling |
| Agent creation fails | Whether `type` is `function`; whether `function.parameters`'s root type is `object`; whether `server.method` is `GET` or `POST`; whether a tool name is duplicated |
| Your endpoint never receives a request | Whether `server` is a sibling of `function`, not nested inside it; whether the URL is an absolute HTTPS URL; whether the referenced template variables exist; whether `body` was mistakenly configured on a `GET` tool |
| A header doesn't contain the model's parameter | `{{args.<name>}}` isn't supported in headers. Use a fixed value directly, or use `template_variables` or `tool_call_id` for dynamic values |
| Your endpoint returns a parameter validation error | Check the body field names, parameter types, and `Content-Type`; template placeholders in the request body can't be concatenated with other text |
| The tool call times out | Check your endpoint's response time against `timeout_ms`; timed-out calls aren't retried automatically |
| Your endpoint returns success, but the model can't use the result | Confirm it returned a `2xx` status; a non-empty response must be safe UTF-8 text under 1 MiB; the engine doesn't support extracting specific fields from a JSON response |
| A `POST` action might run more than once | Use `tool_call_id` to correlate the call on your side and handle duplicate requests in your own service. Don't assume Agora will roll back or retry the request for you |

## References

- [Start a conversational AI agent](/en/api-reference/api-ref/conversational-ai/join#request-body-properties-llm-tools)
