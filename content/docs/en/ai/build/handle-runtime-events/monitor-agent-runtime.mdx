...
| Alert on agent failures from your backend | Webhooks | [`110 agent error`](/en/ai/reference/event-types#110-agent-error), [`102 agent left`](/en/ai/reference/event-types#102-agent-left) |
| Monitor latency and runtime health | Toolkit for live UX, webhooks for aggregation | `onAgentMetrics`, [`111 agent metrics`](/en/ai/reference/event-types#111-agent-metrics) |
| Track when sessions start and stop | Webhooks | [`101 agent joined`](/en/ai/reference/event-types#101-agent-joined), [`102 agent left`](/en/ai/reference/event-types#102-agent-left) |
| Retrieve conversation history after a session | Webhooks or REST API | [`103 agent history`](/en/ai/reference/event-types#103-agent-history), `history`, `turns` |
...
For the first-step setup of both client and server paths, see [Get runtime events](get-runtime-events). For the full client integration flow, see [Listen to agent runtime events on client apps](event-notifications).
...
- [`110 agent error`](/en/ai/reference/event-types#110-agent-error) for structured module failures
- [`102 agent left`](/en/ai/reference/event-types#102-agent-left) to understand whether the session terminated abnormally
...
Use [`111 agent metrics`](/en/ai/reference/event-types#111-agent-metrics) when performance must be monitored over time:
