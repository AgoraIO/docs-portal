---
title: Listen to agent runtime events on client apps
description: Use toolkit callbacks to update the UI and react to agent events during a live session.
---
Real-time Conversational AI applications require responsive user interfaces that react to agent events. This page explains how to implement agent event handling with the Conversational AI Engine toolkit. Use this page when you need in-session events in a mobile or web client. If you need backend monitoring, alerting, or post-session analysis, use [Receive agent runtime events on your server](webhooks) instead.
...
- [Interrupt agents](/en/ai/build/shape-the-conversation/interrupt-agent)
- [Display live transcripts](/en/ai/build/transcripts)
- [Monitor agent status, errors, and performance](monitor-agent-runtime)
- [Set optimal audio parameters](/en/ai/best-practices/audio-setup) for iOS and Android
...
- Implemented the Conversational AI Engine [REST quickstart](/en/ai/get-started/quickstart).
- Integrated Agora Video SDK v4.5.1 or later and completed the [audio and video quickstart](/en/introduction/realtime-audio-video).
...
- Maintained active and authenticated RTC and Signaling instances that persist beyond the component lifecycle. The toolkit does not manage RTC or Signaling initialization, lifecycle, or authentication.

## Implementation

<Tabs groupId="platform" persist>
<TabsList>
...
1. **Integrate the toolkit**
   ...
   Copy the [`convoaiApi`](https://github.com/AgoraIO-Community/Conversational-AI-Demo/tree/main/Android/scenes/convoai/src/main/java/io/agora/scene/convoai/convoaiApi) folder to your project and import the toolkit before calling the toolkit API. See [Folder structure](#folder-structure) to understand the role of each file.
...
5. **Add a conversational AI agent to the channel**
   ...
   To [start a conversational AI agent](/en/api-reference/conversational-ai/rest-api/agent/join), configure:
...
1. **Integrate the toolkit**
   ...
   Copy the [`ConversationalAIAPI`](https://github.com/AgoraIO-Community/Conversational-AI-Demo/tree/main/iOS/Scenes/ConvoAI/ConvoAI/ConvoAI/Classes/ConversationalAIAPI) folder to your project and import the toolkit before calling the toolkit API. See [Folder structure](#folder-structure) to understand the role of each file.
...
1. **Integrate the toolkit**
   ...
   Copy the [`conversational-ai-api`](https://github.com/AgoraIO-Community/Conversational-AI-Demo/tree/main/Web/Scenes/VoiceAgent/src/conversational-ai-api) file to your project and import the toolkit before calling the toolkit API. See [Folder structure](#folder-structure) to understand the role of each file.
