...
description: Configure a custom LLM to output audio directly, bypassing TTS, by streaming Base64-encoded PCM audio data with optional transcript and word-level subtitle fields.
---
...
- A reference implementation of a conversational AI agent that includes the [basic logic](../get-started/quickstart.md) for interacting with an AI agent.
- If you plan to implement subtitles for audio output, read the [Display live subtitles](transcripts.md) documentation and complete the required configuration.
...
To configure the output mode, set the `llm.output_modalities` field when you [Start a conversational AI agent](../../api-reference/conversational-ai/rest-api/agent/join.md), as follows:
...
To adapt your LLM service, refer to [Custom LLM](custom-llm.md) for guidance on transforming your API to meet these requirements.
...
Include these fields in the `llm.params` object of the [Start a conversational AI agent](../../api-reference/conversational-ai/rest-api/agent/join.md) request.
...
### Subtitle alignment
When you [Display live subtitles](transcripts.md), the Conversational AI Engine can use the `audio.words` field to segment the audio. The engine aligns subtitles based on the `start_ts`, `end_ts`, and `duration` fields within `audio.words`.
...
> - When you [Broadcast a custom message using the TTS module](../../api-reference/conversational-ai/rest-api/agent/speak.md).
