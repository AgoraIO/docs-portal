...
- **MCP tool calls**: When the agent invokes tools through an [MCP server](../tools/mcp-tools), response times can increase significantly. Filler words bridge this gap while the agent waits for tool results.
...
```go
fillerWords := &agentkit.FillerWordsConfig{
