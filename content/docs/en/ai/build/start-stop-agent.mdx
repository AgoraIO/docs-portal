...
![](https://assets-docs.agora.io/images/conversational-ai/ai-agent-tech.svg)
</details>
...
- The following information from Agora Console:
  ...
  - [Customer ID and Customer secret](/en/api-reference/api-ref/conversational-ai/authentication#generate-customer-id-and-customer-secret): Used for HTTP authentication when calling the REST APIs.
...
- Implemented the [Voice Calling](../../realtime-media/voice/quickstart) or [Video Calling](../../realtime-media/video/quickstart) quickstart.
...
Call the `join` endpoint to create an agent instance that joins an Agora channel. For the Curl request, pass in the `channel` name and `token` for agent authentication. To generate your base64-encoded credentials, see [RESTful authentication](/en/api-reference/api-ref/conversational-ai/authentication).
