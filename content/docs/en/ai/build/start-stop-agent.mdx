...
A user joins a channel and triggers your business server to start an agent via the Conversational AI Engine API. The agent joins the same channel and communicates with the user through voice, using the specified LLM, TTS service, and Agora's low-latency backend. When the conversation ends, the business server stops the agent and the user leaves the channel.
...
- The following information from Agora Console:
  - [App ID](../reference/enable-conversational-ai#get-the-app-id): The string identifier for your project.
  - [App Certificate](../reference/enable-conversational-ai#get-the-app-certificate): Required by the SDK to generate authentication tokens automatically.
  - For Curl only: [Customer ID and Customer secret](../../api-reference/conversational-ai/rest-api/authentication#generate-customer-id-and-customer-secret) and a [temporary token](../reference/enable-conversational-ai#generate-temporary-tokens).
...
This section introduces how to use the Agora Conversational AI SDKs or REST API to start and stop a conversational AI agent. In a production environment, implement this logic on your business server.

### Start a conversational AI agent

Call the SDK method or the `join` endpoint to create an agent instance that joins an Agora channel. For the Curl request, pass in the `channel` name and `token` for agent authentication. To generate your base64-encoded credentials, see [RESTful authentication](../../api-reference/conversational-ai/rest-api/authentication).
