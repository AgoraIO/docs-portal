...
:::info[Info]
Building your agent in Agent Studio? See [Manage integrations](../../studio/build/integrations#mcp-servers).
:::
...
:::note
MLLM agents support MCP tool-use. For an MLLM agent, add `mcp_servers` under `mllm` instead of `llm`. All three Agents SDKs accept MCP servers on every MLLM vendor class. In the Go SDK, use the `McpServers` field, except on `NewOpenAIGPTLive`, where the equivalent field is `McpServerConfigs`.
:::
