...
---
Set up a working voice agent in under five minutes. This page walks you through installing Agora Skills to give your AI coding assistant the official quickstarts and Agora CLI workflows. You then use the CLI to log in, clone the official starter, and run it locally. You can follow the CLI steps yourself or paste the sample prompt and let your assistant handle setup for you.
...
- **[Monitor runtime behavior](../build/handle-runtime-events/monitor-agent-runtime)**: Choose the client events and webhooks you need for live status, metrics, and backend monitoring.
...
- **[Retrieve conversation history](../build/handle-runtime-events/retrieve-session-history)**: Store messages, timestamps, and per-turn diagnostics after a session.
...
- **[Record agent conversations](../best-practices/record-agent-conversation)**: Configure Cloud Recording without disrupting the audio experience.
- **[Understand pricing](../reference/pricing)**: Review Conversational AI usage and billing before launch.
