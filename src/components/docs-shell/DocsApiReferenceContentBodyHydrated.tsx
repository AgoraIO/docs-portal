...
const DocsApiReferenceContentBodyClient = lazy(() =>
  import('./DocsApiReferenceContentBody.client').then((module) => ({
    default: module.DocsApiReferenceContentBodyClient,
  })),
);

export function DocsApiReferenceContentBodyHydrated({
  contentPath,
}: {
  contentPath: string;
}) {
  return (
    <Suspense fallback={<DocsContentSkeleton />}>
      <DocsApiReferenceContentBodyClient contentPath={contentPath} />
    </Suspense>
  );
}
