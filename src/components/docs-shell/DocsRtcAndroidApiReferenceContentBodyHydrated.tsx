...
const DocsRtcAndroidApiReferenceContentBodyClient = lazy(() =>
  import('./DocsRtcAndroidApiReferenceContentBody.client').then((module) => ({
    default: module.DocsRtcAndroidApiReferenceContentBodyClient,
  })),
);

export function DocsRtcAndroidApiReferenceContentBodyHydrated({
  contentPath,
}: {
  contentPath: string;
}) {
  return (
    <Suspense fallback={<DocsContentSkeleton />}>
      <DocsRtcAndroidApiReferenceContentBodyClient contentPath={contentPath} />
    </Suspense>
  );
}
