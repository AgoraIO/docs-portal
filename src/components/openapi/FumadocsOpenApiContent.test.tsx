...
describe('FumadocsOpenApiContent', () => {
  ...
  it('keeps generated language tabs when an operation does not define x-codeSamples', async () => {
    ...
    const generatedPanel = within(generatedPreview).getByRole('tabpanel');
    expect(generatedPanel.closest('.openapi-request-code-tabs')).toHaveClass(
      'openapi-request-code-tabs',
      'my-0',
    );
    expect(
      generatedPanel.querySelector('.fd-scroll-container'),
    ).toHaveAttribute('data-openapi-code-viewport', '');
    expect(generatedPanel.querySelector('figure.shiki')).toHaveClass(
      'bg-fd-secondary',
      '-mx-px',
      '-mb-px',
      'last:rounded-b-xl',
    );
    expect(generatedPanel.querySelector('.token-command')).toBeInTheDocument();
    expect(generatedPanel.querySelector('.token-flag')).toBeInTheDocument();
    expect(
      within(generatedPreview).getByRole('button', { name: '复制文本' }),
    ).toBeInTheDocument();
    expect(
      screen
    ...
    expect(
      screen.getByText('响应示例').closest('.openapi-response-example'),
    ).not.toHaveAttribute('data-markdown-ignore');
  });
  ...
  it('keeps same-language code samples on distinct tabs', async () => {
    render(
      <FumadocsOpenApiContent
        pageProps={{
          operations: [{ method: 'post', path: '/duplicate-languages' }],
          payload: {
            bundled: {
              info: { title: 'Duplicate language samples' },
              openapi: '3.2.0',
              paths: {
                '/duplicate-languages': {
                  post: {
                    responses: { '200': { description: 'OK' } },
                    'x-codeSamples': [
                      { lang: 'bash', source: 'curl first' },
                      { lang: 'bash', source: 'curl second' },
                    ],
                  },
                },
              },
    ...
    const tabs = await screen.findAllByRole('tab', { name: 'bash' });
    expect(tabs).toHaveLength(2);
    const examples = within(
      tabs[0].closest('.openapi-request-examples') as HTMLElement,
    );
    fireEvent.mouseDown(tabs[1]);
    expect(tabs[1]).toHaveAttribute('aria-selected', 'true');
    expect(examples.getByRole('tabpanel')).toHaveTextContent('curl second');
  });
  ...
  it('renders authorization with official parameter sections', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Header Parameters' }),
    ).toBeInTheDocument();
    expect(screen.getAllByText('Authorization').length).toBeGreaterThan(0);
    expect(
      screen.getByRole('heading', { name: 'Path Parameters' }),
    ).toBeInTheDocument();
    expect(screen.getByText('appid')).toBeVisible();
  ...
  it('prefers an explicit Authorization header parameter over localized securitySchemes in zh-CN', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: '请求 Header' }),
    ).toBeInTheDocument();
    expect(screen.getByText('Authorization')).toBeVisible();
    expect(screen.getByText(/支持 RTC Token/)).toBeVisible();
    expect(
  ...
  it('uses the official array type navigation without wrapper rows', async () => {
    render(
      <FumadocsOpenApiContent
        locale="zh-CN"
        pageProps={{
    ...
    const remoteRow = (await screen.findByText('remote_rtc_uids')).closest(
      'div.border-t',
    ) as HTMLElement;
  ...
  it('renders referenced path and header parameters', async () => {
    ...
    const pathSection = screen.getByRole('heading', {
  ...
  it('renders scalar parameters, response headers, callouts, and parameter metadata', async () => {
    ...
    const pathSection = pathParameters.nextElementSibling as HTMLElement;
    const headerSection = screen.getByRole('heading', {
      name: 'Header Parameters',
    }).nextElementSibling as HTMLElement;
    ...
    expect(within(querySection).getByText('0 < value < 1')).toBeInTheDocument();
    ...
    expect(
      within(headerSection).getByText('"request-123"'),
    ).toBeInTheDocument();
  ...
  it('renders response body schema field descriptions', async () => {
                ...
                '/{region}/v1/projects/{appId}/rtls/ingress/streamkeys': {
                  post: {
                    ...
                    responses: {
                      ...
                      '204': { description: 'No response content.' },
                      '401': {
                        content: {
                          'application/json': {
                            schema: {
    ...
    expect(
      screen.getByRole('heading', { name: '响应 Header' }),
    ).toBeInTheDocument();
    expect(
      document.getElementById('response-headers-200-x-request-id'),
    ).toBeInTheDocument();
    ...
    fireEvent.click(
      screen.getByRole('button', { name: '401 application/json' }),
    );
    ...
    fireEvent.click(screen.getByRole('button', { name: '204' }));
    expect(screen.getByText('响应 Body 为空')).toBeInTheDocument();
  });

  it('renders the localized fieldless state for a real empty response schema', async () => {
    render(
      <FumadocsOpenApiContent
        locale="zh-CN"
        pageProps={{
          operations: [{ method: 'get', path: '/empty-schema' }],
          payload: {
            bundled: {
              info: { title: 'Empty schema API' },
              openapi: '3.2.0',
              paths: {
                '/empty-schema': {
                  get: {
                    responses: {
                      '200': {
                        ...
                        description: 'No response fields.',
                      },
                    },
    ...
    expect(screen.getByText('Schema 中没有字段')).toBeInTheDocument();
  });

  it('keeps a real primitive response schema renderable', async () => {
    render(
      <FumadocsOpenApiContent
        locale="zh-CN"
        pageProps={{
          operations: [{ method: 'get', path: '/primitive-schema' }],
          payload: {
            bundled: {
              info: { title: 'Primitive schema API' },
              openapi: '3.2.0',
              paths: {
                '/primitive-schema': {
                  get: {
    ...
    expect(screen.queryByText('Schema 中没有字段')).not.toBeInTheDocument();
    expect(screen.getAllByText('string').length).toBeGreaterThan(0);
    expect(screen.getAllByText('ready').length).toBeGreaterThan(0);
  });

  it('matches fieldless states to boolean and map schema rendering', async () => {
    render(
      <FumadocsOpenApiContent
        locale="zh-CN"
        pageProps={{
          operations: [{ method: 'get', path: '/schema-shapes' }],
          payload: {
            bundled: {
              info: { title: 'Schema shapes API' },
              openapi: '3.2.0',
              paths: {
                '/schema-shapes': {
                  get: {
    ...
    const panelFor = (status: string) => {
      const trigger = screen.getByRole('button', {
        name: `${status} application/json`,
      });
    ...
    expect(panelFor('200').queryByText('Schema 中没有字段')).toBeNull();
    expect(panelFor('201').getAllByText('^x-').length).toBeGreaterThan(0);
    expect(panelFor('202').getByText('Schema 中没有字段')).toBeVisible();
    expect(panelFor('203').queryByText('Schema 中没有字段')).toBeNull();
    expect(panelFor('203').getAllByText('string').length).toBeGreaterThan(0);
    expect(panelFor('204').getByText('Schema 中没有字段')).toBeVisible();
    expect(panelFor('205').queryByText('Schema 中没有字段')).toBeNull();
    expect(panelFor('205').getAllByText('object').length).toBeGreaterThan(0);
  });
  ...
  it('renders all parameter locations through the shared field row contract', async () => {
    ...
    const cookieSection = screen.getByRole('heading', {
    ...
    const queryRow = within(querySection)
      .getByText('state')
      .closest('div.border-t') as HTMLElement;
    const headerRow = within(headerSection)
      .getByText('X-Trace-ID')
      .closest('div.border-t') as HTMLElement;
    const cookieRow = within(cookieSection)
      .getByText('session')
      .closest('div.border-t') as HTMLElement;
    ...
    const responseRow = within(responseSection)
      ...
      .closest('.openapi-response-header-row') as HTMLElement;
    ...
    expect(
      within(responseRow).getByRole('link', {
        name: 'Copy link to X-Request-ID',
      }),
    ).toHaveAttribute('href', '#response-headers-200-x-request-id');
  });

  it('localizes official parameter headings and keeps restrained requiredness', async () => {
    render(
      <FumadocsOpenApiContent
        locale="zh-CN"
        pageProps={{
                    ...
                    'x-codeSamples': [
                      {
                        lang: 'bash',
                        label: 'curl',
                        source: 'curl https://example.com/items',
                      },
                    ],
                  },
                },
    ...
    expect(
      screen.getAllByRole('button', { name: '复制文本' }).length,
    ).toBeGreaterThan(0);
  });
});
