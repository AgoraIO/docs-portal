...
describe('docs content regressions', () => {
  ...
  function findMdxTabs(source: string) {
    return [...source.matchAll(/<Tabs\b([^>]*)>([\s\S]*?)<\/Tabs>/g)].map(
      (match) => ({
        attrs: match[1],
        line: source.slice(0, match.index).split('\n').length,
        values: [
          ...match[2].matchAll(/<TabsTrigger\b[^>]*value="([^"]+)"/g),
  ...
  it('keeps AI language and platform tabs persisted by preference group', () => {
    const expectedPreferenceGroups = new Map<string, string>([
      ['python|typescript|go|rest-api', 'ai-sdk-language'],
      ['python|typescript|go', 'ai-sdk-language'],
      ['typescript|python|go', 'ai-sdk-language'],
      ['android|ios|web', 'ai-toolkit-platform'],
      ['android|ios', 'ai-toolkit-platform'],
    ...
    for (const file of listMarkdownFiles(resolve(docsRoot, 'ai'))) {
      const relativePath = relative(process.cwd(), file);
      const source = readFileSync(file, 'utf8');

      for (const tab of findMdxTabs(source)) {
        const valuesKey = tab.values.join('|');
        const groupId = expectedPreferenceGroups.get(valuesKey);

        if (tab.values.includes('curl')) {
          offenders.push(
            `${relativePath}:${tab.line}: use rest-api, not curl, as the REST API tab value`,
          );
        }
        ...
        if (!tab.attrs.includes(`groupId="${groupId}"`)) {
          offenders.push(
            `${relativePath}:${tab.line}: expected groupId="${groupId}" for ${valuesKey}`,
          );
        }

        if (!/\bpersist\b/.test(tab.attrs)) {
          offenders.push(
            `${relativePath}:${tab.line}: expected persist for ${valuesKey}`,
          );
        }
      }
    }
