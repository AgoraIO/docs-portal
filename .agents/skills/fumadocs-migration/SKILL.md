...
5. Update or create `meta.json` from legacy sidebar order. Do not migrate executable JS metadata.
6. Mark deferred generated API, unresolved OpenAPI lane work, and missing standards in the audit report.
7. Verify with `bun run types:check`; add `bun run docs:links`, `bun run test`, or `bun run build` when the migration touches links, code, routing, or shell behavior.
